                     ...
                     const vfft_codelet_registry *reg, size_t minr)
{
    ...
    {
        size_t r = *rp;
        if (r < minr || r > rem || rem % r || d >= MAX_STAGES)
            continue;
                            ...
                            const vfft_codelet_registry *reg)
{
    ...
    for (size_t s = 0; s < nf; s++)
    {
        ...
        st->radix = R;
        st->K = K;
        st->N_remaining = N / (R * K);
        if (R < VFFT_MAX_RADIX)
        {
            st->fwd = reg->fwd[R];
            st->bwd = reg->bwd[R];
            st->tw_fwd = reg->tw_fwd[R];
            st->tw_bwd = reg->tw_bwd[R];
            st->tw_dif_fwd = reg->tw_dif_fwd[R];
            st->tw_dif_bwd = reg->tw_dif_bwd[R];
        }
        if (!st->fwd)
        {
            free(p);
                     ...
                     double *ir, double *ii, double *vr, double *vi)
{
    ...
    if (!p)
        return -1.0;
    for (int i = 0; i < 5; i++)
        vfft_execute_fwd(p, ir, ii, vr, vi);
    int reps = N <= 512 ? 5000 : N <= 2048 ? 2000
                             : N <= 8192   ? 1000
                             : N <= 32768  ? 200
                                           : 100;
    double best = 1e18;
...
static void fstr(const size_t *f, size_t n, char *b, size_t sz)
{
    ...
    {
        if (i)
            p += (size_t)snprintf(b + p, sz - p, "x");
        p += (size_t)snprintf(b + p, sz - p, "%zu", f[i]);
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (size_t ni = 0; ni < nu; ni++)
    {
        ...
        g_nfacts = 0;
        {
            size_t tmp[MAX_STAGES];
            enum_rec(N, tmp, 0, &reg, 2);
        }
        ...
        if (cp)
        {
            ...
            {
                if (s)
                    pos += (size_t)snprintf(cfact + pos, sizeof(cfact) - pos, "x");
                pos += (size_t)snprintf(cfact + pos, sizeof(cfact) - pos, "%zu", cp->stages[s].radix);
            }
            for (int w = 0; w < 5; w++)
                vfft_execute_fwd(cp, ir, ii, vr, vi);
            int reps = N <= 512 ? 5000 : N <= 2048 ? 2000
                                     : N <= 8192   ? 1000
                                     : N <= 32768  ? 200
                                                   : 100;
        ...
        if (verbose)
        {
            ...
            for (size_t i = 0; i < g_nfacts; i++)
                ord[i] = i;
            for (size_t i = 1; i < g_nfacts; i++)
            {
                size_t k = ord[i];
                double kn = ans[k] > 0 ? ans[k] : 1e18;
                size_t j = i;
                while (j > 0 && (ans[ord[j - 1]] <= 0 || ans[ord[j - 1]] > kn))
                {
                    ord[j] = ord[j - 1];
                    j--;
                }
        ...
        char bl[128];
        fstr(g_facts[bi].factors, g_facts[bi].nfactors, bl, sizeof(bl));
        printf("\n  PLANNER:  %-24s %9.0f ns\n", cfact, cns);
        printf("  OPTIMAL:  %-24s %9.0f ns", bl, bns);
        if (cns > 0 && bns > 0 && cns > bns * 1.05)
            printf("  (%.1fx faster!)", cns / bns);
        else if (cns > 0 && bns > 0)
            printf("  (planner OK)");
        printf("\n\n");
