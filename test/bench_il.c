...
/* R=16 DIF now in unified fft_radix16_dispatch.h — no separate DIF header */
/* R=20 DIF now in unified fft_radix20_dispatch.h — no separate DIF header */
...
int main(void)
{
    ...
    for (il_radix_t *ir = radixes; ir->R; ir++) {
        ...
        for (const size_t *kp = K_VALUES; *kp; kp++) {
            ...
            const char *winner;
            if (il_wins)
                winner = (native_ns > 0 && native_ns <= hybrid_ns) ?
                    "\033[92mIL(nat)\033[0m" : "\033[92mIL(hyb)\033[0m";
            else if (ratio > 1.05)
                winner = (native_ns > 0 && native_ns <= hybrid_ns) ?
                    "\033[93mil(nat)?\033[0m" : "\033[93mil(hyb)?\033[0m";
            else if (ratio < 0.90)
                winner = "SPLIT";
            else
                winner = "~tie";
    ...
    for (il_radix_t *ir = radixes; ir->R; ir++) {
        ...
        if (!have_split || !have_il) continue;
