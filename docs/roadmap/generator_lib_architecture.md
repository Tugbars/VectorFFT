...
| **M6** | 🔴 **Thread `Render.ctx`. Delete `emit_state.ml`.** (6a) the 22 config flags; (6b) the ~10 scratch cells. | ✅ at default env **AND** `VFFT_DUP=1` | Cheaper than feared — ~5 functions gain `~ctx` (§11.3). 6b is the semantically load-bearing half: `il_stash`'s correctness depends on the *scheduler* emitting re/im in adjacent order, enforced only by a runtime `failwith` at `emit_c.ml:2004`. **Standing rule if a row moves: "the gate decides, not the argument" — record it as a latent bug the corpus encodes and re-bank with a numeric gate; do not preserve the leak.** (D-2: `current_tw_perpos`/`current_tw_linear`, `codelet_oop.ml:1965-1966`, never cleared, leaking into six later quadrants.) Apply the xcheck pattern to the flag reads. 🟢 **M6.0 EXECUTED 2026-08-14 (Knobs, the M5 row's second half, landed beside its consumers)**: `kernel/knobs.ml`+`.mli` — the env REGISTRY: 8 byte-affecting keys + 8 Trace keys (the §11.5 split), each read ONCE lazily; all **29 scattered `Sys.getenv` sites in schedule/fma_passes/simplify swapped** (zero getenv left in the three modules), consumers keep their historical parses on the raw `string option` so bytes cannot move (typed recipes arrive with ctx at M6.1). Trace liveness verified (FACTOR_TRACE through Knobs produces the trace), **full corpus gate PASS**. 🟢 **M6.1 EXECUTED 2026-08-14**: `Emit_render.Scratch` landed — the 10 per-emission cells (ls_mode, regalloc, emit_position, fence_only, il_seen, il_pending, il_stash, dup_barrier_tags, unpin_candidates, hoisted_const_tags) as ONE record, created fresh by each driver (gen_main + codelet_oop's emit_codelet/emit_k1_mono + codelet_zsplit). §11.3's measurement confirmed to the letter: the doc's 4 emit_render defs + il_in_name = 5 gained `~sc` (plus wrappers/dispatchers the compiler enumerated: render_node_def, body_preamble, filter_inline_set_cross_pass, emit_load_edge, oop's 8-helper chain). THREE reset disciplines RETIRED: il_reset became a Scratch function, emit_k1_mono's Fun.protect fence save/restore deleted outright (fresh-per-emission makes it meaningless), gen_main's conditional dup_barrier_tags replacement became a fill of the driver's own record — D-1 is now unrepresentable. 21/21 build, cx PASS, **full corpus gate PASS**, and the row's dual-env requirement met: **VFFT_DUP=1 byte-identical** vs pre-change references (modulo the pre-existing random /tmp/vfft_dup_order* name stamped in comments — nondeterministic by design, recorded not hidden). 🟢 **M6.2 EXECUTED 2026-08-14 — M6 COMPLETE, `emit_state.ml` DELETED.** `Emit_render.Cfg` = the per-emission CONFIG VIEW (immutable value, flows FORWARD): the driver builds it from the descriptor (gen_main's ccfg record replaces every remaining global write), emit_c receives `~cfg` (kernel type — T1's direction preserved: the kernel never names Codelet), and THE THREE TWIDDLE BACK-EDGES ARE DEAD — codelet_oop passes Tw_perpos/Tw_linear(nlegs), zsplit passes Tw_zsplit(off) with its Fun.protect resets converted to local ref updates, the D-2 leak site and emit_k1_mono's hand neutralize-list deleted. The tw trio turned out to carry PAYLOADS (linear=nlegs int, zsplit=offset string) — the enum grew them plus historical-shape projections. Both `open Emit_state` lines died as warning-33 errors — the compiler itself announced the wean. 21/21 build, cx PASS, **VFFT_DUP=1 6/6 byte-identical, full corpus gate PASS with xcheck armed** — with the back-edge buffer gone from the tree and its stanza. STILL REMAINING: M6.1 Render.ctx + Scratch threading (~5 functions per §11.3), M6.2 config reads from the descriptor + DELETE emit_state — the X6 per-cell ledger is the map; gate at default env AND VFFT_DUP=1. |
| **M7** | **Extract `Simd`** from `emit_c`'s **feature-blind** lattice ranges + the 4 hand copies + the ≥5 transposes. Text-preserving. | ✅ | ⚠ **Scope corrected twice.** (a) The stated justification — *"`codelet_oop`'s UnitLeg path `failwith`s to this day"* — is **wrong**: those stubs have **zero callers repo-wide** and 22 UnitLeg codelets ship and reproduce 100 %. M7 is de-duplication, not unblocking. (b) 🔴 **M7 cannot be both text-preserving and feature-blind as originally scoped**: the lattice ranges read `strided_r2c`/`strided_r2c_bwd` at **11 lines / 14 occurrences** (⚠ **corrected from “24 sites”**, which was inherited from a review note: `grep -o 'strided_r2c[a-z_]*' emit_c.ml | wc -l` = 14, on lines 547,552,562,566,574,588,636,747,1084,3269,3605 — 9 lines in the 522–1445 load lattice, 2 in the 3186–3994 store lattice. The substantive point stands; the magnitude does not), and the r2c/c2r math §15 declares out-of-scope is *physically inside them* (`emit_c.ml:3269` sits inside the 3190–3994 store lattice; `:843-860` inside the 522–1445 load lattice). **Therefore M7 extracts ONLY the feature-blind sub-ranges; the r2c-conditional regions stay put until M8 moves them into `Real`.** The file names this seam itself at `:4117-4125`. 🟢 **M7 EXECUTED 2026-08-14**: `kernel/simd.ml`+`.mli` — the four FEATURE-BLIND plain arms of the strided load/store lattices moved BYTE-VERBATIM (guarded line-splice; zero `cfg.`/`sc.`/`Knobs.` reads verified per span): AVX2 4×4 load (91 lines), AVX-512 8×8 load (222), AVX2 inverse-4×4 store (90), AVX-512 inverse-8×8 store (233) → `Simd.load_transpose_4x4/8x8 ~buf ~groups` / `Simd.store_transpose_4x4/8x8 ~buf ~groups`; emit_c 4015→3360 lines. The il_in/il_out arms (which physically CONTAIN the r2c merge/split branches) and the r2c store arms stay in emit_c per the corrected scope — they relocate into their families at M8. Hand copies in codelet_oop/codelet_cil/cx_render NOT consolidated (different emitted-text shapes; the X-ray’s “6 modules → 1” stays partial until M8 relocation makes the texts comparable). Clean build, **full corpus gate PASS — 1403/1432, drift 0; strided/avx2 32/32, strided/avx512 22/22.** |
| **M8** | 🔴 **Feature modules — PER FAMILY, SMALLEST FIRST, each independently gated + xchecked.** `Cascade_z` (32) → `Real` (318) → `C2c_il` (233) → `C2c_split` (849). What remains of `emit_c` becomes `Emit_body`. | ✅ each | **THE RISKIEST STEP — see §16 R1. GATED ON A GO/NO-GO PROBE.** Four small red-or-green events instead of one large one. ⚠ **`Cascade_z` first is for an early visible payoff, NOT for risk information** — `codelet_zsplit` already has zero own refs and `Fun.protect`s its one borrowed global, so that sub-step **cuts nothing and carries no risk signal**. The first *informative* sub-step is `Real`. 🔴 **Do NOT promise the spill path shrinks:** it reads **2** distinct globals across ~152 branch points, is 100 % unchanged since the tree move, and stays 700–900 lines. **It moves whole; it does not decompose.** 🟢 **M8.1–M8.4 EXECUTED 2026-08-14 (each gated, all PASS 1403/1432 drift 0):** (1) `Cascade_z` = codelet_zsplit RENAMED (as priced: cuts nothing). (2) *(infrastructure)* `emit_c.ml` → **`emit_body.ml`** (module Emit_body; the two zero-caller `emit_strided_*` failwith stubs DELETED; the G7 `.mli` deferred to M8.5 when the hook surface is final). (3) **`Real` — the informative cut, GREEN**: kernel gains `Emit_body.family_hooks` — 5 positional `(Buffer.t -> unit) option` slots (strided_prologue / strided_locals / strided_load / strided_store / trailer), `no_hooks` default; the cfg dispatch tests STAY kernel-side and `failwith` LOUDLY if a family-owned arm is reached unhooked. `gen/real.ml` (700 lines) hosts, moved VERBATIM: the §6a36 fused conjugate-split stores (both widths), the §6a38/§6a45 merge-load prologues, the rio alias prologue, the c2r _hx lane decls, and the hc_ranged advance trailer — every span verified closed over ONLY buf/cfg/isa/radix before the cut. gen_main routes real-family cfgs (r2cf/r2cb/r2c_term*/hc*/r2r/strided_r2c*) through Real. ⚠ **Scope deviation, recorded:** the thin preamble-ladder arms (body_preamble + v-loop glue, 3–10 lines each) STAY in Emit_body — moving them drags sc/spill/assigns/anyk_tail/emit_v_loop_header across the seam for zero de-duplication (§9's own “differs only in ABI” rule). §5.4's “expected byte seam” (the r2c math) did NOT fire — the move was verbatim, bytes held. (4) `C2c_il` = codelet_cil RENAMED (§9.1 prices it as a rename). X3's load-bearing condition holds by construction: the spill cluster never moved — it stays WHOLE in Emit_body. REMAINING: M8.5 `C2c_split` (codelet_oop absorb + in_place/twidsq/oop-generic/strided-il arms via the same hooks) + the Emit_body `.mli` (G7) + the G6 disposition (record codelet_oop's private 578-line spill emitter as surviving duplication or merge it — owner call). 🟢🟢 **M8.5 EXECUTED 2026-08-14 — M8 COMPLETE, all four family events gated green (1403/1432 drift 0 each).** (a) `codelet_oop.ml` → **`gen/c2c_split.ml`** (module C2c_split; 35 gen_main refs + codelet.ml comment + dune; banner carries the G6 record). (b) family_hooks grew THREE slots (`strided_idx` / `strided_load_il` / `strided_store_il`); the strided-il lattice arms (il permute-index decls, il_in deinterleave loads ×4/×8, il_out interleave stores ×4/×8, ~340 lines) moved VERBATIM into **`C2c_split.emit_engine`** — the engine route: gen_main's whole non-real emit_codelet path now enters through it, so **Emit_body has exactly TWO consumers (Real, C2c_split) and zero direct gen_main calls** — G7's prediction, landed. real.ml switched to `{ no_hooks with ... }` so future slot growth is non-breaking. (c) **`emit_body.mli` written (G7 satisfied)** — family_hooks (field order matching), no_hooks, emit_codelet's full optional-heavy signature, the no-op survivor. (d) **G6 DISPOSED: surviving duplication, recorded** in §15 (item 4↳) + the c2c_split banner — merging the private 578-line spill emitter is a numerical-campaign job. STAYED engine-side, recorded: the ip-il index decls (uses woven through emit_store's render path — the engine moves whole or not at all), the plain-Simd lattice defaults, the 1-line ilo_nt sfence, the thin preamble-ladder arms (same rule as M8.3), and the SPILL CLUSTER — whole, untouched, per X3's load-bearing condition. |
| **M9** | **Three dune libraries** (`vfft_kernel` / `vfft_cx` / `vfft_gen`, with `(implicit_transitive_deps false)`) **+ the remaining `.mli`** (chain tails, emitters). | ✅ | Interfaces and packaging only. 24 of 34 modules have zero external consumers ⇒ near-zero `bin/` churn (but 9 files `open Vfft_v2`, including all 4 debug tools). **This is where T1 stops being a rule and becomes a build error.** ⚠ Attempting the split while back-edges are live produces a dune cycle reported as an unreadable error that gets blamed on the refactor — which is why it lands after M6. ⚠ `(private_modules)` is **not** the mechanism (§9.2). 🟢 **PHYSICAL PRECURSOR EXECUTED 2026-08-14 (owner request: "put the old lib to another folder… the 'current' lib too — so I can follow the new files easier")**: `lib/` now spans `kernel/` (21 modules incl. layout + the emit chain pending decomposition), `cx/` (7), `gen/` (6: codelet_oop/zsplit/cil, dft_r2c, coverage, gen_main) via `(include_subdirs unqualified)` — ONE library still, every module name and reference unchanged, 19/19 build + full gate PASS. New-architecture modules now land in their destined folder on arrival; M9 proper converts the folders into the three enforced libraries. (Also: `generator/old-lib/` moved to `src/dag-fft-compiler/archive/old-lib/` the same day — §9's revised owner decision.)  🟢🟢 **M9 EXECUTED 2026-08-14 — T1 IS NOW A BUILD ERROR.** (a) THREE LIBRARIES: `vfft_kernel` (unix) / `vfft_cx` (→kernel) / `vfft_gen` (→kernel,cx,unix), each **`(wrapped false)`** so every module name and intra-repo reference survived unchanged (basenames globally unique by repo law); `(implicit_transitive_deps false)` in dune-project; `lib/dune` (the vfft_v2 stanza) DELETED. (b) CONSUMER CHURN was larger than the row's “9 files”: **18 sources + 4 dune stanzas** — bin (13 exes), bin_test (6), plus ⚠ `emit_tool/` and `facdrv/` which the row never inventoried (the corpus gate's own 19-target build caught them — the gate audits the plan). `open Vfft_v2` lines deleted and `Vfft_v2.` prefixes stripped (unwrapped = global names). (c) **The split compiled FIRST TRY** — zero layering violations existed: M1's chain cuts + M6's forward-flow + M8's two-consumer engine made T1 true before it was enforced. (d) M9b: the seven missing kernel `.mli` (algsimp 41 · dft 33 · dft_recurse 37 · dft_select 7 · emit_render 120 · fma_passes 19 · simplify 22 lines) installed as full-surface interfaces DERIVED via `ocamlc -i` against the built cmis — the surfaces are tiny because M1's requalification already shrank them; kernel is now 21/21 `.mli`-covered. Exempt per §10, recorded: the cx seven (M12a's business), gen's single-consumer families/driver, `dft_r2c` (=Real_math, owner-ratified), `coverage` (deferred to M10's Corpus inversion). Full corpus gate PASS after each half (1403/1432, drift 0). |
| **M10** | 🔴 **`Corpus` from `recipes`.** | ⚠️ **CROSS-TREE — the point of no return** | ⚠ **Re-priced.** `Coverage.files` is not internal: it feeds `bin/gen_set.ml:62` **and six registry emitters**, which are the `(deps …)` of **12 `(mode promote)` rules writing TRACKED headers** — `registry_avx2.h` (93 KB), `registry_avx512.h` (95 KB), and the rfft/oop/trig/strided/c2r pairs — consumed by the C build, whose two systems already **disagree about what the corpus is** (CMake 598 files and globs a nonexistent `codelets/il/`; `build.py` 863). `coverage.ml:29-31` says so itself: *"regen + registry emit must ship together."* Raising `Coverage` 75 % → ~98.5 % lands **71 additions in existing quadrants**, so registries change, so new ABI-typed slots appear, so the runtime dispatcher changes. **Everything up to M9 is a `git revert` inside `generator/lib`; M10 is the first step whose output is consumed by C code outside the generator.** Requires its own C-side gate.  🟢 **M10a EXECUTED 2026-08-14 — the INVERSION, output-preserving; the raise is NOT taken.** `coverage.ml` → **`corpus.ml`+`.mli`**: the family matrices moved verbatim as PRIVATE scaffold (`matrix_files`); the public surface derives through `Codelet.t` — `cells` / `files` (via `to_argv`) — under TWO LAWS enforced lazily at first use: **round-trip verbatim** (`of_argv (tail@[--emit-c])` must `to_argv` back EXACTLY ⇒ provenance == coverage == regen recipe is now a CHECKED identity) and **uniqueness** (per-dir filename + global canonical argv). Laws fire in gen_set + the six registry emitters — the tree/registry writers refuse a lawless corpus; `gen_radix` never pays (lazy). All 12 quadrants passed both laws first run. 7 consumers rewired on exact code tokens; ⚠ the emitted banner text "Coverage.files" in the 12 headers KEPT deliberately — rewording waits for the first real registry change. `expected_counts` deleted (zero consumers). **THE C-SIDE GATE for an output-preserving step = registry byte-identity, and it held**: `dune build @default` re-promoted all 12 registry headers BYTE-IDENTICAL (git-diff empty) ⇒ the C build is provably untouched; full corpus gate PASS. ⚠ **DISCOVERED PRE-EXISTING DRIFT**: `plan_executors.h` (last promoted 2026-07-22, 40c48156) is STALE vs `spike_wisdom.txt` (refreshed 2026-08-13, a1e9d932 — the zr2c kind-5 arc): re-promotion produces +1172/−52 lines. REVERTED here — no byte change rides a structural step; it needs its own intentional re-promotion + C build check. **REMAINING (M10b, unstarted, owner-gated): the coverage RAISE** 75%→~98.5% (71 additions ⇒ registries change ⇒ new ABI slots ⇒ dispatcher) — the true announced byte-changing event, with the full C-side gate (build both C systems + runtime gates); cil/zil corpus entry stays coupled to M12a.  🟢🟢 **M10b EXECUTED 2026-08-15 — the raise, made OUTPUT-PRESERVING by design: 75 cells, FOUR REGISTRY-INVISIBLE QUADRANTS.** The additions dispatch via wisdom/il2p, never via registry slots — so instead of "registries change ⇒ new ABI slots ⇒ dispatcher changes", the new quadrants (`oop-edges-avx2/512`, `strided-r2c-avx2/512`) are named by NO registry emitter: **all 12 registries re-promoted BYTE-IDENTICAL, the C build provably untouched — the C-side gate reduces to registry byte-identity and it held.** Cells: the 32 zr2c strided-r2c pairs (the D2 arc's production kernels), 37 oop edge/variant cells (post-tw · UL/UGUL · spec extras), the 6 k1_mono champions — LITERAL rows from recorded provenance, corpus laws green, **entry gate 75/75 byte-identical** (warm gen_set to temp vs shipped; the M12b argv[0] normalization is what makes their stamps replay-stable). Excluded with cause (recorded in corpus.ml): the 29 non-reproducers (pool-sunset / announced-numeric-event material), the 9 sed-renamed zil variants (emitter suffix knob needed), and radix256_r2c_term_ls_r8_avx512 — its recorded argv has --isa AFTER --emit-c (the M5 placement quirk emit_one cannot reproduce); it enters when that argv order normalizes, naturally with its known ULP fix. **Corpus: 1074 → 1402/1432 cells (75.0% → 97.9%).** Full corpus gate PASS. ⚠ REMAINING adjacent hygiene, not taken here: the CMake-598 vs build.py-863 corpus-view reconciliation (its own C-side audit + build gates) and the roadmapped plan_executors.h re-promotion. |
| **M11** | **`Pipeline` becomes the sole cascade.** Delete the inline copy at `gen_main.ml:906-1430`; rewire `bin/dbg_eval.ml:121-134`. | ⚠️ **default env only** | 🔴 **The one step the gate cannot fully see.** **Split it:** (a) the *mechanical* unification, with the two divergent passes preserved as explicit `pipeline_recipe` fields so default emission is byte-identical — gateable, ship it; (b) the *behavioural* question *"should `VFFT_FORCE_REASSOC` now fire on oop and zsplit?"* — an **owner decision needing a race, not a diff**. Run the gate for (a) under a **6-cell env matrix** (`VFFT_FORCE_REASSOC`, `VFFT_DUP`, `VFFT_COLLECT_M`, `VFFT_DEEP_COLLECT`, `VFFT_SCHED_ORDER`, `VFFT_NO_SUBDEDUP`), recording a baseline per cell.  🟢 **M11a EXECUTED 2026-08-14 — Pipeline IS the sole cascade; the mechanical half, gated on the env matrix.** gen_main's ~555-line inline cascade DELETED; `Pipeline.prepare_codelet` gained `~recipe : recipe` — **REQUIRED, not optional** (OCaml can't erase a trailing optional with no positional; and explicit-at-every-caller is the better surface anyway: each route DECLARES its arms). `recipe = { butterfly_share : bool; dup : dup_ctx option }` where `dup_ctx = { uarch; barrier_sink }` — the two historically-gen_main-only arms travel as fields: the main driver enables both (dup block moved VERBATIM, two substitutions: barrier sink + uarch via ctx); c2c_split/cascade_z pass `default_recipe` (historical behavior preserved). The spill remap chain gained the bsm walk (empty table ⇒ identity for default_recipe callers). reassoc env-override stays CALLER policy in gen_main — M11b's question untouched. ⚠ **dbg_eval RECLASSIFIED**: it is a per-pass numeric PROBER (steps inside the cascade with check_t between passes), not a third cascade copy — "rewiring" it to one call would destroy its purpose; instead it gained a "Pipeline (sole cascade)" cross-check tail (PASS 1.1e-15 on the dct1 N=9 reproducer). **ENV MATRIX GATE: 7 cells × 26-recipe sample (all routes: ip flat/log3/prime/blocked-spill, oop n1/t1p/spec/post-tw, strided plain/bwd/r2c, rfft, c2r, trig, cil, zsplit, k1_mono) — ALL 26/26 BYTE-IDENTICAL pre-vs-post in every cell**: default, FORCE_REASSOC=1, FORCE_REASSOC=0, DUP=1 (temp-order-file name normalized), COLLECT_M=1, DEEP_COLLECT=1, NO_SUBDEDUP=1. ⚠ recorded substitution: the VFFT_SCHED_ORDER axis is exercised THROUGH the DUP=1 cell — the var only ever carries the dup flow's per-DAG temp file (Knobs.sched_order); a standalone cell has no meaningful value. Full corpus gate PASS. ⚠ Knobs NOTE (pre-existing, recorded): `sched_order` is lazy once-per-process; in a warm gen_set process a dup putenv AFTER the first force would be invisible — inert today (gen_set never runs VFFT_DUP=1). **REMAINING: M11b** — should oop/zsplit adopt the env override + arms? OWNER RACE, not a diff. 🟢🟢 **M11b EXECUTED 2026-08-14 — RACED, VERDICT: NO. The policy stands; M11 COMPLETE.** Override WIRED into c2c_split + cascade_z (byte-identical default, gate PASS) ⇒ VFFT_FORCE_REASSOC now reaches all three routes as a DIAGNOSTIC knob. CENSUS under FORCE_REASSOC=1: 1128/1432 BODY_DIFFERS (oop 145/147, zsplit 31/32, cil 0). STATIC: CT/pow2 −17..23% vector ops (r128 oop −547, k1_mono128 −194) but primes +5..23%. 🔴 **39/121 avx2 oop reassoc variants are INVALID C** (undeclared t-values; oop emitter declared/reference sets diverge under reassoc DAGs; all R∈{16..128} oop shapes; zsplit 31/31 clean) — recorded defect, deliberately unfixed (diagnostic-only path that loses). RACE (m11b_ab.c per the cil_ab protocol, 5 cells × 3 lanes incl __ctl floor, correctness ≤5e-15 first): **10/10 cell-runs ra/ship ≥ 1.0 across two runs (1.08–1.57, noisy-day magnitudes NOT banked; ctl drift ±12%)** — reassoc NEVER wins; the −20% static cut is a TIME LOSS (the static≠time META is 4-for-4; reassoc flattens butterfly FMA chains → worse ILP). **needs_reassoc UNCHANGED; butterfly_share/dup stay main-route-only.** Artifacts: docs/research/m11b_reassoc_race/ (CONCLUSIONS.md + harness). |
| **M12a** | **The IL state fix.** `Cx_ir`'s 7 cells + `Cx_math`'s 3 + `Cx_render`'s 1 become a `Cx_ctx` created per emission; the 265 zil files enter `Corpus` and therefore the gate. | ✅ | **The campaign's shared blind spot.** 233 shipped files (16 %), an emitter absent from `Coverage` entirely, and the subsystem that **regrew a globals buffer within weeks of its own cleanup**. cil is safe today **only because it is not in `Coverage`**; adding it to the warm `gen_set` process activates its missing resets (`codelet_cil.ml:168-180` launders 4 labelled args into `Cx_ir` globals and never resets them) — **so M12a's state fix must land before or with the `Corpus` entry, never after.**  🟢🟢 **M12a EXECUTED 2026-08-15 — Cx_ctx LANDED, cil IS IN THE CORPUS, and the warm process is PROVEN CLEAN.** (a) **`Cx_ir.ctx`** = the 9 policy/knob cells as ONE per-emission record (tw_log3/tw_pre/st_turn/st_turn_gs + MUTABLE mono_spill_slots + tangent/w32_combine/wing_enabled/rotfma capturing their VFFT_CX_* envs at creation; `make_ctx` ORs the --cil-tangent CLI flag). The launder block (codelet_cil:171–183) is GONE; C2c_il.emit/emit_k1 create ctx and thread it (compiler-guided wean: butterfly_pair/dft_cx/dft_small/dft_chain/render gained `~ctx`; gen_main's `Cx_math.tangent := true` CLI write became a local ref + params). Full corpus gate PASS. (b) **THE CORPUS ENTRY: two quadrants** `zil-boundary` (32) + `zil-pure` (221) as **LITERAL cells derived from recipes.tsv** — the sets are benchmark-derived, no honest comprehension exists; excluded: 6 stale pure_il, 6 tangent sunset copies, 9 sed-renamed replay rows (regen would overwrite shipped bytes). dir_of_quadrant special-cases the two names. All 253 pass the corpus LAWS (round-trip + uniqueness). (c) **THE PROOF THE ROW DEMANDED — warm gen_set regen of all 253 in ONE process: 221/253 fully byte-identical, 32/32 boundary cells BODY-identical** — the sole delta is the argv[0] token in the provenance line (the shipped 32 recorded a full WSL path; gen_set stamps the logical name): **the KNOWN §14.3 123-file hermeticity item, queued for its own announced regen — NOT a state leak.** Zero cross-emission contamination in either family — the regrown-globals blind spot is structurally closed. |
| **M12b** | 🔴 **`codelet_cil` gains a `provenance_block` call.** | ❌ **BYTE-CHANGING — announced baseline regeneration** | ⚠ **Re-classified.** `provenance_block` emits **before `#include`**; adding the call to `codelet_cil` moves **233 files from `IDENTICAL` to `PROLOGUE_ONLY`** ⇒ **GATE FAIL on 16 % of the corpus**. This is arithmetic, not risk. It gets exactly the treatment the argv[0] fix gets: a **separately announced, separately reviewed baseline regeneration**, never folded into a structural step. It is the prerequisite for raising machine-readable reproducibility past 84 % and for making the 221 recovered recipes *recorded*.  🟢🟢 **M12b EXECUTED 2026-08-15 — THE ANNOUNCED REGEN, widened to cover the argv[0] item: 84.0% → 99.4% self-describing.** (a) **Hermetic argv[0]**: normalized on the RESOLVED array inside `provenance_block` — ⚠ the doc's "one line in the None arm" would have been DEAD CODE: `Gen_main.run` sets `provenance_argv` unconditionally (caught because the first fix silently did nothing — tested by exec -a before trusting the gate). (b) **Provenance for the silent emitters**: `C2c_il.emit` + `C2c_il.emit_k1` + **`C2c_split.emit_k1_mono`** (the sixth entry point — the cil edit alone missed the 6 k1_mono files; the flip-count audit caught it: 33 oop flips where 39 were due). Env-overrides watchlist extended with the CX/cascade knobs (unset in every baseline run ⇒ zero byte impact). (c) **THE REGEN: 329 files** (221 pure_il + 6 k1_mono gained headers; 102 argv[0] normalizations across oop/rfft/strided/boundary) — every one audited BODY-IDENTICAL (strip-provenance diff EMPTY, 329/329) before copy-in; the untouchables (16 avx512_regen orphans, 6 tangent, 6 stale pure_il, 1 rfft) held their recorded classes. Verdict-transition audit: previously-IDENTICAL → {IDENTICAL, PROLOGUE_ONLY} ONLY, zero body flips, both pre-regen runs. (d) recipes.tsv: 329 shas + 102 argv0 columns normalized; `manifest` + `record` re-blessed per G4; **final verify GATE PASS on the new baseline.** Self-describing files: 1203/1432 (84.0%) → **1424/1432 (99.4%)** — the residual 8 = the 6 tangent sunset copies + 2 stale pure_il, all in the known non-reproducing population. |
...
4. **Expect the line count to go UP first.** The one directly comparable data point in this repo: `02f7c633`
   ...
   ↳ 🟢 **(G6, recorded 2026-08-14) `C2c_split` keeps a private 578-line body/spill emitter** — `emit_body_spill` (plus `emit_body_monolithic`/`emit_butterfly_body`), duplicating the kernel engine's spill machinery. **SURVIVING DUPLICATION by the M8.5 disposition**: merging it into `Emit_body` is a byte-risk numerical-campaign job with its own gate, not a structural move. Recorded in the c2c_split.ml banner too.
