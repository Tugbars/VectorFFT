...
conjugate twiddle. The blocked transpose threads by blocks; the 2D child
threads by its own verdict.

The natural class therefore costs one sweep over the scrambled class —
MKL's six-step pays its transposes too — and a transposed-write twin of the
row terminator (the row's outputs stored k2-major directly) is the lever if
the race asks for it; it is not in this design.
...
by blocks with the same pool. `VFFT_K1_FS=N1xN2` pins a split for a probe
(never banks). No fallback: a cell whose splits all refuse (a missing row
plan) refuses.
...
- [ ] 6. Race and bank at 2^18..2^22, both classes, both placements, T=1 and
      T=8, on a quiet machine; the shipped store gains the rows.
- [ ] 7. Measure against MKL (quiet machine); rule what the numbers say at
      262144 (ZTURN-T or the four-step) and on the natural class's transpose.
- [ ] 8. Records: `v1_0_results.md` (the 1D K=1 section's band extended, in
