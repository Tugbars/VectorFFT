...
## 3. The N ≥ 2048 tier is ZTURN-T. The cascade is deleted.

The boundary-split cascade this section once described was deleted whole on
2026-09-15 (files, wisdom tokens, codelets, OCaml); ZTURN-T replaces it at
every N from 2048 to 262144, alone (the owner's ruling of 2026-09-13). Its
stage kernels keep the cascade's layout contract, packed `z` at the buffer
boundary and split re/im planes in the interior, and live in
`codelets/zil/avx2/ztt/` (README there); the pow2 cells run as the fused
drivers in `generator/generated/fused_codelets/`. The design is
`docs/design/ztt_odd_design.md` and the ZTURN-T notes it links; the odd
middle radices `msz`/`mszt` that share the layout live in
`codelets/zil/avx2/flat/odd_mid/`.
