...
| verdict | `chain= wl= tf= sw= ro= rbk= turn= csk=` on the cell's `lay=il` row, one race at one thread; at T > 1 the same race runs every arm through its route's threaded walk and banks them on the cell's row at that T (`nthreads=T` in the key, the same tokens), served at that T only |
...
| the race, the bank | `_il2d_axis_race` in `il2d_tier.h`; tokens `ro= rbk= turn= csk=` on the cell's row at the plan's T (`nthreads=`; `wisdom2_2d_reader.h`) |
