...
its partitions at the plan's T and banks `cmt= cmts= cmtp=` on its own row at that T (`nthreads=`).
The cycle form binds its cycles over the plane team it serves.
...
`nthreads=` row, `cmt= cmts= cmtp=`; the flat arm's `chain1=`), plus whatever the natural form
