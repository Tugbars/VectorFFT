...
|---|---|---|---|---|
| `calibrate_k1.exe` | `build_tuned/benches/calibrate_k1.c` | `<argv1>/wisdom2_oop.txt` (kind-3 + kind-4) | **PORTED 2026-08-20**: thin driver → `vfft_sp_dp_plan_and_bank` → planner opens a writable vw2 store at `<argv1>`, banks via `vw2_oop_bank_entry`, `vw2_save`. `_sp_merge_bank` + `k1_bank_tmp.txt` DELETED (tombstone in `dp_planner_split_oop.h`); sub-2048 kind-4 filter now lives in the family codec. Stale spike banner fixed | **Wave 1: DONE** (smoke: legacy file byte-unchanged, fresh verdict banked concrete-key) |
| `calibrate_zchain.exe` | `build_tuned/benches/calibrate_zchain.c` | `<argv1>/wisdom2_oop.txt` (kind-4) | **PORTED 2026-08-20**: thin driver → `vfft_il_dp_bank_scr_top` (`dp_planner_il.h`) — entry built by the planner's emit path (zt_tw/zt_l1 carried by the ONE constructor; the silent-width-drop class is unrepresentable). Own `bank()` + hand-built entry DELETED | **Wave 1: DONE** (smoke 2048: winner banked `metric=joint2`, freeze held) |
| `zturn_wisdom_width_gate.exe` | `build_tuned/benches/zturn_wisdom_width_gate.c` | `<--wisdir>/wisdom2_oop.txt` (synthetic kind-4 injections, byte-restored) + `_wisdom_width_gate.log` tap | **PORTED 2026-08-20**: thin driver → `vfft_oop_width_gate_run` (`src/core/oop/oop_width_gate.h`); injections go through `vw2_oop_bank_entry` (shipped constructor, no hand encoding), store byte-snapshot/restored. The hand-encoder is gone | **Wave 1: DONE** (6/6 PASS on the dual scratch; also flushed out the kind-4 metric-law bug — see campaign doc) |
