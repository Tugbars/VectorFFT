/**
 * @file fft_radix7_sse2_n1.h
 * @brief SSE2 Radix-7 Rader Butterfly - TWIDDLE-LESS (N1) First Stage Optimization
 *
 * @details
 ...
 * ✅✅ PRESERVED: Separate mul+add (SSE2 has no FMA)
 * ✅✅ PRESERVED: U2 pipeline (process k, k+2 simultaneously)
 * ✅✅ PRESERVED: Store-time adds (frees 6 XMM registers)
 * 
 * CODE REUSE:
 * ===========
 * This header REUSES most functions from fft_radix7_sse2.h:
 * - load_7_lanes_sse2_soa
 * - store_7_lanes_sse2_soa / store_7_lanes_sse2_stream_soa
 * - broadcast_rader_twiddles_sse2_soa
 * - compute_y0_tree_sse2_soa
 * - permute_rader_inputs_sse2_soa
 * - rader_convolution_roundrobin_sse2_soa
 * - cmul_sse2_soa / cmul_add_sse2_soa
 * 
...
#ifndef FFT_RADIX7_SSE2_N1_H
#define FFT_RADIX7_SSE2_N1_H
...
#include "fft_radix7_sse2.h"  // Reuse most functions from standard version!

//==============================================================================
// CONFIGURATION (reuse from standard header)
//==============================================================================

#define R7_SSE2_N1_ALIGNMENT R7_SSE2_ALIGNMENT
#define R7_SSE2_N1_WIDTH R7_SSE2_WIDTH
#define R7_SSE2_N1_U2_WIDTH R7_SSE2_U2_WIDTH
 ...
 */
__attribute__((always_inline)) static inline void prefetch_7_lanes_sse2_n1_soa(
    int k, int K,
...
/**
 * @brief Single radix-7 butterfly - 2-wide SSE2 TRUE SoA - N1 VARIANT
 * @details
 ...
 *    - 2-wide processing (full SSE2 utilization)
 *    - Aligned loads/stores
 *    - Store-time adds (frees 6 XMM registers!)
 *    - Separate mul+add (SSE2 has no FMA)
 * 
 ...
 */
__attribute__((always_inline)) static inline void radix7_butterfly_single_sse2_n1_soa(
    int k, int K,
    const double *restrict in_re,
    const double *restrict in_im,
    ...
    bool use_nt)
{
    ...
    // Standard version would call: apply_stage_twiddles_sse2_soa(...)
 ...
 */
__attribute__((always_inline)) static inline void radix7_butterfly_dual_sse2_n1_soa(
    int ka, int kb, int K,
    const double *restrict in_re,
    const double *restrict in_im,
    const __m128d rader_tw_re[6],
    ...
    bool use_nt)
{
    ...
    //==========================================================================
    // Standard version would call apply_stage_twiddles_sse2_soa() twice here
 ...
 * @param out_im Output imaginary components (16-byte aligned)
 */
static void radix7_stage_sse2_n1_soa(
    int K,
    const double *restrict in_re,
    const double *restrict in_im,
    ...
    double *restrict out_im)
{
    ...
    for (; k <= K - R7_SSE2_N1_U2_WIDTH; k += R7_SSE2_N1_U2_WIDTH)
    {
        ...
        radix7_butterfly_dual_sse2_n1_soa(k, k + R7_SSE2_N1_WIDTH, K,
                                          in_re, in_im,
                                          rader_tw_re, rader_tw_im,
    ...
    for (; k <= K - R7_SSE2_N1_WIDTH; k += R7_SSE2_N1_WIDTH)
    {
