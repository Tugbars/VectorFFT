//==============================================================================
// fft_radix7_bv.c - Inverse Radix-7 Rader Butterfly (P0+P1 OPTIMIZED!)
//==============================================================================
//
...
//   - Rader twiddles use positive sign (inverse direction, precomputed by cache)
//   - Convolution algorithm is identical (only twiddle sign differs)
//
...
/**
 * @brief Ultra-optimized inverse radix-7 Rader butterfly (P0+P1 version)
 *
 ...
 * @param stage_tw      Precomputed SoA stage twiddles (6 blocks of K, inverse sign)
    ...
    int sub_len)
{
    ...
    // ─────────────────────────────────────────────────────────────────────
    // Fetch Rader Convolution Twiddles (SoA, INVERSE direction)
    // ─────────────────────────────────────────────────────────────────────

    const fft_twiddles_soa *rader_tw = get_rader_twiddles_soa(7, FFT_INVERSE);
    ...
    for (; k + 3 < K; k += 4)
    {
        ...
        if (use_streaming)
        {
            ...
            RADIX7_PIPELINE_4_BV_AVX512_STREAM_HOISTED_SOA_SPLIT(k, K, sub_outputs, stage_tw,
                                                                 tw_brd_re, tw_brd_im,
        ...
        else
        {
            ...
            RADIX7_PIPELINE_4_BV_AVX512_HOISTED_SOA_SPLIT(k, K, sub_outputs, stage_tw,
                                                          tw_brd_re, tw_brd_im,
                                                          output_buffer, sub_len);
        }
    }
    ...
    for (; k + 1 < K; k += 2)
    {
        ...
        {
            // P0+P1 optimized pipeline with streaming stores
            RADIX7_PIPELINE_2_BV_AVX2_STREAM_HOISTED_SOA_SPLIT(k, K, sub_outputs, stage_tw,
                                                               tw_brd_re, tw_brd_im,
                                                               output_buffer, sub_len);
        }
        else
        {
            // P0+P1 optimized pipeline with normal stores
            RADIX7_PIPELINE_2_BV_AVX2_HOISTED_SOA_SPLIT(k, K, sub_outputs, stage_tw,
                                                        tw_brd_re, tw_brd_im,
                                                        output_buffer, sub_len);
        }
    }
    ...
    for (; k < K; k++)
    {
        RADIX7_BUTTERFLY_SCALAR_BV_SOA(k, K, sub_outputs, stage_tw, rader_tw, output_buffer, sub_len);
    }
}
