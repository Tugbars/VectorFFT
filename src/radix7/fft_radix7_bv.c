 ...
 * @brief TRUE END-TO-END SoA Radix-7 FFT Implementation - Backward (Inverse)
 *
 * @details
 * This module implements native SoA radix-7 backward FFT using Rader's algorithm:
 * - Standard twiddle version (fft_radix7_bv) for general stages
 * - Twiddle-less n1 version (fft_radix7_bv_n1) for first stage
 *
...
/**
 * @brief Process range [k_start, k_end) with cascading SIMD - BACKWARD
 *
 * @details
 * Cascading pattern:
 ...
 */
static void radix7_process_range_soa_bv(
    int k_start,
    int k_end,
...
//==============================================================================
// MAIN FUNCTION: Radix-7 FFT Butterfly - NATIVE SoA - BACKWARD (WITH TWIDDLES)
//==============================================================================

/**
 * @brief Radix-7 FFT butterfly - NATIVE SoA - Backward (Inverse) FFT
 *
 * @details
 * Standard radix-7 DIT butterfly using Rader's algorithm for prime radix.
 * Uses precomputed stage and Rader twiddles.
 *
 ...
 *
 * @note For first stage where all twiddles ≈ 1, use fft_radix7_bv_n1() instead
 */
void fft_radix7_bv(
    double *restrict out_re,
    double *restrict out_im,
    ...
    int sub_len)
{
    ...
    radix7_process_range_soa_bv(0, K, in_re, in_im, out_re, out_im,
...
/**
 * @brief Process range [k_start, k_end) with cascading SIMD - N1 BACKWARD
 * 
 * @details
 ...
 * @param use_nt Use non-temporal stores
 */
static void radix7_process_range_soa_bv_n1(
    int k_start,
    int k_end,
    const double *restrict in_re,
    const double *restrict in_im,
...
//==============================================================================
// MAIN FUNCTION: Radix-7 FFT Butterfly - NATIVE SoA - BACKWARD (NO TWIDDLES)
//==============================================================================

/**
 * @brief Radix-7 FFT butterfly - NATIVE SoA - Backward FFT - NO TWIDDLES (n1)
 * 
 * @details
    ...
    int K)
{
    ...
    radix7_process_range_soa_bv_n1(0, K, in_re, in_im, out_re, out_im,
                                   rader_tw_re, rader_tw_im,
                                   K, use_nt);
