/**
 * @file fft_radix7_avx512_n1.h
 * @brief AVX-512 Radix-7 Rader Butterfly - TWIDDLE-LESS (N1) First Stage Optimization
 *
 * @details
 ...
 * ✅✅ PRESERVED: FMA usage (AVX-512 advantage)
 * ✅✅ PRESERVED: U2 pipeline (process k, k+8 simultaneously)
 * ✅✅ PRESERVED: Store-time adds (frees 6 ZMM registers)
 * 
 * CODE REUSE:
 * ===========
 * This header REUSES most functions from fft_radix7_avx512.h:
 * - load_7_lanes_avx512_soa
 * - store_7_lanes_avx512_soa / store_7_lanes_avx512_stream_soa
 * - broadcast_rader_twiddles_avx512_soa
 * - compute_y0_tree_avx512_soa
 * - permute_rader_inputs_avx512_soa
 * - rader_convolution_roundrobin_avx512_soa
 * - cmul_fma_avx512_soa / cmul_add_fma_avx512_soa
 * 
 * ONLY DIFFERENCE: Butterfly functions skip apply_stage_twiddles() call!
 *
 * TARGET: x86-64 CPUs with AVX-512F + AVX-512DQ (Skylake-X and later)
...
#ifndef FFT_RADIX7_AVX512_N1_H
#define FFT_RADIX7_AVX512_N1_H
...
#include "fft_radix7_avx512.h"  // Reuse most functions from standard version!

//==============================================================================
// CONFIGURATION (reuse from standard header)
//==============================================================================

#define R7_AVX512_N1_ALIGNMENT R7_AVX512_ALIGNMENT
#define R7_AVX512_N1_WIDTH R7_AVX512_WIDTH
#define R7_AVX512_N1_U2_WIDTH R7_AVX512_U2_WIDTH
 ...
 */
__attribute__((always_inline)) static inline void prefetch_7_lanes_avx512_n1_soa(
    int k, int K,
...
/**
 * @brief Single radix-7 butterfly - 8-wide AVX-512 TRUE SoA - N1 VARIANT
 * @details
 ...
 *    - 8-wide processing (full AVX-512 utilization)
 *    - Aligned loads/stores
 *    - Store-time adds (frees 6 ZMM registers!)
 * 
 * ⚡⚡⚡ N1 DIFFERENCE: NO apply_stage_twiddles() call!
 * ⚡⚡⚡ Saves 6 complex multiplies per butterfly
 * ⚡⚡⚡ 20-30% faster than standard version
 *
 * Process one butterfly: k..k+7 (8 complex values)
 *
 ...
 */
__attribute__((always_inline)) static inline void radix7_butterfly_single_avx512_n1_soa(
    int k, int K,
    const double *restrict in_re,
    const double *restrict in_im,
    ...
    bool use_nt)
{
    ...
    // Standard version would call: apply_stage_twiddles_avx512_soa(...)
    ...
    // STEP 6 & 7: Assemble outputs at store-time (CRITICAL: frees 6 ZMM!)
    ...
    if (use_nt)
    {
        store_7_lanes_avx512_stream_soa(k, K, out_re, out_im,
                                        y0_re, y0_im,
                                        _mm512_add_pd(x0_re, v0_re), _mm512_add_pd(x0_im, v0_im),  // y1
                                        _mm512_add_pd(x0_re, v4_re), _mm512_add_pd(x0_im, v4_im),  // y2
                                        _mm512_add_pd(x0_re, v5_re), _mm512_add_pd(x0_im, v5_im),  // y3
                                        _mm512_add_pd(x0_re, v2_re), _mm512_add_pd(x0_im, v2_im),  // y4
                                        _mm512_add_pd(x0_re, v1_re), _mm512_add_pd(x0_im, v1_im),  // y5
                                        _mm512_add_pd(x0_re, v3_re), _mm512_add_pd(x0_im, v3_im)); // y6
    }
    else
    {
 ...
 *
 * U2 PIPELINE STRUCTURE (AVX-512):
 * =================================
 * Process k and k+8 in parallel to maximize ILP.
 *
 * Key optimizations:
 ...
 * - Register reuse: 32 ZMM registers provide ample headroom
 * - Store-time adds: no ya1-ya6, yb1-yb6 temporaries (frees 12 ZMM!)
 * - Total register pressure: ~20 ZMM (comfortably within 32 ZMM budget)
 *
 ...
 */
__attribute__((always_inline)) static inline void radix7_butterfly_dual_avx512_n1_soa(
    int ka, int kb, int K,
    const double *restrict in_re,
    const double *restrict in_im,
    const __m512d rader_tw_re[6],
    ...
    bool use_nt)
{
    ...
    //==========================================================================
    // Standard version would call apply_stage_twiddles_avx512_soa() twice here
    ...
    // Do adds inline to avoid materializing ya1-ya6 and yb1-yb6 (frees 12 ZMM!)
    ...
    if (use_nt)
    {
        ...
        store_7_lanes_avx512_stream_soa(ka, K, out_re, out_im,
                                        ya0_re, ya0_im,
                                        _mm512_add_pd(xa0_re, va0_re), _mm512_add_pd(xa0_im, va0_im),  // ya1
                                        _mm512_add_pd(xa0_re, va4_re), _mm512_add_pd(xa0_im, va4_im),  // ya2
                                        _mm512_add_pd(xa0_re, va5_re), _mm512_add_pd(xa0_im, va5_im),  // ya3
                                        _mm512_add_pd(xa0_re, va2_re), _mm512_add_pd(xa0_im, va2_im),  // ya4
                                        _mm512_add_pd(xa0_re, va1_re), _mm512_add_pd(xa0_im, va1_im),  // ya5
                                        _mm512_add_pd(xa0_re, va3_re), _mm512_add_pd(xa0_im, va3_im)); // ya6

        // Butterfly B
        store_7_lanes_avx512_stream_soa(kb, K, out_re, out_im,
                                        yb0_re, yb0_im,
                                        _mm512_add_pd(xb0_re, vb0_re), _mm512_add_pd(xb0_im, vb0_im),  // yb1
                                        _mm512_add_pd(xb0_re, vb4_re), _mm512_add_pd(xb0_im, vb4_im),  // yb2
                                        _mm512_add_pd(xb0_re, vb5_re), _mm512_add_pd(xb0_im, vb5_im),  // yb3
                                        _mm512_add_pd(xb0_re, vb2_re), _mm512_add_pd(xb0_im, vb2_im),  // yb4
                                        _mm512_add_pd(xb0_re, vb1_re), _mm512_add_pd(xb0_im, vb1_im),  // yb5
                                        _mm512_add_pd(xb0_re, vb3_re), _mm512_add_pd(xb0_im, vb3_im)); // yb6
    }
    else
    {
        // Butterfly A
 ...
 * @param out_im Output imaginary components (64-byte aligned)
 */
static void radix7_stage_avx512_n1_soa(
    int K,
    const double *restrict in_re,
    const double *restrict in_im,
    ...
    double *restrict out_im)
{
    ...
    if (!verify_r7_avx512_alignment(in_re, in_im, out_re, out_im))
    {
    ...
    for (; k <= K - R7_AVX512_N1_U2_WIDTH; k += R7_AVX512_N1_U2_WIDTH)
    {
        ...
        radix7_butterfly_dual_avx512_n1_soa(k, k + R7_AVX512_N1_WIDTH, K,
                                            in_re, in_im,
                                            rader_tw_re, rader_tw_im,
    ...
    for (; k <= K - R7_AVX512_N1_WIDTH; k += R7_AVX512_N1_WIDTH)
    {
