...
static void check(const char *name, double err, double tol) {
    if (err <= tol) { g_pass++; printf("  PASS %-40s err=%.2e\n", name, err); }
    else            { g_fail++; printf("  FAIL %-40s err=%.2e > %.2e\n", name, err, tol); }
}
...
static void test_twiddled_roundtrip(int K) {
    ...
    fill(a_re,K,100+K); fill(a_im,K,200+K);
    fill(b_re,K,300+K); fill(b_im,K,400+K);
    fill(c_re,K,500+K); fill(c_im,K,600+K);
    fill(d_re,K,700+K); fill(d_im,K,800+K);
    fill(e_re,K,900+K); fill(e_im,K,1000+K);
    fill(f_re,K,1100+K); fill(f_im,K,1200+K);
    fill(g_re,K,1300+K); fill(g_im,K,1400+K);

    /* Generate unit-modulus twiddles */
    for (int k = 0; k < K; k++) {
        double ang1 = 2.0*M_PI*k/(7.0*K);
        tw1r[k]=cos(ang1);  tw1i[k]=sin(ang1);
        double ang2 = 2.0*ang1;
        tw2r[k]=cos(ang2);  tw2i[k]=sin(ang2);
        double ang3 = 3.0*ang1;
        tw3r[k]=cos(ang3);  tw3i[k]=sin(ang3);
    }
    ...
    free(a_re);free(a_im);free(b_re);free(b_im);free(c_re);free(c_im);
    free(d_re);free(d_im);free(e_re);free(e_im);free(f_re);free(f_im);
    free(g_re);free(g_im);
    free(y0r);free(y0i);free(y1r);free(y1i);free(y2r);free(y2i);
    free(y3r);free(y3i);free(y4r);free(y4i);free(y5r);free(y5i);free(y6r);free(y6i);
    free(z0r);free(z0i);free(z1r);free(z1i);free(z2r);free(z2i);
    free(z3r);free(z3i);free(z4r);free(z4i);free(z5r);free(z5i);free(z6r);free(z6i);
    free(tw1r);free(tw1i);free(tw2r);free(tw2i);free(tw3r);free(tw3i);
}
...
static void test_cross_check(int K) {
    ...
    fill(a_re,K,2000+K);fill(a_im,K,2100+K);
    fill(b_re,K,2200+K);fill(b_im,K,2300+K);
    fill(c_re,K,2400+K);fill(c_im,K,2500+K);
    fill(d_re,K,2600+K);fill(d_im,K,2700+K);
    fill(e_re,K,2800+K);fill(e_im,K,2900+K);
    fill(f_re,K,3000+K);fill(f_im,K,3100+K);
    fill(g_re,K,3200+K);fill(g_im,K,3300+K);

    for (int k=0;k<K;k++){
        double a1=2.0*M_PI*k/(7.0*K);
        tw1r[k]=cos(a1);tw1i[k]=sin(a1);
        tw2r[k]=cos(2*a1);tw2i[k]=sin(2*a1);
        tw3r[k]=cos(3*a1);tw3i[k]=sin(3*a1);
    }
    ...
    free(a_re);free(a_im);free(b_re);free(b_im);free(c_re);free(c_im);
    free(d_re);free(d_im);free(e_re);free(e_im);free(f_re);free(f_im);free(g_re);free(g_im);
    free(sy0r);free(sy0i);free(sy1r);free(sy1i);free(sy2r);free(sy2i);
    free(sy3r);free(sy3i);free(sy4r);free(sy4i);free(sy5r);free(sy5i);free(sy6r);free(sy6i);
    free(vy0r);free(vy0i);free(vy1r);free(vy1i);free(vy2r);free(vy2i);
    free(vy3r);free(vy3i);free(vy4r);free(vy4i);free(vy5r);free(vy5i);free(vy6r);free(vy6i);
    free(tw1r);free(tw1i);free(tw2r);free(tw2i);free(tw3r);free(tw3i);
}
...
int main(void) {
    ...
    for (int i = 0; i < (int)(sizeof(rt_Ks)/sizeof(rt_Ks[0])); i++) {
        ...
        double *z5r=(double*)aligned_alloc(64,K*8),*z5i=(double*)aligned_alloc(64,K*8);
        double *z6r=(double*)aligned_alloc(64,K*8),*z6i=(double*)aligned_alloc(64,K*8);
        fill(ar,K,5000+K);fill(ai,K,5100+K);fill(br,K,5200+K);fill(bi,K,5300+K);
        fill(cr,K,5400+K);fill(ci,K,5500+K);fill(dr,K,5600+K);fill(di,K,5700+K);
        fill(er,K,5800+K);fill(ei,K,5900+K);fill(fr,K,6000+K);fill(fi,K,6100+K);
        fill(gr,K,6200+K);fill(gi,K,6300+K);
        ...
        for(int k2=0;k2<K;k2++) for(int j=0;j<7;j++){
            double e1=fabs(zrs[j][k2]/7.0-origs_r[j][k2]);
            double e2=fabs(zis[j][k2]/7.0-origs_i[j][k2]);
            if(e1>mx)mx=e1; if(e2>mx)mx=e2;
        }
        char label[80]; snprintf(label,80,"N1 roundtrip K=%d",K);
        check(label,mx,3.0e-15);
        free(ar);free(ai);free(br);free(bi);free(cr);free(ci);free(dr);free(di);
        free(er);free(ei);free(fr);free(fi);free(gr);free(gi);
        free(y0r);free(y0i);free(y1r);free(y1i);free(y2r);free(y2i);
        free(y3r);free(y3i);free(y4r);free(y4i);free(y5r);free(y5i);free(y6r);free(y6i);
        free(z0r);free(z0i);free(z1r);free(z1i);free(z2r);free(z2i);
        free(z3r);free(z3i);free(z4r);free(z4i);free(z5r);free(z5i);free(z6r);free(z6i);
