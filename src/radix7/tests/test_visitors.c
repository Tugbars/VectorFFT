...
static tw_table_t *alloc_twiddles(int L) {
    ...
    for (int s = 0; s < L; s++) {
        ...
        tw[s].tw1_re = (double *)aligned_alloc(64, bytes);
        tw[s].tw1_im = (double *)aligned_alloc(64, bytes);
        tw[s].tw2_re = (double *)aligned_alloc(64, bytes);
        tw[s].tw2_im = (double *)aligned_alloc(64, bytes);
        tw[s].tw3_re = (double *)aligned_alloc(64, bytes);
        tw[s].tw3_im = (double *)aligned_alloc(64, bytes);
        ...
        for (int k = K; k < alloc_n; k++) {
            ...
            tw[s].tw3_re[k] = 0; tw[s].tw3_im[k] = 0;
        }
    }
    return tw;
}
...
static void free_twiddles(tw_table_t *tw, int L) {
    for (int s = 1; s < L; s++) {
        free(tw[s].tw1_re); free(tw[s].tw1_im);
        free(tw[s].tw2_re); free(tw[s].tw2_im);
        free(tw[s].tw3_re); free(tw[s].tw3_im);
    }
    free(tw);
}
...
static void test_roundtrip(const fft_r7_vtable_t *vt, int L) {
    int N = pow7(L);
    double *re = (double *)aligned_alloc(64, N * 8);
    double *im = (double *)aligned_alloc(64, N * 8);
    ...
    free(re); free(im); free(or_); free(oi_);
}
...
static void test_naive(const fft_r7_vtable_t *vt, int L) {
    ...
    double *re = (double *)aligned_alloc(64, N * 8);
    double *im = (double *)aligned_alloc(64, N * 8);
    double *Rre = (double *)aligned_alloc(64, N * 8);
    double *Rim = (double *)aligned_alloc(64, N * 8);
    ...
    free_twiddles(tw, L); free(perm);
    free(re); free(im); free(Rre); free(Rim); free(Nre); free(Nim);
}
...
int main(void) {
    ...
    printf("\n--- Naive Cross-Check ---\n");
    for (int L = 1; L <= 4; L++) test_naive(&vt, L);
