...
#ifdef _WIN32
#include <malloc.h>
static inline void *r5_aligned_alloc(size_t align, size_t size) {
    return _aligned_malloc(size, align);
}
static inline void r5_aligned_free(void *ptr) {
    _aligned_free(ptr);
}
#else
static inline void *r5_aligned_alloc(size_t align, size_t size) {
...
#endif
#include <windows.h>
static inline double r5_now_sec(void) {
...
#define R5_PRAGMA_NO_AUTOVEC _Pragma("GCC optimize(\"O2\",\"no-tree-loop-vectorize\")")
#else
#define R5_PRAGMA_NO_AUTOVEC /* not needed on ICX/Clang/MSVC */
#endif

#endif /* FFT_R5_PLATFORM_H */
