/**
 * @file  fft_radix5_fv.c
 * @brief Radix-5 forward stage visitor
 *
 * The centralized planner calls fft_radix5_visit_forward() once per DIT stage.
 ...
 *   - Twiddle precomputation (BLOCKED2: tw1, tw2)
 *   - Passing K = 5^s and num_groups = N / (5·K) per stage
 *
 * For stage 0 (K=1): pass tw pointers as NULL → uses N1 butterfly.
 * For stage s>0:      pass BLOCKED2 twiddle arrays → uses twiddled butterfly.
 */
 ...
 * The R5_PRAGMA_NO_AUTOVEC macro handles this portably (no-op on ICX/Clang).
 */
 ...
 *
 * Override R5_BUTTERFLY_API: headers default to static inline for
 * direct-include use (tests, benchmarks). Here we need extern linkage
 * so the linker can resolve vtable function pointers.
 */
#define R5_BUTTERFLY_API /* extern linkage for library build */
    ...
    const double *tw2_re, const double *tw2_im)
{
    ...
    if (tw1_re == NULL) {
        /* Stage 0: K=1, N1 butterfly, 5 adjacent elements per group */
