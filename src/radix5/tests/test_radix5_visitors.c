...
static void free_twiddles(tw_table_t *tw, int L)
{
    for (int s = 1; s < L; s++)
    {
        r5_aligned_free(tw[s].tw1_re);
        r5_aligned_free(tw[s].tw1_im);
        r5_aligned_free(tw[s].tw2_re);
        r5_aligned_free(tw[s].tw2_im);
    }
    free(tw);
}
...
static void test_naive(const fft_r5_vtable_t *vt, int L)
{
    int N = pow5(L);
    if (N > 3125)
        return; /* skip for L≥6, O(N²) too slow */
