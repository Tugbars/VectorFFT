/**
 * @file fft_radix5_bv_native_soa.c
 * @brief TRUE END-TO-END SoA Radix-5 FFT Implementation - BACKWARD (INVERSE)
 *
 * @details
 * This module implements a native Structure-of-Arrays (SoA) radix-5 FFT (backward/inverse)
...
/**
 * @brief Process radix-5 butterflies in range [k_start, k_end) - NATIVE SoA - BACKWARD
 *
 * @details
 ...
 */
static void radix5_process_range_native_soa_bv(
    ...
    int use_streaming)
{
    ...
    if (use_streaming)
    {
        for (; k + 15 < k_end; k += 16)
        {
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512_STREAM(k + 8, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 15 < k_end; k += 16)
        {
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512(k + 8, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX5_PIPELINE_8_NATIVE_SOA_BV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2_STREAM(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX5_PIPELINE_4_NATIVE_SOA_BV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2_STREAM(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX5_PIPELINE_2_NATIVE_SOA_BV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
    ...
    for (; k < k_end; k++)
    {
        RADIX5_PIPELINE_1_NATIVE_SOA_BV_SCALAR(k, K, in_re, in_im, out_re, out_im, stage_tw);
    }
...
//==============================================================================
// MAIN FUNCTION: Radix-5 Backward Transform - NATIVE SoA
//==============================================================================

/**
 * @brief Execute one stage of radix-5 FFT - NATIVE SoA - BACKWARD (INVERSE)
 *
 * @details
 ...
 *       if (stage % 2 == 0)
 *           fft_radix5_bv_native_soa(buf_b_re, buf_b_im, buf_a_re, buf_a_im, tw[stage], K[stage]);
 *       else
 *           fft_radix5_bv_native_soa(buf_a_re, buf_a_im, buf_b_re, buf_b_im, tw[stage], K[stage]);
 *   }
 *
 *   // Convert output once
    ...
    int K)
{
    ...
    */
    ...
    radix5_process_range_native_soa_bv(
        out_re, out_im,
        in_re, in_im,
        stage_tw,
        K,
