...
/* Per-butterfly cycle cost for radix R, calibrated from VTune measurements
 * at K=256 (docs/vtune-profiles/vtune_r{R}_codelet_k256.md). Derived as
 * ns_per_call * freq_GHz / K, which is unambiguous (we ignore the docs'
 * "CPE" column because its element-normalisation differs from ours).
 *
 * The static op count from the profile is a poor predictor of cycle cost:
 * pure ops/SIMD says R=64 (1924 ops) is 14.6x R=8 (132 ops), but VTune
 * shows R=64 is 49x R=8 in real cycles. The dominant per-radix bottleneck
 * shifts with R:
 *   R=2..8     compute-peak / DFT critical path (50-86% retiring)
 *   R=10..25   composite or Winograd, mostly compute-bound (50-65%)
 *   R=16/32    store-bound + L1 DTLB overflow (~25-35% retiring)
 *   R=64       load+store DTLB overflow (~27% retiring)
 *
 * Unmeasured radixes (R=2,3,5,6,7,17,19) get hand-fit estimates based on
 * the cycle/op ratio observed in their nearest measured neighbours
 * (~0.05-0.07 at small R, climbing to 0.15-0.25 at large R).
 *
 * Numbers are from t1_dit_fwd (twiddled, used at stages 1+). n1 (no
 * twiddle, stage 0) is a few percent cheaper but the *ranking* between
 * radixes is identical — we use the same table for both. */
static inline double _radix_per_butterfly_cycles_avx2(int R) {
    switch (R) {
        case  2: return   2.0;   /* est: tiny SIMD codelet, ~1.0 cycle/elem
                                  * (loop dispatch + per-stage memory-pass
                                  *  overhead dominates for R=2 — bumped
                                  *  empirically from naive 0.9 because the
                                  *  bench showed model picking R=2 outer
                                  *  stages it shouldn't). */
        case  3: return   2.5;   /* est: similar reasoning to R=2 */
        case  4: return   2.64;  /* measured: 118.6 ns × 5.71 GHz / 256 */
        case  5: return   5.1;   /*  est: 85 ops × 0.06 */
        case  6: return   7.0;   /*  est: ~120 ops × 0.06 */
        case  7: return   9.7;   /*  est: 161 ops × 0.06 */
        case  8: return   9.71;  /* measured: 437.8 ns × 5.68 GHz / 256 */
        case 10: return  19.86;  /* measured: 892 × 5.7 / 256 */
        case 11: return  29.69;  /* measured: 1333 × 5.7 / 256 */
        case 12: return  25.18;  /* measured: 1128 × 5.71 / 256 (lower-bound; var) */
        case 13: return  39.32;  /* measured: 1766 × 5.7 / 256 */
        case 16: return  36.27;  /* measured: 1631.6 × 5.69 / 256 (post-prefetch) */
        case 17: return  68.7;   /*  est: 458 ops × 0.15 */
        case 19: return  81.3;   /*  est: 542 ops × 0.15 */
        case 20: return  55.77;  /* measured: 2500.6 × 5.71 / 256 */
        case 25: return 113.9;   /* measured: 5113 × 5.7 / 256 */
        case 32: return 150.78;  /* measured: 6777.6 × 5.696 / 256 (post-defer) */
        case 64: return 472.32;  /* measured: 21185.9 × 5.705 / 256 */
        default: return 0.0;     /* unknown — fall back to ops/SIMD */
    }
}
 ...
 *
 * For radixes with measured CPE we use the empirical value directly.
 * For unmeasured radixes we use ops/SIMD-width as a starting estimate.
 * The latter under-counts decoder/TLB pressure for huge codelets, but is
 * fine for small/prime radixes (R=2/3/5/7) which never hit those limits. */
static inline double _radix_butterfly_cost_variant(
    const stride_radix_profile_t *p, int R, int isa_avx512)
{
    /* AVX-512 doesn't have measured CPE yet — for now we scale AVX2 numbers
     * by a constant factor of (4/8) since AVX-512 doubles SIMD width, but
     * actual AVX-512 codelets often have different bottlenecks. Calibrate
     * later when we have real AVX-512 VTune data. */
    if (!isa_avx512) {
        double cpe = _radix_per_butterfly_cycles_avx2(R);
                                           ...
                                           int isa_avx512)
{
    ...
    double c = _radix_butterfly_cost_variant(p, R, isa_avx512);
