...
static inline void _stride_execute_fwd_slice_range(const stride_plan_t *plan,
                                                  double *re, double *im,
                                                  size_t slice_K, size_t full_K,
                                                  int start_stage, int stop_stage)
{
    for (int s = start_stage; s < stop_stage; s++)
    {
...
/* ── _from: run stages start_stage..num_stages-1 (the original behavior) ── */
static inline void _stride_execute_fwd_slice_from(const stride_plan_t *plan,
                                                  double *re, double *im,
                                                  size_t slice_K, size_t full_K,
                                                  int start_stage)
{
    _stride_execute_fwd_slice_range(plan, re, im, slice_K, full_K,
                                    start_stage, plan->num_stages);
}

/* ── _until: run stages start_stage..stop_stage-1 (stop BEFORE stop_stage).
 *    Model (b) uses stop_stage = num_stages-1 so the last stage is left for the
 *    fused r2c_term_laststage codelet. ── */
...
static inline void stride_execute_fwd(const stride_plan_t *plan,
                                      double *re, double *im)
{
    if (plan->override_fwd) { plan->override_fwd(plan->override_data, re, im); return; }
...
static inline void stride_execute_bwd(const stride_plan_t *plan,
                                      double *re, double *im)
{
    if (plan->override_bwd) { plan->override_bwd(plan->override_data, re, im); return; }
