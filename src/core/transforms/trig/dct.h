 ...
 *
 * DCT-II (the JPEG/MPEG/AAC workhorse) and its inverse DCT-III.
 ...
 *
 * Requires N even (the R2C inner requires even input length).
 *
 * MT: the pre-permute and post-process passes K-split across the pool
 * (below); the inner R2C threads itself.
 */
#ifndef STRIDE_DCT_H
#define STRIDE_DCT_H
...
/* Specialized straight-line N=8 DCT-II / DCT-III codelets (the JPEG block
 * size). */
#if defined(__AVX2__)
...
/* ═══════════════════════════════════════════════════════════════
 * EXECUTE -- DCT-III (the inverse of DCT-II up to scale 2N)
 *
...
static void _dct3_execute_fwd(void *data, double *re, double *im) {
...
#if defined(__AVX2__)
    /* Fast path: specialized straight-line codelet for N=8 (JPEG block size).
     * Bypasses Makhoul + inner R2C entirely. */
    if (N == 8) {
        memcpy(d->buf_re, re, (size_t)N * K * sizeof(double));
        dct3_n8_avx2(d->buf_re, re, K);
        return;
    }
#endif
     ...
     *    The halfcomplex bin layout: buf[i] -> our buf_re[i],
...
/* DCT-III — the inverse of DCT-II up to scale factor 2N.
