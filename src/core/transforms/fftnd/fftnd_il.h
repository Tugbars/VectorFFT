...
/* the strip widths admitted at this cell: {8,16,32,64} columns with the
...
{
    static const int SW[] = { 8, 16, 32, 64 };
    int n = 0, i;
                                       ...
                                       size_t K)
{
    ...
    int sws[4], nsw = 0, sw = 0, maxsw = 0, sw_best = 0, tot = 0;    /* the strip widths */
    ...
    if (nat)
    {
        ...
        int bnf = 0, i, sp[4], nsp;
        nsp = _ilnd_sw_pool(d, sp, 4);
    ...
    if (nthr > 1)
    {
        ...
        else if (usable_w && !cfg->recalibrate && bcmt >= 0 && bcmtt == nthr)
        {
            ...
            if (mtf == 2 && (d->nsw <= 0 || d->mt != 2 || _ilnd_sw_pool(d, sws, 4) == 0))
