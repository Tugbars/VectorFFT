...
/* the PREFIX PAIR (il2d_large_plane_design.md, 2026-09-15): stages s and
 * s+1 of the wide prefix in ONE sweep of the plane. Stage s+1's digit e
 * (rows m*L1 + e + k*D1 within a stage-s block, m < R0, k < R1, where
 * L1 = L[s+1] = D0 = L[s]/R0) closes both stages: stage s runs on the R1
 * digit ranges {e + k*D1} of the block, then stage s+1 on the R0 blocks of
 * span L1 at digit e — R0*R1 rows x cw columns per unit, L2-resident, the
 * plane read once and written once instead of once per stage. The
 * arithmetic per element is the two stages' own (the same kernels on the
 * same table entries) in another traversal: bitwise the two-stage walk.
 * Forward: stage s reads src and writes dst, stage s+1 in place on dst;
 * reverse (the backward's reversed prefix): s+1 reads src, s in place.
 * Digits [e_lo, e_hi) of stage s+1 are the pool's unit axis. Legal when
 * both stages carry a digit axis (D > 1) and L[s+1] == L[s]/R[s]. */
static int _il2d_col_prefix_pair_ok(int s, const int *Rst, const int *Lst)
{
    const int R0 = Rst[s], R1 = Rst[s + 1];
    return Lst[s] / R0 > 1 && Lst[s + 1] / R1 > 1 && Lst[s + 1] == Lst[s] / R0;
}
static void _il2d_col_prefix_pair(const double *src, double *dst, int N1, size_t rn,
                                  int s, const int *Rst, const int *Lst,
                                  vfft_il2p_fn const *fns, double *const *tabs,
                                  int reverse, size_t cw, int e_lo, int e_hi)
{
    const int R0 = Rst[s], R1 = Rst[s + 1];
    const size_t L0 = (size_t)Lst[s], L1 = (size_t)Lst[s + 1];
    const size_t D0 = L0 / (size_t)R0, D1 = L1 / (size_t)R1;
    const int nblk = N1 / (int)L0;
    int e, bb, k, m;
    size_t k0;
    if (cw == 0 || cw > rn) cw = rn;
    for (bb = 0; bb < nblk; bb++)
        for (e = e_lo; e < e_hi; e++)
            for (k0 = 0; k0 < rn; k0 += cw)
            {
                const size_t w = (rn - k0 < cw) ? rn - k0 : cw;
                const size_t boff = 2 * ((size_t)bb * L0 * rn);
                if (!reverse)
                {
                    for (k = 0; k < R1; k++)
                    {
                        const size_t d = (size_t)e + (size_t)k * D1;
                        const size_t off = boff + 2 * (d * rn + k0);
                        fns[s](src + off, NULL, dst + off, NULL,
                               tabs[s] + d * (size_t)(R0 - 1) * 8, NULL,
                               D0 * rn, rn, D0 * rn, 1, w);
                    }
                    for (m = 0; m < R0; m++)
                    {
                        const size_t off = boff + 2 * (((size_t)m * L1 + (size_t)e) * rn + k0);
                        fns[s + 1](dst + off, NULL, dst + off, NULL,
                                   tabs[s + 1] + (size_t)e * (size_t)(R1 - 1) * 8, NULL,
                                   D1 * rn, rn, D1 * rn, 1, w);
                    }
                }
                else
                {
                    ...
                    {
                        const size_t off = boff + 2 * (((size_t)m * L1 + (size_t)e) * rn + k0);
                        fns[s + 1](src + off, NULL, dst + off, NULL,
                                   tabs[s + 1] + (size_t)e * (size_t)(R1 - 1) * 8, NULL,
                                   D1 * rn, rn, D1 * rn, 1, w);
                    }
                    for (k = 0; k < R1; k++)
                    {
                        const size_t d = (size_t)e + (size_t)k * D1;
                        const size_t off = boff + 2 * (d * rn + k0);
                        fns[s](dst + off, NULL, dst + off, NULL,
                               tabs[s] + d * (size_t)(R0 - 1) * 8, NULL,
                               D0 * rn, rn, D0 * rn, 1, w);
                    }
                }
...
/* the probe switch (VFFT_IL2D_PAIR=1, VFFT_IL2D_PAIRCW=columns, default 512) */
static int _il2d_pair_probe(size_t *cw)
{
    const char *e = getenv("VFFT_IL2D_PAIR"), *c = getenv("VFFT_IL2D_PAIRCW");
    *cw = (c && atoi(c) > 0) ? (size_t)atoi(c) : 512;
    return e && atoi(e) > 0;
}
