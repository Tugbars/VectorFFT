/* bench_r4_su_spill.c — Hand, Topo, SU+Spill on R=4. */
...
int main(int c,char**v){
    ...
    fr(bH_r,4*K,0xa1); fr(bH_i,4*K,0xa2);
    fr(twr,3*K,0xb1); fr(twi,3*K,0xb2);

    memcpy(bT_r,bH_r,4*K*8); memcpy(bT_i,bH_i,4*K*8);
    memcpy(bSU_r,bH_r,4*K*8); memcpy(bSU_i,bH_i,4*K*8);

    cH(); cT(); cSU();

    double e_t = max_rel(bH_r,bT_r,4*K);
    double e_su = max_rel(bH_r,bSU_r,4*K);
