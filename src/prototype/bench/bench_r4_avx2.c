/* bench_r4_avx2.c — Topo vs SU+Spill on R=4 AVX2. */
...
int main(int c,char**v){
    ...
    bT_r=aa(4*K); bT_i=aa(4*K);
    bSU_r=aa(4*K); bSU_i=aa(4*K);
    twr=aa(3*K); twi=aa(3*K);

    fr(bT_r,4*K,0xa1); fr(bT_i,4*K,0xa2);
    memcpy(bSU_r,bT_r,4*K*8); memcpy(bSU_i,bT_i,4*K*8);
    fr(twr,3*K,0xb1); fr(twi,3*K,0xb2);

    cT(); cSU();

    double e_re = max_rel(bT_r,bSU_r,4*K);
    double e_im = max_rel(bT_i,bSU_i,4*K);
    ...
    printf("K=%5zu  T=%7.1f SU=%7.1f | SU/T=%.3f\n", K,tT,tSU,tSU/tT);
    return 0;
}
