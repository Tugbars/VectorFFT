/* bench_r4_bwd.c — R=4 DIT in-place bwd: hand vs ours (recipe). */
...
static void hand_bwd(double *r, double *i, const double *tr, const double *ti,
    ...
    radix4_t1_dit_bwd_avx512(r, i, tr, ti, ios, me);
}

__attribute__((target("avx512f")))
void radix4_t1_dit_bwd_avx512_gen_inplace_su_spill(
...
static void cSU(){radix4_t1_dit_bwd_avx512_gen_inplace_su_spill(bSU_r,bSU_i,twr,twi,K,K);}
...
int main(int c,char**v){
    ...
    /* Caller provides forward-convention W; both fwd and bwd codelets
     * read the same array. Bwd internally conjugates. */
    ...
    printf("R=4 K=%5zu  H=%7.1f SU=%7.1f | SU/H=%.3f\n", K,tH,tSU,tSU/tH);
    return 0;
}
