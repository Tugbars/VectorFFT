...
int main(int argc, char **argv) {
    ...
    double err_topo_re = max_rel_err(g_rio_re_hand, g_rio_re_topo, 16 * K);
    double err_topo_im = max_rel_err(g_rio_im_hand, g_rio_im_topo, 16 * K);
    double err_topo = err_topo_re > err_topo_im ? err_topo_re : err_topo_im;

    if (err_topo > 1e-10 ) {
        printf("CORRECTNESS FAIL: topo_err=%.2e\n", err_topo);
        return 2;
    }
