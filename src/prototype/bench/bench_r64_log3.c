/* bench_r64_log3.c — flat vs log3 vs recipe-log3 at R=64. */
...
int main(int c,char**v){
    ...
    bL_r=aa(64*K); bL_i=aa(64*K);
    bLR_r=aa(64*K); bLR_i=aa(64*K);
    twr=aa(63*K); twi=aa(63*K);

    unsigned s=0xa1; for(size_t i=0;i<64*K;i++){s=s*1103515245u+12345u;bF_r[i]=(double)((int)(s>>8)&0x7fffff)/(double)0x800000-0.5;}
    s=0xa2; for(size_t i=0;i<64*K;i++){s=s*1103515245u+12345u;bF_i[i]=(double)((int)(s>>8)&0x7fffff)/(double)0x800000-0.5;}
    memcpy(bL_r,bF_r,64*K*8); memcpy(bL_i,bF_i,64*K*8);
    memcpy(bLR_r,bF_r,64*K*8); memcpy(bLR_i,bF_i,64*K*8);
    ...
    double e_re = max_rel(bF_r,bL_r,64*K);
    double e_im = max_rel(bF_i,bL_i,64*K);
    double er_re = max_rel(bF_r,bLR_r,64*K);
    double er_im = max_rel(bF_i,bLR_i,64*K);
    double max_err = e_re;
    ...
    printf("K=%5zu  F=%9.0f L=%9.0f LR=%9.0f | L/F=%.3f LR/F=%.3f LR/L=%.3f\n",
