/* bench_r64_dif.c — R=64 in-place DIF: Hand vs ours (Topo).
 *
 * Tests correctness AND performance of our DIF emission. */
...
int main(int c,char**v){
    ...
    /* Same twiddle layout as DIT: tw_re[(j-1)*K + k] = cos(angle_jk) */
    ...
    double e_re = max_rel(bH_r,bT_r,64*K);
    double e_im = max_rel(bH_i,bT_i,64*K);
    ...
    printf("K=%5zu  H=%9.0f T=%9.0f | T/H=%.3f\n", K,tH,tT,tT/tH);
    return 0;
}
