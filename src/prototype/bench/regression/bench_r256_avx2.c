/* bench_r256_avx2.c — Monolithic R=256 codelets vs multi-stage 16×16.
 *
 * Question: with AVX2 (16 YMM regs) and modern GCC, can a single 8K-line
 * monolithic R=256 codelet beat the equivalent two-pass 16×16 build?
 *
 * Contestants (per K ∈ {4, 32, 256}):
 *   1. mono_n1         radix256_n1_fwd
 *   2. mono_t1         radix256_t1_dit_fwd        (fed identity twiddles)
 *   3. mono_t1s        radix256_t1s_dit_fwd       (fed identity twiddles)
 *   4. mono_t1_log3    radix256_t1_dit_log3_fwd   (fed identity twiddles)
 *   5. mono_t1s_log3   radix256_t1s_dit_log3_fwd  (fed identity twiddles)
 *   6. multi_16x16     r16_n1 × 16  →  twid mul  →  r16_n1 × 16
 *
 * Layout: rio[K*p + k] where p∈[0,256) is the DFT index and k∈[0,K) the
 * SIMD batch lane.  Stride 1 across k (innermost), K across p.
 *
 * Note on the twiddled monolithic variants:
 *   These codelets compute "external twiddle mul + DFT" — designed as
 *   the inner stage of a larger CT decomposition.  With identity
 *   twiddles (W=1+0i) they still execute the full multiply pass; the
 *   resulting timing tells us "what does the twiddle-mul layer cost
 *   when you don't need it?"  Correctness is NOT checked for these —
 *   they compute a different function than plain DFT-256.
 */
...
static double *g_tw_re, *g_tw_im;          /* monolithic codelet twiddles */
static double *g_stage_tw_re, *g_stage_tw_im;  /* inter-stage 16×16 W */

/* ── Multi-stage 16×16 implementation ─────────────────────────────── */

/* Precompute inter-stage twiddles W[a*16+b] = exp(-2πi * a*b / 256). */
static void precompute_stage_twiddles(void) {
    for (int a = 0; a < 16; a++) {
        for (int b = 0; b < 16; b++) {
            double phi = -2.0 * M_PI * (double)(a * b) / 256.0;
            g_stage_tw_re[a * 16 + b] = cos(phi);
            g_stage_tw_im[a * 16 + b] = sin(phi);
        }
    }
}

/* Inter-stage complex multiply: rio[K*(16*a+b)+k] *= W[a*16+b].
 * SIMD batch lanes k step by 4 (AVX2).  W is constant across the K
 * dimension so we broadcast. */
__attribute__((target("avx2,fma")))
...
/* ── Repeat count by K (target ~5-10 ms per trial) ─────────────────── */
...
/* ── Sanity check: mono_n1 and multi_16x16 should compute the same
 *    DFT-256 modulo a digit-reversal permutation on output. We verify
 *    by comparing the multisets of complex magnitudes (sorted vectors
 *    of |x|² should match within tolerance). ───────────────────────── */
...
static int validate_mono_vs_multi(void) {
    size_t N = 256 * g_K;
    double *input_re = aa(N), *input_im = aa(N);
    fr(input_re, N, 0x77);
    fr(input_im, N, 0x88);

    /* run mono_n1 */
    memcpy(g_rio_re, input_re, N * sizeof(double));
    memcpy(g_rio_im, input_im, N * sizeof(double));
    call_mono_n1();
    double *mag_mono = aa(N);
    for (size_t i = 0; i < N; i++)
        mag_mono[i] = g_rio_re[i] * g_rio_re[i] + g_rio_im[i] * g_rio_im[i];

    /* run multi_16x16 on same input */
    memcpy(g_rio_re, input_re, N * sizeof(double));
    memcpy(g_rio_im, input_im, N * sizeof(double));
    call_multi_16x16();
    ...
    af(input_re); af(input_im); af(mag_mono); af(mag_multi);
...
static void run_K(size_t K) {
    ...
    g_rio_re = aa(N);
    g_rio_im = aa(N);
    /* Twiddle table for monolithic codelets — generous size, random fill. */
    g_tw_re = aa((size_t)256 * K);
    g_tw_im = aa((size_t)256 * K);
    fr(g_tw_re, (size_t)256 * K, 0xc1);
    fr(g_tw_im, (size_t)256 * K, 0xc2);
    /* Inter-stage W table (16×16). */
    g_stage_tw_re = aa(16 * 16);
    g_stage_tw_im = aa(16 * 16);
    ...
    if (rc != 0) {
        printf("  K=%-4zu  VALIDATION FAIL (mono_n1 vs multi_16x16 magnitudes differ)\n", K);
    }

    int reps = reps_for_K(K);
    int trials = 7;

    /* Re-seed input before each bench so we're not measuring something that
     * has gone NaN over many repeats. */
    fr(g_rio_re, N, 0x11); fr(g_rio_im, N, 0x22);
    double t_multi   = bench(call_multi_16x16,   reps, trials);
    fr(g_rio_re, N, 0x11); fr(g_rio_im, N, 0x22);
...
int main(int argc, char **argv) {
    printf("════════════════════════════════════════════════════════════════════════\n");
    printf("  R=256 monolithic vs multi-stage 16×16 — AVX2\n");
    printf("  Compiler: gcc -O3 -mavx2 -mfma\n");
    printf("  Layout:   rio[K*p + k], stride 1 across batch, K across DFT idx\n");
    printf("════════════════════════════════════════════════════════════════════════\n");
