/* bench_r8_dif.c — R=8 in-place DIF: hand vs ours (recipe). */
...
static void hand_fwd(double *r, double *i, const double *tr, const double *ti,
    ...
    radix8_t1_dif_fwd_avx512(r, i, tr, ti, ios, me);
}
...
static void cSU(){radix8_t1_dif_fwd_avx512_gen_inplace_su_spill(bSU_r,bSU_i,twr,twi,K,K);}
...
int main(int c,char**v){
    ...
    unsigned s=0xa1; for(size_t i=0;i<8*K;i++){s=s*1103515245u+12345u;bH_r[i]=(double)((int)(s>>8)&0x7fffff)/(double)0x800000-0.5;}
    s=0xa2; for(size_t i=0;i<8*K;i++){s=s*1103515245u+12345u;bH_i[i]=(double)((int)(s>>8)&0x7fffff)/(double)0x800000-0.5;}
    ...
    double e_re = max_rel(bH_r,bSU_r,8*K);
    double e_im = max_rel(bH_i,bSU_i,8*K);
