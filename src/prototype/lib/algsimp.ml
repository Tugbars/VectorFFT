...
(* ── EXPERIMENTAL: factor_common_terms ────────────────────────────────
 * Generalizes factor_common_muls to non-constant shared factors.
 *
 * factor_common_muls finds c*a + c*b → c*(a+b) only when c is Const.
 * FFTW's collectM allows any non-NaN shared first factor — including
 * Load nodes, intermediate Plus, etc.
 *
 * For each flat-sum term that's a Mul(a, b), we consider BOTH operands
 * as candidate shared factors and bucket the term under each operand's
 * tag. After enumeration, any bucket with ≥2 entries represents a
 * factoring opportunity: those terms all share that operand. We greedily
 * pick the largest bucket, factor it, then re-bucket the remainder.
 *
 * Correctness: factoring c*a + c*b → c*(a+b) saves 1 mul regardless of
 * whether c is materialized elsewhere. The (a+b) inner sum hash-conses
 * naturally; if it's already in the DAG, we get sharing for free; if
 * not, it's a new node but the trade is still 1 mul saved per shared
 * factor.
 *
 * Run via gen_radix --factor-terms flag (off by default). Doc 48
 * measures the impact across radixes. *)
let factor_common_terms (assigns : (Expr.elem_ref * t) list)
    : (Expr.elem_ref * t) list =
  let cache : (int, t) Hashtbl.t = Hashtbl.create 256 in
  let changed = ref false in

  (* Use-count over the whole DAG (excluding our reconstruction). Same
   * mechanism as share_subsums and fma_lift. If a Mul node has > 1
   * consumer in the original DAG, factoring it would create a new path
   * while leaving the original (still-referenced) Mul in place — net
   * INCREASE in node count. We only factor Muls with use_count == 1. *)
  ...
  let single_use n =
    (try Hashtbl.find use_count n.tag with Not_found -> 0) = 1 in
  ...
  (* For a term that's Mul(a, b) with use_count == 1 (no other consumers
   * in the DAG), return [(a_tag, b); (b_tag, a)] — pair each candidate
   * factor with what's left if you extract it. Multi-use Muls are
   * rejected: factoring them creates new paths while leaving the
   * originals reachable, INCREASING node count rather than decreasing. *)
  let mul_candidates (term : t) : (int * t * t) list =
    if not (single_use term) then []
    else match term.node with
    | NK_Mul (a, b) -> [(a.tag, a, b); (b.tag, b, a)]
    | _ -> []
  in

  (* Greedy: find the factor key with the most occurrences, factor that
   * group, repeat. Returns new term list + whether any factoring fired. *)
  let factor_terms (terms : (int * t) list) : (int * t) list * bool =
    let any_fired = ref false in
    let work = ref terms in
    let progress = ref true in
    while !progress do
      progress := false;
      (* Build bucket: factor_tag → list of (sign, factor_node, other_factor, original_term_index) *)
      let buckets : (int, (int * t * t * int) list) Hashtbl.t =
        Hashtbl.create 16 in
      List.iteri (fun idx (sign, term) ->
        List.iter (fun (key, factor, other) ->
          let cur = try Hashtbl.find buckets key with Not_found -> [] in
          Hashtbl.replace buckets key ((sign, factor, other, idx) :: cur)
        ) (mul_candidates term)
      ) !work;
      (* Find the bucket with the most entries (≥ 2). Tie-break: lowest factor tag
       * (deterministic). *)
      ...
      Hashtbl.iter (fun key entries ->
        ...
        if n >= 2 then
          match !best with
          | None -> best := Some (key, entries, n)
          | Some (_, _, prev_n) ->
            if n > prev_n || (n = prev_n && key < (match !best with Some (k,_,_) -> k | _ -> 0))
            then best := Some (key, entries, n)
      ) buckets;
      match !best with
      | None -> ()  (* No factoring opportunity. *)
      | Some (_factor_tag, entries, _) ->
        (* Factor: collect the other-factors with signs, build inner sum,
         * multiply by the shared factor. The original terms get replaced
         * by one factored term. *)
        any_fired := true;
        changed := true;
        progress := true;
        let factor_node = match entries with
          | (_, f, _, _) :: _ -> f
          | [] -> failwith "factor_common_terms: empty entries"
        in
        let inner_terms = List.map (fun (s, _, other, _) -> (s, other)) entries in
        let inner_sum = rebuild_sum inner_terms in
        let factored_term = mk_mul factor_node inner_sum in
        let consumed_indices = List.map (fun (_, _, _, i) -> i) entries in
        (* Build new term list: keep non-consumed terms, append the factored one. *)
        let new_terms = List.filter_map (fun (i, term_pair) ->
          if List.mem i consumed_indices then None else Some term_pair
        ) (List.mapi (fun i x -> (i, x)) !work) in
        work := (1, factored_term) :: new_terms
    done;
    (!work, !any_fired)
  in
    ...
    | None ->
      let r = match n.node with
        ...
        | NK_Fma (a, b, c, n1, n2) ->
          ...
          if a' == a && b' == b && c' == c then n
          else hashcons (NK_Fma (a', b', c', n1, n2))
      in
      Hashtbl.add cache n.tag r;
      r
  in
...
(* ── end factor_common_terms ──────────────────────────────────────── *)

(* Diagnostic-only: count factoring opportunities without rewriting.
 * An "opportunity" = a sum that contains ≥2 single-use Mul terms
 * sharing a non-constant factor. If this count is 0, factor_common_terms
 * has nothing to find regardless of how cleverly we rewrite. *)
let count_factor_opportunities (assigns : (Expr.elem_ref * t) list) : int =
  let use_count : (int, int) Hashtbl.t = Hashtbl.create 256 in
  let visited : (int, unit) Hashtbl.t = Hashtbl.create 256 in
  let bump tag =
    let c = try Hashtbl.find use_count tag with Not_found -> 0 in
  ...
  let seen2 : (int, unit) Hashtbl.t = Hashtbl.create 256 in
  let rec visit e =
    if not (Hashtbl.mem seen2 e.tag) then begin
      Hashtbl.add seen2 e.tag ();
      match e.node with
      | NK_Add (a, b) | NK_Sub (a, b) ->
        let terms = flatten 1 e in
        let buckets : (int, int) Hashtbl.t = Hashtbl.create 8 in
        List.iter (fun (_, term) ->
          if single_use term then
            match term.node with
            | NK_Mul (x, y) ->
              (match x.node with NK_Const _ -> () | _ ->
                 let c = try Hashtbl.find buckets x.tag with Not_found -> 0 in
                 Hashtbl.replace buckets x.tag (c + 1));
              (match y.node with NK_Const _ -> () | _ ->
                 let c = try Hashtbl.find buckets y.tag with Not_found -> 0 in
                 Hashtbl.replace buckets y.tag (c + 1))
            | _ -> ()
        ) terms;
        Hashtbl.iter (fun _ count ->
          if count >= 2 then incr opportunities
        ) buckets;
        visit a; visit b
      | NK_Mul (a, b) -> visit a; visit b
      | NK_Neg a -> visit a
      | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
        visit a; visit b; visit c; visit d
      | NK_Fma (a, b, c, _, _) -> visit a; visit b; visit c
      | NK_Const _ | NK_Load _ -> ()
    end
  in
  List.iter (fun (_, e) -> visit e) assigns;
