...
/* Prototype codelet coverage — matches src/prototype/scripts/generate_codelets.sh. */
                                           ...
                                           size_t me, int isa_avx512)
{
    ...
    if (is_first_stage) {
        ...
        if (_stride_total_ops(p) == 0) {
            /* Some radixes are n1-only (e.g. R=3) — t1 slot is empty,
             * fall back to n1 ops for an inner-stage estimate. */
            p = isa_avx512 ? &stride_radix_profile_n1_avx512[R]
                           : &stride_radix_profile_n1_avx2[R];
        }
    }
    int ops = _stride_total_ops(p);
    if (ops == 0) return (double)R * 4.0;
...
static inline double stride_score_strided_codelet(int R, int isa_avx512) {
    ...
    if (R <= 0 || R >= STRIDE_RADIX_PROFILE_MAX_R) return (double)R * 4.0;
    const stride_radix_profile_t *p = isa_avx512
        ? &stride_radix_profile_n1_avx512[R]
        : &stride_radix_profile_n1_avx2[R];
    int ops = _stride_total_ops(p);
    if (ops == 0) return (double)R * 4.0;
    ops += _strided_transpose_overhead(R, isa_avx512);
    int simd_width = isa_avx512 ? 8 : 4;
    return (double)ops / (double)simd_width;
}

/* Score a single-call trig codelet. `kind` ∈ {"dct2","dct3","dct4",
 * "dst2","dst3","dht"}. Returns cycles (ops / SIMD width), or a
 * size-linear fallback when the codelet isn't available at this N. */
static inline double stride_score_trig_codelet(const char *kind, int N,
                                                int isa_avx512) {
    if (!_n_has_trig_codelet(N)) return (double)N * 4.0;
    if (N <= 0 || N >= STRIDE_RADIX_PROFILE_MAX_R) return (double)N * 4.0;
    const stride_radix_profile_t *p = NULL;
    #define _PICK(name) \
        p = isa_avx512 ? &stride_radix_profile_##name##_avx512[N] \
                       : &stride_radix_profile_##name##_avx2[N]
    if      (strcmp(kind, "dct2") == 0) _PICK(dct2);
    else if (strcmp(kind, "dct3") == 0) _PICK(dct3);
    else if (strcmp(kind, "dct4") == 0) _PICK(dct4);
    else if (strcmp(kind, "dst2") == 0) _PICK(dst2);
    else if (strcmp(kind, "dst3") == 0) _PICK(dst3);
    else if (strcmp(kind, "dht")  == 0) _PICK(dht);
    else return (double)N * 4.0;
    #undef _PICK
    int ops = _stride_total_ops(p);
    if (ops == 0) return (double)N * 4.0;
    int simd_width = isa_avx512 ? 8 : 4;
    return (double)ops / (double)simd_width;
}
