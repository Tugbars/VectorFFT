/**
 * @file fft_radix13_butterfly_avx512_part1.h
 * @brief Radix-13 Butterfly AVX-512 Implementation - Part 1
 *
 * @details
 * OPTIMIZATIONS PRESERVED FROM RADIX-11:
 ...
 * ✅ Fixed SIMD shuffles (no out-of-range indices)
 * ✅ LO/HI split for register reuse
 *
 * NEW FOR RADIX-13:
 * - 13 input/output lanes (6 symmetric pairs + DC)
 * - Geometric constants for radix-13
 * - Adjusted load/store macros for 13 lanes
 *
...
#ifndef FFT_RADIX13_BUTTERFLY_AVX512_PART1_H
#define FFT_RADIX13_BUTTERFLY_AVX512_PART1_H
...
#define R13_VECTOR_WIDTH 8
#define R13_REQUIRED_ALIGNMENT 64
#elif defined(__AVX2__)
#define R13_PARALLEL_THRESHOLD 4096
#define R13_VECTOR_WIDTH 4
#define R13_REQUIRED_ALIGNMENT 32
#elif defined(__SSE2__)
#define R13_PARALLEL_THRESHOLD 8192
#define R13_VECTOR_WIDTH 2
#define R13_REQUIRED_ALIGNMENT 16
#else
#define R13_PARALLEL_THRESHOLD 16384
#define R13_VECTOR_WIDTH 1
#define R13_REQUIRED_ALIGNMENT 8
#endif

#define R13_CACHE_LINE_BYTES 64
#define R13_DOUBLES_PER_CACHE_LINE (R13_CACHE_LINE_BYTES / sizeof(double))
#define R13_CACHE_BLOCK_SIZE 1024

#ifndef R13_LLC_BYTES
#define R13_LLC_BYTES (8 * 1024 * 1024)
#endif

#ifndef R13_FORCE_NT
#define R13_USE_NT_STORES 0
#else
#define R13_USE_NT_STORES R13_FORCE_NT
#endif

#ifndef R13_PREFETCH_DISTANCE
#if defined(__AVX512F__)
#define R13_PREFETCH_DISTANCE 24
#elif defined(__AVX2__)
#define R13_PREFETCH_DISTANCE 20
#elif defined(__SSE2__)
#define R13_PREFETCH_DISTANCE 16
#else
#define R13_PREFETCH_DISTANCE 8
#endif
#endif

#ifndef R13_PREFETCH_HINT
#define R13_PREFETCH_HINT _MM_HINT_T0
#endif
...
typedef struct
{
    __m512d c1, c2, c3, c4, c5, c6;
    __m512d s1, s2, s3, s4, s5, s6;
} radix13_consts_avx512;

/**
 * @brief Broadcast radix-13 geometric constants to AVX-512 registers
 * @details CRITICAL: Call ONCE before main loop to hoist constants (5-10% speedup)
 */
static inline radix13_consts_avx512 broadcast_radix13_consts_avx512(void)
{
    radix13_consts_avx512 KC;
    KC.c1 = _mm512_set1_pd(C13_1);
    KC.c2 = _mm512_set1_pd(C13_2);
    KC.c3 = _mm512_set1_pd(C13_3);
    KC.c4 = _mm512_set1_pd(C13_4);
    KC.c5 = _mm512_set1_pd(C13_5);
    KC.c6 = _mm512_set1_pd(C13_6);
    KC.s1 = _mm512_set1_pd(S13_1);
    KC.s2 = _mm512_set1_pd(S13_2);
    KC.s3 = _mm512_set1_pd(S13_3);
    KC.s4 = _mm512_set1_pd(S13_4);
    KC.s5 = _mm512_set1_pd(S13_5);
    KC.s6 = _mm512_set1_pd(S13_6);
    return KC;
}

//==============================================================================
// FIXED INTERLEAVE/DEINTERLEAVE HELPERS (PROVEN CORRECT FROM RADIX-11)
//==============================================================================
 ...
 *
 * FIX: Replaced out-of-range permutexvar with fixed shuffles
 */
static inline __m512d extract_re_avx512(__m512d z)
{
    ...
    // 0x00 = 0b00000000: for each pair, select first element
    ...
    // Step 3: Final per-lane selection to get [r0, r1, r2, r3, r4, r5, r6, r7]
...
static inline __m512d extract_im_avx512(__m512d z)
{
    ...
    // Step 3: Final per-lane selection to get [i0, i1, i2, i3, i4, i5, i6, i7]
...
/**
 * @brief Load 13 complex lanes with CORRECT interleaving
 * @details Loads 16 complex numbers (8 lo + 8 hi) across 13 frequency lanes
 *
 * Memory layout (SoA): in_re[lane * K + k], in_im[lane * K + k]
 * where lane ∈ [0,12], k ∈ [0, K-1]
 */
#define LOAD_13_LANES_AVX512_NATIVE_SOA_FULL(k, K, in_re, in_im,             \
    ...
    {                                                                        \
        ...
        __m512d re11_lo = _mm512_loadu_pd(&(in_re)[11 * (K) + (k)]);         \
        __m512d im11_lo = _mm512_loadu_pd(&(in_im)[11 * (K) + (k)]);         \
        __m512d re11_hi = _mm512_loadu_pd(&(in_re)[11 * (K) + (k) + 8]);     \
        __m512d im11_hi = _mm512_loadu_pd(&(in_im)[11 * (K) + (k) + 8]);     \
        x11_lo = interleave_ri_avx512(re11_lo, im11_lo);                     \
        x11_hi = interleave_ri_avx512(re11_hi, im11_hi);                     \
        __m512d re12_lo = _mm512_loadu_pd(&(in_re)[12 * (K) + (k)]);         \
        __m512d im12_lo = _mm512_loadu_pd(&(in_im)[12 * (K) + (k)]);         \
        __m512d re12_hi = _mm512_loadu_pd(&(in_re)[12 * (K) + (k) + 8]);     \
        __m512d im12_hi = _mm512_loadu_pd(&(in_im)[12 * (K) + (k) + 8]);     \
        x12_lo = interleave_ri_avx512(re12_lo, im12_lo);                     \
        x12_hi = interleave_ri_avx512(re12_hi, im12_hi);                     \
    } while (0)
 ...
 * @details Branchless masked loads for partial vectors (1-15 elements remaining)
 */
#define LOAD_13_LANES_AVX512_NATIVE_SOA_MASKED(k, K, mask_lo, mask_hi,                  \
                                               in_re, in_im,                            \
                                               x0_lo, x0_hi, x1_lo, x1_hi,              \
                                               x2_lo, x2_hi, x3_lo, x3_hi,              \
                                               x4_lo, x4_hi, x5_lo, x5_hi,              \
    ...
    {                                                                                   \
        ...
        x10_lo = interleave_ri_avx512(re10_lo, im10_lo);                                \
        x10_hi = interleave_ri_avx512(re10_hi, im10_hi);                                \
        __m512d re11_lo = _mm512_maskz_loadu_pd(mask_lo, &(in_re)[11 * (K) + (k)]);     \
        __m512d im11_lo = _mm512_maskz_loadu_pd(mask_lo, &(in_im)[11 * (K) + (k)]);     \
        __m512d re11_hi = _mm512_maskz_loadu_pd(mask_hi, &(in_re)[11 * (K) + (k) + 8]); \
        __m512d im11_hi = _mm512_maskz_loadu_pd(mask_hi, &(in_im)[11 * (K) + (k) + 8]); \
        x11_lo = interleave_ri_avx512(re11_lo, im11_lo);                                \
        x11_hi = interleave_ri_avx512(re11_hi, im11_hi);                                \
        __m512d re12_lo = _mm512_maskz_loadu_pd(mask_lo, &(in_re)[12 * (K) + (k)]);     \
        __m512d im12_lo = _mm512_maskz_loadu_pd(mask_lo, &(in_im)[12 * (K) + (k)]);     \
        __m512d re12_hi = _mm512_maskz_loadu_pd(mask_hi, &(in_re)[12 * (K) + (k) + 8]); \
        __m512d im12_hi = _mm512_maskz_loadu_pd(mask_hi, &(in_im)[12 * (K) + (k) + 8]); \
        x12_lo = interleave_ri_avx512(re12_lo, im12_lo);                                \
        x12_hi = interleave_ri_avx512(re12_hi, im12_hi);                                \
    } while (0)
...
/**
 * @brief Store 13 complex lanes (FULL) - split for better ILP
 * @details Extracts re/im and stores to SoA layout
 * Split into LO and HI for instruction-level parallelism (3-8% speedup)
 */
#define STORE_13_LANES_AVX512_NATIVE_SOA_FULL(k, K, out_re, out_im,           \
                                              y0_lo, y0_hi, y1_lo, y1_hi,     \
                                              y2_lo, y2_hi, y3_lo, y3_hi,     \
                                              y4_lo, y4_hi, y5_lo, y5_hi,     \
                                              y6_lo, y6_hi, y7_lo, y7_hi,     \
                                              y8_lo, y8_hi, y9_lo, y9_hi,     \
                                              y10_lo, y10_hi, y11_lo, y11_hi, \
                                              y12_lo, y12_hi)                 \
    do                                                                        \
    {                                                                         \
        ...
        __m512d re11_lo = extract_re_avx512(y11_lo);                          \
        __m512d im11_lo = extract_im_avx512(y11_lo);                          \
        __m512d re11_hi = extract_re_avx512(y11_hi);                          \
        __m512d im11_hi = extract_im_avx512(y11_hi);                          \
        _mm512_storeu_pd(&(out_re)[11 * (K) + (k)], re11_lo);                 \
        _mm512_storeu_pd(&(out_re)[11 * (K) + (k) + 8], re11_hi);             \
        _mm512_storeu_pd(&(out_im)[11 * (K) + (k)], im11_lo);                 \
        _mm512_storeu_pd(&(out_im)[11 * (K) + (k) + 8], im11_hi);             \
        __m512d re12_lo = extract_re_avx512(y12_lo);                          \
        __m512d im12_lo = extract_im_avx512(y12_lo);                          \
        __m512d re12_hi = extract_re_avx512(y12_hi);                          \
        __m512d im12_hi = extract_im_avx512(y12_hi);                          \
        _mm512_storeu_pd(&(out_re)[12 * (K) + (k)], re12_lo);                 \
        _mm512_storeu_pd(&(out_re)[12 * (K) + (k) + 8], re12_hi);             \
        _mm512_storeu_pd(&(out_im)[12 * (K) + (k)], im12_lo);                 \
        _mm512_storeu_pd(&(out_im)[12 * (K) + (k) + 8], im12_hi);             \
    } while (0)
 ...
 * @details Branchless tail handling with AVX-512 predicate masks (2-5% speedup)
 */
#define STORE_13_LANES_AVX512_NATIVE_SOA_LO_MASKED(k, K, remaining_lo,         \
                                                   out_re, out_im,             \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            __m512d re11 = extract_re_avx512(y11_lo);                          \
            __m512d im11 = extract_im_avx512(y11_lo);                          \
            __m512d re12 = extract_re_avx512(y12_lo);                          \
            __m512d im12 = extract_im_avx512(y12_lo);                          \
            ...
            _mm512_mask_storeu_pd(&(out_re)[11 * (K) + (k)], mask, re11);      \
            _mm512_mask_storeu_pd(&(out_re)[12 * (K) + (k)], mask, re12);      \
            ...
            _mm512_mask_storeu_pd(&(out_im)[11 * (K) + (k)], mask, im11);      \
            _mm512_mask_storeu_pd(&(out_im)[12 * (K) + (k)], mask, im12);      \
        }                                                                      \
    } while (0)
 ...
 */
#define STORE_13_LANES_AVX512_NATIVE_SOA_HI_MASKED(k, K, remaining_hi,         \
                                                   out_re, out_im,             \
    ...
    {                                                                          \
        ...
        {                                                                      \
            ...
            __m512d re11 = extract_re_avx512(y11_hi);                          \
            __m512d im11 = extract_im_avx512(y11_hi);                          \
            __m512d re12 = extract_re_avx512(y12_hi);                          \
            __m512d im12 = extract_im_avx512(y12_hi);                          \
            ...
            _mm512_mask_storeu_pd(&(out_re)[11 * (K) + (k) + 8], mask, re11);  \
            _mm512_mask_storeu_pd(&(out_re)[12 * (K) + (k) + 8], mask, re12);  \
            ...
            _mm512_mask_storeu_pd(&(out_im)[11 * (K) + (k) + 8], mask, im11);  \
            _mm512_mask_storeu_pd(&(out_im)[12 * (K) + (k) + 8], mask, im12);  \
        }                                                                      \
    } while (0)
...
/**
 * @brief Apply stage twiddles to inputs x1..x12 (x0 has unit twiddle)
 * @details Complex multiplication: x * W = (x_re + i*x_im) * (w_re + i*w_im)
 *          Uses FMA for efficiency: tmp_re = x_re*w_re - x_im*w_im
 *                                    tmp_im = x_re*w_im + x_im*w_re
 */
#define APPLY_STAGE_TWIDDLES_R13_AVX512_SOA_NATIVE(k, K, x1, x2, x3, x4, x5, x6, \
    ...
    {                                                                            \
        ...
        {                                                                        \
            ...
            w_re = _mm512_loadu_pd(&stage_tw->re[10 * K + k]);                   \
            w_im = _mm512_loadu_pd(&stage_tw->im[10 * K + k]);                   \
            x_re = extract_re_avx512(x11);                                       \
            x_im = extract_im_avx512(x11);                                       \
            tmp_re = _mm512_fmsub_pd(x_re, w_re, _mm512_mul_pd(x_im, w_im));     \
            tmp_im = _mm512_fmadd_pd(x_re, w_im, _mm512_mul_pd(x_im, w_re));     \
            x11 = interleave_ri_avx512(tmp_re, tmp_im);                          \
            w_re = _mm512_loadu_pd(&stage_tw->re[11 * K + k]);                   \
            w_im = _mm512_loadu_pd(&stage_tw->im[11 * K + k]);                   \
            x_re = extract_re_avx512(x12);                                       \
            x_im = extract_im_avx512(x12);                                       \
            tmp_re = _mm512_fmsub_pd(x_re, w_re, _mm512_mul_pd(x_im, w_im));     \
            tmp_im = _mm512_fmadd_pd(x_re, w_im, _mm512_mul_pd(x_im, w_re));     \
            x12 = interleave_ri_avx512(tmp_re, tmp_im);                          \
        }                                                                        \
    } while (0)
 ...
 */
#define APPLY_STAGE_TWIDDLES_R13_AVX512_SOA_NATIVE_MASKED(k, K, mask, x1, x2,    \
                                                          x3, x4, x5, x6, x7,    \
                                                          x8, x9, x10, x11, x12, \
                                                          stage_tw, sub_len)     \
    do                                                                           \
    {                                                                            \
        ...
        {                                                                        \
            ...
            tmp_im = _mm512_fmadd_pd(x_re, w_im, _mm512_mul_pd(x_im, w_re));     \
            x10 = interleave_ri_avx512(tmp_re, tmp_im);                          \
            w_re = _mm512_maskz_loadu_pd(mask, &stage_tw->re[10 * K + k]);       \
            w_im = _mm512_maskz_loadu_pd(mask, &stage_tw->im[10 * K + k]);       \
            x_re = extract_re_avx512(x11);                                       \
            x_im = extract_im_avx512(x11);                                       \
            tmp_re = _mm512_fmsub_pd(x_re, w_re, _mm512_mul_pd(x_im, w_im));     \
            tmp_im = _mm512_fmadd_pd(x_re, w_im, _mm512_mul_pd(x_im, w_re));     \
            x11 = interleave_ri_avx512(tmp_re, tmp_im);                          \
            w_re = _mm512_maskz_loadu_pd(mask, &stage_tw->re[11 * K + k]);       \
            w_im = _mm512_maskz_loadu_pd(mask, &stage_tw->im[11 * K + k]);       \
            x_re = extract_re_avx512(x12);                                       \
            x_im = extract_im_avx512(x12);                                       \
            tmp_re = _mm512_fmsub_pd(x_re, w_re, _mm512_mul_pd(x_im, w_im));     \
            tmp_im = _mm512_fmadd_pd(x_re, w_im, _mm512_mul_pd(x_im, w_re));     \
 ...
 */
#define RADIX13_BUTTERFLY_CORE_AVX512(x0, x1, x2, x3, x4, x5, x6, x7, x8,                                      \
                                      x9, x10, x11, x12, t0, t1, t2, t3, t4,                                   \
                                      t5, s0, s1, s2, s3, s4, s5, y0)                                          \
    do                                                                                                         \
    {                                                                                                          \
        t0 = _mm512_add_pd(x1, x12);                                                                           \
        t1 = _mm512_add_pd(x2, x11);                                                                           \
        t2 = _mm512_add_pd(x3, x10);                                                                           \
        t3 = _mm512_add_pd(x4, x9);                                                                            \
        t4 = _mm512_add_pd(x5, x8);                                                                            \
        t5 = _mm512_add_pd(x6, x7);                                                                            \
        s0 = _mm512_sub_pd(x1, x12);                                                                           \
        s1 = _mm512_sub_pd(x2, x11);                                                                           \
        s2 = _mm512_sub_pd(x3, x10);                                                                           \
        s3 = _mm512_sub_pd(x4, x9);                                                                            \
        s4 = _mm512_sub_pd(x5, x8);                                                                            \
        s5 = _mm512_sub_pd(x6, x7);                                                                            \
        y0 = _mm512_add_pd(x0,                                                                                 \
                           _mm512_add_pd(t0,                                                                   \
                                         _mm512_add_pd(t1,                                                     \
                                                       _mm512_add_pd(t2,                                       \
 ...
 */
#define RADIX13_REAL_PAIR1_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
...
#define RADIX13_REAL_PAIR2_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
        __m512d term = _mm512_fmadd_pd(KC.c2, t0,                                                                                     \
                                       _mm512_fmadd_pd(KC.c4, t1,                                                                     \
                                                       _mm512_fmadd_pd(KC.c6, t2,                                                     \
                                                                       _mm512_fmadd_pd(KC.c5, t3,                                     \
                                                                                       _mm512_fmadd_pd(KC.c3, t4,                     \
                                                                                                       _mm512_mul_pd(KC.c1, t5)))))); \
        real_out = _mm512_add_pd(x0, term);                                                                                           \
    } while (0)

#define RADIX13_REAL_PAIR3_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
        __m512d term = _mm512_fmadd_pd(KC.c3, t0,                                                                                     \
                                       _mm512_fmadd_pd(KC.c6, t1,                                                                     \
                                                       _mm512_fmadd_pd(KC.c4, t2,                                                     \
                                                                       _mm512_fmadd_pd(KC.c1, t3,                                     \
                                                                                       _mm512_fmadd_pd(KC.c5, t4,                     \
                                                                                                       _mm512_mul_pd(KC.c2, t5)))))); \
        real_out = _mm512_add_pd(x0, term);                                                                                           \
    } while (0)

#define RADIX13_REAL_PAIR4_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
        __m512d term = _mm512_fmadd_pd(KC.c4, t0,                                                                                     \
                                       _mm512_fmadd_pd(KC.c5, t1,                                                                     \
                                                       _mm512_fmadd_pd(KC.c1, t2,                                                     \
                                                                       _mm512_fmadd_pd(KC.c6, t3,                                     \
                                                                                       _mm512_fmadd_pd(KC.c2, t4,                     \
                                                                                                       _mm512_mul_pd(KC.c3, t5)))))); \
        real_out = _mm512_add_pd(x0, term);                                                                                           \
    } while (0)

#define RADIX13_REAL_PAIR5_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
                                       ...
                                       _mm512_fmadd_pd(KC.c3, t1,                                                                     \
                                                       _mm512_fmadd_pd(KC.c5, t2,                                                     \
                                                                       _mm512_fmadd_pd(KC.c2, t3,                                     \
                                                                                       _mm512_fmadd_pd(KC.c6, t4,                     \
                                                                                                       _mm512_mul_pd(KC.c4, t5)))))); \
        real_out = _mm512_add_pd(x0, term);                                                                                           \
    } while (0)

#define RADIX13_REAL_PAIR6_AVX512(x0, t0, t1, t2, t3, t4, t5, KC, real_out)                                                           \
    do                                                                                                                                \
    {                                                                                                                                 \
        __m512d term = _mm512_fmadd_pd(KC.c6, t0,                                                                                     \
                                       _mm512_fmadd_pd(KC.c1, t1,                                                                     \
                                                       _mm512_fmadd_pd(KC.c2, t2,                                                     \
                                                                       _mm512_fmadd_pd(KC.c3, t3,                                     \
                                                                                       _mm512_fmadd_pd(KC.c4, t4,                     \
                                                                                                       _mm512_mul_pd(KC.c5, t5)))))); \
        real_out = _mm512_add_pd(x0, term);                                                                                           \
    } while (0)
 ...
 */
#define RADIX13_IMAG_PAIR1_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                             \
    do                                                                                                                                \
    {                                                                                                                                 \
                                       ...
                                       _mm512_fmadd_pd(KC.s2, s1,                                                                     \
                                                       _mm512_fmadd_pd(KC.s3, s2,                                                     \
                                                                       _mm512_fmadd_pd(KC.s4, s3,                                     \
                                                                                       _mm512_fmadd_pd(KC.s5, s4,                     \
                                                                                                       _mm512_mul_pd(KC.s6, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                     \
    } while (0)

#define RADIX13_IMAG_PAIR2_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                               \
    do                                                                                                                                  \
    {                                                                                                                                   \
        __m512d base = _mm512_fmadd_pd(KC.s2, s0,                                                                                       \
                                       _mm512_fmadd_pd(KC.s4, s1,                                                                       \
                                                       _mm512_fmadd_pd(KC.s6, s2,                                                       \
                                                                       _mm512_fnmadd_pd(KC.s5, s3,                                      \
                                                                                        _mm512_fnmadd_pd(KC.s3, s4,                     \
                                                                                                         _mm512_mul_pd(KC.s1, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                       \
    } while (0)

#define RADIX13_IMAG_PAIR3_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
        __m512d base = _mm512_fmadd_pd(KC.s3, s0,                                                                                        \
                                       _mm512_fmadd_pd(KC.s6, s1,                                                                        \
                                                       _mm512_fnmadd_pd(KC.s4, s2,                                                       \
                                                                        _mm512_fnmadd_pd(KC.s1, s3,                                      \
                                                                                         _mm512_fnmadd_pd(KC.s5, s4,                     \
                                                                                                          _mm512_mul_pd(KC.s2, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                        \
    } while (0)

#define RADIX13_IMAG_PAIR4_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
        __m512d base = _mm512_fmadd_pd(KC.s4, s0,                                                                                        \
                                       _mm512_fnmadd_pd(KC.s5, s1,                                                                       \
                                                        _mm512_fnmadd_pd(KC.s1, s2,                                                      \
                                                                         _mm512_fmadd_pd(KC.s6, s3,                                      \
                                                                                         _mm512_fnmadd_pd(KC.s2, s4,                     \
                                                                                                          _mm512_mul_pd(KC.s3, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                        \
    } while (0)

#define RADIX13_IMAG_PAIR5_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
                                       ...
                                       _mm512_fnmadd_pd(KC.s3, s1,                                                                       \
                                                        _mm512_fnmadd_pd(KC.s5, s2,                                                      \
                                                                         _mm512_fnmadd_pd(KC.s2, s3,                                     \
                                                                                          _mm512_fmadd_pd(KC.s6, s4,                     \
                                                                                                          _mm512_mul_pd(KC.s4, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                        \
    } while (0)

#define RADIX13_IMAG_PAIR6_FV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                 \
    do                                                                                                                                    \
    {                                                                                                                                     \
        __m512d base = _mm512_fmadd_pd(KC.s6, s0,                                                                                         \
                                       _mm512_fnmadd_pd(KC.s1, s1,                                                                        \
                                                        _mm512_fnmadd_pd(KC.s2, s2,                                                       \
                                                                         _mm512_fnmadd_pd(KC.s3, s3,                                      \
                                                                                          _mm512_fnmadd_pd(KC.s4, s4,                     \
                                                                                                           _mm512_mul_pd(KC.s5, s5)))))); \
        rot_out = rotate_by_minus_i_avx512(base);                                                                                         \
    } while (0)
 ...
 */
#define RADIX13_IMAG_PAIR1_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                             \
    do                                                                                                                                \
    {                                                                                                                                 \
        __m512d base = _mm512_fmadd_pd(KC.s1, s0,                                                                                     \
                                       _mm512_fmadd_pd(KC.s2, s1,                                                                     \
...
#define RADIX13_IMAG_PAIR2_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                               \
    do                                                                                                                                  \
    {                                                                                                                                   \
        __m512d base = _mm512_fmadd_pd(KC.s2, s0,                                                                                       \
                                       _mm512_fmadd_pd(KC.s4, s1,                                                                       \
...
#define RADIX13_IMAG_PAIR3_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
        __m512d base = _mm512_fmadd_pd(KC.s3, s0,                                                                                        \
                                       _mm512_fmadd_pd(KC.s6, s1,                                                                        \
...
#define RADIX13_IMAG_PAIR4_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
        __m512d base = _mm512_fmadd_pd(KC.s4, s0,                                                                                        \
                                       _mm512_fnmadd_pd(KC.s5, s1,                                                                       \
...
#define RADIX13_IMAG_PAIR5_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                \
    do                                                                                                                                   \
    {                                                                                                                                    \
        __m512d base = _mm512_fmadd_pd(KC.s5, s0,                                                                                        \
                                       _mm512_fnmadd_pd(KC.s3, s1,                                                                       \
...
#define RADIX13_IMAG_PAIR6_BV_AVX512(s0, s1, s2, s3, s4, s5, KC, rot_out)                                                                 \
    do                                                                                                                                    \
    {                                                                                                                                     \
        __m512d base = _mm512_fmadd_pd(KC.s6, s0,                                                                                         \
                                       _mm512_fnmadd_pd(KC.s1, s1,                                                                        \
 ...
 */
#define RADIX13_ASSEMBLE_PAIR_AVX512(real_part, rot_part, y_m, y_conj) \
    do                                                                 \
    {                                                                  \
...
//==============================================================================
// FORWARD BUTTERFLY - FULL (16 complex elements)
//==============================================================================

/**
 * @brief Radix-13 forward butterfly - processes 16 complex numbers (8 lo + 8 hi)
 ...
 */
#define RADIX13_BUTTERFLY_FV_AVX512_NATIVE_SOA_FULL(k, K, in_re, in_im,        \
                                                    stage_tw, out_re, out_im,  \
                                                    sub_len, KC)               \
    do                                                                         \
    {                                                                          \
        /* ============================================================== */   \
        /* LOAD ALL 13 LANES (16 complex numbers: 8 lo + 8 hi)          */     \
        /* ============================================================== */   \
        __m512d x0_lo, x0_hi, x1_lo, x1_hi, x2_lo, x2_hi, x3_lo, x3_hi;        \
        __m512d x4_lo, x4_hi, x5_lo, x5_hi, x6_lo, x6_hi, x7_lo, x7_hi;        \
        __m512d x8_lo, x8_hi, x9_lo, x9_hi, x10_lo, x10_hi, x11_lo, x11_hi;    \
        __m512d x12_lo, x12_hi;                                                \
        LOAD_13_LANES_AVX512_NATIVE_SOA_FULL(k, K, in_re, in_im,               \
                                             x0_lo, x0_hi, x1_lo, x1_hi,       \
                                             x2_lo, x2_hi, x3_lo, x3_hi,       \
                                             x4_lo, x4_hi, x5_lo, x5_hi,       \
                                             x6_lo, x6_hi, x7_lo, x7_hi,       \
        ...
        APPLY_STAGE_TWIDDLES_R13_AVX512_SOA_NATIVE(k, K, x1_lo, x2_lo,         \
                                                   x3_lo, x4_lo, x5_lo,        \
                                                   x6_lo, x7_lo, x8_lo,        \
                                                   x9_lo, x10_lo, x11_lo,      \
        ...
        RADIX13_REAL_PAIR2_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
                                  t4_lo, t5_lo, KC, real2_lo);                 \
        RADIX13_REAL_PAIR3_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
                                  t4_lo, t5_lo, KC, real3_lo);                 \
        RADIX13_REAL_PAIR4_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
                                  t4_lo, t5_lo, KC, real4_lo);                 \
        RADIX13_REAL_PAIR5_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
                                  t4_lo, t5_lo, KC, real5_lo);                 \
        RADIX13_REAL_PAIR6_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
        ...
        RADIX13_IMAG_PAIR1_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot1_lo);                      \
        RADIX13_IMAG_PAIR2_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot2_lo);                      \
        RADIX13_IMAG_PAIR3_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot3_lo);                      \
        RADIX13_IMAG_PAIR4_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot4_lo);                      \
        RADIX13_IMAG_PAIR5_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot5_lo);                      \
        RADIX13_IMAG_PAIR6_FV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot6_lo);                      \
        /* Assemble conjugate pairs: y_m and y_(13-m) */                       \
        ...
        RADIX13_ASSEMBLE_PAIR_AVX512(real2_lo, rot2_lo, y2_lo, y11_lo);        \
        RADIX13_ASSEMBLE_PAIR_AVX512(real3_lo, rot3_lo, y3_lo, y10_lo);        \
        RADIX13_ASSEMBLE_PAIR_AVX512(real4_lo, rot4_lo, y4_lo, y9_lo);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real5_lo, rot5_lo, y5_lo, y8_lo);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real6_lo, rot6_lo, y6_lo, y7_lo);         \
        ...
        APPLY_STAGE_TWIDDLES_R13_AVX512_SOA_NATIVE(k + 8, K, x1_hi, x2_hi,     \
                                                   x3_hi, x4_hi, x5_hi,        \
                                                   x6_hi, x7_hi, x8_hi,        \
                                                   x9_hi, x10_hi, x11_hi,      \
        ...
        RADIX13_REAL_PAIR1_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real1_hi);                 \
        RADIX13_REAL_PAIR2_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real2_hi);                 \
        RADIX13_REAL_PAIR3_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real3_hi);                 \
        RADIX13_REAL_PAIR4_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real4_hi);                 \
        RADIX13_REAL_PAIR5_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real5_hi);                 \
        RADIX13_REAL_PAIR6_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real6_hi);                 \
        __m512d rot1_hi, rot2_hi, rot3_hi, rot4_hi, rot5_hi, rot6_hi;          \
        RADIX13_IMAG_PAIR1_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot1_hi);                      \
        RADIX13_IMAG_PAIR2_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot2_hi);                      \
        RADIX13_IMAG_PAIR3_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot3_hi);                      \
        RADIX13_IMAG_PAIR4_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot4_hi);                      \
        RADIX13_IMAG_PAIR5_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot5_hi);                      \
        RADIX13_IMAG_PAIR6_FV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot6_hi);                      \
        __m512d y1_hi, y2_hi, y3_hi, y4_hi, y5_hi, y6_hi;                      \
        __m512d y7_hi, y8_hi, y9_hi, y10_hi, y11_hi, y12_hi;                   \
        RADIX13_ASSEMBLE_PAIR_AVX512(real1_hi, rot1_hi, y1_hi, y12_hi);        \
        RADIX13_ASSEMBLE_PAIR_AVX512(real2_hi, rot2_hi, y2_hi, y11_hi);        \
        RADIX13_ASSEMBLE_PAIR_AVX512(real3_hi, rot3_hi, y3_hi, y10_hi);        \
        RADIX13_ASSEMBLE_PAIR_AVX512(real4_hi, rot4_hi, y4_hi, y9_hi);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real5_hi, rot5_hi, y5_hi, y8_hi);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real6_hi, rot6_hi, y6_hi, y7_hi);         \
        /* Store HI half */                                                    \
        __m512d re0_hi = extract_re_avx512(y0_hi);                             \
        __m512d im0_hi = extract_im_avx512(y0_hi);                             \
        _mm512_storeu_pd(&(out_re)[0 * (K) + (k) + 8], re0_hi);                \
...
//==============================================================================
// FORWARD BUTTERFLY - TAIL (1-15 remaining elements)
//==============================================================================

/**
 * @brief Radix-13 forward butterfly - tail handling with masks
 * @details BRANCHLESS tail handling (2-5% speedup)
 *          Computes masks once, uses predicated loads/stores
 */
#define RADIX13_BUTTERFLY_FV_AVX512_NATIVE_SOA_TAIL(k, K, remaining,           \
                                                    in_re, in_im,              \
                                                    stage_tw, out_re, out_im,  \
                                                    sub_len, KC)               \
    do                                                                         \
    {                                                                          \
        /* Compute masks once (branchless) */                                  \
        ...
        __m512d x8_lo, x8_hi, x9_lo, x9_hi, x10_lo, x10_hi, x11_lo, x11_hi;    \
        __m512d x12_lo, x12_hi;                                                \
        LOAD_13_LANES_AVX512_NATIVE_SOA_MASKED(k, K, mask_lo, mask_hi,         \
                                               in_re, in_im,                   \
                                               x0_lo, x0_hi, x1_lo, x1_hi,     \
                                               x2_lo, x2_hi, x3_lo, x3_hi,     \
                                               x4_lo, x4_hi, x5_lo, x5_hi,     \
        ...
        RADIX13_ASSEMBLE_PAIR_AVX512(real5_lo, rot5_lo, y5_lo, y8_lo);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real6_lo, rot6_lo, y6_lo, y7_lo);         \
        STORE_13_LANES_AVX512_NATIVE_SOA_LO_MASKED(k, K, remaining_lo,         \
                                                   out_re, out_im,             \
                                                   y0_lo, y1_lo, y2_lo,        \
                                                   y3_lo, y4_lo, y5_lo,        \
                                                   y6_lo, y7_lo, y8_lo,        \
        ...
        /* ============================================================== */   \
        BEGIN_REGISTER_SCOPE                                                   \
        APPLY_STAGE_TWIDDLES_R13_AVX512_SOA_NATIVE_MASKED(k + 8, K,            \
                                                          mask_hi, x1_hi,      \
                                                          x2_hi, x3_hi,        \
                                                          x4_hi, x5_hi,        \
                                                          x6_hi, x7_hi,        \
        ...
        RADIX13_ASSEMBLE_PAIR_AVX512(real5_hi, rot5_hi, y5_hi, y8_hi);         \
        RADIX13_ASSEMBLE_PAIR_AVX512(real6_hi, rot6_hi, y6_hi, y7_hi);         \
        STORE_13_LANES_AVX512_NATIVE_SOA_HI_MASKED(k, K, remaining_hi,         \
                                                   out_re, out_im,             \
                                                   y0_hi, y1_hi, y2_hi,        \
                                                   y3_hi, y4_hi, y5_hi,        \
                                                   y6_hi, y7_hi, y8_hi,        \
...
//==============================================================================
// BACKWARD BUTTERFLY - FULL (16 complex elements)
//==============================================================================

/**
 * @brief Radix-13 backward butterfly - processes 16 complex numbers
 ...
 */
#define RADIX13_BUTTERFLY_BV_AVX512_NATIVE_SOA_FULL(k, K, in_re, in_im,        \
                                                    stage_tw, out_re, out_im,  \
                                                    sub_len, KC)               \
    do                                                                         \
    {                                                                          \
        ...
        RADIX13_REAL_PAIR6_AVX512(x0_lo, t0_lo, t1_lo, t2_lo, t3_lo,           \
                                  t4_lo, t5_lo, KC, real6_lo);                 \
        /* BACKWARD: Use BV macros (rotate_by_plus_i) */                       \
        __m512d rot1_lo, rot2_lo, rot3_lo, rot4_lo, rot5_lo, rot6_lo;          \
        RADIX13_IMAG_PAIR1_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot1_lo);                      \
        RADIX13_IMAG_PAIR2_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot2_lo);                      \
        RADIX13_IMAG_PAIR3_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot3_lo);                      \
        RADIX13_IMAG_PAIR4_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot4_lo);                      \
        RADIX13_IMAG_PAIR5_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot5_lo);                      \
        RADIX13_IMAG_PAIR6_BV_AVX512(s0_lo, s1_lo, s2_lo, s3_lo, s4_lo,        \
                                     s5_lo, KC, rot6_lo);                      \
        __m512d y1_lo, y2_lo, y3_lo, y4_lo, y5_lo, y6_lo;                      \
        __m512d y7_lo, y8_lo, y9_lo, y10_lo, y11_lo, y12_lo;                   \
        RADIX13_ASSEMBLE_PAIR_AVX512(real1_lo, rot1_lo, y1_lo, y12_lo);        \
        ...
        RADIX13_REAL_PAIR6_AVX512(x0_hi, t0_hi, t1_hi, t2_hi, t3_hi,           \
                                  t4_hi, t5_hi, KC, real6_hi);                 \
        __m512d rot1_hi, rot2_hi, rot3_hi, rot4_hi, rot5_hi, rot6_hi;          \
        RADIX13_IMAG_PAIR1_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot1_hi);                      \
        RADIX13_IMAG_PAIR2_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot2_hi);                      \
        RADIX13_IMAG_PAIR3_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot3_hi);                      \
        RADIX13_IMAG_PAIR4_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot4_hi);                      \
        RADIX13_IMAG_PAIR5_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot5_hi);                      \
        RADIX13_IMAG_PAIR6_BV_AVX512(s0_hi, s1_hi, s2_hi, s3_hi, s4_hi,        \
                                     s5_hi, KC, rot6_hi);                      \
        __m512d y1_hi, y2_hi, y3_hi, y4_hi, y5_hi, y6_hi;                      \
        __m512d y7_hi, y8_hi, y9_hi, y10_hi, y11_hi, y12_hi;                   \
        RADIX13_ASSEMBLE_PAIR_AVX512(real1_hi, rot1_hi, y1_hi, y12_hi);        \
...
//==============================================================================
// BACKWARD BUTTERFLY - TAIL (1-15 remaining elements)
//==============================================================================

/**
 * @brief Radix-13 backward butterfly - tail handling with masks
 * @details Identical to forward tail, but uses BV macros instead of FV
 */
#define RADIX13_BUTTERFLY_BV_AVX512_NATIVE_SOA_TAIL(k, K, remaining,           \
                                                    in_re, in_im,              \
                                                    stage_tw, out_re, out_im,  \
                                                    sub_len, KC)               \
    do                                                                         \
 ...
 *
 * void radix13_fft_forward_pass(size_t K, const double *in_re,
 ...
 *     {
 *         RADIX13_BUTTERFLY_FV_AVX512_NATIVE_SOA_FULL(k, K, in_re, in_im,
 *                                                     stage_tw, out_re, out_im,
 *                                                     sub_len, KC);
 *     }
 *
 ...
 *     if (remaining > 0)
 *     {
 *         RADIX13_BUTTERFLY_FV_AVX512_NATIVE_SOA_TAIL(main_iterations, K,
