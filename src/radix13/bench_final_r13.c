...
static void naive(int dir, size_t K, const double *ir, const double *ii, double *nr, double *ni)
{
    for (size_t k = 0; k < K; k++)
        for (size_t m = 0; m < 13; m++)
        {
            ...
            {
                double a = dir * 2.0 * M_PI * (double)m * (double)n / 13.0;
...
static void bench(size_t K, int warm, int trials)
{
    ...
    for (size_t k = 0; k < K; k++)
        for (size_t n = 0; n < 13; n++)
        {
            fin[k * 13 + n][0] = ir[n * K + k];
            fin[k * 13 + n][1] = ii_[n * K + k];
        }
    int na[1] = {13};
    ...
    {
        double *p1 = aa64(N), *p2 = aa64(N), *p3 = aa64(N), *p4 = aa64(N);
        r13_pack(ir, ii_, p1, p2, K, 1);
        for (int i = 0; i < warm; i++)
            r13_genfft_packed_fwd_scalar(p1, p2, p3, p4, K);
        for (int t = 0; t < trials; t++)
        {
            double t0 = get_ns();
            r13_genfft_packed_fwd_scalar(p1, p2, p3, p4, K);
...
{
    printf("════════════════════════════════════════════════════════════════\n");
    printf("  VectorFFT DFT-13 (scalar");
#ifdef __AVX2__
    printf(" + AVX2");
#endif
#ifdef __AVX512F__
    ...
    {
        t++;
        p += verify("scalar fwd", Ks[i], radix13_genfft_fwd_scalar, -1);
    }
    for (int i = 0; i < 5; i++)
    {
        t++;
        p += verify("scalar bwd", Ks[i], radix13_genfft_bwd_scalar, +1);
    }
...
#ifdef __AVX2__
    {
        ...
        {
            t++;
            p += verify("avx2 fwd", Ka[i], radix13_genfft_fwd_avx2, -1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify("avx2 bwd", Ka[i], radix13_genfft_bwd_avx2, +1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify_packed("packed avx2", Ka[i], 4, r13_genfft_packed_fwd_avx2);
        }
    }
#else
...
#ifdef __AVX512F__
    {
        ...
        {
            t++;
            p += verify("avx512 fwd", Kz[i], radix13_genfft_fwd_avx512, -1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify("avx512 bwd", Kz[i], radix13_genfft_bwd_avx512, +1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify_packed("packed avx512", Kz[i], 8, r13_genfft_packed_fwd_avx512);
        }
    }
#else
