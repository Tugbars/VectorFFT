...
static void naive(int dir,size_t K,const double*ir,const double*ii,double*nr,double*ni){
    for(size_t k=0;k<K;k++) for(int m=0;m<13;m++){
        double sr=0,si=0;
        for(int n=0;n<13;n++){double a=dir*2.0*M_PI*m*n/13.0;
...
static int verify_packed(const char*l,size_t K,size_t T,
    ...
    r13_pack(ir,ii_,pir,pii,K,T);
    fn(pir,pii,por,poi,K);
    r13_unpack(por,poi,sor,soi,K,T);
...
static void bench(size_t K, int warm, int trials){
    ...
    for(size_t k=0;k<K;k++) for(int n=0;n<13;n++){
        fin[k*13+n][0]=ir[n*K+k]; fin[k*13+n][1]=ii_[n*K+k];}
    int na[1]={13};
    fftw_plan plan=fftw_plan_many_dft(1,na,(int)K,
        fin,NULL,1,13,fout,NULL,1,13,FFTW_FORWARD,FFTW_MEASURE);
    ...
    {size_t T=8;
     ...
     r13_pack(ir,ii_,p1,p2,K,T);
     for(int i=0;i<warm;i++) r13_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
     for(int t=0;t<trials;t++){double t0=get_ns();
         r13_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
         double dt=get_ns()-t0;if(dt<ns_z)ns_z=dt;}
     r32_aligned_free(p1);r32_aligned_free(p2);r32_aligned_free(p3);r32_aligned_free(p4);}
    {size_t T=4;
     double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
     r13_pack(ir,ii_,p1,p2,K,T);
     for(int i=0;i<warm;i++) r13_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
     for(int t=0;t<trials;t++){double t0=get_ns();
         r13_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
...
int main(void){
    printf("════════════════════════════════════════════════════════════════\n");
    printf("  VectorFFT DFT-13 — FINAL (scalar + AVX2 + AVX-512)\n");
    printf("  206 ops, 20 constants (genfft-derived arithmetic)\n");
    printf("════════════════════════════════════════════════════════════════\n\n");

    int p=0,t=0;
    printf("── Correctness ──\n");
    size_t Ks[]={1,7,8,64,1024};
    for(int i=0;i<5;i++){t++;p+=verify("scalar fwd",Ks[i],radix13_genfft_fwd_scalar,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("scalar bwd",Ks[i],radix13_genfft_bwd_scalar,+1);}
    size_t Ka[]={4,8,16,64,256};
    for(int i=0;i<5;i++){t++;p+=verify("avx2 fwd",Ka[i],radix13_genfft_fwd_avx2,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("avx2 bwd",Ka[i],radix13_genfft_bwd_avx2,+1);}
    size_t Kz[]={8,16,64,256,1024};
    for(int i=0;i<5;i++){t++;p+=verify("avx512 fwd",Kz[i],radix13_genfft_fwd_avx512,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("avx512 bwd",Kz[i],radix13_genfft_bwd_avx512,+1);}
    for(int i=0;i<5;i++){t++;p+=verify_packed("packed avx2",Ka[i],4,r13_genfft_packed_fwd_avx2);}
    for(int i=0;i<5;i++){t++;p+=verify_packed("packed avx512",Kz[i],8,r13_genfft_packed_fwd_avx512);}
    {size_t K=8,N=13*K;
     double *ir=aa64(N),*ii_=aa64(N),*ar=aa64(N),*ai=aa64(N),*zr=aa64(N),*zi=aa64(N);
     fill_rand(ir,N,7777); fill_rand(ii_,N,8888);
     radix13_genfft_fwd_avx2(ir,ii_,ar,ai,K);
     radix13_genfft_fwd_avx512(ir,ii_,zr,zi,K);
     double err=0;
