 ...
 * @brief Radix-8 Backward FFT Stage Driver — ISA Dispatch
 *
 * @details
 * Native SoA radix-8 DIF backward butterfly.
...
/*============================================================================
 * DISPATCH: BLOCKED4 BACKWARD
 *============================================================================*/

static void dispatch_blocked4_backward(
    double *out_re,
    double *out_im,
    const double *in_re,
    const double *in_im,
    ...
    int K)
{
    ...
    if (K >= AVX512_MIN_K && (K & 7) == 0) {
        radix8_stage_blocked4_backward_avx512((size_t)K, in_re, in_im, out_re, out_im, tw);
        return;
    }
#endif
#if defined(__AVX2__)
...
#endif
    radix8_stage_blocked4_backward_scalar((size_t)K, in_re, in_im, out_re, out_im, tw);
}

/*============================================================================
 * DISPATCH: BLOCKED2 BACKWARD
 *============================================================================*/

static void dispatch_blocked2_backward(
    double *out_re,
    double *out_im,
    const double *in_re,
    const double *in_im,
...
#if defined(__AVX512F__)
    if (K >= AVX512_MIN_K && (K & 7) == 0) {
        radix8_stage_blocked2_backward_avx512((size_t)K, in_re, in_im, out_re, out_im, tw);
        return;
    }
#endif
#if defined(__AVX2__)
...
#endif
    radix8_stage_blocked2_backward_scalar((size_t)K, in_re, in_im, out_re, out_im, tw);
}

/*============================================================================
 * DISPATCH: N1 BACKWARD
 *============================================================================*/

static void dispatch_n1_backward(
    double *out_re,
    double *out_im,
    const double *in_re,
    const double *in_im,
...
#if defined(__AVX512F__)
    if (K >= AVX512_MIN_K && (K & 7) == 0) {
        radix8_stage_n1_backward_avx512((size_t)K, in_re, in_im, out_re, out_im);
        return;
    }
#endif
#if defined(__AVX2__)
    if (K >= AVX2_MIN_K && (K & 3) == 0) {
        radix8_stage_n1_backward_avx2((size_t)K, in_re, in_im, out_re, out_im);
        return;
    }
#endif
    radix8_stage_n1_backward_scalar((size_t)K, in_re, in_im, out_re, out_im);
}

/*============================================================================
 * PUBLIC: fft_radix8_bv — Backward with twiddles
 *============================================================================*/
    ...
    int K)
{
    if (K <= RADIX8_BLOCKED4_THRESHOLD)
        dispatch_blocked4_backward(out_re, out_im, in_re, in_im, tw4, K);
    else
        dispatch_blocked2_backward(out_re, out_im, in_re, in_im, tw2, K);
}

/*============================================================================
 * PUBLIC: fft_radix8_bv_n1 — Backward twiddle-less
 *============================================================================*/
...
void fft_radix8_bv_n1(
    ...
    int K)
{
    dispatch_n1_backward(out_re, out_im, in_re, in_im, K);
}