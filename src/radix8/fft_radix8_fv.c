 ...
 * 
 * @details
 * This module implements native SoA radix-8 forward FFT with:
 * - Hybrid blocked twiddle system (BLOCKED4 for K≤256, BLOCKED2 for K>256)
 * - Standard twiddle version (fft_radix8_fv)
 * - Twiddle-less n1 version (fft_radix8_fv_n1) for first stage
...
//==============================================================================
// HELPER: Process Range (Native SoA) - BLOCKED4 - FORWARD
//==============================================================================

static void radix8_process_range_blocked4_fv(
...
#ifdef __AVX512F__
    radix8_stage_blocked4_forward_avx512(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
#ifdef __AVX2__
    radix8_stage_blocked4_forward_avx2(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
#ifdef __SSE2__
    radix8_stage_blocked4_forward_sse2(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
    radix8_stage_blocked4_forward_scalar(K, in_re, in_im, out_re, out_im, stage_tw);
}

//==============================================================================
// HELPER: Process Range (Native SoA) - BLOCKED2 - FORWARD
//==============================================================================

static void radix8_process_range_blocked2_fv(
    double *restrict out_re,
    double *restrict out_im,
    const double *restrict in_re,
    const double *restrict in_im,
...
#ifdef __AVX512F__
    radix8_stage_blocked2_forward_avx512(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
#ifdef __AVX2__
    radix8_stage_blocked2_forward_avx2(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
#ifdef __SSE2__
    radix8_stage_blocked2_forward_sse2(K, in_re, in_im, out_re, out_im, stage_tw);
    return;
#endif
    
    radix8_stage_blocked2_forward_scalar(K, in_re, in_im, out_re, out_im, stage_tw);
}
...
static void radix8_process_range_n1_fv(
    double *restrict out_re,
    double *restrict out_im,
    const double *restrict in_re,
    const double *restrict in_im,
...
#ifdef __AVX512F__
    radix8_stage_n1_forward_avx512(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
#ifdef __AVX2__
    radix8_stage_n1_forward_avx2(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
#ifdef __SSE2__
    radix8_stage_n1_forward_sse2(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
    radix8_stage_n1_forward_scalar(K, in_re, in_im, out_re, out_im);
}

//==============================================================================
// MAIN FUNCTION: Radix-8 DIF Butterfly - NATIVE SoA - FORWARD (WITH TWIDDLES)
//==============================================================================

/**
 * @brief Radix-8 DIF butterfly - Native SoA - Forward FFT - WITH TWIDDLES
 * 
 * @details
    ...
    int K)
{
    ...
    if (mode == RADIX8_TW_BLOCKED4)
    {
        ...
        radix8_process_range_blocked4_fv(out_re, out_im, in_re, in_im,
                                         stage_tw_blocked4, K, N);
    }
    else
    {
        // K > 256: Use BLOCKED2 (2 twiddle blocks loaded, 2 derived, 3 sign-flipped)
        radix8_process_range_blocked2_fv(out_re, out_im, in_re, in_im,
                                         stage_tw_blocked2, K, N);
    }
}

//==============================================================================
// MAIN FUNCTION: Radix-8 DIF Butterfly - NATIVE SoA - FORWARD (NO TWIDDLES)
//==============================================================================

/**
 * @brief Radix-8 DIF butterfly - Native SoA - Forward FFT - NO TWIDDLES (n1)
 * 
 * @details
 * Twiddle-less variant for first radix-8 stage or when all W1=...=W7=1.
 * 50-70% faster than standard version due to:
 * - No twiddle loads
 * - No complex multiplications
 * - Reduced register pressure
 * 
 * Use this for the very first stage of a forward mixed-radix FFT where
    ...
    int K)
{
    ...
    radix8_process_range_n1_fv(out_re, out_im, in_re, in_im, K, N);
}