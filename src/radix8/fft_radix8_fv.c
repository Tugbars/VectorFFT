/**
 * @file fft_radix8_fv_native_soa.c
 * @brief TRUE END-TO-END SoA Radix-8 FFT Implementation - FORWARD
 *
 * @details
 * This module implements a native Structure-of-Arrays (SoA) radix-8 FFT (forward)
...
/**
 * @brief Process radix-8 butterflies in range [k_start, k_end) - NATIVE SoA - FORWARD
 *
 * @details
 ...
 */
static void radix8_process_range_native_soa_fv(
    ...
    int use_streaming)
{
    ...
    if (use_streaming)
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512_STREAM(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, 0, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX8_PIPELINE_4_FV_NATIVE_SOA_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2_STREAM(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, 0, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX8_PIPELINE_2_FV_NATIVE_SOA_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2_STREAM(k + 1, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2(k + 1, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k < k_end; k++)
        {
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, 0, k_end);
        }
    }
    else
    {
        for (; k < k_end; k++)
        {
            RADIX8_PIPELINE_1_FV_NATIVE_SOA_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, 0, k_end);
        }
    }
#else
    ...
    for (; k < k_end; k++)
    {
        RADIX8_PIPELINE_1_FV_NATIVE_SOA_SCALAR(k, K, in_re, in_im, out_re, out_im, stage_tw);
    }
#endif
...
//==============================================================================
// MAIN FUNCTION: Radix-8 Forward Transform - NATIVE SoA
//==============================================================================

/**
 * @brief Execute one stage of radix-8 FFT - NATIVE SoA - FORWARD
 *
 * @details
 ...
 *       if (stage % 2 == 0)
 *           fft_radix8_fv_native_soa(buf_b_re, buf_b_im, buf_a_re, buf_a_im, tw[stage], K[stage]);
 *       else
 *           fft_radix8_fv_native_soa(buf_a_re, buf_a_im, buf_b_re, buf_b_im, tw[stage], K[stage]);
 *   }
 *
 *   // Convert output once
    ...
    int K)
{
    ...
    radix8_process_range_native_soa_fv(
        out_re, out_im,
        in_re, in_im,
        stage_tw,
        K,
