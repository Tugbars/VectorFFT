//==============================================================================
// fft_radix8_fv.c - Forward Radix-8 Butterfly (SOA VERSION - FULLY OPTIMIZED)
//==============================================================================
//
...
/**
 * @brief Ultra-optimized forward radix-8 butterfly (SoA version)
 * 
 ...
 * @param stage_tw      Precomputed SoA twiddles (7 blocks of K, forward sign)
    ...
    int sub_len)
{
    ...
    // Forward rotation mask: -i - UNCHANGED
    ...
    for (; k + 3 < K; k += 4)
    {
        ...
        if (use_streaming) {
            RADIX8_PIPELINE_4_FV_AVX512_STREAM_SOA(k, K, sub_outputs, stage_tw, output_buffer,
                                                   rot_mask, vw81_re, vw81_im, vw83_re, vw83_im);
        } else {
            RADIX8_PIPELINE_4_FV_AVX512_SOA(k, K, sub_outputs, stage_tw, output_buffer,
                                            rot_mask, vw81_re, vw81_im, vw83_re, vw83_im);
        }
    }
    ...
    for (; k + 1 < K; k += 2)
    {
        ...
        if (use_streaming)
        {
            RADIX8_PIPELINE_2_FV_AVX2_STREAM_SOA(k, K, sub_outputs, stage_tw, output_buffer,
                                                 rot_mask, vw81_re, vw81_im, vw83_re, vw83_im);
        }
        else
        {
            RADIX8_PIPELINE_2_FV_AVX2_SOA(k, K, sub_outputs, stage_tw, output_buffer,
                                          rot_mask, vw81_re, vw81_im, vw83_re, vw83_im);
        }
    }
    ...
    for (; k < K; k++)
    {
        ...
        //======================================================================
        // Apply W_8 geometric twiddles (forward) - UNCHANGED
        //======================================================================
 ...
 *    - Zero shuffle overhead on 7 twiddle loads
 *    - Direct re/im array access
