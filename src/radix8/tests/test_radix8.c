...
int main(void) {
    ...
    for (int ki = 0; ki < 12; ki++) {
        ...
        double *twr=alloc64(7*K),*twi=alloc64(7*K);
        ...
        for (int fwd = 1; fwd >= 0; fwd--) {
            ...
            radix8_notw_dit_kernel_fwd_scalar(ir,ii,out_r,out_i,K);
            if (!fwd) radix8_notw_dit_kernel_bwd_scalar(ir,ii,out_r,out_i,K);
            else radix8_notw_dit_kernel_fwd_scalar(ir,ii,out_r,out_i,K);
            double e = maxerr(ref_r,ref_i,out_r,out_i,NN);
            int ok = e < 1e-11;
            total++; passed += ok;
            printf("  notw %s K=%-4zu scalar  err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");

            if (K >= 4 && (K%4)==0) {
                if (fwd) radix8_notw_dit_kernel_fwd_avx2(ir,ii,out_r,out_i,K);
                else     radix8_notw_dit_kernel_bwd_avx2(ir,ii,out_r,out_i,K);
                e = maxerr(ref_r,ref_i,out_r,out_i,NN);
                ok = e < 1e-11;
                total++; passed += ok;
                printf("  notw %s K=%-4zu avx2    err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");
            }
            if (K >= 8 && (K%8)==0) {
                if (fwd) radix8_notw_dit_kernel_fwd_avx512(ir,ii,out_r,out_i,K);
                else     radix8_notw_dit_kernel_bwd_avx512(ir,ii,out_r,out_i,K);
                e = maxerr(ref_r,ref_i,out_r,out_i,NN);
                ok = e < 1e-11;
                total++; passed += ok;
                printf("  notw %s K=%-4zu avx512  err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");
            }

            /* TW */
            ref_tw_dft8(ir,ii,ref_r,ref_i,K,fwd);

            if (fwd) radix8_tw_dit_kernel_fwd_scalar(ir,ii,out_r,out_i,twr,twi,K);
            else     radix8_tw_dit_kernel_bwd_scalar(ir,ii,out_r,out_i,twr,twi,K);
            e = maxerr(ref_r,ref_i,out_r,out_i,NN);
            ok = e < 1e-11;
            total++; passed += ok;
            printf("  tw   %s K=%-4zu scalar  err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");

            if (K >= 4 && (K%4)==0) {
                if (fwd) radix8_tw_dit_kernel_fwd_avx2(ir,ii,out_r,out_i,twr,twi,K);
                else     radix8_tw_dit_kernel_bwd_avx2(ir,ii,out_r,out_i,twr,twi,K);
                e = maxerr(ref_r,ref_i,out_r,out_i,NN);
                ok = e < 1e-11;
                total++; passed += ok;
                printf("  tw   %s K=%-4zu avx2    err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");
            }
            if (K >= 8 && (K%8)==0) {
                if (fwd) radix8_tw_dit_kernel_fwd_avx512(ir,ii,out_r,out_i,twr,twi,K);
                else     radix8_tw_dit_kernel_bwd_avx512(ir,ii,out_r,out_i,twr,twi,K);
                e = maxerr(ref_r,ref_i,out_r,out_i,NN);
                ok = e < 1e-11;
                total++; passed += ok;
                printf("  tw   %s K=%-4zu avx512  err=%.1e %s\n", dir, K, e, ok?"PASS":"FAIL");
            }
        }
