//==============================================================================
// fft_radix16_bv.c - Inverse Radix-16 Butterfly (Optimized with SoA Twiddles)
//==============================================================================
    ...
    int sub_len)
{
    ...
    for (; k + 3 < K; k += 4)
    {
        ...
        PREFETCH_STAGE_TW_AVX512(k, PREFETCH_DISTANCE_AVX512, stage_tw, sub_len);

        if (use_streaming)
        {
            RADIX16_BUTTERFLY_BV_SOA_AVX512_STREAM(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
        else
        {
            RADIX16_BUTTERFLY_BV_SOA_AVX512(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
    ...
    for (; k + 1 < K; k += 2)
    {
        ...
        PREFETCH_STAGE_TW_AVX2(k, PREFETCH_L1, stage_tw, sub_len);

        if (use_streaming)
        {
            RADIX16_BUTTERFLY_BV_SOA_AVX2_STREAM(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
        else
        {
            RADIX16_BUTTERFLY_BV_SOA_AVX2(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
