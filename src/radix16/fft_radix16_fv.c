//==============================================================================
// fft_radix16_fv.c - Forward Radix-16 Butterfly (SOA VERSION - FULLY OPTIMIZED)
//==============================================================================
//
// OPTIMIZATIONS:
//   ✅ Full AVX-512 support (4 complex values/iteration)
//   ✅ Hoisted W_4 constants outside loops
//   ✅ 2-stage radix-4 decomposition with fused W_4 twiddles
//   ✅ Single-level prefetching
//   ✅ Alignment hints
//   ✅ SOA layout (zero shuffle overhead on 15 twiddle loads!)
//   ✅ Software pipelined twiddle application
//   ✅ Reduced register pressure (≤24 zmm)
//
...
/**
 * @brief Ultra-optimized forward radix-16 butterfly (SoA version)
 * 
 ...
 * @param stage_tw      Precomputed SoA twiddles (15 blocks of K, forward sign)
    ...
    int sub_len)
{
    ...
    // Forward rotation mask for radix-4: -i
    ...
    for (; k + 7 < K; k += 4)
    {
        ...
        if (use_streaming) {
            RADIX16_BUTTERFLY_FV_SOA_AVX512_STREAM(k, K, sub_outputs, stage_tw_re, stage_tw_im, 
                                                   sub_len, output_buffer, rot_mask, neg_mask);
        } else {
            RADIX16_BUTTERFLY_FV_SOA_AVX512(k, K, sub_outputs, stage_tw_re, stage_tw_im, 
                                            sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
    ...
    for (; k + 3 < K; k += 2)
    {
        ...
        if (use_streaming)
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX2_STREAM(k, K, sub_outputs, stage_tw_re, stage_tw_im,
                                                 sub_len, output_buffer, rot_mask, neg_mask);
        }
        else
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX2(k, K, sub_outputs, stage_tw_re, stage_tw_im,
                                          sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
