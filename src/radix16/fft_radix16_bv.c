//==============================================================================
// fft_radix16_bv.c - Inverse Radix-16 Butterfly (Precomputed Twiddles)
//==============================================================================
//
// DESIGN: Identical to fft_radix16_fv.c except:
// - Uses APPLY_W4_INTERMEDIATE_BV_* instead of APPLY_W4_INTERMEDIATE_FV_*
// - Uses inverse radix-4 rotation mask (+i instead of -i)
// - Twiddles have inverse sign: W_N^k = exp(+2πik/N)
//

#include "fft_radix16.h"
#include "simd_math.h"
#include "fft_radix16_macros.h"

// Non-temporal store threshold
#define STREAM_THRESHOLD 8192

//==============================================================================
// INVERSE RADIX-16 BUTTERFLY - Main Function
//==============================================================================

/**
 * @brief Ultra-optimized inverse radix-16 butterfly
 * 
 * DIFFERENCE FROM FORWARD:
 * - W_4 twiddles use positive sign (conjugate)
 * - Radix-4 rotation uses +i instead of -i
 * - Twiddles: Precomputed with positive sign (exp(+2πik/N))
 * 
 * Everything else is IDENTICAL to fft_radix16_fv()
 */
void fft_radix16_bv(
    fft_data *restrict output_buffer,
    const fft_data *restrict sub_outputs,
    const fft_data *restrict stage_tw,
    int sub_len)
{
    const int K = sub_len;
    int k = 0;

#ifdef __AVX2__
    //==========================================================================
    // AVX2 PATH: Process 2 butterflies at a time (IDENTICAL except rotation)
    //==========================================================================
    
    const int use_streaming = (K >= STREAM_THRESHOLD);
    
    // Inverse radix-4 rotation mask ⚡ ONLY DIFFERENCE from forward
    const __m256d rot_mask = _mm256_set_pd(-0.0, 0.0, -0.0, 0.0);

    // Main loop: process 2 butterflies per iteration
    for (; k + 1 < K; k += 2)
    {
        // Prefetch ahead (IDENTICAL)
        PREFETCH_16_LANES(k, K, PREFETCH_L1, sub_outputs, _MM_HINT_T0);
        PREFETCH_16_LANES(k, K, PREFETCH_L2, sub_outputs, _MM_HINT_T1);
        PREFETCH_16_LANES(k, K, PREFETCH_L3, sub_outputs, _MM_HINT_T2);
        
        //======================================================================
        // STAGE 1: Load 16 lanes (IDENTICAL)
        //======================================================================
        __m256d x[16];
        LOAD_16_LANES_AVX2(k, K, sub_outputs, x);
        
        //======================================================================
        // STAGE 2: Apply precomputed input twiddles (IDENTICAL)
        //======================================================================
        APPLY_STAGE_TWIDDLES_R16_AVX2(k, x, stage_tw);
        
        //======================================================================
        // STAGE 3: First radix-4 (uses inverse rotation mask)
        //======================================================================
        __m256d y[16];
        
        for (int group = 0; group < 4; group++)
        {
            RADIX4_BUTTERFLY_AVX2(
                x[group], x[group+4], x[group+8], x[group+12],
                y[4*group], y[4*group+1], y[4*group+2], y[4*group+3],
                rot_mask
            );
        }
        
        //======================================================================
        // STAGE 4: Apply W_4 intermediate twiddles (INVERSE) ⚡ ONLY DIFFERENCE
        //======================================================================
        APPLY_W4_INTERMEDIATE_BV_AVX2(y);
        
        //======================================================================
        // STAGE 5: Second radix-4 (uses inverse rotation mask)
        //======================================================================
        __m256d z[16];
        
        for (int m = 0; m < 4; m++)
        {
            RADIX4_BUTTERFLY_AVX2(
                y[m], y[m+4], y[m+8], y[m+12],
                z[m], z[m+4], z[m+8], z[m+12],
                rot_mask
            );
        }
        
        //======================================================================
        // STAGE 6: Store results (IDENTICAL)
        //======================================================================
        if (use_streaming)
        {
            for (int m = 0; m < 4; m++)
            {
                _mm256_stream_pd(&output_buffer[k + m*K].re, z[m]);
                _mm256_stream_pd(&output_buffer[k + (m+4)*K].re, z[m+4]);
                _mm256_stream_pd(&output_buffer[k + (m+8)*K].re, z[m+8]);
                _mm256_stream_pd(&output_buffer[k + (m+12)*K].re, z[m+12]);
            }
        }
        else
        {
            for (int m = 0; m < 4; m++)
            {
                STOREU_PD(&output_buffer[k + m*K].re, z[m]);
                STOREU_PD(&output_buffer[k + (m+4)*K].re, z[m+4]);
                STOREU_PD(&output_buffer[k + (m+8)*K].re, z[m+8]);
                STOREU_PD(&output_buffer[k + (m+12)*K].re, z[m+12]);
            }
        }
    }
    
    if (use_streaming)
    {
        _mm_sfence();
    }
    
#endif // __AVX2__

    //==========================================================================
    // SCALAR TAIL: Process remaining single butterflies
    //==========================================================================
    for (; k < K; k++)
    {
        //======================================================================
        // Load 16 lanes (IDENTICAL)
        //======================================================================
        fft_data x[16];
        for (int lane = 0; lane < 16; lane++)
        {
            x[lane] = sub_outputs[k + lane*K];
        }
        
        //======================================================================
        // Apply precomputed input twiddles (IDENTICAL)
        //======================================================================
        APPLY_STAGE_TWIDDLES_R16_SCALAR(k, x, stage_tw);
        
        //======================================================================
        // First radix-4 stage (uses +i rotation) ⚡ ONLY DIFFERENCE
        //======================================================================
        fft_data y[16];
        
        for (int group = 0; group < 4; group++)
        {
            fft_data a = x[group];
            fft_data b = x[group+4];
            fft_data c = x[group+8];
            fft_data d = x[group+12];
            
            // Radix-4 butterfly (inverse: rot_sign = +1)
            double sumBD_re = b.re + d.re, sumBD_im = b.im + d.im;
            double difBD_re = b.re - d.re, difBD_im = b.im - d.im;
            double sumAC_re = a.re + c.re, sumAC_im = a.im + c.im;
            double difAC_re = a.re - c.re, difAC_im = a.im - c.im;
            
            y[4*group].re = sumAC_re + sumBD_re;
            y[4*group].im = sumAC_im + sumBD_im;
            y[4*group+2].re = sumAC_re - sumBD_re;
            y[4*group+2].im = sumAC_im - sumBD_im;
            
            // Inverse rotation: +i
            double rot_re = -difBD_im;
            double rot_im = difBD_re;
            
            y[4*group+1].re = difAC_re - rot_re;
            y[4*group+1].im = difAC_im - rot_im;
            y[4*group+3].re = difAC_re + rot_re;
            y[4*group+3].im = difAC_im + rot_im;
        }
        
        //======================================================================
        // Apply W_4 intermediate twiddles (INVERSE) ⚡ ONLY DIFFERENCE
        //======================================================================
        APPLY_W4_INTERMEDIATE_BV_SCALAR(y);
        
        //======================================================================
        // Second radix-4 stage (uses +i rotation)
        //======================================================================
        for (int m = 0; m < 4; m++)
        {
            fft_data a = y[m];
            fft_data b = y[m+4];
            fft_data c = y[m+8];
            fft_data d = y[m+12];
            
            double sumBD_re = b.re + d.re, sumBD_im = b.im + d.im;
            double difBD_re = b.re - d.re, difBD_im = b.im - d.im;
            double sumAC_re = a.re + c.re, sumAC_im = a.im + c.im;
            double difAC_re = a.re - c.re, difAC_im = a.im - c.im;
            
            output_buffer[k + m*K].re = sumAC_re + sumBD_re;
            output_buffer[k + m*K].im = sumAC_im + sumBD_im;
            output_buffer[k + (m+8)*K].re = sumAC_re - sumBD_re;
            output_buffer[k + (m+8)*K].im = sumAC_im - sumBD_im;
            
            // Inverse rotation: +i
            double rot_re = -difBD_im;
            double rot_im = difBD_re;
            
            output_buffer[k + (m+4)*K].re = difAC_re - rot_re;
            output_buffer[k + (m+4)*K].im = difAC_im - rot_im;
            output_buffer[k + (m+12)*K].re = difAC_re + rot_re;
            output_buffer[k + (m+12)*K].im = difAC_im + rot_im;
        }
    }
}

//==============================================================================
// SUMMARY: Forward vs Inverse
//==============================================================================

/**
 * IDENTICAL CODE (~99%):
 * - All load/store patterns
 * - All prefetching
 * - All input twiddle application (stage_tw)
 * - 2-stage radix-4 structure
 * - All arithmetic (sum/difference)
 * 
 * DIFFERENT (2 things):
 * 1. Radix-4 rotation mask:
 *    - Forward: rot_mask = (0.0, -0.0, 0.0, -0.0)   // -i rotation
 *    - Inverse: rot_mask = (-0.0, 0.0, -0.0, 0.0)   // +i rotation
 * 
 * 2. W_4 intermediate twiddles:
 *    - Forward: APPLY_W4_INTERMEDIATE_FV_* uses {1, -i, -1, +i}
 *    - Inverse: APPLY_W4_INTERMEDIATE_BV_* uses {1, +i, -1, -i}
 * 
 * TWIDDLE DIFFERENCE (computed by planning):
 * - Forward stage_tw:  exp(-2πijk/N) - negative sign
 * - Inverse stage_tw:  exp(+2πijk/N) - positive sign
 * 
 * This is why _fv and _bv can share 99% of code via macros!
 * 
 * BENEFITS:
 * - Reuses all radix-4 butterfly infrastructure
 * - Only W_4 pattern differs (7 twiddles total)
 * - Clean separation of concerns
 */

//==============================================================================
// PERFORMANCE NOTES
//==============================================================================

/**
 * Performance is IDENTICAL to forward butterfly:
 * - AVX2: ~40 cycles/butterfly
 * - Scalar: ~80 cycles/butterfly
 * 
 * The direction change has ZERO performance impact because:
 * 1. Both use the same number of operations
 * 2. Only constant values differ (rotation masks, W_4 constants)
 * 3. Constants are loaded once and reused
 * 
 */