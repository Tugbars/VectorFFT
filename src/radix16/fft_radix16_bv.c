    ...
    int sub_len)
{
    ...
    for (; k + 3 < K; k += 4)
    {
        ...
        if (use_streaming)
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX512_STREAM(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
        else
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX512(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
    ...
    for (; k + 1 < K; k += 2)
    {
        ...
        if (use_streaming)
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX2_STREAM(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
        else
        {
            RADIX16_BUTTERFLY_FV_SOA_AVX2(k, K, sub_outputs, stage_tw, sub_len, output_buffer, rot_mask, neg_mask);
        }
    }
