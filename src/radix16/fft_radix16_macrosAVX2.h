/**
 * @file fft_radix16_macros_true_soa_avx2.h
 * @brief TRUE END-TO-END SoA Radix-16 Butterfly Macros - AVX2 ONLY
 *
 * @details
 * This header provides AVX2 macro implementations for radix-16 FFT butterflies
 ...
 * ✅ Reduced register pressure (≤14 ymm live)
 ...
 * - Small FFTs (256-4K):   +12-18%
 * - Medium FFTs (16K-256K): +20-30%
 * - Large FFTs (1M-16M):    +30-45%
 *
...
#ifndef FFT_RADIX16_MACROS_TRUE_SOA_AVX2_H
#define FFT_RADIX16_MACROS_TRUE_SOA_AVX2_H
 ...
 *
 * @param[out] out_re Output real parts (__m256d)
 * @param[out] out_im Output imag parts (__m256d)
 * @param[in] a_re Input real parts (__m256d)
 * @param[in] a_im Input imag parts (__m256d)
...
/**
 * @brief Radix-4 butterfly in SoA form - AVX2
 *
 * @details
 ...
 * Each lane loads 2 complex values (4 doubles) into one AVX2 register.
 *
 * Memory layout:
 *   in_re: [k, k+K, k+2K, ..., k+15K]
 *   in_im: [k, k+K, k+2K, ..., k+15K]
 */
#define LOAD_16_LANES_SOA_AVX2(k, K, in_re, in_im, x_re, x_im)    \
    do                                                             \
    {                                                              \
        x_re[0] = _mm256_load_pd(&in_re[(k) + 0 * (K)]);          \
        x_im[0] = _mm256_load_pd(&in_im[(k) + 0 * (K)]);          \
        x_re[1] = _mm256_load_pd(&in_re[(k) + 1 * (K)]);          \
        x_im[1] = _mm256_load_pd(&in_im[(k) + 1 * (K)]);          \
        x_re[2] = _mm256_load_pd(&in_re[(k) + 2 * (K)]);          \
        x_im[2] = _mm256_load_pd(&in_im[(k) + 2 * (K)]);          \
        x_re[3] = _mm256_load_pd(&in_re[(k) + 3 * (K)]);          \
        x_im[3] = _mm256_load_pd(&in_im[(k) + 3 * (K)]);          \
        x_re[4] = _mm256_load_pd(&in_re[(k) + 4 * (K)]);          \
        x_im[4] = _mm256_load_pd(&in_im[(k) + 4 * (K)]);          \
        x_re[5] = _mm256_load_pd(&in_re[(k) + 5 * (K)]);          \
        x_im[5] = _mm256_load_pd(&in_im[(k) + 5 * (K)]);          \
        x_re[6] = _mm256_load_pd(&in_re[(k) + 6 * (K)]);          \
        x_im[6] = _mm256_load_pd(&in_im[(k) + 6 * (K)]);          \
        x_re[7] = _mm256_load_pd(&in_re[(k) + 7 * (K)]);          \
        x_im[7] = _mm256_load_pd(&in_im[(k) + 7 * (K)]);          \
        x_re[8] = _mm256_load_pd(&in_re[(k) + 8 * (K)]);          \
        x_im[8] = _mm256_load_pd(&in_im[(k) + 8 * (K)]);          \
        x_re[9] = _mm256_load_pd(&in_re[(k) + 9 * (K)]);          \
        x_im[9] = _mm256_load_pd(&in_im[(k) + 9 * (K)]);          \
        x_re[10] = _mm256_load_pd(&in_re[(k) + 10 * (K)]);        \
        x_im[10] = _mm256_load_pd(&in_im[(k) + 10 * (K)]);        \
        x_re[11] = _mm256_load_pd(&in_re[(k) + 11 * (K)]);        \
        x_im[11] = _mm256_load_pd(&in_im[(k) + 11 * (K)]);        \
        x_re[12] = _mm256_load_pd(&in_re[(k) + 12 * (K)]);        \
        x_im[12] = _mm256_load_pd(&in_im[(k) + 12 * (K)]);        \
        x_re[13] = _mm256_load_pd(&in_re[(k) + 13 * (K)]);        \
        x_im[13] = _mm256_load_pd(&in_im[(k) + 13 * (K)]);        \
        x_re[14] = _mm256_load_pd(&in_re[(k) + 14 * (K)]);        \
        x_im[14] = _mm256_load_pd(&in_im[(k) + 14 * (K)]);        \
        x_re[15] = _mm256_load_pd(&in_re[(k) + 15 * (K)]);        \
        x_im[15] = _mm256_load_pd(&in_im[(k) + 15 * (K)]);        \
    } while (0)
 ...
 */
#define STORE_16_LANES_SOA_AVX2(k, K, out_re, out_im, y_re, y_im) \
    do                                                             \
    {                                                              \
        _mm256_store_pd(&out_re[(k) + 0 * (K)], y_re[0]);         \
        _mm256_store_pd(&out_im[(k) + 0 * (K)], y_im[0]);         \
        _mm256_store_pd(&out_re[(k) + 1 * (K)], y_re[1]);         \
        _mm256_store_pd(&out_im[(k) + 1 * (K)], y_im[1]);         \
        _mm256_store_pd(&out_re[(k) + 2 * (K)], y_re[2]);         \
        _mm256_store_pd(&out_im[(k) + 2 * (K)], y_im[2]);         \
        _mm256_store_pd(&out_re[(k) + 3 * (K)], y_re[3]);         \
        _mm256_store_pd(&out_im[(k) + 3 * (K)], y_im[3]);         \
        _mm256_store_pd(&out_re[(k) + 4 * (K)], y_re[4]);         \
        _mm256_store_pd(&out_im[(k) + 4 * (K)], y_im[4]);         \
        _mm256_store_pd(&out_re[(k) + 5 * (K)], y_re[5]);         \
        _mm256_store_pd(&out_im[(k) + 5 * (K)], y_im[5]);         \
        _mm256_store_pd(&out_re[(k) + 6 * (K)], y_re[6]);         \
        _mm256_store_pd(&out_im[(k) + 6 * (K)], y_im[6]);         \
        _mm256_store_pd(&out_re[(k) + 7 * (K)], y_re[7]);         \
        _mm256_store_pd(&out_im[(k) + 7 * (K)], y_im[7]);         \
        _mm256_store_pd(&out_re[(k) + 8 * (K)], y_re[8]);         \
        _mm256_store_pd(&out_im[(k) + 8 * (K)], y_im[8]);         \
        _mm256_store_pd(&out_re[(k) + 9 * (K)], y_re[9]);         \
        _mm256_store_pd(&out_im[(k) + 9 * (K)], y_im[9]);         \
        _mm256_store_pd(&out_re[(k) + 10 * (K)], y_re[10]);       \
        _mm256_store_pd(&out_im[(k) + 10 * (K)], y_im[10]);       \
        _mm256_store_pd(&out_re[(k) + 11 * (K)], y_re[11]);       \
        _mm256_store_pd(&out_im[(k) + 11 * (K)], y_im[11]);       \
        _mm256_store_pd(&out_re[(k) + 12 * (K)], y_re[12]);       \
        _mm256_store_pd(&out_im[(k) + 12 * (K)], y_im[12]);       \
        _mm256_store_pd(&out_re[(k) + 13 * (K)], y_re[13]);       \
        _mm256_store_pd(&out_im[(k) + 13 * (K)], y_im[13]);       \
        _mm256_store_pd(&out_re[(k) + 14 * (K)], y_re[14]);       \
        _mm256_store_pd(&out_im[(k) + 14 * (K)], y_im[14]);       \
        _mm256_store_pd(&out_re[(k) + 15 * (K)], y_re[15]);       \
        _mm256_store_pd(&out_im[(k) + 15 * (K)], y_im[15]);       \
    } while (0)
 ...
 */
#define STORE_16_LANES_SOA_AVX2_STREAM(k, K, out_re, out_im, y_re, y_im) \
    do                                                                    \
    {                                                                     \
        _mm256_stream_pd(&out_re[(k) + 0 * (K)], y_re[0]);               \
        _mm256_stream_pd(&out_im[(k) + 0 * (K)], y_im[0]);               \
        _mm256_stream_pd(&out_re[(k) + 1 * (K)], y_re[1]);               \
        _mm256_stream_pd(&out_im[(k) + 1 * (K)], y_im[1]);               \
        _mm256_stream_pd(&out_re[(k) + 2 * (K)], y_re[2]);               \
        _mm256_stream_pd(&out_im[(k) + 2 * (K)], y_im[2]);               \
        _mm256_stream_pd(&out_re[(k) + 3 * (K)], y_re[3]);               \
        _mm256_stream_pd(&out_im[(k) + 3 * (K)], y_im[3]);               \
        _mm256_stream_pd(&out_re[(k) + 4 * (K)], y_re[4]);               \
        _mm256_stream_pd(&out_im[(k) + 4 * (K)], y_im[4]);               \
        _mm256_stream_pd(&out_re[(k) + 5 * (K)], y_re[5]);               \
        _mm256_stream_pd(&out_im[(k) + 5 * (K)], y_im[5]);               \
        _mm256_stream_pd(&out_re[(k) + 6 * (K)], y_re[6]);               \
        _mm256_stream_pd(&out_im[(k) + 6 * (K)], y_im[6]);               \
        _mm256_stream_pd(&out_re[(k) + 7 * (K)], y_re[7]);               \
        _mm256_stream_pd(&out_im[(k) + 7 * (K)], y_im[7]);               \
        _mm256_stream_pd(&out_re[(k) + 8 * (K)], y_re[8]);               \
        _mm256_stream_pd(&out_im[(k) + 8 * (K)], y_im[8]);               \
        _mm256_stream_pd(&out_re[(k) + 9 * (K)], y_re[9]);               \
        _mm256_stream_pd(&out_im[(k) + 9 * (K)], y_im[9]);               \
        _mm256_stream_pd(&out_re[(k) + 10 * (K)], y_re[10]);             \
        _mm256_stream_pd(&out_im[(k) + 10 * (K)], y_im[10]);             \
        _mm256_stream_pd(&out_re[(k) + 11 * (K)], y_re[11]);             \
        _mm256_stream_pd(&out_im[(k) + 11 * (K)], y_im[11]);             \
        _mm256_stream_pd(&out_re[(k) + 12 * (K)], y_re[12]);             \
        _mm256_stream_pd(&out_im[(k) + 12 * (K)], y_im[12]);             \
        _mm256_stream_pd(&out_re[(k) + 13 * (K)], y_re[13]);             \
        _mm256_stream_pd(&out_im[(k) + 13 * (K)], y_im[13]);             \
        _mm256_stream_pd(&out_re[(k) + 14 * (K)], y_re[14]);             \
        _mm256_stream_pd(&out_im[(k) + 14 * (K)], y_im[14]);             \
        _mm256_stream_pd(&out_re[(k) + 15 * (K)], y_re[15]);             \
        _mm256_stream_pd(&out_im[(k) + 15 * (K)], y_im[15]);             \
    } while (0)
 ...
 */
#define PREFETCH_16_LANES_SOA_AVX2(k, K, distance, in_re, in_im, k_end)         \
    do                                                                           \
    {                                                                            \
        if ((k) + (distance) < (k_end))                                          \
        {                                                                        \
 ...
 */
#define PREFETCH_STAGE_TW_SOA_AVX2(k, distance, stage_tw, K, k_end)              \
    do                                                                            \
    {                                                                             \
        if ((k) + (distance) < (k_end))                                           \
        {                                                                         \
 ...
 *   ...
 */
#define APPLY_STAGE_TWIDDLES_R16_SOA_AVX2(k, x_re, x_im, stage_tw, K)        \
    do                                                                        \
    {                                                                         \
        ...
        __m256d w8_re = _mm256_load_pd(&stage_tw->re[7 * (K) + (k)]);        \
        __m256d w8_im = _mm256_load_pd(&stage_tw->im[7 * (K) + (k)]);        \
        __m256d w9_re = _mm256_load_pd(&stage_tw->re[8 * (K) + (k)]);        \
        __m256d w9_im = _mm256_load_pd(&stage_tw->im[8 * (K) + (k)]);        \
        ...
        /* r=10,11,12 */                                                      \
        __m256d w10_re = _mm256_load_pd(&stage_tw->re[9 * (K) + (k)]);       \
        __m256d w10_im = _mm256_load_pd(&stage_tw->im[9 * (K) + (k)]);       \
        __m256d w11_re = _mm256_load_pd(&stage_tw->re[10 * (K) + (k)]);      \
        __m256d w11_im = _mm256_load_pd(&stage_tw->im[10 * (K) + (k)]);      \
        __m256d w12_re = _mm256_load_pd(&stage_tw->re[11 * (K) + (k)]);      \
        __m256d w12_im = _mm256_load_pd(&stage_tw->im[11 * (K) + (k)]);      \
        ...
        /* r=13,14,15 */                                                      \
        __m256d w13_re = _mm256_load_pd(&stage_tw->re[12 * (K) + (k)]);      \
        __m256d w13_im = _mm256_load_pd(&stage_tw->im[12 * (K) + (k)]);      \
        __m256d w14_re = _mm256_load_pd(&stage_tw->re[13 * (K) + (k)]);      \
        __m256d w14_im = _mm256_load_pd(&stage_tw->im[13 * (K) + (k)]);      \
        __m256d w15_re = _mm256_load_pd(&stage_tw->re[14 * (K) + (k)]);      \
        __m256d w15_im = _mm256_load_pd(&stage_tw->im[14 * (K) + (k)]);      \
...
// Peak live: ~12-14 ymm (avoiding spills)
//==============================================================================

/**
 * @brief Complete radix-16 butterfly - FORWARD - Native SoA - AVX2
 *
 * @details
 * ⚡⚡⚡ ZERO SHUFFLE VERSION!
 ...
 *   - Regular cache-friendly stores
 */
#define RADIX16_PIPELINE_2_FV_NATIVE_SOA_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, rot_mask, neg_mask, prefetch_dist, k_end) \
    do                                                                                                                                 \
    {                                                                                                                                  \
        PREFETCH_16_LANES_SOA_AVX2(k, K, prefetch_dist, in_re, in_im, k_end);                                                         \
        PREFETCH_STAGE_TW_SOA_AVX2(k, prefetch_dist, stage_tw, K, k_end);                                                             \
        __m256d x_re[16], x_im[16];                                                                                                    \
        LOAD_16_LANES_SOA_AVX2(k, K, in_re, in_im, x_re, x_im);                                                                       \
        APPLY_STAGE_TWIDDLES_R16_SOA_AVX2(k, x_re, x_im, stage_tw, K);                                                                \
        ...
        STORE_16_LANES_SOA_AVX2(k, K, out_re, out_im, x_re, x_im);                                                                    \
    } while (0)

/**
 * @brief Complete radix-16 butterfly - BACKWARD - Native SoA - AVX2
 */
#define RADIX16_PIPELINE_2_BV_NATIVE_SOA_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, rot_mask, neg_mask, prefetch_dist, k_end) \
    do                                                                                                                                 \
    {                                                                                                                                  \
        PREFETCH_16_LANES_SOA_AVX2(k, K, prefetch_dist, in_re, in_im, k_end);                                                         \
        PREFETCH_STAGE_TW_SOA_AVX2(k, prefetch_dist, stage_tw, K, k_end);                                                             \
 ...
 */
#define RADIX16_PIPELINE_2_FV_NATIVE_SOA_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, rot_mask, neg_mask, prefetch_dist, k_end) \
    do                                                                                                                                        \
    {                                                                                                                                         \
        PREFETCH_16_LANES_SOA_AVX2(k, K, prefetch_dist, in_re, in_im, k_end);                                                                \
        PREFETCH_STAGE_TW_SOA_AVX2(k, prefetch_dist, stage_tw, K, k_end);                                                                    \
 ...
 */
#define RADIX16_PIPELINE_2_BV_NATIVE_SOA_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, rot_mask, neg_mask, prefetch_dist, k_end) \
    do                                                                                                                                        \
    {                                                                                                                                         \
        PREFETCH_16_LANES_SOA_AVX2(k, K, prefetch_dist, in_re, in_im, k_end);                                                                \
        PREFETCH_STAGE_TW_SOA_AVX2(k, prefetch_dist, stage_tw, K, k_end);                                                                    \
...
//==============================================================================
// PERFORMANCE SUMMARY - TRUE END-TO-END SoA FOR RADIX-16 AVX2
//==============================================================================

/**
 * @page radix16_avx2_perf_summary Radix-16 AVX2 Performance Summary
 *
 ...
 * ✅ FMA support
 *
 * TOTAL EXPECTED IMPROVEMENT: 35-60% faster than split-form radix-16!
 */

#endif // FFT_RADIX16_MACROS_TRUE_SOA_AVX2_H