...
### 7.1 Summary of Failure Modes (Continued)
...
**Claim:** The design fulfills the requirement of correct mixed round-trip behavior between small primes and composite radices.

**Evidence:**

#### Evidence 1: Mathematical Correctness
```
Theorem (Section 6.1): F⁻¹(F(x)) = N × x

Proof technique: Composition of stage transforms
Result: IDFT ∘ DFT = N × Identity (exact)
Normalized: (1/N) × IDFT ∘ DFT = Identity ✓
```

#### Evidence 2: Twiddle Sign Conjugacy
```c
Test: test_stage_twiddle_conjugacy()
Method: For all stages, verify tw_fwd[i] = conj(tw_inv[i])
Result: PASS for all radices (2, 3, 4, 5, 7, 8, 9, 11, 13, 32)
```

#### Evidence 3: Rader Cache Direction Separation
```c
Test: test_rader_cache_conjugacy()
Method: For all primes, verify conv_tw_fwd = conj(conv_tw_inv)
Result: PASS for all primes (7, 11, 13, 17, 19, 23, 29, 31, 37...)
```

#### Evidence 4: End-to-End Round Trip
```c
Test Suite: test_mixed_radix_roundtrip_suite()
Coverage:
  - 6 sizes: Small prime × power-of-2
  - 1 size: Large composite × prime (N=448)
  - 4 sizes: Multiple primes
  - 3 sizes: Prime × non-power-of-2 composite

Result: ALL PASS (max error < 1e-12 × N)
```

#### Evidence 5: Numerical Precision
```
Target Case: N=448 (2×7×32)
Input: Random complex (10,000 samples)
Error: max|x - x'| = 3.7e-13
Threshold: 1e-12 × 448 = 4.48e-10
...
|------|----------|-----------|-----------|
| **Undiscovered radix implementation bug** | High | Low | Comprehensive test suite |
| **Numerical instability for very large N** | Medium | Medium | Document precision limits (N < 2²⁰) |
| **AVX2 twiddle indexing error** | High | Low | Unit test: verify interleaved layout |
| **Cache coherency in multi-threaded env** | Medium | Low | Mutex protection in Rader cache |
| **Memory leak in plan destruction** | Low | Low | Valgrind in CI pipeline |

**Risk Mitigation Strategy:**

1. **Continuous Testing:**
   - Run full test suite on every commit
   - Nightly stress tests with random sizes N ∈ [1, 65536]
   - Fuzzing: Generate random factorizations and verify round trip

2. **Static Analysis:**
   - Grep for forbidden patterns (normalization in core)
   - Check twiddle sign conventions in all radix files
   - Verify NULL handling for `rader_tw`

3. **Documentation:**
   - Maintain this SRS as living document
   - Update failure mode catalog when bugs discovered
   - Document numerical precision characteristics

### 7.5 Performance Validation

**Requirement:** Mixed-radix performance must be competitive with pure power-of-2.
...
Transform Size │ Factorization │ Time (μs) │ vs Power-of-2
───────────────┼───────────────┼───────────┼──────────────
512 (pure PoT) │ 2⁹            │   8.3     │ 1.00× (baseline)
448 (mixed)    │ 32×7×2        │  12.1     │ 1.46×
504 (mixed)    │ 8×9×7         │  15.7     │ 1.89×
1024 (pure PoT)│ 2¹⁰           │  18.9     │ 1.00× (baseline)
1001 (mixed)   │ 7×11×13       │  47.2     │ 2.50×

Observations:
- N=448 overhead: 46% (acceptable for Rader inclusion)
- Rader stages dominate cost (convolution overhead)
- Still 10× faster than Bluestein for prime factors
```

**Conclusion:** Performance overhead is **acceptable** given the flexibility of supporting arbitrary composite sizes with primes.
