...
#ifdef HAS_AVX512
    //------------------------------------------------------------------
    // AVX-512 PATH: Split 16x unrolling into two phases for register pressure
    //------------------------------------------------------------------
    const int PREFETCH_AHEAD = 32;
...
#define RADIX4_BUTTERFLY_AVX512(a, b2, c2, d2, y0, y1, y2, y3)    \
    ...
    {
        // Unified prefetch for all lanes
        if (k + PREFETCH_AHEAD < quarter)
        {
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD + quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD + 2 * quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD + 3 * quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&stage_tw[3 * (k + PREFETCH_AHEAD)], _MM_HINT_T0);
        }
...
#ifdef __AVX2__
    //------------------------------------------------------------------
    // AVX2 PATH: Software pipelined 8x unroll
    //------------------------------------------------------------------
    const int PREFETCH_AHEAD_AVX2 = 16;
...
#define RADIX4_BUTTERFLY_AVX2(a, b2, c2, d2, y0, y1, y2, y3)  \
    ...
    {
        // Unified prefetch
        if (k + PREFETCH_AHEAD_AVX2 < quarter)
        {
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD_AVX2], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD_AVX2 + quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD_AVX2 + 2 * quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&sub_outputs[k + PREFETCH_AHEAD_AVX2 + 3 * quarter], _MM_HINT_T0);
            _mm_prefetch((const char *)&stage_tw[3 * (k + PREFETCH_AHEAD_AVX2)], _MM_HINT_T0);
        }
