 ...
 * 
 * @details
 * This module implements native SoA radix-4 forward FFT with:
 * - Standard twiddle version (fft_radix4_fv)
 * - Twiddle-less n1 version (fft_radix4_fv_n1) for first stage
 * 
...
//==============================================================================
// HELPER: Process Range (Native SoA) - WITH TWIDDLES
//==============================================================================
    ...
    int use_streaming)
{
...
#ifdef __AVX512F__
    {
        ...
        radix4_stage_baseptr_fv_avx512(N, range_K, a_re, a_im,
...
#ifdef __SSE2__
    {
        ...
        radix4_stage_baseptr_fv_sse2(N, range_K, a_re, a_im,
                                     out_re + k_start, out_im + k_start,
                                     &tw_local, is_write_only, is_cold_out);
        return;
    }
...
//==============================================================================
// HELPER: Process Range (Native SoA) - NO TWIDDLES (N1)
//==============================================================================

/**
 * @brief Process radix-4 butterflies - NO TWIDDLES (n1 variant)
 * 
 * This is 40-60% faster than the standard version when W1=W2=W3=1.
 * Uses cascaded SIMD dispatch for optimal tail handling.
 */
    ...
    int k_end)
{
    ...
    //==========================================================================
    // CASCADED SIMD DISPATCH FOR OPTIMAL TAIL HANDLING
    //==========================================================================
    ...
    for (; k + 8 <= range_K; k += 8)
    {
        fft_radix4_n1_forward_stage_avx512(N, 8,
                                           a_re + k, a_im + k,
                                           y0_re + k, y0_im + k);
    }
#endif
...
#ifdef __AVX2__
    // Process 4-wide chunks (remainder from AVX-512 or main loop)
    if (k + 4 <= range_K)
    {
        fft_radix4_n1_forward_stage_avx2(N, 4,
                                         a_re + k, a_im + k,
                                         y0_re + k, y0_im + k);
        k += 4;
    }
...
#ifdef __SSE2__
    // Process 2-wide chunks (remainder from AVX2)
    if (k + 2 <= range_K)
    {
        fft_radix4_n1_forward_stage_sse2(N, 2,
                                         a_re + k, a_im + k,
                                         y0_re + k, y0_im + k);
        k += 2;
    }
    ...
    // Scalar cleanup (0-1 elements remaining)
    for (; k < range_K; k++)
    {
        radix4_butterfly_n1_scalar_fv_avx512(k,
...
//==============================================================================
// MAIN FUNCTION: Radix-4 DIF Butterfly - NATIVE SoA - FORWARD (WITH TWIDDLES)
//==============================================================================
...
//==============================================================================
// MAIN FUNCTION: Radix-4 DIF Butterfly - NATIVE SoA - FORWARD (NO TWIDDLES)
//==============================================================================

/**
 * @brief Radix-4 DIF butterfly - NATIVE SoA - Forward FFT - NO TWIDDLES (n1)
 * 
 * @details
 * Twiddle-less variant for first radix-4 stage or when all W1=W2=W3=1.
 * This is 40-60% faster than the standard version due to:
 * - No twiddle loads (saves 6 memory accesses per iteration)
 * - No complex multiplies (saves 9 FMAs per iteration)
 * - Reduced register pressure
 * 
 * USAGE:
 * @code
 * // First radix-4 stage (all twiddles = 1)
 * fft_radix4_fv_n1(out_re, out_im, in_re, in_im, K);
 * 
 * // Subsequent stages (need twiddles)
 * fft_radix4_fv(out_re, out_im, in_re, in_im, stage_tw, K);
 * @endcode
 * 
    ...
    int K)
{
    ...
    // N1 variant doesn't need NT store heuristic - handled internally by each SIMD impl
    
    radix4_process_range_native_soa_fv_n1(out_re, out_im, in_re, in_im,
                                          K, N, 0, K);
}
