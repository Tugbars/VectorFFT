...
//==============================================================================
// COMPLEX MULTIPLICATION - AVX-512 (processes 4 butterflies)
//==============================================================================
 ...
 *
 * Identical logic to AVX2 version but processes 4 complex values at once.
 */
...
/**
 * @brief FORWARD rotation: -i * difBD using AVX-512
 *
...
/**
 * @brief INVERSE rotation: +i * difBD using AVX-512
 *
 * Processes 4 complex values using AVX-512 permute and XOR.
 * (a + bi) * (+i) = -b + ai
 */
...
/**
 * @brief Assemble final outputs from butterfly intermediates (AVX-512)
 *
...
/**
 * @brief AVX-512: Apply stage twiddles for 4 butterflies (kk, kk+1, kk+2, kk+3)
 *
 * Loads and applies twiddle factors to lanes 1, 2, and 3 for four butterflies.
...
/**
 * @brief Load 4 lanes for 4 butterflies (kk, kk+1, kk+2, kk+3)
 *
 * Loads strided data from sub_outputs into AVX-512 registers.
 * Each register holds 4 complex values (8 doubles).
 */
...
/**
 * @brief Store 4 outputs for 4 butterflies (AVX-512)
 *
 * Stores four 512-bit vectors (each with 4 complex values) to output_buffer.
...
/**
 * @brief Prefetch 4 lanes ahead for AVX-512 (4 butterflies)
 *
 * Prefetches more aggressively than AVX2 due to higher throughput.
 * Issues prefetch for all 4 strided lanes plus twiddles.
 */
#define PREFETCH_4_LANES_AVX512(k, K, distance, sub_outputs, stage_tw, hint)          \
    do                                                                                \
    {                                                                                 \
        if ((k) + (distance) < K)                                                     \
        {                                                                             \
            ...
            _mm_prefetch((const char *)&stage_tw[(k) + (distance)], hint);            \
        }                                                                             \
    } while (0)
...
/**
 * @brief Complete AVX-512 radix-4 butterfly pipeline (FORWARD, 4 butterflies)
 *
 * Processes 4 butterflies in one macro call:
 * - Load 4 lanes
 * - Apply twiddles
 * - Butterfly core
 * - Forward rotation
...
/**
 * @brief Complete AVX-512 radix-4 butterfly pipeline (INVERSE, 4 butterflies)
 *
 * Identical to forward except uses inverse rotation.
 */
