    ...
    int use_streaming)
{
    ...
    if (use_streaming)
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512_STREAM(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, 0, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX4_PIPELINE_4_NATIVE_SOA_BV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN512, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2_STREAM(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, 0, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX4_PIPELINE_2_NATIVE_SOA_BV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN256, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2_STREAM(k + 1, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2(k + 1, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, prefetch_dist, k_end);
        }
    }
    ...
    if (use_streaming)
    {
        for (; k < k_end; k++)
        {
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, 0, k_end);
        }
    }
    else
    {
        for (; k < k_end; k++)
        {
            RADIX4_PIPELINE_1_NATIVE_SOA_BV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, SIGN128, 0, k_end);
        }
    }
#else
