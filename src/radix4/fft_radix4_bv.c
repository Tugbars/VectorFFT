    ...
    int use_streaming)
{
...
#ifdef __AVX512F__
    {
        ...
        fft_radix4_tw_backward_stage_avx512((size_t)N, (size_t)range_K,
                                            a_re, a_im,
    ...
    int k_end)
{
...
#ifdef __AVX512F__
    fft_radix4_n1_backward_stage_avx512((size_t)N, (size_t)K,
                                        in_re, in_im, out_re, out_im);
#elif defined(__AVX2__)
    fft_radix4_n1_backward_stage_avx2((size_t)N, (size_t)K,
                                      in_re, in_im, out_re, out_im);
#else
    ...
    for (int k = 0; k < K; k++)
        radix4_butterfly_n1_scalar_bv((size_t)k,
    ...
    const double *restrict in_re, const double *restrict in_im, int K)
{
    ...
    out_im = (double*)__builtin_assume_aligned(out_im, REQUIRED_ALIGNMENT);
#endif
    radix4_process_range_native_soa_bv_n1(out_re, out_im, in_re, in_im, K, N, 0, K);
}
