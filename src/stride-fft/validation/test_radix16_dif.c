/**
 * test_radix16_dif.c — Scalar-vs-AVX2 cross-check for R=16 DIF codelet.
 *
 * Runs scalar DIF and AVX2 DIF on identical inputs/twiddles. If the
 * generator is self-consistent (scalar and AVX2 emit implement the same
 * mathematical function), outputs must agree to ~FMA-rounding precision
 * (1e-13). This catches SIMD-emission bugs without needing to know the
 * correct spec.
 *
 * Also cross-checks DIT the same way as a baseline sanity.
 *
 * K=256 to exercise the full codelet body (matches VTune setup).
 */
...
#include "../codelets/scalar/fft_radix16_scalar_ct_t1_dit.h"
#include "../codelets/scalar/fft_radix16_scalar_ct_t1_dif.h"
...
int main(void) {
    ...
    radix16_t1_dif_fwd_scalar(re_s, im_s, tw_re, tw_im, K, 0, K, 1);
    radix16_t1_dif_fwd_avx2  (re_v, im_v, tw_re, tw_im, K, K);
    ...
    memcpy(re_s, re_src, nelem * sizeof(double)); memcpy(im_s, im_src, nelem * sizeof(double));
    memcpy(re_v, re_src, nelem * sizeof(double)); memcpy(im_v, im_src, nelem * sizeof(double));
    radix16_t1_dif_bwd_scalar(re_s, im_s, tw_re, tw_im, K, 0, K, 1);
    radix16_t1_dif_bwd_avx2  (re_v, im_v, tw_re, tw_im, K, K);
    ...
    /* Test 3: DIT fwd — baseline scalar vs AVX2 sanity */
    memcpy(re_s, re_src, nelem * sizeof(double)); memcpy(im_s, im_src, nelem * sizeof(double));
    memcpy(re_v, re_src, nelem * sizeof(double)); memcpy(im_v, im_src, nelem * sizeof(double));
    radix16_t1_dit_fwd_scalar(re_s, im_s, tw_re, tw_im, K, 0, K, 1);
    radix16_t1_dit_fwd_avx2  (re_v, im_v, tw_re, tw_im, K, K);
    dre = max_abs_diff(re_s, re_v, nelem);
    dim = max_abs_diff(im_s, im_v, nelem);
