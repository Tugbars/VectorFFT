...
static void naive(int dir,size_t K,const double*ir,const double*ii,double*nr,double*ni){
    for(size_t k=0;k<K;k++) for(int m=0;m<19;m++){
        double sr=0,si=0;
        for(int n=0;n<19;n++){double a=dir*2.0*M_PI*m*n/19.0;
...
static void bench(size_t K, int warm, int trials){
    ...
    for(size_t k=0;k<K;k++) for(int n=0;n<19;n++){
        fin[k*19+n][0]=ir[n*K+k]; fin[k*19+n][1]=ii_[n*K+k];}
    int na[1]={19};
    fftw_plan plan=fftw_plan_many_dft(1,na,(int)K,
        fin,NULL,1,19,fout,NULL,1,19,FFTW_FORWARD,FFTW_MEASURE);
    ...
    {size_t T=8; double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
     r19_pack(ir,ii_,p1,p2,K,T);
     for(int i=0;i<warm;i++) r19_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
     for(int t=0;t<trials;t++){double t0=get_ns();
         r19_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
    ...
    {size_t T=4; double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
     r19_pack(ir,ii_,p1,p2,K,T);
     for(int i=0;i<warm;i++) r19_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
     for(int t=0;t<trials;t++){double t0=get_ns();
         r19_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
...
int main(void){
    printf("════════════════════════════════════════════════════════════════\n");
    printf("  VectorFFT DFT-19 — NEW (FFTW has NO hard-coded N=19 codelet)\n");
    printf("  542 ops, 26 constants (genfft-generated, not shipped by FFTW)\n");
    printf("  FFTW falls back to generic Rader solver for N=19\n");
    printf("════════════════════════════════════════════════════════════════\n\n");

    int p=0,t=0;
    printf("── Correctness ──\n");
    size_t Ks[]={1,7,8,64,1024};
    for(int i=0;i<5;i++){t++;p+=verify("scalar fwd",Ks[i],radix19_genfft_fwd_scalar,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("scalar bwd",Ks[i],radix19_genfft_bwd_scalar,+1);}
    size_t Ka[]={4,8,16,64,256};
    for(int i=0;i<5;i++){t++;p+=verify("avx2 fwd",Ka[i],radix19_genfft_fwd_avx2,-1);}
    size_t Kz[]={8,16,64,256,1024};
    for(int i=0;i<5;i++){t++;p+=verify("avx512 fwd",Kz[i],radix19_genfft_fwd_avx512,-1);}
    /* Cross-ISA */
    {size_t K=8,N=19*K;
     double *ir=aa64(N),*ii_=aa64(N),*ar=aa64(N),*ai=aa64(N),*zr=aa64(N),*zi=aa64(N);
     fill_rand(ir,N,7777); fill_rand(ii_,N,8888);
     radix19_genfft_fwd_avx2(ir,ii_,ar,ai,K);
     radix19_genfft_fwd_avx512(ir,ii_,zr,zi,K);
     double err=0;
    ...
    printf("  NOTE: FFTW has NO hard-coded N=19 — uses generic solver\n");
