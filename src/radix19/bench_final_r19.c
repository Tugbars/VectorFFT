...
static void naive(int dir, size_t K, const double *ir, const double *ii, double *nr, double *ni)
{
    for (size_t k = 0; k < K; k++)
        for (size_t m = 0; m < 19; m++)
        {
            ...
            {
                double a = dir * 2.0 * M_PI * (double)m * (double)n / 19.0;
                         ...
                         void (*fn)(const double *, const double *, double *, double *, size_t))
{
    ...
    r19_pack(ir, ii_, pir, pii, K, T);
    fn(pir, pii, por, poi, K);
    r19_unpack(por, poi, sor, soi, K, T);
...
static void bench(size_t K, int warm, int trials)
{
    ...
    for (size_t k = 0; k < K; k++)
        for (size_t n = 0; n < 19; n++)
        {
            fin[k * 19 + n][0] = ir[n * K + k];
            fin[k * 19 + n][1] = ii_[n * K + k];
        }
    int na[1] = {19};
    ...
    {
        double *p1 = aa64(N), *p2 = aa64(N), *p3 = aa64(N), *p4 = aa64(N);
        r19_pack(ir, ii_, p1, p2, K, 1);
        for (int i = 0; i < warm; i++)
            r19_genfft_packed_fwd_scalar(p1, p2, p3, p4, K);
        for (int t = 0; t < trials; t++)
        {
            double t0 = get_ns();
            r19_genfft_packed_fwd_scalar(p1, p2, p3, p4, K);
...
{
    printf("════════════════════════════════════════════════════════════════\n");
    printf("  VectorFFT DFT-19 (scalar");
#ifdef __AVX2__
    printf(" + AVX2");
#endif
#ifdef __AVX512F__
    ...
    {
        t++;
        p += verify("scalar fwd", Ks[i], radix19_genfft_fwd_scalar, -1);
    }
    for (int i = 0; i < 5; i++)
    {
        t++;
        p += verify("scalar bwd", Ks[i], radix19_genfft_bwd_scalar, +1);
    }
...
#ifdef __AVX2__
    {
        ...
        {
            t++;
            p += verify("avx2 fwd", Ka[i], radix19_genfft_fwd_avx2, -1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify("avx2 bwd", Ka[i], radix19_genfft_bwd_avx2, +1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify_packed("packed avx2", Ka[i], 4, r19_genfft_packed_fwd_avx2);
        }
    }
#else
...
#ifdef __AVX512F__
    {
        ...
        {
            t++;
            p += verify("avx512 fwd", Kz[i], radix19_genfft_fwd_avx512, -1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify("avx512 bwd", Kz[i], radix19_genfft_bwd_avx512, +1);
        }
        for (int i = 0; i < 5; i++)
        {
            t++;
            p += verify_packed("packed avx512", Kz[i], 8, r19_genfft_packed_fwd_avx512);
        }
    }
#else
