...
class Target:
    def __init__(self, isa):
        if isa == 'avx512':
            ...
            self.ld_macro = None
            self.st_macro = None
            self.guard = 'FFT_RADIX32_SCALAR_NOTW_H'
            self.scalar = True
        else:
...
class E:
    def __init__(self, tgt):
        ...
        self.vt = tgt.vtype
        self.p = tgt.prefix
        self.sc = getattr(tgt, 'scalar', False)
    def o(s,t=""): s.L.append("    "*s.ind+t)
    def c(s,t): s.o(f"/* {t} */")
    def b(s): s.L.append("")
    def add(s,a,b):
        return f"({a})+({b})" if s.sc else f"{s.p}_add_pd({a},{b})"
    def sub(s,a,b):
        return f"({a})-({b})" if s.sc else f"{s.p}_sub_pd({a},{b})"
    def mul(s,a,b):
        return f"({a})*({b})" if s.sc else f"{s.p}_mul_pd({a},{b})"
    def neg(s,a):
        return f"-({a})" if s.sc else f"{s.p}_xor_pd({a},sign_flip)"
    def fma(s,a,b,c):
        return f"({a})*({b})+({c})" if s.sc else f"{s.p}_fmadd_pd({a},{b},{c})"
    def fms(s,a,b,c):
        return f"({a})*({b})-({c})" if s.sc else f"{s.p}_fmsub_pd({a},{b},{c})"
    ...
    def emit_twiddle(s,dst,src,e,tN,d):
        ...
        if typ=='one':
            ...
            label=wN_label(e,tN)
            tw_pre = "" if s.sc else "tw_"
            s.o(f"{{ {s.vt} tr={src}_re;")
            if fwd:
                s.o(f"  {dst}_re={s.fms(f'{src}_re',f'{tw_pre}{label}_re',s.mul(f'{src}_im',f'{tw_pre}{label}_im'))};")
                s.o(f"  {dst}_im={s.fma('tr',f'{tw_pre}{label}_im',s.mul(f'{src}_im',f'{tw_pre}{label}_re'))}; }}")
            else:
                s.o(f"  {dst}_re={s.fma(f'{src}_re',f'{tw_pre}{label}_re',s.mul(f'{src}_im',f'{tw_pre}{label}_im'))};")
                s.o(f"  {dst}_im={s.fms(f'{src}_im',f'{tw_pre}{label}_re',s.mul('tr',f'{tw_pre}{label}_im'))}; }}")
...
def emit_notw_kernel(em, d, itw_set):
    ...
    em.b()
    if sc:
        em.o(f"double spill_re[{spill_doubles}], spill_im[{spill_doubles}];")
    else:
    ...
    if itw_set:
        if sc:
            em.c(f"Internal W32 constants defined at file scope")
        else:
...
def main():
    if len(sys.argv) < 2 or sys.argv[1] not in ('avx512', 'avx2', 'scalar'):
    ...
    if sc:
        em.L.append(f" * Scalar (no SIMD), NFUSE={tgt.nfuse}, 8x4 decomposition")
    else:
