...
#pragma GCC unroll 1
    for (size_t k = 0; k < K; k++)
    {
         ...
         *
         * Group g: reads stripes {g, g+8, g+16, g+24}
         * Twiddles: W1[k], W2[k], W3[k]=W1×W2 (shared across groups)
         *
         * Register budget per group:
         *   8 (inputs) + 4 (intermediates) + 2 (twiddle cmul) = 14 peak
         * Twiddle W1,W2 reloaded from L1-cached p1_re/p1_im per group.
         * W3 derivation costs 1 cmul per group (could be hoisted, but
         * 14 + 2(W3) = 16 is already at limit; per-group is safer).
         *============================================================*/
        for (size_t g = 0; g < 8; g++)
        {
            /* Load 4 radix-4 inputs from strided positions */
            ...
            /* Store to micro-buffer in bin-major order */
            tmp_re[0 * 8 + g] = y0r;
            tmp_im[0 * 8 + g] = y0i;
            tmp_re[1 * 8 + g] = y1r;
            tmp_im[1 * 8 + g] = y1i;
         ...
         *
         * Bin b: reads micro-buffer[b*8 + 0..7]
         *        writes output stripes {b*8+0 .. b*8+7}
         *
         * Register budget:
        ...
        for (size_t b = 0; b < 4; b++)
        {
            ...
            else /* TW_MODE_BLOCKED4 (or RECURRENCE — TODO) */
            {
         ...
         *
         * Scalar step = 1 double = 8 bytes. Cache line = 64 bytes.
         * Fire prefetch every 8th iteration to hit once per line.
         *============================================================*/
        if ((k & 7) == 0)
        {
            ...
            if (kpf < K)
            {
                /* Prefetch pass-1 inputs (4 of 32 stripes per group) */
                ...
                /* Prefetch pass-2 twiddles (mode-dependent) */
                if (mode == TW_MODE_BLOCKED8)
                {
                    for (int j = 0; j < 7; j++)
                    {
...
#pragma GCC unroll 1
    for (size_t k = 0; k < K; k++)
    {
        ...
        for (size_t g = 0; g < 8; g++)
        {
            ...
            /* Same twiddles as forward (conjugation is in butterfly) */
            {
                double W1r = p1_re[0 * K + k];
                double W1i = p1_im[0 * K + k];
                double W2r = p1_re[1 * K + k];
