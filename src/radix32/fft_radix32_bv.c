/**
 * @file fft_radix32_bv.c
 * @brief Native SoA Radix-32 FFT Implementation - Backward (bv)
 * 
 * @details
 * This module implements native SoA radix-32 backward (inverse) FFT with:
 * - Standard twiddle version (fft_radix32_bv)
 * - Twiddle-less n1 version (fft_radix32_bv_n1) for first stage [FUTURE]
 * 
...
static void radix32_process_range_native_soa_bv(
    ...
    int use_streaming)
{
...
#ifdef __AVX512F__
    radix32_stage_dit_backward_soa_avx512(range_K,
...
#ifdef __AVX2__
    radix32_stage_dit_backward_soa_avx2(range_K,
                                        in_re_range, in_im_range,
                                        out_re_range, out_im_range,
                                        stage_tw_opaque);
    return;
    ...
    // Scalar fallback
    radix32_stage_dit_backward_soa_scalar(range_K,
                                          in_re_range, in_im_range,
                                          out_re_range, out_im_range,
                                          stage_tw_opaque);
}
...
//==============================================================================
// HELPER: Process Range (Native SoA) - NO TWIDDLES (N1) - BACKWARD [STUB]
//==============================================================================

static void radix32_process_range_native_soa_bv_n1(
    double *restrict out_re,
    double *restrict out_im,
    const double *restrict in_re,
    const double *restrict in_im,
...
//==============================================================================
// MAIN FUNCTION: Radix-32 DIT Backward - NATIVE SoA - WITH TWIDDLES
//==============================================================================

/**
 * @brief Radix-32 DIT backward (inverse) butterfly - NATIVE SoA - WITH TWIDDLES
 * 
 * @details
 * Implements 2×16 Cooley-Tukey decomposition:
 * 1. Two radix-16 sub-FFTs (even/odd indices) - BACKWARD butterflies
    ...
    int K)
{
    ...
    // Process full range
    radix32_process_range_native_soa_bv(out_re, out_im, in_re, in_im,
                                        stage_tw_opaque, K, N, 0, K,
                                        use_streaming);
}

//==============================================================================
// MAIN FUNCTION: Radix-32 DIT Backward - NATIVE SoA - NO TWIDDLES (N1) [STUB]
//==============================================================================

/**
 * @brief Radix-32 DIT backward (inverse) butterfly - NATIVE SoA - NO TWIDDLES (n1)
 * 
 * @details
    ...
    int K)
{
    ...
    radix32_process_range_native_soa_bv_n1(out_re, out_im, in_re, in_im,
                                           K, N, 0, K);
}
