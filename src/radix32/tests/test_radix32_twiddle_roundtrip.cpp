...
struct AlignedBuf
{
    ...
    explicit AlignedBuf(size_t n)
        : ptr(static_cast<double *>(std::aligned_alloc(32, n * sizeof(double)))), count(n)
    {
...
struct TwiddleTables
{
    ...
    ~TwiddleTables()
    {
        for (auto *p : p2_re_bufs)
            delete p;
        for (auto *p : p2_im_bufs)
            delete p;
    }
...
TEST_P(Radix32TwiddleTest, EnergyPreservation)
{
    ...
    for (size_t k = 0; k < K; k++)
    {
        ...
        {
            size_t idx = s * K + k;
            e_in += (*in_re)[idx] * (*in_re)[idx] + (*in_im)[idx] * (*in_im)[idx];
            e_out += (*freq_re)[idx] * (*freq_re)[idx] + (*freq_im)[idx] * (*freq_im)[idx];
        }
