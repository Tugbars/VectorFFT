...
static int test_vs_avx2(size_t K, tw_mode_t mode) {
    ...
    r32_aligned_free(in_re); r32_aligned_free(in_im); r32_aligned_free(sr); r32_aligned_free(si); r32_aligned_free(ar); r32_aligned_free(ai);
    r32_aligned_free(sbr); r32_aligned_free(sbi); r32_aligned_free(abr); r32_aligned_free(abi); r32_aligned_free(tr); r32_aligned_free(ti);
    tw_free(&tw);
    return pass;
}
...
static int test_roundtrip(size_t K) {
    ...
    r32_aligned_free(in_re); r32_aligned_free(in_im); r32_aligned_free(fwd_re); r32_aligned_free(fwd_im);
    r32_aligned_free(bwd_re); r32_aligned_free(bwd_im); tw_free(&tw);
    return pass;
}
...
static int test_linearity(size_t K, tw_mode_t mode) {
    ...
    r32_aligned_free(xr); r32_aligned_free(xi); r32_aligned_free(yr); r32_aligned_free(yi); r32_aligned_free(zr); r32_aligned_free(zi);
    r32_aligned_free(Fxr); r32_aligned_free(Fxi); r32_aligned_free(Fyr); r32_aligned_free(Fyi); r32_aligned_free(Fzr); r32_aligned_free(Fzi);
    tw_free(&tw);
    return pass;
}
...
static int test_parseval(size_t K, tw_mode_t mode) {
    ...
    r32_aligned_free(xr); r32_aligned_free(xi); r32_aligned_free(Xr); r32_aligned_free(Xi); tw_free(&tw);
    return pass;
}
...
static int test_rec_vs_blocked8(size_t K, int tile_len)
{
    ...
    for (size_t i = 0; i < n; i++)
    {
        double e = fabs(rec_re[i] - b8_re[i]); if (e > max_err) max_err = e;
        e = fabs(rec_im[i] - b8_im[i]); if (e > max_err) max_err = e;
    }
    ...
    r32_aligned_free(in_re); r32_aligned_free(in_im); r32_aligned_free(rec_re); r32_aligned_free(rec_im);
    r32_aligned_free(b8_re); r32_aligned_free(b8_im); r32_aligned_free(seed_re_buf); r32_aligned_free(seed_im_buf);
    tw_free(&tw_b8);
    return pass;
}
...
static int test_rec_roundtrip(size_t K, int tile_len)
{
    ...
    for (size_t i = 0; i < 8 * K; i++) { seed_re[i] = 1.0; seed_im[i] = 0.0; }
    rec.seed_re = seed_re;
    rec.seed_im = seed_im;
    rec.K = K;
    rec.tile_len = tile_len;
    for (int j = 0; j < 8; j++) { rec.delta_re[j] = 1.0; rec.delta_im[j] = 0.0; }
    ...
    r32_aligned_free(in_re); r32_aligned_free(in_im); r32_aligned_free(fwd_re); r32_aligned_free(fwd_im);
    r32_aligned_free(bwd_re); r32_aligned_free(bwd_im); r32_aligned_free(seed_re); r32_aligned_free(seed_im);
    tw_free(&tw);
    return pass;
}
