/**
 * @file fft_radix64_fv_n1.c
 * @brief Radix-64 N1 Forward FFT - SIMD Dispatch Wrapper
 * 
 * @details
 * This module provides the public API for radix-64 N1 forward transforms.
 ...
 * 
 * USE CASE:
 * - First stage of forward mixed-radix FFT (N = 64×K)
...
static void radix64_process_range_n1_fv(
...
#ifdef __AVX512F__
    radix64_stage_dit_forward_n1_soa_avx512(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
#ifdef __AVX2__
    radix64_stage_dit_forward_n1_soa_avx2(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
#ifdef __SSE2__
    radix64_stage_dit_forward_n1_soa_sse2(K, in_re, in_im, out_re, out_im);
    return;
#endif
    
    radix64_stage_dit_forward_n1_soa_scalar(K, in_re, in_im, out_re, out_im);
}

//==============================================================================
// MAIN FUNCTION: Radix-64 DIT Butterfly - NATIVE SoA - FORWARD (NO TWIDDLES)
//==============================================================================

/**
 * @brief Radix-64 DIT butterfly - Native SoA - Forward FFT - NO TWIDDLES (n1)
 * 
 * @details
 * Twiddle-less variant for first radix-64 stage or when all stage twiddles = 1.
 * 
 * PERFORMANCE GAINS:
 * - 40-50% faster than standard radix-64
 ...
 * 2. Apply W₆₄ merge twiddles to outputs 1-7
    ...
    int K)
{
    ...
    radix64_process_range_n1_fv(out_re, out_im, in_re, in_im, K, N);
}