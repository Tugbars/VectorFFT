#!/usr/bin/env python3
"""
gen_r64_tw.py — DFT-64 split twiddled codelet (8×8 CT + log3), multi-ISA.

Targets: avx2, avx512
Emits: DIT fwd/bwd + DIF fwd/bwd in one header file.

v2 optimizations:
  1. Flattened twiddle derivation tree (depth-3 for ILP)
  2. Sequential cross twiddles (data×col×row, no temp regs)
  3. Static arrays for internal W64 constants (compiler uses broadcast)
  4. XOR negation for ×j/×(-j)/×(-1) (1-cycle latency)

Usage:
  python3 gen_r64_tw.py avx2   > fft_radix64_avx2_tw.h
  python3 gen_r64_tw.py avx512 > fft_radix64_avx512_tw.h
"""
import math, sys
...
class ISA:
    ...
    def fmsub(self, a,b,c): return f'{self.P}_fmsub_pd({a},{b},{c})'
    def fmadd(self, a,b,c): return f'{self.P}_fmadd_pd({a},{b},{c})'
    def fnmadd(self,a,b,c): return f'{self.P}_fnmadd_pd({a},{b},{c})'
    def load(self, addr):   return f'{self.P}_load_pd({addr})'
    def store(self, addr, v): return f'{self.P}_store_pd({addr},{v})'
    def set1(self, v):      return f'{self.P}_set1_pd({v})'
    def neg(self, v):       return f'{self.P}_xor_pd({v}, sign_mask)'
...
def emit_r8_butterfly_split(isa, em, fwd, xr, xi, yr, yi):
    ...
    em.o(f'  const {T} B0r={add("opr","orr")}, B0i={add("opi","ori")};')
    # B2 (W8^2 = ×j, handled implicitly in combine)
    em.o(f'  const {T} B2r={sub("opr","orr")}, B2i={sub("opi","ori")};')
    ...
    # Pair 2: A2 ± j*B2 (implicit ×j: swap re/im)
...
def emit_r8(isa, em, fwd, xr, xi, yr, yi):
    """ISA dispatch: split butterfly for AVX2 (16 regs), monolithic for AVX-512 (32 regs)."""
    if isa.name == 'avx2':
...
# Extended bases: inline-derive from core pairs (a × b)
COL_EXTENDED = {5: ('ew1r','ew1i','ew4r','ew4i'),   # ew5 = ew1 × ew4
                6: ('ew3r','ew3i','ew3r','ew3i'),   # ew6 = ew3²
                7: ('ew3r','ew3i','ew4r','ew4i')}   # ew7 = ew3 × ew4
ROW_EXTENDED = {40: ('ew8r','ew8i','ew32r','ew32i'),   # ew40 = ew8 × ew32
                48: ('ew16r','ew16i','ew32r','ew32i'),  # ew48 = ew16 × ew32
                56: ('ew24r','ew24i','ew32r','ew32i')}  # ew56 = ew24 × ew32
...
def _emit_inline_derive_cmul(isa, em, xr, xi, ar, ai, br, bi, fwd):
    """Inline: derive tmp = a × b, then x *= tmp (or x *= conj(tmp)). All scoped."""
    T = isa.T
...
def gen_file(isa_name):
    ...
    L = ['/**',
         f' * @file fft_radix64_{isa_name}_tw.h',
         f' * @brief DFT-64 {ISA_U} split — 8x8 CT + log3 twiddles (v2)',
         f' * Optimizations: depth-3 log3 tree, sequential cross tw,',
         f' * static W64 arrays (broadcast), XOR negation.',
         f' * Vector width: {isa.C} doubles, k-step: {isa.C}',
         f' * Generated by gen_r64_tw.py {isa_name}',
         ' */', '',
