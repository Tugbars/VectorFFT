    ...
    for (size_t k = 0; k < K; k++)
        for (size_t m = 0; m < 17; m++) {
            ...
            for (size_t n = 0; n < 17; n++) {
                double a = dir * 2.0 * M_PI * (double)m * (double)n / 17.0;
...
static int verify(const char *l, size_t K,
    ...
    for (size_t i=0;i<N;i++) {
        double e=fmax(fabs(gr[i]-nr[i]),fabs(gi[i]-ni[i])); if(e>err) err=e;
        double m=fmax(fabs(nr[i]),fabs(ni[i])); if(m>mag) mag=m;
    }
    double rel=mag>0?err/mag:err; int p=rel<1e-13;
...
static int verify_packed(const char *l, size_t K, size_t T,
    ...
    r17_pack(ir,ii_,pir,pii,K,T);
    fn(pir,pii,por,poi,K);
    r17_unpack(por,poi,sor,soi,K,T);
...
static void bench(size_t K, int warm, int trials) {
    ...
    for (size_t k=0;k<K;k++) for (size_t n=0;n<17;n++) {
        fin[k*17+n][0]=ir[n*K+k]; fin[k*17+n][1]=ii_[n*K+k];
    }
    int na[1]={17};
    fftw_plan plan=fftw_plan_many_dft(1,na,(int)K,
        fin,NULL,1,17,fout,NULL,1,17,FFTW_FORWARD,FFTW_MEASURE);
    ...
    { size_t T=8;
      ...
      r17_pack(ir,ii_,p1,p2,K,T);
      for (int i=0;i<warm;i++) r17_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
      for (int t=0;t<trials;t++) {
          double t0=get_ns(); r17_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
          double dt=get_ns()-t0; if(dt<ns_best) ns_best=dt;
      }
      best_label="avx512";
    ...
    { size_t T=4;
      double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
      r17_pack(ir,ii_,p1,p2,K,T);
      for (int i=0;i<warm;i++) r17_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
      for (int t=0;t<trials;t++) {
          double t0=get_ns(); r17_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
          double dt=get_ns()-t0; if(dt<ns_best) ns_best=dt;
      }
      best_label="avx2";
      r32_aligned_free(p1);r32_aligned_free(p2);r32_aligned_free(p3);r32_aligned_free(p4);
...
#else
    { double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
      r17_pack(ir,ii_,p1,p2,K,1);
      for (int i=0;i<warm;i++) r17_genfft_packed_fwd_scalar(p1,p2,p3,p4,K);
      for (int t=0;t<trials;t++) {
          double t0=get_ns(); r17_genfft_packed_fwd_scalar(p1,p2,p3,p4,K);
          double dt=get_ns()-t0; if(dt<ns_best) ns_best=dt;
      }
      r32_aligned_free(p1);r32_aligned_free(p2);r32_aligned_free(p3);r32_aligned_free(p4);
    }
...
int main(void) {
    printf("════════════════════════════════════════════════════════════════\n");
    printf("  VectorFFT DFT-17 (scalar");
#ifdef __AVX2__
    printf(" + AVX2");
#endif
#ifdef __AVX512F__
    ...
    for (int i=0;i<5;i++) { t++; p+=verify("scalar fwd",Ks[i],radix17_genfft_fwd_scalar,-1); }
    for (int i=0;i<5;i++) { t++; p+=verify("scalar bwd",Ks[i],radix17_genfft_bwd_scalar,+1); }

#ifdef __AVX2__
    { size_t Ka[]={4,8,16,64,256};
      for (int i=0;i<5;i++) { t++; p+=verify("avx2 fwd",Ka[i],radix17_genfft_fwd_avx2,-1); }
      for (int i=0;i<5;i++) { t++; p+=verify("avx2 bwd",Ka[i],radix17_genfft_bwd_avx2,+1); }
      for (int i=0;i<5;i++) { t++; p+=verify_packed("packed avx2",Ka[i],4,r17_genfft_packed_fwd_avx2); }
    }
#else
    ...
    { size_t Kz[]={8,16,64,256,1024};
      for (int i=0;i<5;i++) { t++; p+=verify("avx512 fwd",Kz[i],radix17_genfft_fwd_avx512,-1); }
      for (int i=0;i<5;i++) { t++; p+=verify("avx512 bwd",Kz[i],radix17_genfft_bwd_avx512,+1); }
      for (int i=0;i<5;i++) { t++; p+=verify_packed("packed avx512",Kz[i],8,r17_genfft_packed_fwd_avx512); }
    }
#else
    ...
    { size_t K=8, N=17*K;
      ...
      radix17_genfft_fwd_avx2(ir,ii_,ar,ai,K);
      radix17_genfft_fwd_avx512(ir,ii_,zr,zi,K);
      double err=0;
      for (size_t i=0;i<N;i++) { double e=fmax(fabs(ar[i]-zr[i]),fabs(ai[i]-zi[i])); if(e>err) err=e; }
      int ok=err<1e-15; t++;p+=ok;
