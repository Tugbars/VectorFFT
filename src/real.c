 ...
 *       OPTIMIZED: Preallocated workspace buffers eliminate malloc/free in hot paths.
 */
 ...
 *
 * OPTIMIZED: Preallocates workspace buffer to eliminate malloc/free in exec functions.
 *
...
fft_real_object fft_real_init(int signal_length, int transform_direction)
{
    ...
    {
        return NULL; // Removed fprintf for production
    }
    ...
    real_obj->tw_re = NULL;
    real_obj->tw_im = NULL;
    real_obj->workspace = NULL; // NEW: workspace buffer
    ...
    // NEW: Allocate persistent workspace buffer (32-byte aligned)
 ...
 *
 * OPTIMIZED: Uses preallocated workspace buffer from real_obj - NO malloc/free overhead!
 *
 ...
 * @return 0 on success, -1 for invalid inputs.
...
{
    // Validate inputs
    if (!real_obj || !real_obj->cobj || !real_obj->workspace || !input_data || !output_data)
    {
        return -1; // Removed fprintf for production
    }
    const int H = real_obj->halfN;

    // Use preallocated workspace buffer - NO MALLOC!
    ...
    // No free() needed - workspace is persistent!
    return 0;
}
 ...
 * twiddle factor arrays, and workspace buffer. Safe to call with NULL or partially
 * initialized objects.
 *
