...
typedef void (*rfft_r2cb_fn)(const double *in_re, const double *in_im,
                             double *out_re,
                             ptrdiff_t is, ptrdiff_t os_re, size_t vl);
...
/* ===== VFFT_RFFT_TRACE: record forward moves to invert for c2r ===== */
#ifdef VFFT_RFFT_TRACE
#include <stdio.h>
typedef struct { const char*op; int d; long ire,iim,ore,oim; long is,os; int r,m,k; size_t Q; } rfft_move_t;
extern rfft_move_t g_rfft_moves[4096];
extern int g_rfft_nmoves;
static inline void _trace2(const char*op,int d,const double*ib,const double*ob,
   const double*ire,const double*iim,const double*ore,const double*oim,
   long is,long os,int r,int m,int k,size_t Q){
  rfft_move_t*mv=&g_rfft_moves[g_rfft_nmoves++];
  mv->op=op; mv->d=d;
  mv->ire = ire?(long)(ire-ib):-1; mv->iim = iim?(long)(iim-ib):-1;
  mv->ore = ore?(long)(ore-ob):-1; mv->oim = oim?(long)(oim-ob):-1;
  mv->is=is; mv->os=os; mv->r=r; mv->m=m; mv->k=k; mv->Q=Q;
}
#define RFFT_TRACE2(...) _trace2(__VA_ARGS__)
#else
#define RFFT_TRACE(...) ((void)0)
#endif
    ...
    for (size_t b = 0; b < K; b += p->Kb) {
        ...
        /* LEAF: per-group calls on this slab */
        {
            ...
            for (size_t g = 0; g < p->S; g++) {
                p->leaf(x + g * K + b,
                        ...
                        dst + g * K + b + NK,
                        SK, SK, -SK, bw);
                RFFT_TRACE2("leaf", -1, x, dst, x+g*K+b, NULL, dst+g*K+b, dst+g*K+b+NK, (long)SK, (long)SK, p->leaf_r, 0, (int)g, 0);
            }
#ifdef VFFT_RFFT_PROFILE
        ...
        for (int d = p->nf - 2; d >= 0; d--) {
            ...
            for (size_t q = 0; q < Qfold; q++) {
                ...
                RFFT_TRACE2("k0", d, cur, nxt, curq, NULL, nxtq, nxtq+NK, (long)QK, (long)QmK, r, m, 0, Q);
#ifdef VFFT_RFFT_PROFILE
                ...
                for (int k = 1; k <= st->kmax; k++) {
                    ...
                    st->hc(curq + (Q * (size_t)(r * k)) * K,
                           ...
                           QK, QmK, vlf);
                    RFFT_TRACE2("hc", d, cur, nxt, curq+(Q*(size_t)(r*k))*K, curq+(Q*(size_t)(r*(m-k)))*K, nxtq+(Q*(size_t)k)*K, nxtq+(Q*(size_t)(m-k))*K, (long)QK, (long)QmK, r, m, k, Q);
                }
#ifdef VFFT_RFFT_PROFILE
