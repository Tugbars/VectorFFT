#!/bin/bash
# build_demo_v4_score.sh — DP planner on a single cell.
set -e
...
$CC $CFLAGS \
    -I $PROTO_CORE \
    -I $GENERATED \
    "$PROTO_CORE/demo/demo_v4_score.c" \
    @$RSP \
    -o $OUT -lm 2>&1 | grep -v warning | tail -3

echo "[build_demo_v4_score] built $OUT"
