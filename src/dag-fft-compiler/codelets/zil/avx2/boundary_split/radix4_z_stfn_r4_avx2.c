...
/* ===================== PROVENANCE =====================
 * Generated by: ./_build/default/bin/gen_radix.exe 4 --zp-stfn --zp-r0 4 --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=stfn radix=4 dir=fwd isa=avx2 r0=4 (ZTURN-S sectioned geometry)
...
void radix4_z_stfn_r4_fwd_avx2(
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t k = 0; k + 4 <= count; k += 4) {
        ...
        const __m256d lane_re_0 = _mm256_loadu_pd(&zin[2*(size_t)kn + 0]);
        const __m256d lane_im_0 = _mm256_loadu_pd(&zin[2*(size_t)kn + 4]);
        const __m256d lane_re_1 = _mm256_loadu_pd(&zin[2*((size_t)2*OLs + kn) + 0]);
        const __m256d lane_im_1 = _mm256_loadu_pd(&zin[2*((size_t)2*OLs + kn) + 4]);
        const __m256d lane_re_2 = _mm256_loadu_pd(&zin[2*((size_t)1*OLs + kn) + 0]);
        const __m256d lane_im_2 = _mm256_loadu_pd(&zin[2*((size_t)1*OLs + kn) + 4]);
        const __m256d lane_re_3 = _mm256_loadu_pd(&zin[2*((size_t)3*OLs + kn) + 0]);
        const __m256d lane_im_3 = _mm256_loadu_pd(&zin[2*((size_t)3*OLs + kn) + 4]);
