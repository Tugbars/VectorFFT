...
| **IL-edged mids** | `msz` (3/5/7/9/15, fwd) | the `msg` body with **interleaved z on both edges** (deinterleave on load, reinterleave on store, lanes left unordered: unpack only, no `permute4x64`) — the split body between interleaved edges, for the flat odd-N DIT engine (`src/core/oop/il_flatdit.h`); takes ANY count through the il_odd_count_tail §3 arms (2-column VEX-128, 1-column scalar re-renders of the same DAG). Raced 2026-09-05: 2.1–2.5× on run-4 stages, parity on long runs of 5/7, loses at radix 9 and on runs of 3 |
