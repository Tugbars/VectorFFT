 ...
 * tlf (ZTURN-T last: tmg's combine + REINT packed stores at leg*OLs + k = natural interleaved out, group-looped over distinct in/out pointers), bwd (table conjugated by the create).
...
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 8 --zp-tlfb --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=tlf radix=8 dir=bwd isa=avx2
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t g = 0; g < Gs; g++) {
        _ztlf8b_body(ip, op, tw_re, Ls, OLs, count);
        ip += 2 * (size_t)8 * Ls;
        op += 2 * (size_t)8 * OLs;
    }
}
