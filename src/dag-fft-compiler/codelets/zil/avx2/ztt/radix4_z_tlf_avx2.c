 ...
 * tlf (ZTURN-T last: tmg's combine + REINT packed stores at leg*OLs + k = natural interleaved out, group-looped over distinct in/out pointers), fwd.
...
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 4 --zp-tlf --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=tlf radix=4 dir=fwd isa=avx2
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t g = 0; g < Gs; g++) {
        _ztlf4f_body(ip, op, tw_re, Ls, OLs, count);
        ip += 2 * (size_t)4 * Ls;
        op += 2 * (size_t)4 * OLs;
    }
}
