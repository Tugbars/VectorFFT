 ...
 * tlfi (ZTURN-T IN-PLACE last: tlf with every output stream prefetched 4 column quads ahead — the caller's buffer went cold under the plane; same arithmetic, bitwise tlf), fwd.
...
/* ===================== PROVENANCE =====================
 * Generated by: gen_radix.exe 4 --zp-tlfi --isa avx2 --uarch raptor_lake_avx2 --emit-c
 * Env overrides: (none)
 * Family: zsplit-pipeline
 * kind=tlfi radix=4 dir=fwd isa=avx2
    ...
    size_t Ls, size_t Gs, size_t OLs, size_t OGs, size_t count)
{
    ...
    for (size_t g = 0; g < Gs; g++) {
        _ztlfi4f_body(ip, op, tw_re, Ls, OLs, count);
        ip += 2 * (size_t)4 * Ls;
        op += 2 * (size_t)4 * OLs;
    }
}
