 ...
 * *** EXPERIMENT (arbitrary-K tail) — NOT built by default. ***
 * Bulk full-vectors + tail. -DVFFT_TAIL_MASKED => one forward masked vector pass
 * (vector-efficient remainder: cost flat in rem). Else scalar remainder loop.
 * t1s twiddle is broadcast (set1, lane-independent) so only rio loads/stores are
 * masked; the forward base=floor reads lanes DISJOINT from the bulk -> in-place
 * safe. Even-K (me%4==0) is byte-identical to the original.
 * ====================================================== */
#include <immintrin.h>
#include <stddef.h>
    ...
    size_t me)
{
    ...
    if (k < me) {
#if defined(VFFT_TAIL_MASKED)  /* one forward masked vector pass; tw broadcast (no mask) */
...
#else  /* scalar (1-wide SSE) remainder loop */
        for (; k < me; k++) {
            ...
            rio_re[3*ios + k] = t33; rio_im[3*ios + k] = t34;
            rio_re[2*ios + k] = t40; rio_im[2*ios + k] = t42;
            rio_re[1*ios + k] = t43; rio_im[1*ios + k] = t44;
            rio_re[0*ios + k] = t45; rio_im[0*ios + k] = t46;
        }
#endif
    }
}
