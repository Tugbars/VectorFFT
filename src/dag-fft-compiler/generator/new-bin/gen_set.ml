...
let emit_one (path : string) (argv_tail : string list) : unit =
  ...
  Fun.protect
    ~finally:(fun () ->
      ...
      Unix.close saved)
    (fun () -> Vfft_v2r.Gen_main.run argv)
...
let () =
  ...
  let targets =
    ...
    | [] | [ "all" ] -> Vfft_v2r.Coverage.quadrants
    | l -> l
  in
  let total = ref 0 in
  List.iter
    (fun q ->
      let files = Vfft_v2r.Coverage.files q in
      let dir = Filename.concat !root (Vfft_v2r.Coverage.dir_of_quadrant q) in
      mkdir_p dir;
      List.iter
        (fun (name, tail) ->
