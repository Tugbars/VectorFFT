...
let () =
  ...
  let a = Ir.mk_load (Expr.Input (0, true)) in
  let b = Ir.mk_load (Expr.Input (1, true)) in
  let c = Ir.mk_load (Expr.Input (2, true)) in
  let ab = Ir.mk_add a b in
  let ac = Ir.mk_add a c in
  let result = Ir.mk_add ab ac in
  Printf.printf
    "Tags: a=%d b=%d c=%d ab=%d ac=%d result=%d\n"
    a.tag
    b.tag
    c.tag
    ab.tag
  ...
  let inp k = Ir.mk_load (Expr.Input (k, true)) in
  let l0 = inp 100 in
  let l1 = inp 101 in
  let l2 = inp 102 in
  let l3 = inp 103 in
  let l4 = inp 104 in
  let n1 = Ir.mk_add l0 l1 in
  let n2 = Ir.mk_add n1 l2 in
  let n3 = Ir.mk_add n2 l3 in
  let n4 = Ir.mk_add n3 l4 in
  (* Interleave loads and adds — peak should stay small. *)
  let schedule2 = [ l0; l1; n1; l2; n2; l3; n3; l4; n4 ] in
  Printf.printf
    "Tags2: l0=%d l1=%d n1=%d l2=%d n2=%d l3=%d n3=%d l4=%d n4=%d\n"
    l0.tag
    l1.tag
    n1.tag
    l2.tag
