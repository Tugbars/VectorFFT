...
let () =
  ...
  let input =
    Regalloc.prepare_for_simple_codelet ~raw_scheduled:raw_dup ~assigns ()
  in
  ...
  (match
     Regalloc.allocate ~isa:Isa.avx512 ~scheduled:input.scheduled
  ...
  in
  let input2 =
    Regalloc.prepare_for_simple_codelet_from_oref ~raw_scheduled:raw_oref
      ~assigns ()
  in
