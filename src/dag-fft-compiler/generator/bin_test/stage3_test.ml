...
let () =
  ...
  let input = Regalloc.prepare_for_simple_codelet ~raw_scheduled:raw_dup ~assigns () in
  ...
  (match
     Regalloc.allocate
       ...
       ()
   with
   | Regalloc.Allocated _ -> Printf.printf "Regalloc.allocate on prep output: PASS\n"
   | Regalloc.Overflow _ ->
  ...
  (try
   ...
   | Failure msg ->
     if
       try
         let _ = Str.search_forward (Str.regexp "I1") msg 0 in
         true
       with
       | Not_found -> false
  ...
  in
  let input2 =
    Regalloc.prepare_for_simple_codelet_from_oref ~raw_scheduled:raw_oref ~assigns ()
  in
