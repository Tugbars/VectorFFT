...
let () =
  ...
  (try
     while true do
       ...
       then (
         incr total;
         let argv = String.split_on_char ' ' line |> List.filter (fun s -> s <> "") in
         match Codelet.of_argv argv with
         | exception Codelet.Parse_error m -> parse_fail := (line, m) :: !parse_fail
         | c ->
   ...
   | End_of_file -> ());
  Printf.printf
    "lines: %d  round-trip OK: %d  parse-fail: %d  order/content-fail: %d\n"
    !total
    !ok
    (List.length !parse_fail)
    (List.length !rt_fail);
  List.iteri
    (fun i (l, m) -> if i < 15 then Printf.printf "PARSE  [%s]  %s\n" m l)
    (List.rev !parse_fail);
  List.iteri
    (fun i (l, b) -> if i < 15 then Printf.printf "RTFAIL\n  rec: %s\n  got: %s\n" l b)
    (List.rev !rt_fail);
