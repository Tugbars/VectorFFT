...
let codelet_exists_t1 ~r ~kind ~orient ~log3:_ ~direction =
  if r = xl_radix
  then kind = T1 && orient = DIT && direction = Fwd
...
let slot_name ~kind ~orient ~log3 ~direction =
  ...
  Printf.sprintf "%s_%s%s_%s" (kind_str kind) (orient_str orient) l3 (dir_str direction)
;;
  ...
  List.iter
    (fun r ->
       foreach_t1_combo (fun ~kind ~orient ~log3 ~direction ->
         if codelet_exists_t1 ~r ~kind ~orient ~log3 ~direction
         then
           Printf.printf
             "extern void %s%s;\n"
...
let emit_struct () =
  ...
  foreach_t1_combo (fun ~kind ~orient ~log3 ~direction ->
    Printf.printf
      "    vfft_proto_codelet_fn %s[VFFT_PROTO_REG_MAX_RADIX];\n"
...
let emit_init ~isa =
  ...
  Printf.printf
    "static inline void vfft_proto_registry_init_%s(vfft_proto_registry_t *reg) {\n"
    isa;
...
let emit_helpers () =
  ...
  foreach_t1_combo (fun ~kind ~orient ~log3 ~direction ->
    ...
    then (
      ...
      Printf.printf
        "static inline int vfft_proto_has_%s_%s%s(const vfft_proto_registry_t *reg, int \
         r) {\n"
        (kind_str kind)
        (orient_str orient)
        (if log3 then "_log3" else "");
      Printf.printf
        "    return r > 0 && r < VFFT_PROTO_REG_MAX_RADIX && reg->%s[r] != NULL;\n"
        slot;
      print_endline "}"))
;;
