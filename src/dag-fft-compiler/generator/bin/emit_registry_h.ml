 ...
 * pow2 families. *)
(* Production set: primes + composites + small/mid pow2 only. The large
 ...
 * so it never emits. *)
let xl_radix = 1024
let all_radixes = standard_radixes
...
let t1_symbol ~r ~kind ~orient ~log3 ~direction ~isa =
  ...
  Printf.sprintf "radix%d_%s_%s_%s%s_%s" r (kind_str kind) (orient_str orient)
    l3 (dir_str direction) isa
...
let codelet_exists_t1 ~r ~kind ~orient ~log3:_ ~direction =
  if r = xl_radix then kind = T1 && orient = DIT && direction = Fwd
  else List.mem r standard_radixes
...
let slot_name ~kind ~orient ~log3 ~direction =
  ...
  Printf.sprintf "%s_%s%s_%s" (kind_str kind) (orient_str orient) l3
    (dir_str direction)
    ...
    (fun kind ->
      List.iter
        (fun orient ->
          List.iter
            (fun log3 ->
              List.iter (fun direction -> f ~kind ~orient ~log3 ~direction) dirs)
            log3s)
        orients)
    kinds
...
let emit_n1_oop_wrappers ~isa =
  ...
  print_endline
    "                memcpy(out_re + (size_t)j*os, in_re + (size_t)j*is, \
     vl*sizeof(double)); \\";
  print_endline
    "                memcpy(out_im + (size_t)j*os, in_im + (size_t)j*is, \
     vl*sizeof(double)); \\";
  print_endline "            } \\";
  print_endline "        } \\";
  print_endline
...
let emit_init ~isa =
  Printf.printf
    "/* Initialize the registry for ISA=%s. Codelets that aren't emitted\n" isa;
  Printf.printf
  ...
  Printf.printf
    "static inline void vfft_proto_registry_init_%s(vfft_proto_registry_t \
     *reg) {\n"
    isa;
    ...
    (fun r ->
      if has_n1 r then begin
        Printf.printf "    reg->n1_fwd[%d] = %s;\n" r
          (n1_wrapper_symbol ~r ~direction:Fwd ~isa);
        Printf.printf "    reg->n1_bwd[%d] = %s;\n" r
          (n1_wrapper_symbol ~r ~direction:Bwd ~isa)
      end)
    all_radixes;
  print_endline "";
      ...
      let lines =
        List.filter_map
          (fun r ->
            if codelet_exists_t1 ~r ~kind ~orient ~log3 ~direction then
              Some
                (Printf.sprintf "    reg->%s[%d] = %s;" slot r
                   (t1_symbol ~r ~kind ~orient ~log3 ~direction ~isa))
            else None)
          all_radixes
      in
...
let emit_helpers () =
  ...
  print_endline "";
  print_endline
    "static inline int vfft_proto_has_n1(const vfft_proto_registry_t *reg, int \
     r) {";
  print_endline
    "    return r > 0 && r < VFFT_PROTO_REG_MAX_RADIX && reg->n1_fwd[r] != \
     NULL;";
  print_endline "}";
  print_endline "";
  foreach_t1_combo (fun ~kind ~orient ~log3 ~direction ->
      let slot = slot_name ~kind ~orient ~log3 ~direction in
      if direction = Fwd then begin
        (* one helper per (kind, orient, log3); checks fwd *)
        Printf.printf
          "static inline int vfft_proto_has_%s_%s%s(const \
           vfft_proto_registry_t *reg, int r) {\n"
          (kind_str kind) (orient_str orient)
          (if log3 then "_log3" else "");
        Printf.printf
          "    return r > 0 && r < VFFT_PROTO_REG_MAX_RADIX && reg->%s[r] != \
           NULL;\n"
          slot;
        print_endline "}"
      end)
...
let emit_header_file ~isa =
  ...
  print_endline " *";
  Printf.printf
    " * Prototype codelet registry for ISA=%s. Function-pointer table\n" isa;
  print_endline
    " * indexed by radix, one slot per (kind × orient × log3? × direction).";
  print_endline
