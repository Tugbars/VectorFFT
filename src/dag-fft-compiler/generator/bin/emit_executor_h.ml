  ...
  if is_blank_or_comment line then None
    ...
    let need k ctx =
      if Array.length toks < k then
        failwith
          (Printf.sprintf "wisdom line %d: %s (have %d tokens): %s" lineno ctx
             (Array.length toks) line)
    in
    ...
    let variants =
      Array.init nf (fun i ->
          variant_of_int (int_of_string toks.(3 + nf + 5 + i)))
    in
...
let t1_symbol ~r ~variant ~direction ?(orient = `Dit) ~isa () =
  ...
  | LOG3, _ ->
      Printf.sprintf "radix%d_t1_%s_log3_%s_%s" r o (dir_str direction) isa
  | T1S, `Dit ->
      Printf.sprintf "radix%d_t1s_dit_%s_%s" r (dir_str direction) isa
  | T1S, `Dif ->
      failwith "T1S not supported in DIF (planner falls back to FLAT)"
...
let executor_name (e : plan_entry) (d : direction) ~isa =
  ...
  let join_var vs =
    String.concat ""
      (List.map (fun v -> string_of_int (variant_code v)) (Array.to_list vs))
  in
  let orient = if e.use_dif_forward then "dif" else "dit" in
  match d with
  | `Fwd ->
      Printf.sprintf "exec_n%d_k%d_%s_v%s_%s_%s_%s" e.n e.k (join_int e.factors)
        (join_var e.variants) orient (dir_str d) isa
  | `Bwd ->
      Printf.sprintf "exec_n%d_k%d_%s_%s_%s_%s" e.n e.k (join_int e.factors)
        orient (dir_str d) isa
  ...
  | `Bwd ->
      ...
      Array.iter
        (fun r ->
          ...
          acc :=
            t1_symbol ~r ~variant:bwd_variant ~direction:`Bwd ~orient ~isa ()
            :: !acc)
        e.factors;
  ...
  | `Fwd ->
      Array.iteri
        (fun s v ->
          ...
          let is_outer = if e.use_dif_forward then s = nf - 1 else s = 0 in
          if is_outer then acc := n1_symbol ~r ~direction:`Fwd ~isa :: !acc
          else begin
            ...
            in
            acc :=
              t1_symbol ~r ~variant:effective_v ~direction:`Fwd ~orient ~isa ()
              :: !acc;
...
let emit_stage_fwd (e : plan_entry) (stage_idx : int) ~isa =
  ...
  let nf = Array.length e.factors in
  let macro =
    if e.use_dif_forward then
      (* DIF: no-twiddle stage is LAST (s = nf-1). *)
      begin if stage_idx = nf - 1 then "VFFT_PROTO_STAGE_DIF_OUTER"
      else
        match v with
      ...
      begin if stage_idx = 0 then "VFFT_PROTO_STAGE_OUTER"
      else
        match v with
...
let sort_for_lookup (entries : plan_entry list) : plan_entry list =
  ...
  in
  let cmp a b =
    let cN = compare a.n b.n in
    if cN <> 0 then cN
    else
      let cK = compare a.k b.k in
      if cK <> 0 then cK
      else
        let cF = compare (Array.to_list a.factors) (Array.to_list b.factors) in
        if cF <> 0 then cF else compare (all_inner_t1s a) (all_inner_t1s b)
    (* false < true → mixed first *)
  in
    ...
    Array.iteri
      (fun s v ->
        if s = outer_stage_idx then ()
          ...
          let effective_v = if e.use_dif_forward && v = T1S then FLAT else v in
          ...
          | LOG3 ->
              Printf.printf
                "\n\
                \        && plan->stages[%d].t1_fwd  != NULL && \
                 plan->stages[%d].use_log3"
                s s
          | FLAT ->
              Printf.printf
                "\n\
...
let emit_lookup_fwd (entries : plan_entry list) ~isa =
  ...
  Printf.printf
    "static inline vfft_proto_exec_fn vfft_proto_lookup_fwd_%s(const \
     stride_plan_t *plan)\n"
    isa;
  Printf.printf "{\n";
  Printf.printf
  ...
  List.iter
    (fun (e : plan_entry) ->
      ...
      let variants_str =
        String.concat ""
          (List.map
             (fun v -> string_of_int (variant_code v))
             (Array.to_list e.variants))
      in
      Printf.printf "    /* Entry: N=%d K=%d factors=%s variants=v%s */\n" e.n
        e.k factors_str variants_str;
      emit_match_body e;
...
let emit_lookup_bwd (entries : plan_entry list) ~isa =
  ...
  Printf.printf
    "static inline vfft_proto_exec_fn vfft_proto_lookup_bwd_%s(const \
     stride_plan_t *plan)\n"
    isa;
  Printf.printf "{\n";
  List.iter
...
let emit_stage_macros () =
  ...
  print_endline
    " *   VFFT_PROTO_STAGE_T1S  (S, R, ISA) — T1S with per-group needs_tw \
     branch";
  print_endline
    " *   VFFT_PROTO_STAGE_LOG3 (S, R, ISA) — LOG3 with per-group needs_tw \
     branch";
  print_endline
    " *                                       (branch is essentially never \
     taken";
  print_endline
  ...
  print_endline
    "            radix##R##_n1_fwd_##ISA(re + base, im + base, NULL, NULL, \
     stride, slice_K); \\";
  print_endline "        } \\";
  print_endline "    }";
  print_endline "";
  ...
  print_endline
    "                /* leg-0 common factor (cf0): codelet only touches legs \
     1..R-1. */ \\";
  print_endline "                const double _cfr = st->cf0_re[g]; \\";
  print_endline "                const double _cfi = st->cf0_im[g]; \\";
  print_endline "                if (_cfr != 1.0 || _cfi != 0.0) \\";
  print_endline
    "                    _stride_cmul_scalar_inplace(re + inv.base, im + \
     inv.base, \\";
  print_endline
    "                                                slice_K, _cfr, _cfi); \\";
  print_endline
    "                radix##R##_t1s_dit_fwd_##ISA(re + inv.base, im + \
     inv.base, \\";
  print_endline
    "                                              inv.tw_re, inv.tw_im, \\";
  print_endline
  ...
  print_endline "            if (inv.tw_re) { \\";
  print_endline
    "                /* LOG3 applies cf0 to ALL R legs (codelet reads raw \
     per-leg). \\";
  print_endline
    "                 * Twiddles come from grp_tw (per-leg LOG3 format), NOT \
     the tape's \\";
  print_endline
    "                 * scalar inv.tw — same as the generic LOG3 path and DIF \
     LOG3. */ \\";
  print_endline "                const double _cfr = st->cf0_re[g]; \\";
  print_endline "                const double _cfi = st->cf0_im[g]; \\";
  print_endline "                if (_cfr != 1.0 || _cfi != 0.0) \\";
  ...
  print_endline
    "                        _stride_cmul_scalar_inplace(re + inv.base + \
     (size_t)_j * stride, \\";
  print_endline
    "                                                    im + inv.base + \
     (size_t)_j * stride, \\";
  ...
  print_endline "            if (!inv.tw_re) { \\";
  print_endline
    "                /* needs_tw=0 group (k_prev=0 in DIT): no twiddle, use \
     n1. */ \\";
  print_endline
  ...
  print_endline "            } \\";
  print_endline
    "            /* leg-0 common factor (cf0), same as T1S; FLAT stages legs \
     1..R-1. */ \\";
  print_endline "            const double _cfr = st->cf0_re[g]; \\";
  print_endline "            const double _cfi = st->cf0_im[g]; \\";
  print_endline "            if (_cfr != 1.0 || _cfi != 0.0) \\";
  ...
  print_endline
    "            for (size_t kb = 0; kb < slice_K; kb += \
     VFFT_PROTO_TW_BLOCK_K) { \\";
  print_endline
  ...
  print_endline
    "                __m256d _nr = _mm256_fmadd_pd(_vi, _vcfi, \
     _mm256_mul_pd(_vr, _vcfr)); \\";
  print_endline
    "                __m256d _ni = _mm256_fnmadd_pd(_vr, _vcfi, \
     _mm256_mul_pd(_vi, _vcfr)); \\";
  ...
  print_endline
    "                __m512d _nr = _mm512_fmadd_pd(_vi, _vcfi, \
     _mm512_mul_pd(_vr, _vcfr)); \\";
  print_endline
    "                __m512d _ni = _mm512_fnmadd_pd(_vr, _vcfi, \
     _mm512_mul_pd(_vi, _vcfr)); \\";
  ...
  print_endline "";
  print_endline
    "/* Backward stage: FUSED inverse butterfly + post-twiddle via t1s_bwd \
     codelet.";
  print_endline " *";
  ...
  print_endline " *";
  print_endline
    " * Executor adds the leg-0 conj(cf0) post-mul (codelet doesn't touch leg \
     0).";
  print_endline " *";
  print_endline
    " * Compared to the pre-fusion path (n1_bwd + R-leg manual cmul loop), this";
  print_endline
    " * eliminates one full load/store pass over R-1 legs — the inverse \
     butterfly's";
  print_endline
  ...
  print_endline "        for (int g = 0; g < num_groups; g++) { \\";
  print_endline
    "            radix##R##_n1_fwd_##ISA(re + group_base[g], im + \
     group_base[g], \\";
  print_endline
...
let emit_header_file (entries : plan_entry list) ~isa =
  ...
  print_endline "     * use). */";
  print_endline
    "    void (*n1_scaled_bwd)(const double *, const double *, double *, \
     double *,";
  print_endline "                          size_t, size_t, size_t, double);";
  ...
  print_endline
    "    int    variants[STRIDE_MAX_STAGES]; /* per-stage twiddle variant";
  print_endline
    "                                        * (0=FLAT 1=LOG3 2=T1S 3=BUF),";
  print_endline
    "                                        * recorded by plan_create_ex so";
  print_endline
    "                                        * OOP wisdom can persist the mix \
     */";
  ...
  print_endline "     * plans. */";
  print_endline "    void (*override_fwd)(void *data, double *re, double *im);";
  print_endline "    void (*override_bwd)(void *data, double *re, double *im);";
  print_endline "    void (*override_destroy)(void *data);";
  print_endline "    void  *override_data;";
  ...
  print_endline "";
  print_endline
    "static inline void _stride_broadcast_2(double *out_re, double *out_im, \
     size_t n,";
  print_endline
  ...
  let emit_for_isa isa =
    let all_externs =
      let fwd_syms =
        List.concat_map (fun e -> collect_externs e `Fwd ~isa) entries
      in
      let bwd_syms =
        List.concat_map (fun e -> collect_externs e `Bwd ~isa) bwd_entries
      in
      List.sort_uniq compare (fwd_syms @ bwd_syms)
    in
    Printf.printf
      "/* Externs for %s codelets called by the executors below. */\n" isa;
    List.iter
      (fun sym ->
        Printf.printf
          "extern void %s(double *rio_re, double *rio_im,\n\
          \                                 const double *tw_re, const double \
           *tw_im,\n\
    ...
    List.iter
      (fun e ->
        emit_executor e `Fwd ~isa;
        print_endline "")
      entries;
    Printf.printf
      "/* ── Backward executors (%s) ─────────────────────────────── */\n" isa;
    List.iter
      (fun e ->
        emit_executor e `Bwd ~isa;
        print_endline "")
      bwd_entries;
  ...
  print_endline
    "typedef void (*vfft_proto_exec_fn)(const stride_plan_t *, double *, \
     double *,";
  ...
  print_endline "#endif /* __AVX512F__ */";
  let _ = isa in
  (* unused now; CLI param kept for backward-compat. *)
  print_endline "";
