...
let parse_wisdom_line (lineno : int) (line : string) : plan_entry option =
  ...
  then None
    ...
    let need k ctx =
      if Array.length toks < k
      then
        failwith
          (Printf.sprintf
             "wisdom line %d: %s (have %d tokens): %s"
             lineno
             ctx
             (Array.length toks)
             line)
    ...
    need total_expected (Printf.sprintf "nf=%d needs %d tokens" nf total_expected);
    let factors = Array.init nf (fun i -> int_of_string toks.(3 + i)) in
    let use_dif_forward = int_of_string toks.(3 + nf + 4) <> 0 in
    let variants =
      Array.init nf (fun i -> variant_of_int (int_of_string toks.(3 + nf + 5 + i)))
    in
...
let dir_str : direction -> string = function
  | `Fwd -> "fwd"
  | `Bwd -> "bwd"
;;
...
let emit_stage_fwd (e : plan_entry) (stage_idx : int) ~isa =
  ...
  let macro =
    ...
    then (* DIF: no-twiddle stage is LAST (s = nf-1). *)
      if stage_idx = nf - 1
      then "VFFT_PROTO_STAGE_DIF_OUTER"
      else (
        match v with
...
let emit_stage_bwd (e : plan_entry) (stage_idx : int) ~isa =
  let r = e.factors.(stage_idx) in
  let macro =
    if e.use_dif_forward then "VFFT_PROTO_STAGE_DIF_BWD" else "VFFT_PROTO_STAGE_BWD"
  in
  Printf.printf "    %s(%d, %2d, %s)\n" macro stage_idx r isa
;;
...
let emit_executor (e : plan_entry) (d : direction) ~isa =
  ...
  (match d with
   ...
   | `Bwd -> Printf.printf " *   (backward: variant-independent — uses n1_bwd only)\n");
  Printf.printf
    " *   orient=%s dir=%s isa=%s */\n"
    orient
      ...
      (fun s v ->
         ...
         then ()
           ...
           | LOG3 ->
             Printf.printf
               "\n\
               \        && plan->stages[%d].t1_fwd  != NULL && plan->stages[%d].use_log3"
               s
               s
           | FLAT ->
             Printf.printf
...
let emit_lookup_fwd (entries : plan_entry list) ~isa =
  ...
  Printf.printf
    "static inline vfft_proto_exec_fn vfft_proto_lookup_fwd_%s(const stride_plan_t *plan)\n"
    isa;
  Printf.printf "{\n";
  ...
  List.iter
    (fun (e : plan_entry) ->
       ...
       let variants_str =
         String.concat
           ""
           (List.map (fun v -> string_of_int (variant_code v)) (Array.to_list e.variants))
       in
       Printf.printf
         "    /* Entry: N=%d K=%d factors=%s variants=v%s */\n"
         e.n
         e.k
         factors_str
         variants_str;
...
let emit_lookup_bwd (entries : plan_entry list) ~isa =
  ...
  Printf.printf
    "static inline vfft_proto_exec_fn vfft_proto_lookup_bwd_%s(const stride_plan_t *plan)\n"
    isa;
  Printf.printf "{\n";
  List.iter
    (fun (e : plan_entry) ->
...
let emit_stage_macros () =
  ...
  print_endline
    "            radix##R##_n1_fwd_##ISA(re + base, im + base, NULL, NULL, stride, \
     slice_K); \\";
  print_endline "        } \\";
  print_endline "    }";
  print_endline "";
  ...
  print_endline
    "                        _stride_cmul_scalar_inplace(re + inv.base + (size_t)_j * \
     stride, \\";
  print_endline
    "                                                    im + inv.base + (size_t)_j * \
     stride, \\";
  print_endline
    "                                                    slice_K, _cfr, _cfi); \\";
  print_endline
  ...
  print_endline "                 * cf already applied to all R legs above. */ \\";
  ...
  print_endline
    "                                            st->grp_tw_im[g][(size_t)j * full_K]); \
     \\";
  print_endline
    "                    radix##R##_t1_dit_log3_fwd_##ISA(re + inv.base + kb, im + \
     inv.base + kb, \\";
  print_endline
    "                                                  tw_buf_re, tw_buf_im, stride, \
     this_K); \\";
  print_endline "                } \\";
  print_endline "            } else { \\";
  print_endline "                radix##R##_n1_fwd_##ISA(re + inv.base, im + inv.base, \\";
  ...
  print_endline
    "                __m256d _nr = _mm256_fmadd_pd(_vi, _vcfi, _mm256_mul_pd(_vr, \
     _vcfr)); \\";
  print_endline
    "                __m256d _ni = _mm256_fnmadd_pd(_vr, _vcfi, _mm256_mul_pd(_vi, \
     _vcfr)); \\";
  ...
  print_endline
    "                __m512d _nr = _mm512_fmadd_pd(_vi, _vcfi, _mm512_mul_pd(_vr, \
     _vcfr)); \\";
  print_endline
    "                __m512d _ni = _mm512_fnmadd_pd(_vr, _vcfi, _mm512_mul_pd(_vi, \
     _vcfr)); \\";
  ...
  print_endline
    "                radix##R##_n1_bwd_##ISA(base_re, base_im, NULL, NULL, stride, \
     slice_K); \\";
  print_endline "                continue; \\";
  print_endline "            } \\";
  print_endline
  ...
  print_endline "             * leg for me<full_K — the DIF K-split bug). */ \\";
  print_endline "            double tw_buf_re[((R)-1) * VFFT_PROTO_TW_BLOCK_K]; \\";
  print_endline "            double tw_buf_im[((R)-1) * VFFT_PROTO_TW_BLOCK_K]; \\";
  print_endline
    "            for (size_t kb = 0; kb < slice_K; kb += VFFT_PROTO_TW_BLOCK_K) { \\";
...
let emit_header_file (entries : plan_entry list) ~isa =
  ...
  print_endline "    /* Scaled OOP-strided n1 backward (r2c fused last stage; NULL";
  print_endline "     * until such codelets exist in this tree — r2c.h guards every";
  print_endline "     * use). */";
  print_endline
  ...
  print_endline "     * section 49). calloc'd creation leaves these NULL for normal";
  print_endline "     * plans. */";
...
let () =
  ...
  while !i < Array.length Sys.argv do
    ...
    if arg = "--isa" && !i + 1 < Array.length Sys.argv
    then (
      isa := Sys.argv.(!i + 1);
      i := !i + 2)
    else if arg = "--wisdom" && !i + 1 < Array.length Sys.argv
    then (
  ...
  if entries = [] then failwith (Printf.sprintf "wisdom file %s contains no entries" path);
