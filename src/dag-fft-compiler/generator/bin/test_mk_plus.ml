...
let rec describe (n : t) : string =
  ...
  | NK_Plus terms ->
    let parts =
      List.map
        (fun (s, t) -> Printf.sprintf "%s%s" (if s > 0 then "+" else "-") (describe t))
        terms
    in
...
let () =
  ...
  check
    "Plus[Plus[+x, -y], +z] flattens to Plus[+x, -y, +z]"
    (match e10.node with
  ...
  let collected_c = Vfft_v2.Algsimp.collect_m [ Output (0, true), canceling ] in
  let result_c = snd (List.hd collected_c) in
  check
    "2*x + (-2*x) → 0"
    (match result_c.node with
     | NK_Const 0.0 -> true
     | _ -> false);
