...
let classify (fname : string) : abi =
  let has s =
    ...
    and lf = String.length fname in
    let rec go i = i + ls <= lf && (String.sub fname i ls = s || go (i + 1)) in
    ls > 0 && go 0
  in
  if has "_r2cf_"
  then R2cf
  else if has "_hc2c_nat_rng_"
  then Hc2c_nat_rng
  else if has "_hc2c_nat_"
  then Hc2c_nat
  else if has "_hc2c_"
  then Hc2c_packed
  else if has "_hc2hc_" && has "_rng_"
  then Hc2hc_rng
  else if has "_hc2hc_"
  then Hc2hc
  else Unknown
...
let radix_of (fname : string) : int option =
  if String.length fname > 5 && String.sub fname 0 5 = "radix"
  then (
...
let () =
  ...
  let proto_for slot sym =
    ...
    | "r2cf" ->
      Printf.sprintf
        "extern void %s(const double*, double*, double*, ptrdiff_t, ptrdiff_t, \
         ptrdiff_t, size_t);"
        sym
    ...
    | "hc2c" | "hc2c_log3" ->
      Printf.sprintf
        "extern void %s(const double*, const double*, double*, double*, double*, \
         double*, const double*, const double*, ptrdiff_t, ptrdiff_t, ptrdiff_t, \
         size_t);"
        sym
    | "hc2c_rng" ->
      Printf.sprintf
