 ...
 *
 *   4. Each row also reports predicted cycles from radix_dims.h
...
#include "generated/radix_dims.h"   /* STRIDE_RADIX_PROFILE_MAX_R + profile tables */
...
/* ─────────── predicted-cycle from the coarse analytic prior ───────────
 * The static op-count profile was removed (the compiler rewrites op
 * counts at codegen). This diagnostic now compares the measured CPE
 * against the same coarse prior the cost model falls back to, so the
 * column still means something useful: how far the measurement moves
...
static double predicted_cyc(int R, int is_n1)
{
    ...
    if (lr < 1.0) lr = 1.0;
    return 5.0 * (double)R * lr / (double)ISA_VEC_WIDTH;
}
...
typedef struct {
    ...
    /* radix_dims.h ops/SIMD predicted cycles (single value, K=BENCH_K). */
...
" * Calibration fingerprint:\n",
    ...
    fprintf(f, "#include \"radix_dims.h\"\n\n");
