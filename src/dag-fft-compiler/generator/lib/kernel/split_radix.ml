...
let const_cmul (xr : expr) (xi : expr) (cr : float) (ci : float) : expr * expr =
  (* TEST: tan-factored / K-shared form, mirror of Dft_recurse.const_cmul *)
  let abs_eq = abs_float cr = abs_float ci in
  if cr = 1.0 && ci = 0.0
  then xr, xi
  else if cr = 0.0 && ci = 1.0
  then Neg xi, xr
  else if cr = 0.0 && ci = -1.0
  then xi, Neg xr
  else if cr = -1.0 && ci = 0.0
  then Neg xr, Neg xi
  else if abs_eq && cr <> 0.0
  then (
...
type dft_callback =
  sign:[ `Fwd | `Bwd ] -> int -> (int -> expr) -> (int -> expr) -> expr array * expr array
...
let newsplit_core (sign : [ `Fwd | `Bwd ]) =
  ...
  in
  let cexp_f nn i =
    let th = 2.0 *. pi *. float_of_int i /. float_of_int nn in
    cos th, sin th
  in
  let real_f (c, _) = c in
  let sec_f nn m =
    let c, _ = cexp_f nn m in
    1.0 /. c
  in
  let rec s_f nn k =
    if nn <= 4
    then 1.0
      ...
      s_f (nn / 4) k4' *. real_f (cexp_f nn k4'))
  in
  let rec sinv_f nn k =
    if nn <= 4
    then 1.0
      ...
      let k4 = abs k mod (nn / 4) in
      let k4' = if k4 <= nn / 8 then k4 else (nn / 4) - k4 in
      sinv_f (nn / 4) k4' *. sec_f nn k4')
  in
  let sdiv2_f nn k = s_f nn k *. sinv_f (2 * nn) k in
  let sdiv4_f nn k =
    let k4 = abs k mod nn in
    sec_f (4 * nn) (if k4 <= nn / 2 then k4 else nn - k4)
  in
  let t_f nn k =
    let er, ei = cexp_f nn k in
    let sd = sdiv4_f (nn / 4) k in
    er *. sd, ei *. sd
  in
  ...
  let ccmul (xr, xi) (cr, ci) =
    ...
    if ci = 0.0
    then Mul (xr, Const cr), Mul (xi, Const cr)
    else if cr = 0.0
    then Neg (Mul (xi, Const ci)), Mul (xr, Const ci)
    else (
      (* Unification of symmetric-angle constants now happens in Ir.mk_const,
       * which dedups on a 14-sig-digit KEY while storing the first-seen
       * full-precision value. Quantizing the inputs here (the previous
       ...
       * accuracy harness. Full precision in, full precision emitted. *)
      let round_13 x = x in
      let cr_r = round_13 cr
      and ci_r = round_13 ci in
      let acr = abs_float cr_r
      and aci = abs_float ci_r in
      let r_abs = min acr aci /. max acr aci in
      if acr >= aci
      then (
        let tn = if ci_r >= 0.0 = (cr_r >= 0.0) then r_abs else -.r_abs in
  ...
  let dft2 (f : int -> expr * expr) =
    let ar, ai = f 0
    and br, bi = f 1 in
    [| Add (ar, br), Add (ai, bi); Sub (ar, br), Sub (ai, bi) |]
  in
