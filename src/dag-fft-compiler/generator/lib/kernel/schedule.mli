...
module type SCHED_NODE =
  sig
    type payload
    type t = { tag : int; node : payload; }
    val preds : t -> t list
...
module Ir_node :
  sig
    ...
    type t = Ir.t = { tag : int; node : payload; }
    val preds : t -> t list
    val latency : Uarch.t -> t -> int
    val is_load : t -> bool
    val is_store : t -> bool
...
module Make :
  functor (N : SCHED_NODE) ->
    sig
      val compute_cp_dist :
        Uarch.t -> N.t list -> N.t list -> (int, int) Hashtbl.t
      val compute_su_number : N.t list -> (int, int) Hashtbl.t
      val su_schedule :
        Uarch.t ->
        (Expr.elem_ref * N.t) list -> (Expr.elem_ref option * N.t) list
    end
val compute_cp_dist :
  Uarch.t -> Ir_node.t list -> Ir_node.t list -> (int, int) Hashtbl.t
val compute_su_number : Ir_node.t list -> (int, int) Hashtbl.t
val su_schedule :
  Uarch.t ->
  (Expr.elem_ref * Ir_node.t) list -> (Expr.elem_ref option * Ir_node.t) list
val su_schedule_subset :
  Uarch.t ->
  gh:bool -> subset:Ir_node.t list -> sinks:Ir_node.t list -> Ir_node.t list
