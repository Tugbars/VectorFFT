...
let factor_const_muls
  ...
  let one_round (assigns : (Expr.elem_ref * t) list) : (Expr.elem_ref * t) list * bool =
    ...
    in
    let rec walk1 e =
      if not (Hashtbl.mem visited1 e.tag)
      then (
    ...
    let credit_factor tag =
      let c =
        try Hashtbl.find factor_parent_count tag with
        | Not_found -> 0
      in
    ...
    let rec scan n =
      if not (Hashtbl.mem visited2 n.tag)
      then (
        Hashtbl.add visited2 n.tag ();
        (match n.node with
         | NK_Add (a, b) | NK_Sub (a, b) ->
           if (not (is_frozen a.tag)) && not (is_frozen b.tag)
           then (
    ...
    let safe_to_factor (m : t) : bool =
      ...
      then false
        ...
        let uses =
          try Hashtbl.find use_count m.tag with
          | Not_found -> 0
        in
        let fuses =
          try Hashtbl.find factor_parent_count m.tag with
          | Not_found -> 0
        in
        uses > 0 && uses = fuses)
    in
            ...
            | NK_Add (a, b) ->
              ...
              (match try_factor a' b' true with
               | Some folded ->
                 fired := true;
                 folded
               | None -> if a' == a && b' == b then n else mk_add_binary a' b')
            | NK_Sub (a, b) ->
              let a' = rewrite a in
              let b' = rewrite b in
              (match try_factor a' b' false with
               | Some folded ->
                 fired := true;
                 folded
               | None -> if a' == a && b' == b then n else mk_sub_binary a' b')
            | NK_CmulRe (a, b, c, d) ->
              let a' = rewrite a in
              let b' = rewrite b in
              let c' = rewrite c in
    ...
    and try_factor (a : t) (b : t) (is_add : bool) : t option =
      if is_frozen a.tag || is_frozen b.tag
      then None
      else (
        match const_mul_of a, const_mul_of b with
          ...
          then
            Printf.eprintf
              "  try_factor(%s, t%d, t%d): const_mul_match=true, safe_a=%b (uses=%d, \
               fuses=%d), safe_b=%b (uses=%d, fuses=%d)\n"
              (if is_add then "Add" else "Sub")
              a.tag
              b.tag
              sa
              (try Hashtbl.find use_count a.tag with
               | Not_found -> -1)
              (try Hashtbl.find factor_parent_count a.tag with
               | Not_found -> -1)
              sb
              (try Hashtbl.find use_count b.tag with
               | Not_found -> -1)
              (try Hashtbl.find factor_parent_count b.tag with
               | Not_found -> -1);
          if sa && sb
          then (
            let inner = if is_add then mk_add_binary xa xb else mk_sub_binary xa xb in
     ...
     Hashtbl.iter
       (fun frozen_orig () ->
          let rec chase t seen =
            if List.mem t seen
            then t (* cycle guard; shouldn't happen *)
            else (
...
let fma_lift
  ...
  let rec rewrite (n : t) : t =
    if is_frozen n.tag
    then n (* preserve frozen tag identity and subtree *)
    else (
      match Hashtbl.find_opt cache n.tag with
      | Some r -> r
      | None ->
...
let multi_use_fma_lift
  ...
  let disqualify_for tag reason =
    if not (Hashtbl.mem disqualify_reason tag)
    then Hashtbl.add disqualify_reason tag reason;
    Hashtbl.replace mul_status tag false
  in
  let note_use (m : t) ~absorbable =
    if is_frozen m.tag
    then disqualify_for m.tag "frozen"
    else (
  ...
  then (
    ...
    Hashtbl.iter
      (fun _ reason ->
         let c =
           try Hashtbl.find by_reason reason with
           | Not_found -> 0
         in
         Hashtbl.replace by_reason reason (c + 1))
      disqualify_reason;
    Hashtbl.iter
      (fun reason c -> Printf.eprintf "  disqualified: %d Muls due to '%s'\n" c reason)
      by_reason);
      ...
      let r =
        ...
        | NK_Add (a, b) ->
          ...
          then (
            ...
            then
              Printf.eprintf
                "  mfl Add(t%d, t%d): a'=t%d (mul=%b absorb=%b) b'=t%d (mul=%b absorb=%b)\n"
                a.tag
                b.tag
                a'.tag
                (is_m a')
        ...
        | NK_Sub (a, b) ->
          ...
          then (
            ...
            then
              Printf.eprintf
                "  mfl Sub(t%d, t%d): a'=t%d (mul=%b absorb=%b) b'=t%d (mul=%b absorb=%b)\n"
                a.tag
                b.tag
                a'.tag
                (is_m a')
...
let fma_addend_factor
  ...
  let identify_kxy (a : t) (b : t) (m1 : t) (m2 : t) : (t * t * t) option =
    let is_const n =
      ...
      | _ -> false
    in
    if is_const a && a.tag = m1.tag
    then Some (a, b, m2)
    else if is_const a && a.tag = m2.tag
    then Some (a, b, m1)
    else if is_const b && b.tag = m1.tag
    then Some (b, a, m2)
    else if is_const b && b.tag = m2.tag
    then Some (b, a, m1)
    else None
  in
  ...
  let safe_to_factor (m : t) : bool =
    ...
    then false
      ...
      in
      let fuses =
        try Hashtbl.find factor_use_count m.tag with
        | Not_found -> 0
      in
      uses > 0 && uses = fuses)
  in
  ...
  then (
    ...
    Hashtbl.iter
      (fun tag _ ->
         if safe_to_factor { tag; node = NK_Const 0.0 (* dummy; only tag used *) }
      ...
      let r =
        ...
        | NK_Fma (a, b, c, nm, na) ->
          ...
          let try_factor () =
            ...
            | _, NK_Mul (m1, m2) when safe_to_factor c -> try_with_mul m1 m2 na
  ...
  then (
    ...
    let rec walk (n : t) =
      if not (Hashtbl.mem reach n.tag)
      then (
...
let flatten_fma_mul_addend
  ...
  let bump t =
    let c =
      try Hashtbl.find use_count t with
      | Not_found -> 0
    in
  ...
  let add_parent_kind addend_kind tag =
    let cur =
      try Hashtbl.find fma_parents tag with
      | Not_found -> []
    in
  ...
  let rec parent_scan (n : t) =
    if not (Hashtbl.mem parent_visited n.tag)
    then (
  ...
  let is_rewriteable_consumer = function
    | "Add" | "Sub" -> true
    | _ -> false
  in
           ...
           (match addend.node with
            | NK_Mul _
              when (not (is_frozen addend.tag))
                   && (try Hashtbl.find use_count addend.tag with
                       | Not_found -> 0)
                      = 1 ->
  ...
  let multiuse_enabled, multiuse_decision_reason =
    ...
    | _ ->
      ...
      then
        ( true
        , Printf.sprintf
            "AUTO ON (count=%d ≤ threshold=%d)"
            multiuse_candidate_count
            multiuse_density_threshold )
      else
        ( false
        , Printf.sprintf
            "AUTO OFF (count=%d > threshold=%d)"
            multiuse_candidate_count
            multiuse_density_threshold )
  in
  let multi_use_safe (n : t) =
    if not multiuse_enabled
    then false
    else (
      match Hashtbl.find_opt fma_parents n.tag with
  ...
  then (
    ...
    Printf.eprintf
      "  [multi-use gate] candidates=%d, decision=%s\n"
      multiuse_candidate_count
      multiuse_decision_reason);
  ...
  let inspect_candidate (n : t) =
    ...
    | NK_Fma (_, _, addend, _, _) ->
      ...
      then (
        ...
        if fma_frz
        then incr n_blocked_fma_frozen
        else if mul_frz
        then incr n_blocked_mul_frozen
        else if not fma_su
        then (
        ...
        then incr n_blocked_mul_multiuse)
    | _ -> ()
  in
  ...
  then (
    ...
    Printf.eprintf
      "  %d Add/Sub seen, %d Fma(_,_,Mul,_,_) candidates found\n"
      !n_addsub_seen
      !n_fma_mul_candidates;
    Printf.eprintf
      "  blocked: fma_multiuse=%d mul_multiuse=%d fma_frozen=%d mul_frozen=%d\n"
      !n_blocked_fma_multiuse
      !n_blocked_mul_multiuse
      !n_blocked_fma_frozen
    ...
    let rec gscan (n : t) =
      if Hashtbl.mem global_visited n.tag
      then ()
      else (
