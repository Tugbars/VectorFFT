 ...
 *   Sits between Schedule (an ordered list of Ir.t) and Emit_c's
 ...
 *    from Algsimp.lift_spill_markers (consumed by Algsimp.make_spill_info)
...
let allocate_stub ~(isa : Isa.t) ~(scheduled : Ir.t list) : allocation =
  let _ = scheduled in
  { isa
 ...
 *
 * Walks a scheduled list of Ir.t nodes and computes the maximum
...
let peak_live_analysis ~(isa : Isa.t) ~(scheduled : Ir.t list) : live_info =
  ...
  List.iteri
    (fun i (e : Ir.t) ->
       ...
       in
       kill e.tag;
       List.iter (fun (p : Ir.t) -> kill p.tag) (Ir.preds e))
    scheduled;
  { peak_live = !peak; peak_at = !peak_at; n_nodes; budget; fits = !peak <= budget }
;;
...
let reg_name_of_isa (isa : Isa.t) (color : int) : string =
  ...
  | _ ->
    failwith
      (Printf.sprintf "Regalloc.reg_name_of_isa: unsupported vec_width=%d" isa.vec_width)
;;
...
let allocate_linear_scan
  ...
  let rec walk_pred (e : Ir.t) (pos : int) =
    Hashtbl.replace last_use e.tag pos;
    if is_inlined e.tag then List.iter (fun p -> walk_pred p pos) (Ir.preds e)
  in
  List.iteri
    (fun i (e : Ir.t) ->
       Hashtbl.replace last_use e.tag i;
       List.iter (fun (p : Ir.t) -> walk_pred p i) (Ir.preds e))
    scheduled;
  ...
  (try
     List.iteri
       (fun i (e : Ir.t) ->
          ...
          if (not (is_skipped e.tag)) && not (is_inlined e.tag)
          then (
...
let allocate_with_spilling
  ...
  let record_use tag pos =
    let lst =
      try Hashtbl.find uses tag with
      | Not_found ->
        let r = ref [] in
  ...
  (match force_last_use with
   | None -> ()
   | Some tbl -> Hashtbl.iter (fun tag forced_pos -> record_use tag forced_pos) tbl);
  ...
  let next_use_after tag pos =
    ...
    then (
      ...
      | Some arr ->
        ...
        let rec find i =
          if i >= len then None else if arr.(i) > pos then Some arr.(i) else find (i + 1)
        in
        find 0)
  in
    ...
    Hashtbl.iter
      (fun tag _color ->
         let self_protect =
           if tag < 0
           then (
             try Hashtbl.find creation_of_sentinel tag = p with
             | Not_found -> false)
           else false
         in
         if not self_protect
  ...
  let do_spill (victim_tag : int) (pos : int) : int =
    ...
    then (
      ...
      let covered =
        try Hashtbl.find covered_of_sentinel victim_tag with
        | Not_found -> []
      in
      ...
      (match underlying_opt with
       | Some underlying ->
         List.iter
           (fun p -> if p > pos then Hashtbl.remove name_overrides (p, underlying))
           covered
       | None -> ());
      ...
      let prev =
        try Hashtbl.find spill_sites pos with
        | Not_found -> []
      in
  ...
  let try_reload (tag : int) (pos : int) : string option =
    ...
    (match Hashtbl.find_opt live_reload_of tag with
     | Some s when Hashtbl.mem allocated s ->
       ...
       if List.mem pos covered
       then Some (Hashtbl.find name_of_sentinel s)
       else
    ...
    | None ->
      ...
      (match color_opt with
       ...
       | Some color ->
         let counter =
           try Hashtbl.find reload_counter tag with
           | Not_found -> 0
         in
         ...
         let decl =
           { reload_tag = tag; reload_name = name; reload_reg = reg; reload_slot = slot }
         in
         let prev =
           try Hashtbl.find reload_sites pos with
           | Not_found -> []
         in
         ...
         List.iter (fun p -> Hashtbl.replace name_overrides (p, tag) name) covered;
         Some name)
  in
  ...
  (match force_last_use with
   ...
   | Some tbl ->
     Hashtbl.iter
       (fun tag pos ->
          let prev =
            try Hashtbl.find forced_at pos with
            | Not_found -> []
          in
  ...
  (try
     List.iteri
       (fun i (e : Ir.t) ->
          ...
          let rec collect_preds_of (p : Ir.t) =
            pred_tags := p.tag :: !pred_tags;
            if is_inlined p.tag then List.iter collect_preds_of (Ir.preds p)
          in
          List.iter collect_preds_of (Ir.preds e);
          ...
          while !changed do
            ...
            List.iter
              (fun tag ->
                 if Hashtbl.mem spilled tag && not (Hashtbl.mem name_overrides (i, tag))
                 then (
          ...
          | Some tags ->
            ...
            while !changed do
              ...
              List.iter
                (fun t ->
                   if Hashtbl.mem spilled t && not (Hashtbl.mem name_overrides (i, t))
                   then (
    ...
    (match Hashtbl.find_opt forced_at n with
     | Some tags ->
       ...
       while !changed do
         ...
         List.iter
           (fun t ->
              if Hashtbl.mem spilled t && not (Hashtbl.mem name_overrides (n, t))
              then (
...
let prepare_for_simple_codelet
  ...
  List.iter (fun (_, (e : Ir.t)) -> Hashtbl.replace force_last_use e.tag n) assigns;
...
let allocate
  ...
  let validate_input () =
    ...
    List.iter
      (fun (e : Ir.t) ->
         if Hashtbl.mem scheduled_tags e.tag
         then
           failwith
             (Printf.sprintf
                "Regalloc.allocate (I1): tag %d appears multiple times in scheduled — \
                 caller must deduplicate (raw SU output contains (None,e) intermediates \
                 AND (Some oref,e) store sinks for the same e; pass deduplicated \
                 emission order)"
                e.tag);
    ...
    | Some tbl ->
      Hashtbl.iter
        (fun tag _pos ->
           if not (Hashtbl.mem scheduled_tags tag)
           then
             failwith
               (Printf.sprintf
                  "Regalloc.allocate (I5): force_last_use entry for t%d, but tag is not \
                   in scheduled — caller likely confused about which tags are in this \
                   allocation's scope"
                  tag))
        tbl
  in
  ...
  let m5_enabled =
    let opt_out =
      try Sys.getenv "VFFT_NO_REGALLOC_M5" = "1" with
      | Not_found -> false
    in
    not opt_out
  in
