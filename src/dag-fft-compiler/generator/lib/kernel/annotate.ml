...
let annotate (entries : (Expr.elem_ref option * Ir.t) list) : scope =
  let arr =
    Array.of_list
      (List.map (fun (oref, n) -> { output_for = oref; alg_node = n }) entries)
  in
...
let emit_scope
      (isa : Isa.t)
      (buf : Buffer.t)
      (render_intermediate : Ir.t -> string)
      (render_store : Expr.elem_ref -> Ir.t -> string)
      (s : scope)
  : unit
  =
