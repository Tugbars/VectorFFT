 ...
 *
 * The hash-consing technique is Frigo's. We extend it with stronger
 ...
 * this module in full, so Ir.mk_add etc. remain the canonical
 ...
 * process-global; Ir.reset clears them and MUST run before each
...
type node_kind =
   ...
   *
   * The list-valued `Plus` follows Frigo's design. The n-ary form enables
   * `collectM`-style simplification ("ax + bx + cx → (a+b+c)x") in one
   * pass, which the binary NK_Add/NK_Sub form cannot express without
   * recursive tree-walking that misses cross-subtree sharing.
   *
...
(* === OF_EXPR MEMOIZATION ===
 *
 * The math layer (Dft_recurse.dft_ct etc.) produces Expr trees with high textual
 ...
 * (radix-16/8 chains at 28-76 eps L2 against a 1-3 eps reference;
 * radix-4 chains, whose constants are exact, matched the reference).
 * Keyed on the magnitude; sign is
