...
val render_load :
  ...
  in_place:bool ->
  t1s:bool ->
  ?twidsq:bool -> ?twidsq_n:int -> ?strided:bool -> Expr.elem_ref -> string
...
val render_node_def_core :
  ...
  ?twidsq:bool ->
  ?twidsq_n:int ->
  ?strided:bool -> isa:Isa.t -> in_place:bool -> t1s:bool -> Ir.t -> string
type scheduler =
    Topological
...
val compute_inline_set :
  sc:Scratch.t -> (Expr.elem_ref * Ir.t) list -> (int, unit) Hashtbl.t
...
val codelet_metadata :
  ...
  tw_broadcast:bool -> peak_live:int -> (Expr.elem_ref * Ir.t) list -> string
val is_fused_tag : Algsimp.spill_info -> int -> bool
val compute_min_slot_pass1 :
  Algsimp.spill_info -> Ir.t list -> (int, int) Hashtbl.t * Ir.t list
val cluster_split_schedule :
  ...
  Ir.t list
val classify_passes :
  Algsimp.spill_info -> Ir.t list -> (int, [ `Pass1 | `Pass2 ]) Hashtbl.t
val filter_inline_set_cross_pass :
  sc:Scratch.t ->
  (Expr.elem_ref * Ir.t) list ->
  Algsimp.spill_info -> Ir.t list -> (int, unit) Hashtbl.t
...
val body_preamble :
  ...
  isa:Isa.t ->
  spill:Algsimp.spill_info option ->
  ?consts:('a * Ir.t) list -> unit -> string
