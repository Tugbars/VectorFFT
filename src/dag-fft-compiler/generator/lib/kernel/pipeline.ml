...
type recipe =
  ...
  ; dup : dup_ctx option (* enable the env-gated (VFFT_DUP=1) selective-duplication arm *)
  }
    ...
    then (
      ...
      let rec loop n cur =
        ...
        then cur
          ...
          let same =
            try List.for_all2 (fun (_, a) (_, b) -> a.Ir.tag = b.Ir.tag) cur next with
    ...
    | Some ctx ->
      (match Sys.getenv_opt "VFFT_DUP" with
       | Some "1" when spill_markers_raw = [] ->
         ...
         List.iter
           (fun (n : Ir.t) -> Hashtbl.replace indeg n.tag (List.length (Ir.preds n)))
           nodes;
