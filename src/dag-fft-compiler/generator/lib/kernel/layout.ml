...
let render p =
  let base =
    Printf.sprintf
      "    %s%s%s,"
      p.ctype
      (if p.restrict_ then "__restrict__ " else "")
      p.name
  in
...
let silencer p =
  if p.silence then Some (Printf.sprintf "    (void)%s;\n" p.name) else None
;;
...
let ip_buffers_of_bools ~il_in ~il_out =
  ...
  then
    invalid_arg
      "Layout: --ip-il-in + --ip-il-out is the banned hybrid combination (one in-place \
       plane cannot be interleaved on both sides — use the OOP il forms)";
