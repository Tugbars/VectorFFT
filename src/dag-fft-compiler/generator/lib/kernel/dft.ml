 ...
 * Dft_select.pick_algorithm comes from the select layer, Dft_recurse.dft from the
 ...
 *     Ir.of_expr lifts into opaque Cmul atoms so reassociation
 ...
 * output feeds Ir.of_assignments.
 ...
 * emits Load(Twiddle(j, ...)), it substitutes the derivation tree.
 ...
 * that Ir.of_expr will lift to Cmul nodes.
 *
...
let dft_expand_twiddled
   ...
   * is preserved so Ir.of_expr can lift it to Cmul opaque atoms.
   *
