...
type twiddle_policy = TP_Flat | TP_Log3 | TP_PowW1
...
val dft_expand_twiddled :
  ...
  ?direction:direction ->
  ?sign:[ `Bwd | `Fwd ] -> ?table_conj:bool -> int -> Expr.assignment list
val dft_expand_twidsq :
  ?direction:direction ->
  ?sign:[ `Bwd | `Fwd ] -> int -> Expr.assignment list
type spill_marker = { slot : int; re_expr : Expr.expr; im_expr : Expr.expr; }
...
val dft_expand_twiddled_spill :
  ...
  ?direction:direction ->
  ?sign:[ `Bwd | `Fwd ] ->
  int -> Expr.assignment list * spill_marker list * (int * int) option
