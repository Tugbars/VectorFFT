...
let emit_z_split ~(kind : string) ~(radix : int) () : string =
  ...
  then
    (* split-input TERMINATOR, 2-QUAD UNROLL-AND-JAM schedule (sterm scheduling
       race 2026-07-24, z_cascade_plan §4.9993; supersedes the single-quad
       §4.998 schedule BIT-IDENTICALLY -- scheduling only): A = cols k..k+3,
       B = cols k+4..k+7 per trip; phases [loads A+B] [TR4 A] [TR4 B] [tw tree
       + CMULs alternated A/B] [BFLY A] [stores A] [BFLY B] [stores B];
       baseline-shaped 4-col tail keeps the count mod 4 contract. ABI
       unchanged: zin = block-split plane, zout = z out (drev comb), tw_re =
       packed [c x4][s x4] per 4 cols at tw_re + 2k, OLs = count = N/R. *)
    zsterm_uj2_head
    ^ z_split_macros
    ^ "\n__attribute__((target(\"avx2,fma\")))\nvoid "
    ^ fname
