...
let emit_k1_mono ~(isa : Isa.t) ~(n : int) : string =
  if isa.Isa.vec_width <> 4 then
    failwith "--k1-mono: avx2 only in M1 (vec_width 4)";
  if n <> 64 then failwith "--k1-mono: N=64 only in M1";
  let r1 = 8 and r2 = 8 in
  ...
  let cfg =
    {
      ...
      buffer = OutOfPlace;
      twiddles = NoTwiddles;
      name = Printf.sprintf "vfft_k1_mono%d_fwd_avx2" n;
    }
  in
  validate cfg;
  let prep = prepare_butterfly cfg in
  ...
  for h = 0 to (r1 / 4) - 1 do
    ...
    emit_lane_decls buf cfg;
    emit_load_edge buf cfg;
    emit_body_monolithic buf cfg prep;
    Buffer.add_string buf
      (Printf.sprintf
