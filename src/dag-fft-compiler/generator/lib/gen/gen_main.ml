...
let run (argv : string array) : unit =
  ...
  in
  let deduped =
    if apply_fma_lift then Fma_passes.fma_lift ~frozen_tags post_trans else post_trans
  in
  ...
  let deduped, factor_tag_remap =
    if apply_fma_lift
    then Fma_passes.factor_const_muls ~frozen_tags deduped
  ...
  let deduped, mfl_tag_remap =
    if apply_fma_lift
    then Fma_passes.multi_use_fma_lift ~frozen_tags deduped
    else deduped, Hashtbl.create 0
  in
  ...
  let deduped, fma_addend_remap =
    if apply_fma_lift
    then Fma_passes.fma_addend_factor ~frozen_tags deduped
    else deduped, Hashtbl.create 0
  in
  ...
  let deduped, flatten_tag_remap =
    if apply_fma_lift
    then Fma_passes.flatten_fma_mul_addend ~frozen_tags deduped
    else deduped, Hashtbl.create 0
  in
  ...
  let spill_info : Algsimp.spill_info option =
    ...
    then (
      ...
      let tag_markers =
        List.map
          ...
          raw_markers
      in
      Some (Algsimp.make_spill_info ?ct:spill_ct ~fuse:!fuse tag_markers))
    else None
  in
  if !emit_c
  then (
