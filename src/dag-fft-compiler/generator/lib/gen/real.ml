                ...
                List.iter
                  (fun (pl, roff, nm) ->
                     Buffer.add_string
                       buf
                       (Printf.sprintf
                          "        %s_%d = _mm256_set_pd(%s[(2*(b+3)+%d)*in_stride + \
                           %d], %s[(2*(b+2)+%d)*in_stride + %d], \
                           %s[(2*(b+1)+%d)*in_stride + %d], %s[(2*(b+0)+%d)*in_stride + \
                           %d]);\n"
                          nm
                          f
                          pl
                  ...
                  (fun (pl, roff, nm) ->
                     ...
                     for k = 0 to 7 do
                       ...
                       Buffer.add_string
                         ...
                         (Printf.sprintf
                            "            const __m512d _mv%d = \
                             _mm512_permutex2var_pd(_mt%d, %s, _mt%d);\n"
                            k
                            ua
                            idx
                            ub)
         ...
         f=0 and Nyquist self-mirroring. Every out_lane_* is consumed. *)
                       ...
                       (fun (pl, sfx) ->
                          ...
                          for k = 0 to 3 do
                            ...
                            Buffer.add_string
                              ...
                              (Printf.sprintf
                                 "            const __m256d _v%d = \
                                  _mm256_%s_pd(x%s%s_%d, x%s%s_%d);\n"
                                 k
                                 op
                                 xs
                                 sfx
                          ...
                          for i = 0 to 3 do
                            ...
                            Buffer.add_string
                              ...
                              (Printf.sprintf
                                 "            \
                                  _mm256_storeu_pd(&%s[(2*(b+%d)+%d)*out_stride + %d], \
                                  _mm256_permute2f128_pd(_v%d, _v%d, %s));\n"
                                 pl
                                 i
                                 roff
                                 j0
              ...
              for f = 4 * full to h do
                ...
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "            _mm256_store_pd(_q, x1r_%d); _mm256_store_pd(_q + 4, \
                      x1i_%d);\n"
                     f
                     f);
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "            _mm256_store_pd(_q + 8, x2r_%d); _mm256_store_pd(_q + \
                      12, x2i_%d);\n"
                     f
                     f);
                ...
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "                out_im[(2*(b+(size_t)_w))*out_stride + %d] = \
                      _q[4+_w];\n"
                     f);
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "                out_re[(2*(b+(size_t)_w)+1)*out_stride + %d] = \
                      _q[8+_w];\n"
                     f);
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "                out_im[(2*(b+(size_t)_w)+1)*out_stride + %d] = \
                      _q[12+_w];\n"
                     f);
                Buffer.add_string buf "            }\n";
                Buffer.add_string buf "        }\n"
         ...
         Nyquist via a 32-double spill. *)
                       ...
                       (fun (pl, sfx) ->
                          ...
                          for k = 0 to 7 do
                            ...
                            Buffer.add_string
                              ...
                              (Printf.sprintf
                                 "            const __m512d _u%d = \
                                  _mm512_%s_pd(x%s%s_%d, x%s%s_%d);\n"
                                 k
                                 op
                                 xs
                                 sfx
                          ...
                          for k = 0 to 7 do
                            ...
                            let ub = ua + 2 in
                            let idx =
                              if k mod 4 < 2 then "_tp_idx_lo" else "_tp_idx_hi"
                            in
                            Buffer.add_string
                              buf
                              (Printf.sprintf
                                 "            const __m512d _v%d = \
                                  _mm512_permutex2var_pd(_u%d, %s, _u%d);\n"
                                 k
                                 ua
                                 idx
                                 ub)
                          ...
                          for i = 0 to 7 do
                            ...
                            Buffer.add_string
                              ...
                              (Printf.sprintf
                                 "            \
                                  _mm512_storeu_pd(&%s[(2*(b+%d)+%d)*out_stride + %d], \
                                  _mm512_shuffle_f64x2(_v%d, _v%d, %s));\n"
                                 pl
                                 i
                                 roff
                                 j0
              ...
              for f = 8 * full to h do
                ...
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "            _mm512_store_pd(_q, x1r_%d); _mm512_store_pd(_q + 8, \
                      x1i_%d);\n"
                     f
                     f);
                Buffer.add_string
                  buf
                  (Printf.sprintf
                     "            _mm512_store_pd(_q + 16, x2r_%d); _mm512_store_pd(_q + \
                      24, x2i_%d);\n"
                     f
                     f);
