 ...
 *
 * The inner complex sub-DFT is delegated to Dft_recurse.dft, so all the c2c
 ...
 *   2. (N/2)-point complex DFT of z (uses Dft_recurse.dft, which dispatches to
...
let dft_r2c_direct ?(sign = `Fwd) (n : int) (input_re : int -> expr)
  ...
  let z_re_arr, z_im_arr = Dft_recurse.dft half z_in_re z_in_im in
 ...
 *)
let dft_r2c_term_pair ?(sign = `Fwd) (n : int) (k : int) : (expr * expr) * (expr * expr) =
...
let dft_expand_r2c_term ?(sign = `Fwd) (n : int) (k : int) : Expr.assignment list =
...
let dft_r2c_term_laststage ?(sign = `Fwd) (np : int) (r : int) (m : int)
  ...
  in
  let zk_re, zk_im = Dft_recurse.dft ~sign r colk_in_re colk_in_im in
  ...
  let colm_in_im j =
    ...
    Add (Mul (lr, ti), Mul (li, tr))
  in
  let zm_re, zm_im = Dft_recurse.dft ~sign r colm_in_re colm_in_im in
  ignore m;
...
let dft_c2r_direct (n : int) (input_re : int -> expr) (input_im : int -> expr)
  ...
  let z_out_re, z_out_im = Dft_recurse.dft ~sign:`Bwd half z_in_re z_in_im in
...
let dft_r2c_first ?(sign = `Fwd) (r : int) (input_re : int -> expr)
  ...
  let z_in_re k = input_re (2 * k) in
  let z_in_im k = input_re ((2 * k) + 1) in
  Dft_recurse.dft ~sign r z_in_re z_in_im
;;
 ...
 *)
let dft_rdft ?(sign = `Fwd) (n : int) (input_re : int -> expr) : expr array * expr array =
  assert (n >= 2);
  let zero_im _ = Const 0.0 in
  let full_re, full_im = Dft_recurse.dft ~sign n input_re zero_im in
  let half = n / 2 in
  let out_re = Array.make (half + 1) (Const 0.0) in
  let out_im = Array.make (half + 1) (Const 0.0) in
  for k = 0 to half do
 ...
 * Emitted in the stride_n1_fn ABI via Emit_state.r2cf_signature. *)
let dft_expand_r2cf ?(sign = `Fwd) (n : int) : Expr.assignment list =
  let input_re k = Load (Input (k, true)) in
  let half = n / 2 in
  let out_re, out_im = dft_rdft ~sign n input_re in
  let im_hi = if n land 1 = 0 then half - 1 else half in
  let res = List.init (half + 1) (fun k -> Expr.Output (k, true), out_re.(k)) in
  let ims =
...
let dft_expand_r2cb ?(sign = `Bwd) (n : int) : Expr.assignment list =
  ...
  let xim k = if k >= 1 && k <= im_hi then Load (Input (k, false)) else Const 0.0 in
  ...
  let out_re, _out_im = Dft_recurse.dft ~sign n in_re in_im in
...
let dft_dct2_trigII (n : int) (input_re : int -> expr) : expr array =
  ...
  in
  let in_im _ = zero in
  let full_re, _ = Dft_recurse.dft ~sign:`Fwd fourn in_re in_im in
...
let dft_dct3 (n : int) (input_re : int -> expr) : expr array =
   ...
   * We reuse Dft_recurse.dft with sign=Bwd on the Hermitian-extended Z.
  ...
  let buf_re, _buf_im = Dft_recurse.dft ~sign:`Bwd n z_in_re z_in_im in
...
let dft_dct4 (n : int) (input_re : int -> expr) : expr array =
  ...
  let ifft_re, ifft_im =
    Dft_recurse.dft ~sign:`Bwd half (fun k -> psi_re.(k)) (fun k -> psi_im.(k))
  in
...
let dft_hc2hc_dit
  ...
  for k = 1 to n - 1 do
    let re, im = Dft.cmul_pattern ~conj (input_re k) (input_im k) (tw_re k) (tw_im k) in
    twiddled_re.(k) <- re;
    twiddled_im.(k) <- im
  done;
  (* c2c DFT *)
  let re_arr, im_arr =
    Dft_recurse.dft ~sign n (fun k -> twiddled_re.(k)) (fun k -> twiddled_im.(k))
  in
...
let dft_hc2hc_dif
  ...
  out_im.(0) <- im_arr.(0);
  for k = 1 to n - 1 do
    let re, im = Dft.cmul_pattern ~conj re_arr.(k) im_arr.(k) (tw_re k) (tw_im k) in
    out_re.(k) <- re;
    out_im.(k) <- im
  done;
  out_re, out_im
...
let dft_expand_hc2hc ?(sign = `Fwd) ?(direction = `Dit) (n : int) : Expr.assignment list =
  let input_re k = Load (Input (k, true)) in
  let input_im k = Load (Input (k, false)) in
...
let dft_hc2c_dif
  ...
  let re_arr, im_arr = Dft_recurse.dft ~sign n (fun k -> s_re.(k)) (fun k -> s_im.(k)) in
  let out_re = Array.make n (Const 0.0) in
  let out_im = Array.make n (Const 0.0) in
  out_re.(0) <- re_arr.(0);
  out_im.(0) <- im_arr.(0);
