...
let emit_store_unitgroup ~(sc : Emit_render.Scratch.t) (buf : Buffer.t) (c : config)
  : unit
  =
  if !current_post_tw
  then (
...
let emit_body_spill
  ...
  let pass1_nodes =
    List.filter
      (fun (e : Ir.t) -> (not (is_const e)) && Hashtbl.find_opt cls e.tag = Some `Pass1)
      prep.reachable_nodes
  in
  let pass2_nodes =
    List.filter
      (fun (e : Ir.t) -> (not (is_const e)) && Hashtbl.find_opt cls e.tag = Some `Pass2)
      prep.reachable_nodes
  in
  let pass1_assigns =
    List.filter
...
let emit_k1_mono
  ...
  (* M12b: the sixth emission entry point was the last provenance-silent
     one — the recipe now rides in the artifact like everywhere else. *)
  Buffer.add_string
    buf
    (Emit_render.provenance_block
       ~family:
         (Printf.sprintf
            ...
            n
            (match r1_opt with
             | Some r1 -> Printf.sprintf " (%dx%d four-step)" r1 (n / r1)
             | None -> "")
            (if il then if sw then ", il-sw" else ", il" else ", split"))
       [ Printf.sprintf "ISA: %s" isa.Isa.name ]);
  Buffer.add_string
    buf
        ...
        Some
          (fun buf ->
            ...
            then (
              ...
              Buffer.add_string
                buf
                "    const __m512i _il_idx_o = _mm512_set_epi64(15, 7, 14, 6, 13, 5, 12, \
                 4);\n");
            if cfg.Cfg.strided_il_in
            then (
              Buffer.add_string
                buf
                "    const __m512i _il_idx_de = _mm512_set_epi64(14, 12, 10, 8, 6, 4, 2, \
                 0);\n";
              Buffer.add_string
                buf
                "    const __m512i _il_idx_do = _mm512_set_epi64(15, 13, 11, 9, 7, 5, 3, \
                 1);\n"))
    ; strided_load_il =
        Some
          (fun buf ->
                  ...
                  Buffer.add_string
                    ...
                    (Printf.sprintf
                       "            const __m256d _row_re_%d = \
                        _mm256_permute4x64_pd(_mm256_unpacklo_pd(_zv0_%d, _zv1_%d), \
                        0xD8);\n"
                       r
                       r
                       r);
                  Buffer.add_string
                    ...
                    (Printf.sprintf
                       "            const __m256d _row_im_%d = \
                        _mm256_permute4x64_pd(_mm256_unpackhi_pd(_zv0_%d, _zv1_%d), \
                        0xD8);\n"
                       r
                       r
                       r)
          ...
          (fun buf ->
            ...
            then (
              ...
              for g = 0 to groups - 1 do
                let j0 = g * 4 in
                let stfn =
                  if cfg.Cfg.strided_ilo_nt
                  then "_mm256_stream_pd"
                  else "_mm256_storeu_pd"
                in
                Buffer.add_string
                  buf
                  (Printf.sprintf
                ...
                for k = 0 to 3 do
                  Buffer.add_string
                    buf
                    (Printf.sprintf
                       "            const __m256d _p%d_lo = _mm256_unpacklo_pd(_u%d_re, \
                        _u%d_im);\n"
                       k
                       k
                       k);
                  Buffer.add_string
                    buf
                    (Printf.sprintf
                       "            const __m256d _p%d_hi = _mm256_unpackhi_pd(_u%d_re, \
                        _u%d_im);\n"
                       k
                       k
                       k)
              ...
              for g = 0 to groups - 1 do
                ...
                let stfn =
                  if cfg.Cfg.strided_ilo_nt
                  then "_mm512_stream_pd"
                  else "_mm512_storeu_pd"
                in
                Buffer.add_string
                  buf
                  (Printf.sprintf
