...
let compute_peak_live ~(subset : Algsimp.t list) ~(sinks : Algsimp.t list)
    (schedule : Algsimp.t list) : int =
      ...
      List.iter
        (fun p ->
          if Hashtbl.mem in_subset p.Algsimp.tag then begin
            let cur =
              try Hashtbl.find remaining_users p.tag with Not_found -> 0
            in
  ...
  List.iter
    (fun n ->
      let preds_in_sub =
        List.filter (fun p -> Hashtbl.mem in_subset p.Algsimp.tag) (preds_of n)
      in
      List.iter
        (fun p ->
          let cur =
            try Hashtbl.find remaining_users p.Algsimp.tag with Not_found -> 0
          in
...
let compute_progress (schedule : Algsimp.t list)
    (cp_dist : (int, int) Hashtbl.t) : int =
  let n = List.length schedule in
  let total = ref 0 in
  List.iteri
    (fun i node ->
      let cp =
        try Hashtbl.find cp_dist node.Algsimp.tag with Not_found -> 0
      in
...
let bb_search ~(uarch : Uarch.t) ~(subset : Algsimp.t list)
    ~(sinks : Algsimp.t list) ~(initial_schedule : Algsimp.t list)
    ~(initial_peak : int) ~(time_budget_sec : float) :
  ...
  let cp_of n = try Hashtbl.find cp_dist n.Algsimp.tag with Not_found -> 0 in
    ...
    (fun n ->
      List.iter
        (fun p ->
          if Hashtbl.mem in_subset p.Algsimp.tag then begin
            let cur =
              try Hashtbl.find initial_users p.tag with Not_found -> 0
            in
  ...
  let initial_ready =
    List.filter (fun n -> Hashtbl.find initial_unsched n.Algsimp.tag = 0) subset
  in

  let scheduled_cp_sum = ref 0 in

  let rec dfs (partial : Algsimp.t list) (ready : Algsimp.t list) (peak : int)
      (progress : int) (step : int) : unit =
      ...
      if sat_peak > !best_sat_peak then ()
        ...
        if prune_secondary then ()
          ...
          let pressure_delta n =
            let preds_in_sub =
              List.filter
                (fun p -> Hashtbl.mem in_subset p.Algsimp.tag)
                (preds_of n)
            in
            let kills =
              List.fold_left
            ...
            in
            let n_users =
              try Hashtbl.find remaining_users n.Algsimp.tag
              with Not_found -> 0
            in
            let is_sink = Hashtbl.mem sink_set n.tag in
              ...
              if check_budget () then ()
                ...
                List.iter
                  (fun u ->
                    ...
                    if new_count = 0 then
                      new_ready_extra := u :: !new_ready_extra)
                  user_list;
                let ready' =
...
let bb_schedule_subset (uarch : Uarch.t) ~(time_budget_sec : float)
    ~(subset : Algsimp.t list) ~(sinks : Algsimp.t list) : Algsimp.t list =
  if subset = [] then []
  else begin
    ...
    let best, _peak, _progress =
      bb_search ~uarch ~subset ~sinks ~initial_schedule:baseline
        ~initial_peak:baseline_peak ~time_budget_sec
    in
    best
  end
