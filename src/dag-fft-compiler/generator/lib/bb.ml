...
let compute_progress (schedule : Algsimp.t list) (cp_dist : (int, int) Hashtbl.t) : int =
  let n = List.length schedule in
  let total = ref 0 in
  List.iteri
    (fun i node ->
       let cp =
         try Hashtbl.find cp_dist node.Algsimp.tag with
         | Not_found -> 0
       in
...
let bb_search
  ...
  let cp_of n =
    try Hashtbl.find cp_dist n.Algsimp.tag with
    | Not_found -> 0
  in
  ...
  let rec dfs
            (partial : Algsimp.t list)
            (ready : Algsimp.t list)
            (peak : int)
            (progress : int)
            (step : int)
    : unit
            ...
            List.sort
              (fun a b ->
                 ...
                 then compare da db
                   ...
                   let ca = cp_of a in
                   let cb = cp_of b in
                   if ca <> cb then compare cb ca else compare a.Algsimp.tag b.tag))
              ready
          in
          List.iter
            (fun n ->
               ...
               then ()
                 ...
                 List.iter
                   ...
                   user_list;
                 let ready' =
                   List.filter (fun x -> x.Algsimp.tag <> n.tag) ready @ !new_ready_extra
                 in
