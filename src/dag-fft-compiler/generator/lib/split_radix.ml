 ...
 *)
let dft_split_radix ~(dft_rec : dft_callback) ?(sign = `Fwd) (n : int)
  ...
  for k = 0 to m - 1 do
    ...
    let t1_re, t1_im =
      const_cmul o1_re.(k) o1_im.(k) (cos theta1) (sin theta1)
    in
    let t3_re, t3_im =
      const_cmul o3_re.(k) o3_im.(k) (cos theta3) (sin theta3)
    in
...
let newsplit_core (sign : [ `Fwd | `Bwd ]) =
  ...
  and combine0 nn (u : (expr * expr) array) (z : (expr * expr) array)
...
let dft_newsplit ~(sign : [ `Fwd | `Bwd ]) (n : int) (input_re : int -> expr)
