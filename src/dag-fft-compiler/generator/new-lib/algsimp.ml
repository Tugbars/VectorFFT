(* algsimp.ml — algebraic rewrite passes over the hash-consed IR.
 *
 * The IR itself (node_kind / t, hash-consing, smart constructors,
 * the Expr lift) lives in ir.ml and is re-exported here in full via
 * `include Ir`, so every `Algsimp.X` reference and `open Algsimp`
 * site compiles unchanged and module-level mutable state (hash-cons
 * table, tag counter, of_expr memo) stays physically single. This
 * file owns what runs ON the IR: spill-marker lifting, the rewrite
 * passes (dedup_sub_pairs, collect_m / deep_collect, the factor and
 * share family, transpose, the FMA cascade, butterfly_share_mul),
 * and DAG statistics / pretty-printing.
 *)
