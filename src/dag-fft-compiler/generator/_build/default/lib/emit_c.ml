...
let render_load ~(isa : Isa.t) ~(in_place : bool) ~(t1s : bool)
    ...
    (match r with
     ...
     | Expr.Twiddle (j, true)  ->
       if !current_tw_perpos then
         Isa.set1_pd_str isa (Printf.sprintf "tw_re[%d*(me/%d) + b/%d]" j isa.vec_width isa.vec_width)
     ...
     | Expr.Twiddle (j, false) ->
       if !current_tw_perpos then
         Isa.set1_pd_str isa (Printf.sprintf "tw_im[%d*(me/%d) + b/%d]" j isa.vec_width isa.vec_width)
  ...
  let stride =
    ...
    else if !r2cf_signature || !r2cb_signature || !hc_strided || !n1_oop_strided then "is"
    else "K"
  in
  let loop_var =
    if twidsq || !r2cf_signature || !r2cb_signature || !hc_strided || !n1_oop_strided || !r2c_term_signature || !r2c_term_laststage then "v" else "k" in
  let tw_stride =
    if in_place then "me"
    else if !hc_strided then "vl"
    else "K" in
  ...
  let render_input_addr j is_re =
    ...
    if !r2c_term_laststage then
      ...
      let stride =
        if !r2cb_signature then (if is_re then "is_re" else "is_im")
        else stride in
  ...
  | Expr.Twiddle (j, true) ->
    ...
    else Isa.loadu_pd isa (Printf.sprintf "tw_re[%d*%s + %s]" j tw_stride loop_var)
  | Expr.Twiddle (j, false)->
    if tw_broadcast then Isa.set1_pd_str isa (Printf.sprintf "tw_im[%d]" j)
    else Isa.loadu_pd isa (Printf.sprintf "tw_im[%d*%s + %s]" j tw_stride loop_var)
  | Expr.Output _ ->
...
let render_hoisted_consts ~(isa : Isa.t) (nodes : t list) : string =
  ...
  if not !hoist_consts_enabled then "" else begin
  ...
  List.iter (fun e ->
    ...
    | NK_Const c ->
      ...
      Buffer.add_string b (Printf.sprintf "    %s\n"
        (Isa.const_decl isa (Printf.sprintf "t%d" e.tag)
...
let render_node_def
  ...
  if Hashtbl.mem hoisted_const_tags e.tag then "" else
...
let compute_inline_set
  ...
  List.iter (fun n ->
    ...
    | NK_Plus _ ->
      Algsimp.nk_plus_unreachable "emit_c.ml use_count walker"
  ) nodes;
...
let codelet_metadata ~(isa : Isa.t) ~(spill : spill_info option)
  ...
  let peak_live =
    ...
    else (Regalloc.peak_live_analysis ~isa ~scheduled:nodes).peak_live in
  let nadd = ref 0 and nsub = ref 0 and nmul = ref 0 and nfma = ref 0
  and ncmul = ref 0 and nneg = ref 0
  and n_vload = ref 0   (* per-iteration vector loads: input data + (non-broadcast) twiddles *)
  and n_swload = ref 0  (* loop-invariant scalar twiddle broadcasts (hoisted, ~free) *) in
  List.iter (fun n -> match n.node with
    | NK_Add _ -> incr nadd | NK_Sub _ -> incr nsub | NK_Mul _ -> incr nmul
    | NK_Fma _ -> incr nfma | NK_Neg _ -> incr nneg
    ...
    | NK_Load r ->
      (match r with
       ...
       | Expr.Output _ -> incr n_vload)  (* defensive: Output as load source is invalid *)
    | NK_Const _ | NK_Plus _ -> ()) nodes;
  let fp_instr = !nadd + !nsub + !nmul + !nfma + 2 * !ncmul + !nneg in
  let flops = !nadd + !nsub + !nmul + 2 * !nfma + 2 * !ncmul + !nneg in
  let ess_loads = !n_vload in               (* per-iteration vector loads only *)
  ...
  Printf.sprintf
    ...
    \ *   essential_io=%d ops (vec_loads=%d + stores=%d)  [+%d hoisted scalar-twiddle loads, not counted]\n\
    ...
    \ */\n"
    fp_instr flops !nadd !nsub !nmul !nfma !ncmul !nneg
    ess_io ess_loads ess_stores !n_swload
    xslots xpass_mem xpass_live
    memory_floor peak_live budget
    (if membound then "MEMORY-BOUND" else "compute-capable")
    (if membound then ">" else "<=")
    (if fits then "fits" else "SPILLS")
    (if fits then "<=" else ">")
    (if xslots > 0 then
       "  [CT: peak_live is max over passes; cross_pass_cut is added explicit spill traffic]"
     else "")
...
let compute_min_slot_pass1 (sp : spill_info) (pass1_nodes : t list)
  ...
  let pass1_rev =
    List.sort (fun (a : t) b -> compare b.tag a.tag) pass1_nodes in
  List.iter (fun (e : t) ->
    let my =
      ...
      | None ->
        ...
        let s_mins = List.filter_map (fun t ->
          Hashtbl.find_opt min_slot t) s_tags in
        (match s_mins with
         | [] -> None
...
let cluster_split_schedule
  ...
  if sp.ct_n2 <= 0 then pass1_blocked_topo
      ...
      let rec go acc cur_cluster cur_acc = function
        ...
        | n :: rest ->
          ...
          if k = cur_cluster then go acc cur_cluster (n :: cur_acc) rest
            ...
            let acc' = match cur_acc with
              | [] -> acc
              | _ -> (cur_cluster, List.rev cur_acc) :: acc in
            go acc' k [n] rest
      in
    ...
    in
    List.concat_map (fun (_cluster_id, group_nodes) ->
      let cluster_sinks = List.filter (fun (e : t) ->
...
let filter_inline_set_cross_pass
    (assigns : (Expr.elem_ref * t) list) (sp : spill_info) (nodes : t list)
    : (int, unit) Hashtbl.t =
...
let provenance_env_overrides () : string =
  let keys = ["VFFT_N1_BLOCK_MIN"; "VFFT_NO_REGALLOC"; "VFFT_PIN_FORCE";
              "VFFT_CT_FACTOR"; "VFFT_SPLIT_RADIX"; "VFFT_COLLECT_M";
              "VFFT_DEEP_COLLECT"] in
  let act = List.filter_map (fun k ->
    match Sys.getenv_opt k with
    | Some v -> Some (k ^ "=" ^ v) | None -> None) keys in
...
let provenance_block ~(family : string) (lines : string list) : string =
  ...
  Buffer.add_string b (Printf.sprintf " * Generated by: %s\n"
    (String.concat " "
       (Array.to_list
          (match !provenance_argv with Some a -> a | None -> Sys.argv))));
  Buffer.add_string b (Printf.sprintf " * Env overrides: %s\n"
...
let emit_codelet
  ...
  let force_fence = try Sys.getenv "VFFT_FORCE_FENCE" = "1" with Not_found -> false in
  ...
  let regalloc_enabled =
    not opt_out && force_pin
  in

  let fence_enabled =
    not opt_out && (force_pin || force_fence)
  in
  ...
  let sched_str = match scheduler with
    | Topological -> "topological (plain dependency order)"
    | Bisection -> "Frigo recursive bisection (repaired, sections 27-28; not a production default)"
    | SU _ -> "the list scheduler (flag name --su; lazy loads, sink-first, cp_dist, SU tiebreak; section 30)"
    | _ -> "annotated/experimental" in
  Buffer.add_string buf (provenance_block ~family [
    ...
    Printf.sprintf "GH pressure mode: %b (auto-rule: vec_regs<=16 && n>=32; +4-8%% documented)" gh;
    (match spill with
     | Some sp when sp.ct_n1 > 0 ->
       Printf.sprintf
         "Construction: BLOCKED two-pass CT %dx%d; seam through L1 scratch by design (doc 58); threshold n>=16 on <=16-reg ISAs else 25 (section 35)"
         sp.ct_n1 sp.ct_n2
     | Some _ ->
       "Construction: blocked (spill recipe, non-CT marker set)"
     | None ->
       "Construction: MONOLITHIC (below blocking threshold, or prime/Direct: no CT pass boundary)");
    Printf.sprintf
      "Regalloc+pinning: %b (gate: log3+avx512+R<=32 OR avx2 n1 R>=16, sections 32/34/35; kill switch VFFT_NO_REGALLOC)"
      regalloc_enabled;
    Printf.sprintf
      "Value fences: %b (asm-volatile +v stops gcc rematerialization, doc 28; exempt: n1 avx2 R in {8,16}, measured better unfenced)"
      fence_enabled;
  ...
  if strided then begin
    ...
    (match spill with
     ...
     | Some sp ->
       ...
       Buffer.add_string buf (Printf.sprintf
         "    %s spill_im[%d];\n" isa.vec_type sp.num_slots));
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
  ...
  end else if !r2cb_signature then begin
    ...
    (match spill with
     ...
     | Some sp ->
       ...
       Buffer.add_string buf (Printf.sprintf
         "    %s spill_im[%d];\n" isa.vec_type sp.num_slots));
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !r2cf_signature then begin
    ...
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !r2c_term_laststage then begin
    ...
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !r2c_term_signature then begin
    ...
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !hc2c_natural then begin
    ...
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !hc_strided then begin
    ...
    if !hc_ranged then
      Buffer.add_string buf "    for (int kc = 0; kc < kcount; kc++) {\n";
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !n1_oop_strided then begin
    ...
    Buffer.add_string buf (render_hoisted_consts ~isa (topo_sort_reachable (List.map snd assigns)));
    Buffer.add_string buf (Printf.sprintf "    for (size_t v = 0; v < vl; v += %d) {\n" isa.vec_width)
  end else if !r2r_signature then begin
  ...
  let render_output_addr k is_re =
    if !r2c_term_laststage then begin
      (* Output(2s) = Xp slot s at Xp[s*osp+v]; Output(2s+1) = Xm slot s. *)
      if k land 1 = 0 then
        Printf.sprintf "%s[%d*osp + %s]" (if is_re then "Xp_re" else "Xp_im") (k/2) loop_var
      else
        Printf.sprintf "%s[%d*osm + %s]" (if is_re then "Xm_re" else "Xm_im") (k/2) loop_var
    end else
    if !r2c_term_signature then begin
      (* Output(0)=X[k]->Xp pair; Output(1)=X[m]->Xm pair, over v lanes. *)
      if k = 0 then
        Printf.sprintf "%s[%s]" (if is_re then "Xp_re" else "Xp_im") loop_var
      else
        Printf.sprintf "%s[%s]" (if is_re then "Xm_re" else "Xm_im") loop_var
    end else
    if !hc2c_natural then begin
      if k <= !hc2c_nat_sstar then
        Printf.sprintf "%s[%d*osp + %s]" (if is_re then "Rp" else "Ip")
          k loop_var
      else
        Printf.sprintf "%s[%d*osm + %s]" (if is_re then "Rm" else "Im")
          (!hc2c_nat_r - 1 - k) loop_var
    end else
    ...
    if twidsq && twidsq_n > 0 then
      ...
      Printf.sprintf "%s[%d*%s + %s]" buf k (out_stride_for is_re) loop_var
  in
  ...
  (match spill with
   | Some sp ->
     ...
     let (min_slot, pass1_blocked_topo) =
       compute_min_slot_pass1 sp pass1_nodes in
     ...
     if sp.ct_n2 > 0 then begin
       ...
       Hashtbl.iter (fun tag _ -> Hashtbl.add first_walk_assigned tag ()) cluster_of_pass2_node;
     ...
     List.iteri (fun i (e : t) -> Hashtbl.replace pass2_pos_of_tag e.tag i)
       pass2_ordered;
     List.iter (fun (_, e) ->
       if !current_store_on_compute
          && Hashtbl.mem pass2_pos_of_tag e.tag then
     ...
     if !current_store_on_compute then
       List.iter (fun ((_, e) as a) ->
         let cur = try Hashtbl.find soc_assigns_by_tag e.tag with Not_found -> [] in
...
let emit_strided_load_preamble ~isa ~radix ~in_re_name ~in_im_name
  ...
  ignore isa; ignore radix; ignore in_re_name; ignore in_im_name;
  ignore group_stride_name;
  failwith "emit_strided_load_preamble: UnitLeg OOP path not yet wired \
            (M2 phase-2 transpose-helper extraction); use --oop-load UG"

let emit_strided_store_postamble ~isa ~radix ~out_re_name ~out_im_name
                                 ~group_stride_name (_buf : Buffer.t) : unit =
  ignore isa; ignore radix; ignore out_re_name; ignore out_im_name;
  ignore group_stride_name;
  failwith "emit_strided_store_postamble: UnitLeg OOP path not yet wired \
            (M2 phase-2 transpose-helper extraction); use --oop-store UG"
