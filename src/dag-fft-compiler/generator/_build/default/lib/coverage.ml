...
let ip_families (isa : string) : (string * string list) list =
  let n1 =
    [ ("n1_fwd", ["--in-place"; "--isa"; isa; "--su"]);
      ("n1_bwd", ["--in-place"; "--isa"; isa; "--su"; "--bwd"]) ] in
  let t1 =
    List.concat_map (fun (tsuf, targ) ->
      List.concat_map (fun (dsuf, darg) ->
        List.concat_map (fun (bsuf, barg) ->
          List.map (fun (lsuf, larg) ->
            let fam = tsuf ^ dsuf ^ bsuf ^ lsuf in
            let args =
              ["--twiddled"; "--in-place"; "--isa"; isa]
              @ targ @ darg @ barg @ larg in
            (fam, args))
            [("", []); ("_log3", ["--log3"])])
          [("_fwd", []); ("_bwd", ["--bwd"])])
        [("_dit", []); ("_dif", ["--dif"])])
...
let oop_base (isa : string) : string list =
  ["--oop"; "--oop-buffer-oop"; "--oop-load"; "UG"; "--oop-store"; "UG";
   "--isa"; isa]
...
let files (quadrant : string) : (string * string list) list =
  ...
  | "oop-avx2" | "oop-avx512" ->
    ...
    let n1 =
      List.map (fun r ->
        (Printf.sprintf "radix%d_n1_oop_%s.c" r isa,
  ...
  | "strided-avx2" | "strided-avx512" ->
    ...
    let radices =
      if isa = "avx2" then strided_radices_avx2 else strided_radices_avx512 in
    List.concat_map (fun r ->
  ...
  | "rfft-avx2" | "rfft-avx512" ->
    ...
    let leaf_only = [32] in   (* big leaves for low-stage-count plans;
    ...
    @ List.concat_map (fun r ->
      [ (Printf.sprintf "radix%d_r2cf_%s.c" r isa,
        ...
        (Printf.sprintf "radix%d_hc2c_nat_rng_fwd_%s.c" r isa,
         [string_of_int r; "--hc2c-nat"; "--ranged"; "--t1s"; "--isa"; isa; "--su"]) ])
      radices
  | "trig-avx2" | "trig-avx512" ->
    ...
    List.concat_map (fun (kind, sizes) ->
      List.map (fun n ->
    ...
    @ List.concat_map (fun r ->
      [ (Printf.sprintf "radix%d_r2cb_%s.c" r isa,
        ...
        (Printf.sprintf "radix%d_hc2hc_dif_bwd_%s.c" r isa,
         [string_of_int r; "--hc2hc"; "--dif"; "--bwd"; "--t1s"; "--isa"; isa; "--su"]);
        (Printf.sprintf "radix%d_hc2hc_dif_log3_bwd_%s.c" r isa,
         [string_of_int r; "--hc2hc"; "--dif"; "--bwd"; "--log3"; "--t1s"; "--isa"; isa; "--su"]);
        (Printf.sprintf "radix%d_hc2hc_dif_rng_bwd_%s.c" r isa,
         [string_of_int r; "--hc2hc"; "--dif"; "--bwd"; "--ranged"; "--t1s"; "--isa"; isa; "--su"]) ])
      radices
  | q -> failwith ("Coverage.files: unknown quadrant " ^ q)

let quadrants =
  ["inplace-avx2"; "inplace-avx512"; "oop-avx2"; "oop-avx512";
   "strided-avx2"; "strided-avx512"; "trig-avx2"; "trig-avx512";
   "rfft-avx2"; "rfft-avx512"; "c2r-avx2"; "c2r-avx512"]
