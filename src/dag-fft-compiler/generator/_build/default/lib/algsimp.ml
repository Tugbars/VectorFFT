...
let nk_plus_unreachable (site : string) : 'a =
  failwith (Printf.sprintf
    "NK_Plus reached site %S which is not yet wired (Commit 2+).      If you see this, a consumer is generating NK_Plus before its readers      are migrated; check the call stack." site)
...
and mk_plus (terms : (int * t) list) : t =
  ...
  let nonconst = List.filter (fun (s, t) ->
    match t.node with
    | NK_Const c -> const_sum := !const_sum +. (float_of_int s *. c); false
    | _ -> true
  ) flat in
  ...
  let expanded = List.concat_map (fun (s, t) ->
    let n = abs s in
  ...
  let with_const =
    if !const_sum = 0.0 then sorted
    else (1, mk_const !const_sum) :: sorted
  in
...
let lift_spill_markers ?(reassoc = true) (markers : Dft.spill_marker list)
  ...
  let node_kind n = match n.node with
    | NK_Const _ -> "Const" | NK_Load _ -> "Load" | NK_Neg _ -> "Neg"
    | NK_Add _ -> "Add" | NK_Sub _ -> "Sub" | NK_Mul _ -> "Mul"
    | NK_Fma _ -> "Fma" | NK_CmulRe _ -> "CmulRe" | NK_CmulIm _ -> "CmulIm"
...
let extract_coefficient (t : t) : float * t =
  ...
  match t.node with
  | NK_Neg inner ->
    let (c, atom) = unsigned inner in (-. c, atom)
  | _ -> unsigned t

let collect_m (assigns : (Expr.elem_ref * t) list) : (Expr.elem_ref * t) list =
  if Sys.getenv_opt "VFFT_COLLECT_M" <> Some "1"      && Sys.getenv_opt "VFFT_DEEP_COLLECT" <> Some "1" then assigns
  else begin
    ...
    let rec rebuild (e : t) : t =
      ...
      | None ->
        let r = match e.node with
          ...
          | NK_Mul (a, b) -> mk_mul (rebuild a) (rebuild b)
          | NK_CmulRe (a, b, c, d) ->
            let (re, _) = mk_cmul (rebuild a) (rebuild b)
                                  (rebuild c) (rebuild d) in re
          | NK_CmulIm (a, b, c, d) ->
            let (_, im) = mk_cmul (rebuild a) (rebuild b)
                                  (rebuild c) (rebuild d) in im
          | NK_Fma (a, b, c, nm, na) ->
...
let rec distribute_term ~(depth : int) (sign, t : int * t) : (int * t) list =
  if depth <= 0 then [(sign, t)]
    ...
    match t.node with
    | NK_Neg inner ->
      distribute_term ~depth (-sign, inner)
    | NK_Mul (a, b) ->
       ...
       | Some c ->
         (match other_part.node with
          ...
          | NK_Mul (m1, m2) ->
            ...
            let rotated_opt = match m1.node, m2.node with
...
let deep_collect ?(depth_limit = 5)
  ...
  if Sys.getenv_opt "VFFT_DEEP_COLLECT" <> Some "1" then assigns
    ...
    and any_mul_exists (c : t) (x : t) (y : t) : bool =
      lookup_node (NK_Mul (
        (if c.tag <= x.tag then c else x),
        (if c.tag <= x.tag then x else c))) <> None
      ||
      lookup_node (NK_Mul (
        (if c.tag <= y.tag then c else y),
        (if c.tag <= y.tag then y else c))) <> None
    ...
    and distribute_use_aware ~depth (sign, t : int * t) : (int * t) list =
      if depth <= 0 then [(sign, t)]
        ...
        | NK_Mul (a, b) ->
          let const_part, other_part = match a.node, b.node with
            | NK_Const _, _ -> Some a, b
            | _, NK_Const _ -> Some b, a
            | _ -> None, t
          in
    ...
    and try_deep_collect (original : t) (rebuilt_binary : t) : t =
      ...
      let distributed = List.concat_map
        (distribute_use_aware ~depth:depth_limit) rebuilt_terms in
      if List.length distributed <= n_input_terms then rebuilt_binary
        ...
        if win then collect_terms_to_tree distributed
        else rebuilt_binary
      end
    in
    List.map (fun (lhs, e) -> (lhs, rebuild e)) assigns
...
let factor_const_muls
  ...
  let one_round (assigns : (Expr.elem_ref * t) list)
    ...
    let rec walk1 e =
      if not (Hashtbl.mem visited1 e.tag) then begin
        ...
        match e.node with
        | NK_Const _ | NK_Load _ -> ()
        | NK_Neg a -> bump_use a.tag; walk1 a
        | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) ->
          bump_use a.tag; bump_use b.tag; walk1 a; walk1 b
        | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
          bump_use a.tag; bump_use b.tag; bump_use c.tag; bump_use d.tag;
          walk1 a; walk1 b; walk1 c; walk1 d
        | NK_Fma (a, b, c, _, _) ->
          bump_use a.tag; bump_use b.tag; bump_use c.tag;
          walk1 a; walk1 b; walk1 c
        | NK_Plus _ -> nk_plus_unreachable "algsimp.ml:1689"
      end
    in
    List.iter (fun (_, e) -> bump_use e.tag; walk1 e) assigns;
    ...
    let rec scan n =
      if not (Hashtbl.mem visited2 n.tag) then begin
        ...
        | NK_Const _ | NK_Load _ -> ()
        | NK_Neg a -> scan a
        | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) -> scan a; scan b
        | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
          scan a; scan b; scan c; scan d
        | NK_Fma (a, b, c, _, _) -> scan a; scan b; scan c
    ...
    let safe_to_factor (m : t) : bool =
      if is_frozen m.tag then false
        ...
        let fuses = try Hashtbl.find factor_parent_count m.tag
                    with Not_found -> 0 in
        uses > 0 && uses = fuses
    in
    ...
    let rec rewrite (n : t) : t =
      ...
      let frozen_short_circuit =
        is_frozen n.tag &&
        (match n.node with
         | NK_Add _ | NK_Sub _ -> false
         | _ -> true)
      in
      if frozen_short_circuit then n
      else match Hashtbl.find_opt cache n.tag with
      | Some r -> r
      | None ->
        let r = match n.node with
          | NK_Const _ | NK_Load _ -> n
    ...
    and try_factor (a : t) (b : t) (is_add : bool) : t option =
      ...
      | Some (ka, xa), Some (kb, xb) when ka.tag = kb.tag ->
        ...
        if Sys.getenv_opt "FACTOR_TRACE" <> None then
          Printf.eprintf "  try_factor(%s, t%d, t%d): const_mul_match=true, safe_a=%b (uses=%d, fuses=%d), safe_b=%b (uses=%d, fuses=%d)\n"
            (if is_add then "Add" else "Sub") a.tag b.tag sa
            (try Hashtbl.find use_count a.tag with Not_found -> -1)
            (try Hashtbl.find factor_parent_count a.tag with Not_found -> -1)
            sb
            (try Hashtbl.find use_count b.tag with Not_found -> -1)
            (try Hashtbl.find factor_parent_count b.tag with Not_found -> -1);
        if sa && sb then begin
          let inner = if is_add then mk_add_binary xa xb
                      else mk_sub_binary xa xb in
          Some (mk_mul ka inner)
        end else None
      | _ -> None
    in
    let new_assigns =
      List.map (fun (oref, e) -> (oref, rewrite e)) assigns in
    (new_assigns, !fired)
  in
  ...
  let rec loop assigns rounds =
    if rounds >= max_rounds then begin
      if trace then Printf.eprintf "factor_const_muls: hit max_rounds %d\n" max_rounds;
      assigns
    end else
      let (next, fired) = one_round assigns in
      if trace then Printf.eprintf "factor_const_muls: round %d, fired=%b\n" rounds fired;
      if fired then loop next (rounds + 1)
      else next
  in
  ...
  (match frozen_tags with
   | None -> ()
   | Some tbl ->
     Hashtbl.iter (fun frozen_orig () ->
       let rec chase t seen =
         if List.mem t seen then t  (* cycle guard; shouldn't happen *)
         else match Hashtbl.find_opt rewrite_log t with
  ...
  let rec scan (n : t) =
    if Hashtbl.mem visited n.tag then ()
      ...
      | NK_Mul (a, b) ->
        ...
        (match a.node with NK_Mul _ -> disqualify_for a.tag "nested-in-mul" | _ -> ());
        (match b.node with NK_Mul _ -> disqualify_for b.tag "nested-in-mul" | _ -> ());
        scan a; scan b
      | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
        List.iter (fun x ->
          match x.node with
      ...
      | NK_Fma (a, b, c, _, _) ->
        ...
        (match a.node with NK_Mul _ -> disqualify_for a.tag "in-fma-mul-slot" | _ -> ());
        (match b.node with NK_Mul _ -> disqualify_for b.tag "in-fma-mul-slot" | _ -> ());
        (match c.node with NK_Mul _ -> disqualify_for c.tag "in-fma-addend" | _ -> ());
    ...
    | None ->
      let r = match n.node with
        ...
        | NK_Add (a, b) ->
          ...
          if trace then begin
            let is_m x = match x.node with NK_Mul _ -> true | _ -> false in
            let abs_a = is_m a' && is_absorbable a' in
            let abs_b = is_m b' && is_absorbable b' in
            if is_m a' || is_m b' then
              Printf.eprintf "  mfl Add(t%d, t%d): a'=t%d (mul=%b absorb=%b) b'=t%d (mul=%b absorb=%b)\n"
        ...
        | NK_Sub (a, b) ->
          ...
          if trace then begin
            ...
            let abs_b = is_m b' && is_absorbable b' in
            if is_m a' || is_m b' then
              Printf.eprintf "  mfl Sub(t%d, t%d): a'=t%d (mul=%b absorb=%b) b'=t%d (mul=%b absorb=%b)\n"
                a.tag b.tag a'.tag (is_m a') abs_a b'.tag (is_m b') abs_b
          end;
      ...
      if r != n && is_frozen n.tag then
        Hashtbl.replace tag_remap n.tag r.tag;
      r
  in
  let new_assigns = List.map (fun (oref, e) -> (oref, rewrite e)) assigns in
...
let fma_addend_factor
  ...
  let identify_kxy (a : t) (b : t) (m1 : t) (m2 : t)
      : (t * t * t) option =
  ...
  let rec scan (n : t) =
    if not (Hashtbl.mem visited n.tag) then begin
      ...
      match n.node with
      | NK_Const _ | NK_Load _ -> ()
      | NK_Neg a -> bump use_count a.tag; scan a
      | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) ->
        bump use_count a.tag; bump use_count b.tag;
        scan a; scan b
      | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
        bump use_count a.tag; bump use_count b.tag;
        bump use_count c.tag; bump use_count d.tag;
        scan a; scan b; scan c; scan d
      | NK_Fma (a, b, c, _, _) ->
        bump use_count a.tag; bump use_count b.tag; bump use_count c.tag;
        scan a; scan b; scan c;
        (* Factor patterns:
  ...
  let safe_to_factor (m : t) : bool =
    if is_frozen m.tag then false
      ...
      let uses = try Hashtbl.find use_count m.tag with Not_found -> 0 in
      let fuses = try Hashtbl.find factor_use_count m.tag
                  with Not_found -> 0 in
      uses > 0 && uses = fuses
  in
  ...
  if Sys.getenv_opt "FMA_ADDEND_TRACE" <> None then begin
    ...
    Hashtbl.iter (fun tag _ ->
      if safe_to_factor { tag; node = NK_Const 0.0 (* dummy; only tag used *) } then
        incr n_candidates
    ) factor_use_count;
        ...
        | NK_Fma (a, b, c, nm, na) ->
          ...
          let try_factor () =
            let try_with_mul m1 m2 na_eff =
              match identify_kxy a' b' m1 m2 with
              | Some (k, x, y) ->
                let folded = match nm, na_eff with
                 | false, false -> mk_mul k (mk_add_binary x y)
                 | false, true  -> mk_mul k (mk_sub_binary x y)
                 | true,  false -> mk_mul k (mk_sub_binary y x)
                 | true,  true  ->
                   mk_neg (mk_mul k (mk_add_binary x y))
                in
                if Sys.getenv_opt "FMA_ADDEND_TRACE" <> None then
                  Printf.eprintf
                    "[fma_addend] rewrite t%d (Fma nm=%b na=%b na_eff=%b) → t%d  K=t%d X=t%d Y=t%d  c=t%d→t%d\n"
                    n.tag nm na na_eff folded.tag k.tag x.tag y.tag c.tag c'.tag;
                Some folded
              | None -> None
            in
...
let flatten_fma_mul_addend
  ...
  let rec scan (n : t) =
    if not (Hashtbl.mem visited n.tag) then begin
      ...
      match n.node with
      | NK_Const _ | NK_Load _ -> ()
      | NK_Neg a -> bump a.tag; scan a
      | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) ->
        bump a.tag; bump b.tag; scan a; scan b
      | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
        bump a.tag; bump b.tag; bump c.tag; bump d.tag;
        scan a; scan b; scan c; scan d
      | NK_Fma (a, b, c, _, _) ->
        bump a.tag; bump b.tag; bump c.tag; scan a; scan b; scan c
      | NK_Plus _ -> nk_plus_unreachable "algsimp.ml:2591"
    end
  in
  List.iter (fun (_, e) -> bump e.tag; scan e) assigns;
  let single_use (n : t) =
    (try Hashtbl.find use_count n.tag with Not_found -> 0) = 1
  in
  ...
  let rec parent_scan (n : t) =
    if not (Hashtbl.mem parent_visited n.tag) then begin
      ...
      match n.node with
      | NK_Const _ | NK_Load _ -> ()
      | NK_Neg a -> note_child a; parent_scan a
      | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) ->
        note_child a; note_child b; parent_scan a; parent_scan b
      | NK_Fma (a, b, c, _, _) ->
        note_child a; note_child b; note_child c;
        parent_scan a; parent_scan b; parent_scan c
      | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
        note_child a; note_child b; note_child c; note_child d;
        parent_scan a; parent_scan b; parent_scan c; parent_scan d
      | NK_Plus _ -> ()
    end
  in
      ...
      if Hashtbl.mem visited n.tag then ()
        ...
        (match n.node with
         | NK_Fma (_, _, addend, _, _) when not (is_frozen n.tag) ->
           (match addend.node with
            | NK_Mul _ when not (is_frozen addend.tag)
                         && (try Hashtbl.find use_count addend.tag with Not_found -> 0) = 1
              ->
              (* Mul addend is single-use; check parent kinds. *)
              (match Hashtbl.find_opt fma_parents n.tag with
               | Some kinds when kinds <> [] && List.for_all is_rewriteable_consumer kinds ->
                 (* Also exclude single-use Fmas — those already rewrite under
                  * the unrelaxed rule and don't count toward density pressure. *)
                 let use = try Hashtbl.find use_count n.tag with Not_found -> 0 in
  ...
  let multiuse_enabled, multiuse_decision_reason =
    ...
    | _ ->
      if multiuse_candidate_count <= multiuse_density_threshold
      then true,  Printf.sprintf "AUTO ON (count=%d ≤ threshold=%d)"
                    multiuse_candidate_count multiuse_density_threshold
      else false, Printf.sprintf "AUTO OFF (count=%d > threshold=%d)"
                    multiuse_candidate_count multiuse_density_threshold
  in

  let multi_use_safe (n : t) =
    if not multiuse_enabled then false
    else match Hashtbl.find_opt fma_parents n.tag with
    | None -> false  (* No parent info (shouldn't happen for valid Fma-Mul) *)
    | Some kinds ->
      kinds <> [] && List.for_all is_rewriteable_consumer kinds
  in
  ...
  let inspect_candidate (n : t) =
    ...
    | NK_Fma (_, _, addend, _, _) ->
      ...
      let is_mul_addend = (match addend.node with NK_Mul _ -> true | _ -> false) in
      if is_mul_addend then begin
        ...
        if fma_frz then incr n_blocked_fma_frozen
          ...
          if Sys.getenv_opt "FLATTEN_FMA_MUL_TRACE" <> None then begin
            ...
            Printf.eprintf "    blocked-fma-multiuse: Fma tag=t%d use_count=%d\n"
              n.tag uc
          end
        end
    ...
    | None ->
      let r = match n.node with
        ...
        | NK_Add (a, b) when not (is_frozen n.tag) ->
          incr n_addsub_seen; inspect_candidate a; inspect_candidate b;
          ...
          (match match_fma_mul a, match_fma_mul b with
           ...
           | Some (ft, fa, fb, mc, md, nm, na), None ->
             ...
             do_flatten ~fma_tag:ft ~p ~fa:(rewrite fa) ~fb:(rewrite fb)
               ~mc:(rewrite mc) ~md:(rewrite md)
               ~inner_nm:nm ~inner_na:false ~outer_nm:na
        ...
        | NK_Sub (a, b) when not (is_frozen n.tag) ->
          ...
          (match match_fma_mul a, match_fma_mul b with
           ...
           | None, Some (ft, fa, fb, mc, md, nm, na) ->
             ...
             do_flatten ~fma_tag:ft ~p ~fa:(rewrite fa) ~fb:(rewrite fb)
               ~mc:(rewrite mc) ~md:(rewrite md)
               ~inner_nm:(not nm) ~inner_na:false ~outer_nm:(not na)
           | Some (ft, fa, fb, mc, md, nm, na), None ->
             (* Sub(F, p) — inner gets neg_add=true to flip p *)
             let p = rewrite b in
             do_flatten ~fma_tag:ft ~p ~fa:(rewrite fa) ~fb:(rewrite fb)
               ~mc:(rewrite mc) ~md:(rewrite md)
               ~inner_nm:nm ~inner_na:true ~outer_nm:na
           | None, None ->
             let a' = rewrite a in let b' = rewrite b in
             if a' == a && b' == b then n else mk_sub_binary a' b')
        | NK_Sub (a, b) ->
  ...
  if Sys.getenv_opt "FLATTEN_FMA_MUL_TRACE" <> None then begin
    ...
    Printf.eprintf "  blocked: fma_multiuse=%d mul_multiuse=%d fma_frozen=%d mul_frozen=%d\n"
      !n_blocked_fma_multiuse !n_blocked_mul_multiuse
      !n_blocked_fma_frozen !n_blocked_mul_frozen;
    ...
    let rec gscan (n : t) =
      if Hashtbl.mem global_visited n.tag then ()
        ...
        (match n.node with
         | NK_Fma (_, _, c, _, _) ->
           (match c.node with
            | NK_Mul _ -> incr global_count
            | _ -> ())
         | _ -> ());
        match n.node with
        | NK_Const _ | NK_Load _ -> ()
        | NK_Neg a -> gscan a
        | NK_Add (a, b) | NK_Sub (a, b) | NK_Mul (a, b) -> gscan a; gscan b
        | NK_Fma (a, b, c, _, _) -> gscan a; gscan b; gscan c
        | NK_CmulRe (a, b, c, d) | NK_CmulIm (a, b, c, d) ->
          gscan a; gscan b; gscan c; gscan d
        | NK_Plus _ -> ()
      end
    in
  ...
  List.iter (fun (f, m_addend) ->
    ...
    | NK_Fma (a, b, _, _, _), NK_Mul (p, q) ->
      ...
      (match m_other.node with
       | NK_Mul _ ->
         let cands = try Hashtbl.find by_addend m_other.tag with Not_found -> [] in
         List.iter (fun f' ->
           if f'.tag <> f.tag then
             match f'.node with
             | NK_Fma (a', b', _, _, _) ->
               ...
               in
               if f_slot_matches_pq && f.tag < f'.tag
                  && not (Hashtbl.mem rewrite_to_share f.tag)
                  && not (Hashtbl.mem rewrite_to_share f'.tag) then begin
                 Hashtbl.add rewrite_to_share f.tag m_other;
                 if Sys.getenv_opt "BSM_TRACE" <> None then
                   Printf.eprintf
                     "[bsm] swap pair: F=t%d (mul=t%d,t%d add=Mul(t%d,t%d)) ↔ F'=t%d → share Mul(t%d,t%d)=t%d\n"
                     f.tag a.tag b.tag p.tag q.tag f'.tag a.tag b.tag m_other.tag
               end
             | _ -> ()
         ) cands
       | _ -> ())
      ...
      let r = match n.node with
        ...
        | NK_Fma (a, b, c, nm, na) ->
          ...
          | Some m_shared, NK_Mul (p_orig, q_orig) ->
             ...
             * New addend: m_shared.
             * New flags: (na, nm). *)
            let _ = p_orig in let _ = q_orig in
