...
let radix_of (fname : string) : int option =
  if String.length fname > 5 && String.sub fname 0 5 = "radix" then
    let rec digits i acc =
      if i < String.length fname && fname.[i] >= '0' && fname.[i] <= '9'
      then digits (i+1) (acc * 10 + (Char.code fname.[i] - Char.code '0'))
...
let () =
  ...
  Arg.parse
    ...
    (fun _ -> ()) "emit_rfft_registry --isa <avx2|avx512>";
  let isa = !isa in
  ...
  let entries =
    List.filter_map (fun (fname_c, _argv) ->
      ...
      match radix_of stem, slot_of (classify stem) (is_log3 stem) (is_dif stem) with
  ...
  Printf.printf " *\n";
  Printf.printf " * Each codelet is wired into the slot chosen by its KIND; the slot's\n";
  Printf.printf " * function-pointer type IS that kind's ABI, so a wrong-ABI wire is a\n";
  Printf.printf " * compile error. Coverage-driven: a new rfft codelet auto-appears here. */\n";
  ...
  let proto_for slot sym =
    ...
    | "r2cf" ->
      Printf.sprintf
        "extern void %s(const double*, double*, double*, ptrdiff_t, ptrdiff_t, ptrdiff_t, size_t);"
        sym
    | "hc2hc" | "hc2hc_log3" | "hc2hc_dif" | "hc2hc_dif_log3" ->
      Printf.sprintf
        "extern void %s(const double*, const double*, double*, double*, const double*, const double*, ptrdiff_t, ptrdiff_t, size_t);"
        sym
    | "hc2hc_rng" ->
      Printf.sprintf
        "extern void %s(const double*, const double*, double*, double*, const double*, const double*, ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t, int, size_t);"
        sym
    | "hc2c" | "hc2c_log3" ->
      Printf.sprintf
        "extern void %s(const double*, const double*, double*, double*, double*, double*, const double*, const double*, ptrdiff_t, ptrdiff_t, ptrdiff_t, size_t);"
        sym
    | "hc2c_rng" ->
      Printf.sprintf
        "extern void %s(const double*, const double*, double*, double*, double*, double*, const double*, const double*, ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t, ptrdiff_t, int, size_t);"
        sym
    | _ -> Printf.sprintf "extern void %s();" sym
  in
  Printf.printf "/* extern declarations (every rfft codelet in coverage) */\n";
  List.iter (fun (sym, slot, _r) ->
    print_endline (proto_for slot sym)
  ) entries;
  Printf.printf "\n";
  ...
  Printf.printf "static inline void rfft_register_all_%s(rfft_codelets_t *reg)\n{\n" isa;
