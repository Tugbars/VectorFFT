...
let emit_one (path : string) (argv_tail : string list) : unit =
  let argv =
    Array.of_list (("gen_radix.exe" :: argv_tail) @ ["--emit-c"]) in
  flush stdout;
...
let () =
    ...
    if i < Array.length Sys.argv then begin
      (match Sys.argv.(i) with
       | "--root" when i + 1 < Array.length Sys.argv ->
         root := Sys.argv.(i + 1); parse (i + 2) |> ignore
       | t -> targets := t :: !targets; parse (i + 1) |> ignore);
    end in
  ignore (parse 1);
  let targets =
  ...
  List.iter (fun q ->
    ...
    let dir =
      Filename.concat !root (Vfft_v2.Coverage.dir_of_quadrant q) in
    mkdir_p dir;
    List.iter (fun (name, tail) ->
