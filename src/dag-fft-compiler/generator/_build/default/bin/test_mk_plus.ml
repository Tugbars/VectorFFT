...
let rec describe (n : t) : string =
  ...
  | NK_Plus terms ->
    let parts = List.map (fun (s, t) ->
      Printf.sprintf "%s%s" (if s > 0 then "+" else "-") (describe t)) terms in
...
let () =
  ...
  check "term order doesn't matter (canonical sort)"
    (e5_ab.tag = e5_ba.tag);
  ...
  check "(+2) + (+3) + x → Plus[+5, +x]"
    (match e7.node with
     | NK_Plus terms ->
       List.exists (fun (s, t) ->
         s = 1 && match t.node with NK_Const 5.0 -> true | _ -> false) terms
       && List.length terms = 2
     | _ -> false);
  ...
  check "Plus[Plus[+x, -y], +z] flattens to Plus[+x, -y, +z]"
    (match e10.node with
     | NK_Plus terms -> List.length terms = 3
     | _ -> false);
  ...
  let collected = Vfft_v2.Algsimp.collect_m
    [(Output (0, true), sum)] in
  ...
  let collected_c = Vfft_v2.Algsimp.collect_m
    [(Output (0, true), canceling)] in
  ...
  let collected_xy = Vfft_v2.Algsimp.collect_m
    [(Output (0, true), xy)] in
  let result_xy = snd (List.hd collected_xy) in
  check "x + y → x + y (no collection)"
    (result_xy.tag = xy_tag_before);
