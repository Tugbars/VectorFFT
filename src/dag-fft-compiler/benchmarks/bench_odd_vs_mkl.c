...
int main(int argc, char **argv) {
    ...
    const char *csv   = (argc >= 3) ? argv[2] : "/tmp/wis/odd_vs_mkl.csv";
    ...
    printf("=== wisdom non-pow2 (odd-radix) plans vs MKL  (relative ratios only; pace=%dms) ===\n", pace_ms);
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (is_pow2(N)) continue;                     /* non-pow2 (odd-radix) cells only */
        size_t K = (size_t)Kl;
    ...
    fclose(f);
    if (out) fclose(out);
    printf("\nbenched %d non-pow2 cells, skipped %d.  CSV -> %s\n", benched, skipped, csv);
    return 0;
}
