...
## 6. Selective pinning (follow-up implementation)

Section 3 identified that M-project's blanket `asm volatile`
barriers prevent gcc's auto-fusion of `Add(Mul(a,b), c) → vfmadd*`
patterns. On R=64 n1 AVX-512 hot path, this costs 43 asm FMAs
(160 unbarriered → 117 barriered with fma_lift) — every multi-use
Mul whose consumer is an Add gets stranded across a barrier gcc
can't see through.

### Implementation

Add a per-emission set `current_unpin_candidates` mirroring
`current_regalloc`. Compute it from the scheduled DAG via a
single-pass walk in `compute_unpin_candidates`:

```ocaml
let compute_unpin_candidates scheduled =
  let mul_tags = Hashtbl.create 64 in
  List.iter (fun n ->
    match n.node with
...
At the M3a emission switch in `render_node_def`, fall through to
`const_decl` when the tag is in this set, even if `current_regalloc`
has a `Reg` binding. The result: NK_Mul nodes with at least one
Add/Sub consumer are emitted as plain `const __m512d t<tag> = ...`
without the `register asm + asm volatile` pair.

Escape hatch: `VFFT_DISABLE_SELECTIVE_PIN=1` reverts to pin-all.

### Validation — asm-level

Selective pinning closes the gcc auto-fusion gap completely:

| Codelet | B baseline asm FMA | D pin-all asm FMA | D selective asm FMA |
|---|---|---|---|
| R=64 n1 AVX-512 | 160 | 117 | **160** ✓ |
| R=32 t1 SU+spill | 118 | 106 | **118** ✓ |
| R=64 t1 SU+spill | 286 | 250 | **286** ✓ |
| R=16 n1 AVX-512 | 16 | 12 | **16** ✓ |

Every codelet under M-project now produces the same asm FMA count
as the unbarriered baseline. 45/45 codelets bit-exact (max_err=0).

Predicate coverage (number of tags actually unpinned):

| Codelet | total pins (pin-all) | unpinned by predicate | % |
|---|---|---|---|
| R=8 n1 | 76 | 2 | 2.6% |
| R=64 n1 | 2045 | 50 | 2.4% |
| R=128 n1 | 5090 | 278 | 5.5% |
| R=32 t1 | 570 | 16 | 2.8% |
| R=64 t1 | 1314 | 51 | 3.9% |
| R=128 t1 | 3031 | 99 | 3.3% |

### Runtime impact

Honest result: the asm-level FMA recovery does **not** translate
into meaningful runtime improvements. 7-trial statistics, M-project
+ fma_lift on for both:

| Codelet | pin-all | selective | ratio | σ | verdict |
|---|---|---|---|---|---|
| R=64 n1 AVX-512 K=256 | 40287 ± 115 | 40703 ± 436 | +1.03% | 0.92 | noise |
| R=8 n1 AVX-512 | 116.45 ± 2.39 | 114.45 ± 2.01 | −1.72% | 0.6 | noise |
| R=16 n1 AVX-512 | 660.56 ± 1.98 | 663.81 ± 6.18 | +0.49% | 0.5 | noise |
| R=8 t1 AVX-512 | 183.30 ± 1.57 | 182.02 ± 1.02 | −0.70% | 0.7 | noise |
| R=16 t1 AVX-512 | 469.07 ± 5.95 | 460.75 ± 5.38 | −1.77% | 1.0 | borderline win |
| R=32 t1 SU+spill | 1258.62 ± 54.24 | 1248.72 ± 11.71 | −0.79% | 0.2 | noise |

All effects within 1σ noise; median trend slightly favors selective
(−0.7% across the matrix) but no codelet shows a statistically
significant runtime difference.

### Why the runtime gain is smaller than the asm-level gap suggests

For R=64 n1: selective pin recovers 43 FMAs (each replaces a
vmulpd + vaddpd pair = saves 1 instruction per FMA, ~43 saved
instructions). But gcc inserts +76 vmovapd reg-reg moves to handle
the now-unpinned Mul values whose dataflow is no longer fixed at
M-project-chosen register positions. Net instruction count is
approximately unchanged. The work that fma_lift saves is roughly
balanced by the moves M-project's pinning was preventing.

This is consistent with M-project's original design goal: minimize
reg-reg moves by pinning everything. Selective pinning trades some
of that move-minimization for gcc's auto-fusion freedom. The
trade-off is approximately neutral at the runtime level — the gcc
auto-fusion gap section 3 identified is real at the asm level but
costs roughly as many moves as it saves FMAs.

### What this means for the broader architecture

Selective pinning is **correct, safe, and conceptually clean** — it
closes the asm-level gap section 3 motivated and produces a
binary that matches the unbarriered FMA count. But the runtime
floor for this codelet class isn't bounded by FMA count alone; it's
bounded by the move-vs-fma trade-off that gcc and M-project balance
differently.

To get further runtime improvement, either:

- **Reduce the vmovapd cost.** This requires M-project to predict
  gcc's RA choices for unpinned values and place the pinned
  consumers in compatible registers. Architecturally interesting
  but a substantial change to the regalloc.
- **Use a different pinning predicate.** Unpinning more (e.g.,
  short-lived values regardless of node kind) would let gcc do
  more reshaping but probably add more moves, not fewer. Unpinning
  less (e.g., only Muls whose consumers are *all* Adds) would
  unpin fewer nodes — already tried implicitly by the current
  predicate's narrowness.
- **Operand-ordering hints in NK_Fma.** Lets us emit FMAs that
  gcc can register-allocate without the extra moves. This is
  the same architectural direction that would address the
  R=15/20/21/35 mixed-radix regression class.

### Ship state

Selective pinning is ON by default in `lib/emit_c.ml`. The escape
hatch `VFFT_DISABLE_SELECTIVE_PIN=1` reverts to pin-all behavior.
No codelet regresses significantly with the new default; some
borderline benefit at smaller t1 sizes. The asm output is cleaner
and matches the unbarriered baseline FMA count, which is valuable
for inspection and future composition with operand-ordering work.
...
- ✓ Selective pinning implemented in `lib/emit_c.ml`. Asm-level
    FMA gap closed completely (R=64 n1: 117 → 160). Runtime impact
    noise-to-marginal (median −0.7% across matrix, σ < 1.0 on all
    tested cells). 45/45 bit-exact correct.
...
- `lib/emit_c.ml`: `current_unpin_candidates` ref, 
  `compute_unpin_candidates` helper, selective-pin branch in the
  M3a emission switch.

Future work: refine Cooley_Tukey gate per Option B above, evaluate
Split_radix activation, investigate the move-vs-fma trade-off
limiting selective pinning's runtime payoff (operand-ordering hints
in NK_Fma are the architecturally clean next step).
