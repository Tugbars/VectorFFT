 ...
 *
 * Predicates:
 *   radix7_prefer_dit_log3 — DIT-log3 codelet won as log3 representative
 *                              AND log3 was cross-protocol winner.
 *   radix7_prefer_dif_log3 — collapsed to always-0 in v1.0 (DIF filter).
 *   radix7_prefer_log3     — union of the above (= dit_log3 in v1.0).
 *   radix7_prefer_t1s      — t1s protocol won.
 *   radix7_prefer_buf      — flat won cross-protocol AND t1_buf_dit
...
static inline int radix7_prefer_buf(size_t me, size_t ios) {
    (void)me; (void)ios;
