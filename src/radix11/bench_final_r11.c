...
static void naive(int dir,size_t K,const double*ir,const double*ii,double*nr,double*ni){
    for(size_t k=0;k<K;k++) for(int m=0;m<11;m++){
        double sr=0,si=0;
        for(int n=0;n<11;n++){double a=dir*2.0*M_PI*m*n/11.0;
...
static int verify(const char*l,size_t K,
    ...
    double rel=mag>0?err/mag:err; int p=rel<5e-14;
...
static void bench(size_t K, int warm, int trials){
    ...
    for(size_t k=0;k<K;k++) for(int n=0;n<11;n++){
        fin[k*11+n][0]=ir[n*K+k]; fin[k*11+n][1]=ii_[n*K+k];}
    int na[1]={11};
    ...
    {size_t T=8;
     ...
     for(int i=0;i<warm;i++) r11_genfft_packed_fwd_avx512(p1,p2,p3,p4,K);
    ...
    {size_t T=4;
     double *p1=aa64(N),*p2=aa64(N),*p3=aa64(N),*p4=aa64(N);
     r11_pack(ir,ii_,p1,p2,K,T);
     for(int i=0;i<warm;i++) r11_genfft_packed_fwd_avx2(p1,p2,p3,p4,K);
     for(int t=0;t<trials;t++){double t0=get_ns();
...
int main(void){
    ...
    for(int i=0;i<5;i++){t++;p+=verify("scalar fwd",Ks[i],radix11_genfft_fwd_scalar,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("scalar bwd",Ks[i],radix11_genfft_bwd_scalar,+1);}
    size_t Ka[]={4,8,16,64,256};
    for(int i=0;i<5;i++){t++;p+=verify("avx2 fwd",Ka[i],radix11_genfft_fwd_avx2,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("avx2 bwd",Ka[i],radix11_genfft_bwd_avx2,+1);}
    size_t Kz[]={8,16,64,256,1024};
    for(int i=0;i<5;i++){t++;p+=verify("avx512 fwd",Kz[i],radix11_genfft_fwd_avx512,-1);}
    for(int i=0;i<5;i++){t++;p+=verify("avx512 bwd",Kz[i],radix11_genfft_bwd_avx512,+1);}
    for(int i=0;i<5;i++){t++;p+=verify_packed("packed avx2",Ka[i],4,r11_genfft_packed_fwd_avx2);}
    for(int i=0;i<5;i++){t++;p+=verify_packed("packed avx512",Kz[i],8,r11_genfft_packed_fwd_avx512);}
    /* Cross-ISA */
    {size_t K=8,N=11*K;
