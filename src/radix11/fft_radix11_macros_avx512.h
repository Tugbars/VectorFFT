/**
 * @file fft_radix11_macros_true_soa_CORRECTED.h
 * @brief CORRECTED TRUE END-TO-END SoA Radix-11 Butterfly Macros
 *
 * @details
 * CRITICAL FIXES APPLIED (v2):
 * =======================
 * ✅ FIX #1: Process BOTH lo/hi halves from unpack operations (was dropping 50% of data)
 * ✅ FIX #2: Add masked tail handling for K % 8 != 0 (prevents memory corruption)
 * ✅ FIX #3: Disable NT stores for scattered radix-11 writes (FFTW approach)
 * ✅ FIX #4: Add cache-blocking for large K (FFTW-style blocking)
 * ✅ FIX #5: CORRECT de-interleave using permutex2var (was duplicating values!)
 * ✅ FIX #6: Restore prefetching with proper configuration
 *
 ...
 * ✅ TRUE END-TO-END SoA
 * ✅ Software pipelining
 * ✅ Prefetching (now ISA-aware)
 *
 * @author FFT Optimization Team
 * @version 5.0 (STORE BUG FIXED + PREFETCH RESTORED)
 * @date 2025
 */

#ifndef FFT_RADIX11_MACROS_TRUE_SOA_CORRECTED_H
#define FFT_RADIX11_MACROS_TRUE_SOA_CORRECTED_H
 ...
 * @details RESTORED from original - ISA-aware
 */
...
{
    radix11_consts_avx512 k;
    k.c1 = _mm512_set1_pd(C11_1);
    k.c2 = _mm512_set1_pd(C11_2);
    k.c3 = _mm512_set1_pd(C11_3);
    k.c4 = _mm512_set1_pd(C11_4);
    k.c5 = _mm512_set1_pd(C11_5);
    k.s1 = _mm512_set1_pd(S11_1);
    k.s2 = _mm512_set1_pd(S11_2);
    k.s3 = _mm512_set1_pd(S11_3);
    k.s4 = _mm512_set1_pd(S11_4);
    k.s5 = _mm512_set1_pd(S11_5);
    return k;
}
...
//==============================================================================
// LOAD MACROS (CORRECT - PROCESSES BOTH LO/HI)
//==============================================================================
...
/**
 * @brief Load with TAIL MASKING for K % 8 != 0
 */
#define LOAD_11_LANES_AVX512_NATIVE_SOA_MASKED(k, K, remaining, in_re, in_im,            \
    ...
    do                                                                                   \
    {                                                                                    \
        __mmask8 mask = (__mmask8)((1ULL << (remaining)) - 1ULL);                        \
...
//==============================================================================
// STORE MACROS (CORRECTED - USES PERMUTEX2VAR)
//==============================================================================

/**
 * @brief Store 11 lanes to SoA buffers - CORRECTED VERSION
 *
 * @details CRITICAL FIX: Use permutex2var to properly extract evens/odds
 *
 * WRONG (old code):
 *   re = _mm512_shuffle_pd(y, y, 0x00)  // Duplicates! [r0,r0,r1,r1,...]
 *
 * CORRECT (new code):
 *   re = _mm512_permutex2var_pd(y_lo, idx_re, y_hi)  // Packs! [r0,r1,r2,...]
 */
#define STORE_11_LANES_AVX512_NATIVE_SOA_FULL(k, K, out_re, out_im,                     \
    ...
    do                                                                                  \
    {                                                                                   \
        /* Get permutation indices (create ONCE at function scope for efficiency) */    \
        __m512i idx_re = get_deinterleave_idx_re();                                     \
        __m512i idx_im = get_deinterleave_idx_im();                                     \
        /* De-interleave using permutex2var (extracts evens/odds correctly) */          \
        ...
        /* Store (normal stores - no NT for scattered writes) */                        \
...
/**
 * @brief Store with TAIL MASKING - CORRECTED VERSION
 */
...
//==============================================================================
// STAGE TWIDDLE APPLICATION (PRESERVED EXACTLY)
//==============================================================================
...
//==============================================================================
// BUTTERFLY CORE AND PAIR COMPUTATIONS (PRESERVED EXACTLY)
//==============================================================================
...
#define RADIX11_IMAG_PAIR1_FV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                           \
    do                                                                                                          \
    {                                                                                                           \
        rot_out = _mm512_fmadd_pd(KC.s1, s0,                                                                    \
...
#define RADIX11_IMAG_PAIR2_FV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                             \
    do                                                                                                            \
    {                                                                                                             \
        rot_out = _mm512_fmadd_pd(KC.s2, s0,                                                                      \
...
#define RADIX11_IMAG_PAIR3_FV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                             \
    do                                                                                                            \
    {                                                                                                             \
        rot_out = _mm512_fmadd_pd(KC.s3, s0,                                                                      \
...
#define RADIX11_IMAG_PAIR4_FV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                             \
    do                                                                                                            \
    {                                                                                                             \
        rot_out = _mm512_fmadd_pd(KC.s4, s0,                                                                      \
...
#define RADIX11_IMAG_PAIR5_FV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                             \
    do                                                                                                            \
    {                                                                                                             \
        rot_out = _mm512_fmadd_pd(KC.s5, s0,                                                                      \
...
#define RADIX11_IMAG_PAIR1_BV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                                                                    \
    do                                                                                                                                                   \
    {                                                                                                                                                    \
        rot_out = _mm512_fnmadd_pd(KC.s1, s0,                                                                                                            \
                                   _mm512_fnmadd_pd(KC.s2, s1,                                                                                           \
                                                    _mm512_fnmadd_pd(KC.s3, s2,                                                                          \
                                                                     _mm512_fnmadd_pd(KC.s4, s3,                                                         \
                                                                                      _mm512_mul_pd(_mm512_set1_pd(-1.0), _mm512_mul_pd(KC.s5, s4)))))); \
    } while (0)

#define RADIX11_IMAG_PAIR2_BV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                                                                  \
    do                                                                                                                                                 \
    {                                                                                                                                                  \
        rot_out = _mm512_fnmadd_pd(KC.s2, s0,                                                                                                          \
                                   _mm512_fnmadd_pd(KC.s4, s1,                                                                                         \
                                                    _mm512_fmadd_pd(KC.s5, s2,                                                                         \
                                                                    _mm512_fmadd_pd(KC.s3, s3,                                                         \
                                                                                    _mm512_mul_pd(_mm512_set1_pd(-1.0), _mm512_mul_pd(KC.s1, s4)))))); \
    } while (0)

#define RADIX11_IMAG_PAIR3_BV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                                                                  \
    do                                                                                                                                                 \
    {                                                                                                                                                  \
        rot_out = _mm512_fnmadd_pd(KC.s3, s0,                                                                                                          \
                                   _mm512_fmadd_pd(KC.s5, s1,                                                                                          \
                                                   _mm512_fmadd_pd(KC.s2, s2,                                                                          \
                                                                   _mm512_fnmadd_pd(KC.s1, s3,                                                         \
                                                                                    _mm512_mul_pd(_mm512_set1_pd(-1.0), _mm512_mul_pd(KC.s4, s4)))))); \
    } while (0)

#define RADIX11_IMAG_PAIR4_BV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                                                                  \
    do                                                                                                                                                 \
    {                                                                                                                                                  \
        rot_out = _mm512_fnmadd_pd(KC.s4, s0,                                                                                                          \
                                   _mm512_fmadd_pd(KC.s3, s1,                                                                                          \
                                                   _mm512_fnmadd_pd(KC.s1, s2,                                                                         \
                                                                    _mm512_fmadd_pd(KC.s5, s3,                                                         \
                                                                                    _mm512_mul_pd(_mm512_set1_pd(-1.0), _mm512_mul_pd(KC.s2, s4)))))); \
    } while (0)

#define RADIX11_IMAG_PAIR5_BV_AVX512(s0, s1, s2, s3, s4, KC, rot_out)                                                                                  \
    do                                                                                                                                                 \
    {                                                                                                                                                  \
        rot_out = _mm512_fnmadd_pd(KC.s5, s0,                                                                                                          \
                                   _mm512_fmadd_pd(KC.s1, s1,                                                                                          \
                                                   _mm512_fnmadd_pd(KC.s4, s2,                                                                         \
                                                                    _mm512_fmadd_pd(KC.s2, s3,                                                         \
                                                                                    _mm512_mul_pd(_mm512_set1_pd(-1.0), _mm512_mul_pd(KC.s3, s4)))))); \
    } while (0)
...
//==============================================================================
// TOP-LEVEL BUTTERFLY MACROS WITH PREFETCH
//==============================================================================

/**
 * @brief Forward radix-11 butterfly with prefetch
 */
...
/**
 * @brief Backward radix-11 butterfly with prefetch
 */
...
// TAIL VARIANTS (complete implementations with masking and prefetch)

/**
 * @brief Forward radix-11 butterfly - TAIL VERSION with masking
 * @details Same as full version but uses masked loads/stores for remaining < 8 elements
 */
#define RADIX11_BUTTERFLY_FV_AVX512_NATIVE_SOA_TAIL(k, K, remaining, in_re, in_im,     \
    ...
    {                                                                                  \
        ...
        LOAD_11_LANES_AVX512_NATIVE_SOA_MASKED(k, K, remaining, in_re, in_im,          \
                                               ...
                                               x8_lo, x8_hi, x9_lo, x9_hi,             \
                                               x10_lo, x10_hi);                        \
        /* Apply stage twiddles to LO half (zeros don't affect math) */                \
        APPLY_STAGE_TWIDDLES_R11_AVX512_SOA_NATIVE(k, K, x1_lo, x2_lo, x3_lo,          \
                                                   x4_lo, x5_lo, x6_lo, x7_lo,         \
                                                   x8_lo, x9_lo, x10_lo,               \
                                                   stage_tw, sub_len);                 \
        ...
        RADIX11_ASSEMBLE_PAIR_AVX512(real4_lo, rot4_lo, y4_lo, y7_lo);                 \
        RADIX11_ASSEMBLE_PAIR_AVX512(real5_lo, rot5_lo, y5_lo, y6_lo);                 \
        /* Check if HI half has valid data (remaining > 4) */                          \
        ...
        /* Process HI half butterfly (zeros if remaining <= 4) */                      \
        __m512d t0_hi, t1_hi, t2_hi, t3_hi, t4_hi;                                     \
        __m512d s0_hi, s1_hi, s2_hi, s3_hi, s4_hi, y0_hi;                              \
        RADIX11_BUTTERFLY_CORE_AVX512(x0_hi, x1_hi, x2_hi, x3_hi, x4_hi,               \
                                      x5_hi, x6_hi, x7_hi, x8_hi, x9_hi,               \
...
#endif // FFT_RADIX11_MACROS_TRUE_SOA_CORRECTED_H