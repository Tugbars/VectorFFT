...
class E:
    ...
    def spill(s,v,slot):
        ...
        if I.name=='scalar':
            ...
            s.o(f"_mm256_store_pd(&sp_re[{slot}*{m}],{v}_re);")
            s.o(f"_mm256_store_pd(&sp_im[{slot}*{m}],{v}_im);")
    def reload(s,v,slot):
        I=s.I; m=I.spill_mul
        if I.name=='scalar':
            s.o(f"{v}_re=sp_re[{slot}]; {v}_im=sp_im[{slot}];")
        else:
            s.o(f"{v}_re=_mm256_load_pd(&sp_re[{slot}*{m}]);")
            s.o(f"{v}_im=_mm256_load_pd(&sp_im[{slot}*{m}]);")
...
def emit_kernel(em, direction, B_kernel):
    ...
    for n in range(5):
        ...
        if I.name == 'scalar':
            ...
            em.o(f"  _mm256_store_pd(&sp_re[{n}*{m}], a_re);")
            em.o(f"  _mm256_store_pd(&sp_im[{n}*{m}], a_im);")
        em.o(f"}}")
    em.b()
    ...
    for n in range(5):
        ...
        if I.name == 'scalar':
            ...
            em.o(f"  or_ = _mm256_load_pd(&sp_re[{5+n}*{m}]); oi = _mm256_load_pd(&sp_im[{5+n}*{m}]);")
        # c[n] = (even[n] + odd[n]) / 10
        ...
        if I.name == 'scalar':
            ...
            em.o(f"  _mm256_store_pd(&sp_re[{n}*{m}], {I.mul(I.add(f'v{n}_re','or_'),'inv10')});")
            em.o(f"  _mm256_store_pd(&sp_im[{n}*{m}], {I.mul(I.add(f'v{n}_im','oi'),'inv10')});")
            em.o(f"  _mm256_store_pd(&sp_re[{5+n}*{m}], {I.mul(I.sub(f'v{n}_re','or_'),'inv10')});")
            em.o(f"  _mm256_store_pd(&sp_im[{5+n}*{m}], {I.mul(I.sub(f'v{n}_im','oi'),'inv10')});")
        em.o(f"}}")
    em.b()
...
def main():
    ...
    if I.name == 'scalar':
        ...
        em.L.append(f"#ifndef {I.prefix}_LD")
        em.L.append(f"#define {I.prefix}_LD(p) _mm256_load_pd(p)")
        em.L.append(f"#endif")
        em.L.append(f"#ifndef {I.prefix}_ST")
        em.L.append(f"#define {I.prefix}_ST(p,v) _mm256_store_pd((p),(v))")
        em.L.append(f"#endif")
    em.L.append(f"")
