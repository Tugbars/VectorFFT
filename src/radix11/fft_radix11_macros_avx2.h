 ...
 * ================================
 * ✅ FIXED: Twiddle application now uses proper vectorized complex multiply (no UB)
 * ✅ FIXED: Geometric constants use scalar FMAs (not complex multiply)
 ...
 * ✅ ADDED: Forward/Inverse compile-time separation for optimal code generation
...
#define RADIX11_FORM_PAIRS_AVX2(in_re, in_im, K, k, a_re, a_im,           \
    ...
    {                                                                     \
        ...
        a_im = _mm256_loadu_pd(&(in_im)[k]);                              \
                                                                          \
        t0_re = _mm256_add_pd(b_re, xk_re);                               \
        t0_im = _mm256_add_pd(b_im, xk_im);                               \
        t1_re = _mm256_add_pd(c_re, j_re);                                \
        t1_im = _mm256_add_pd(c_im, j_im);                                \
        t2_re = _mm256_add_pd(d_re, i_re);                                \
        t2_im = _mm256_add_pd(d_im, i_im);                                \
        t3_re = _mm256_add_pd(e_re, h_re);                                \
        t3_im = _mm256_add_pd(e_im, h_im);                                \
        t4_re = _mm256_add_pd(f_re, g_re);                                \
        t4_im = _mm256_add_pd(f_im, g_im);                                \
                                                                          \
        s0_re = _mm256_sub_pd(b_re, xk_re);                               \
        s0_im = _mm256_sub_pd(b_im, xk_im);                               \
        s1_re = _mm256_sub_pd(c_re, j_re);                                \
        s1_im = _mm256_sub_pd(c_im, j_im);                                \
        s2_re = _mm256_sub_pd(d_re, i_re);                                \
        s2_im = _mm256_sub_pd(d_im, i_im);                                \
        s3_re = _mm256_sub_pd(e_re, h_re);                                \
        s3_im = _mm256_sub_pd(e_im, h_im);                                \
        s4_re = _mm256_sub_pd(f_re, g_re);                                \
        s4_im = _mm256_sub_pd(f_im, g_im);                                \
    } while (0)
...
#define RADIX11_COMPUTE_OUTPUTS_AVX2_FORWARD(KC, a_re, a_im,                                                              \
    ...
    {                                                                                                                     \
        ...
        base1_re = _mm256_setzero_pd();                                                                                   \
        base1_im = _mm256_setzero_pd();                                                                                   \
        ...
        base1_im = _mm256_fmadd_pd(s4_im, KC.s5, base1_im);                                                               \
                                                                                                                          \
        base2_re = _mm256_setzero_pd();                                                                                   \
        base2_im = _mm256_setzero_pd();                                                                                   \
        ...
        base2_im = _mm256_fmadd_pd(s4_im, KC.s1, base2_im);                                                               \
                                                                                                                          \
        base3_re = _mm256_setzero_pd();                                                                                   \
        base3_im = _mm256_setzero_pd();                                                                                   \
        ...
        base3_im = _mm256_fmadd_pd(s4_im, KC.s4, base3_im);                                                               \
                                                                                                                          \
        base4_re = _mm256_setzero_pd();                                                                                   \
        base4_im = _mm256_setzero_pd();                                                                                   \
        ...
        base4_im = _mm256_fmadd_pd(s4_im, KC.s2, base4_im);                                                               \
                                                                                                                          \
        base5_re = _mm256_setzero_pd();                                                                                   \
        base5_im = _mm256_setzero_pd();                                                                                   \
    ...
    int sub_len, int is_forward)
{
    ...
    if (sub_len > 1)
    {
#define APPLY_TW_SCALAR(x_re, x_im, r)             \
    do                                             \
    {                                              \
...
#undef APPLY_TW_SCALAR
    ...
    out_re[k] = y0_re;
    out_im[k] = y0_im;
    out_re[k + K] = real1_re + rot1_re;
    out_im[k + K] = real1_im + rot1_im;
    out_re[k + 2 * K] = real2_re + rot2_re;
    out_im[k + 2 * K] = real2_im + rot2_im;
    out_re[k + 3 * K] = real3_re + rot3_re;
    out_im[k + 3 * K] = real3_im + rot3_im;
    out_re[k + 4 * K] = real4_re + rot4_re;
    out_im[k + 4 * K] = real4_im + rot4_im;
    out_re[k + 5 * K] = real5_re + rot5_re;
    out_im[k + 5 * K] = real5_im + rot5_im;
    out_re[k + 6 * K] = real5_re - rot5_re;
    out_im[k + 6 * K] = real5_im - rot5_im;
    out_re[k + 7 * K] = real4_re - rot4_re;
    out_im[k + 7 * K] = real4_im - rot4_im;
    out_re[k + 8 * K] = real3_re - rot3_re;
    out_im[k + 8 * K] = real3_im - rot3_im;
    out_re[k + 9 * K] = real2_re - rot2_re;
    out_im[k + 9 * K] = real2_im - rot2_im;
    out_re[k + 10 * K] = real1_re - rot1_re;
    out_im[k + 10 * K] = real1_im - rot1_im;
}
