    ...
    size_t K)
{
    ...
    for (size_t k = 0; k < K; k++)
    {
         ...
         */
        {
            double a = ri[1 * K + k], b = ri[10 * K + k];
            T4 = a + b;
            TG = b - a;
        }
        {
            double a = ii[1 * K + k], b = ii[10 * K + k];
            Tk = a - b;
            TR = a + b;
        }
        {
            double a = ii[2 * K + k], b = ii[9 * K + k];
            Tw = a - b;
            TN = a + b;
        }
        {
            double a = ri[2 * K + k], b = ri[9 * K + k];
            T7 = a + b;
            TK = b - a;
        }
        {
            double a = ri[3 * K + k], b = ri[8 * K + k];
            Ta = a + b;
            TH = b - a;
        }
        {
            double a = ii[3 * K + k], b = ii[8 * K + k];
            Tn = a - b;
            TQ = a + b;
        }
        {
            double a = ri[4 * K + k], b = ri[7 * K + k];
            Td = a + b;
            TJ = b - a;
        }
        {
            double a = ii[4 * K + k], b = ii[7 * K + k];
            Tq = a - b;
            TO = a + b;
        }
        {
            double a = ii[5 * K + k], b = ii[6 * K + k];
            Tt = a - b;
            TP = a + b;
        }
        {
            double a = ri[5 * K + k], b = ri[6 * K + k];
            Tg = a + b;
            TI = b - a;
        }
...
__attribute__((target("avx512f,fma"))) static void radix11_genfft_fwd_avx512(
    const double *__restrict__ ri, const double *__restrict__ ii,
    double *__restrict__ ro, double *__restrict__ io,
    size_t K)
{
...
#define FNS _mm512_fnmsub_pd /* -a*b - c */
    ...
    for (size_t k = 0; k < K; k += 8)
    {
        ...
        TM = LD(&ii[0 * K + k]);
        {
            V a = LD(&ri[1 * K + k]), b = LD(&ri[10 * K + k]);
            T4 = ADD(a, b);
            TG = SUB(b, a);
        }
        {
            V a = LD(&ii[1 * K + k]), b = LD(&ii[10 * K + k]);
            Tk = SUB(a, b);
            TR = ADD(a, b);
        }
        {
            V a = LD(&ii[2 * K + k]), b = LD(&ii[9 * K + k]);
            Tw = SUB(a, b);
            TN = ADD(a, b);
        }
        {
            V a = LD(&ri[2 * K + k]), b = LD(&ri[9 * K + k]);
            T7 = ADD(a, b);
            TK = SUB(b, a);
        }
        {
            V a = LD(&ri[3 * K + k]), b = LD(&ri[8 * K + k]);
            Ta = ADD(a, b);
            TH = SUB(b, a);
        }
        {
            V a = LD(&ii[3 * K + k]), b = LD(&ii[8 * K + k]);
            Tn = SUB(a, b);
            TQ = ADD(a, b);
        }
        {
            V a = LD(&ri[4 * K + k]), b = LD(&ri[7 * K + k]);
            Td = ADD(a, b);
            TJ = SUB(b, a);
        }
        {
            V a = LD(&ii[4 * K + k]), b = LD(&ii[7 * K + k]);
            Tq = SUB(a, b);
            TO = ADD(a, b);
        }
        {
            V a = LD(&ii[5 * K + k]), b = LD(&ii[6 * K + k]);
            Tt = SUB(a, b);
            TP = ADD(a, b);
        }
        {
            V a = LD(&ri[5 * K + k]), b = LD(&ri[6 * K + k]);
            Tg = ADD(a, b);
            TI = SUB(b, a);
        }
        ...
        /* Pair (4, 7) */
        {
            ...
            ST(&ro[7 * K + k], SUB(Th, Tx));
            ST(&ro[4 * K + k], ADD(Th, Tx));
            ...
            ST(&io[4 * K + k], ADD(TZ, T10));
            ST(&io[7 * K + k], SUB(T10, TZ));
        }
        ...
        /* Pair (2, 9) */
        {
            ...
            ST(&ro[9 * K + k], SUB(Ty, Tz));
            ST(&ro[2 * K + k], ADD(Ty, Tz));
            ...
            ST(&io[2 * K + k], ADD(TX, TY));
            ST(&io[9 * K + k], SUB(TY, TX));
        }
        ...
        /* Pair (1, 10) */
        {
            ...
            ST(&ro[10 * K + k], SUB(TA, TB));
            ST(&ro[1 * K + k], ADD(TA, TB));
            ...
            ST(&io[1 * K + k], ADD(TV, TW));
            ST(&io[10 * K + k], SUB(TW, TV));
        }
        ...
        /* Pair (3, 8) */
        {
            ...
            ST(&ro[8 * K + k], SUB(TC, TD));
            ST(&ro[3 * K + k], ADD(TC, TD));
            ...
            ST(&io[3 * K + k], ADD(TT, TU));
            ST(&io[8 * K + k], SUB(TU, TT));
        }
        ...
        /* Pair (5, 6) */
        {
            ...
            ST(&ro[6 * K + k], SUB(TE, TF));
            ST(&ro[5 * K + k], ADD(TE, TF));
            ...
            ST(&io[5 * K + k], ADD(TL, TS));
            ST(&io[6 * K + k], SUB(TS, TL));
        }
    }
#undef LD
#undef ST
...
__attribute__((target("avx512f,fma"))) static inline void radix11_genfft_bwd_avx512(
    const double *__restrict__ ri, const double *__restrict__ ii,
    double *__restrict__ ro, double *__restrict__ io,
    size_t K)
{
...
__attribute__((target("avx2,fma"))) static void radix11_genfft_fwd_avx2(
    const double *__restrict__ ri, const double *__restrict__ ii,
    double *__restrict__ ro, double *__restrict__ io,
    size_t K)
{
...
__attribute__((target("avx2,fma"))) static inline void radix11_genfft_bwd_avx2(
    const double *__restrict__ ri, const double *__restrict__ ii,
    double *__restrict__ ro, double *__restrict__ io,
    size_t K)
{
...
__attribute__((target("avx512f,fma"))) static inline void r11_genfft_packed_fwd_avx512(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K)
{
...
__attribute__((target("avx512f,fma"))) static inline void r11_genfft_packed_bwd_avx512(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K)
{
...
__attribute__((target("avx2,fma"))) static inline void r11_genfft_packed_fwd_avx2(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K)
{
...
__attribute__((target("avx2,fma"))) static inline void r11_genfft_packed_bwd_avx2(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K)
{
...
__attribute__((target("avx512f,fma"))) static void r11_tw_walk_packed_fwd_avx512(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K,
...
__attribute__((target("avx2,fma"))) static void r11_tw_walk_packed_fwd_avx2(
    const double *__restrict__ in_re, const double *__restrict__ in_im,
    double *__restrict__ out_re, double *__restrict__ out_im,
    size_t K,
    const double *__restrict__ tw_re, const double *__restrict__ tw_im,
