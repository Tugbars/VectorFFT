/**
 * bench_r16_log3.c -- R=16 AVX2: notw / flat / log3 vs FFTW SIMD (stride-1)
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
#include "fft_radix16_avx2_dit_tw_log3.h"
#include "fft_radix16_avx2_dif_tw_log3.h"
...
static void init_log3_tw(double *twr, double *twi, size_t K) {
    int bases[]={1,2,4,8};
    for(int i=0;i<4;i++) for(size_t k=0;k<K;k++){
...
int main(void) {
    ...
    printf("============================================================================================\n");
    printf("  R=16 AVX2: notw / flat_dit / log3_dit / log3_dif vs FFTW SIMD (stride-1)\n");
    printf("  Twiddle table: flat=15*K*16B, log3=4*K*16B (3.75x smaller)\n");
    printf("============================================================================================\n\n");
    ...
    for(int ki=0;ki<nK;ki++){
        ...
        double *ltwr=aligned_alloc(64,4*K*8),*ltwi=aligned_alloc(64,4*K*8);
        ...
        double log3_kb=2.0*4*K*8/1024.0;

        printf("%-5zu %-6zu %8.1f",K,N,fsimd);

        if(K>=4){double ns=bench_nf((notw_fn)radix16_n1_dit_kernel_fwd_avx2,ir,ii,or_,oi,K,reps);
            printf("  %5.0f(%5.1fx)",ns,fsimd/ns);}else printf("  %14s","---");
        if(K>=4){double ns=bench_fn((tw_fn)radix16_tw_flat_dit_kernel_fwd_avx2,ir,ii,or_,oi,ftwr,ftwi,K,reps);
            printf("  %5.0f(%5.1fx)",ns,fsimd/ns);}else printf("  %14s","---");
        if(K>=4){double ns=bench_fn((tw_fn)radix16_tw_log3_dit_kernel_fwd_avx2,ir,ii,or_,oi,ltwr,ltwi,K,reps);
            printf("  %5.0f(%5.1fx)",ns,fsimd/ns);}else printf("  %14s","---");
        if(K>=4){double ns=bench_fn((tw_fn)radix16_tw_log3_dif_kernel_fwd_avx2,ir,ii,or_,oi,ltwr,ltwi,K,reps);
            printf("  %5.0f(%5.1fx)",ns,fsimd/ns);}else printf("  %14s","---");
