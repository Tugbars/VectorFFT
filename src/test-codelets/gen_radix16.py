...
class Emitter:
    ...
    def emit_load(self, v, n, ke="k"):
        ...
        if self.isa.name == 'scalar':
            self.o(f"{v}_re = in_re[{n}*K+{ke}]; {v}_im = in_im[{n}*K+{ke}];")
        else:
            self.o(f"{v}_re = LD(&in_re[{n}*K+{ke}]);")
            self.o(f"{v}_im = LD(&in_im[{n}*K+{ke}]);")

    def emit_store(self, v, m, ke="k"):
        self.n_store += 2
        if self.isa.name == 'scalar':
            self.o(f"out_re[{m}*K+{ke}] = {v}_re; out_im[{m}*K+{ke}] = {v}_im;")
        else:
            self.o(f"ST(&out_re[{m}*K+{ke}], {v}_re);")
            self.o(f"ST(&out_im[{m}*K+{ke}], {v}_im);")
    ...
    def emit_ext_tw(self, v, tw_idx, d, ke="k"):
        ...
        self.n_load += 2
        if self.isa.name == 'scalar':
            self.o(f"{{ double wr = tw_re[{tw_idx}*K+{ke}], wi = tw_im[{tw_idx}*K+{ke}], tr = {v}_re;")
            if fwd:
            ...
            self.o(f"{{ const {T} wr = LD(&tw_re[{tw_idx}*K+{ke}]);")
            self.o(f"  const {T} wi = LD(&tw_im[{tw_idx}*K+{ke}]);")
...
def main():
    ...
    parser.add_argument('--variant', default='dit_tw',
                        choices=['notw', 'dit_tw', 'dif_tw', 'dit_tw_log3', 'dif_tw_log3', 'all'])
    ...
    variants = ['notw', 'dit_tw', 'dif_tw', 'dit_tw_log3', 'dif_tw_log3'] if args.variant == 'all' else [args.variant]
