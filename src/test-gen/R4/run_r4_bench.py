"""
run_r4_bench.py — full R=4 bench with parallel compile and incremental
...
Usage:
    python run_r4_bench.py --phase generate
    python run_r4_bench.py --phase build
    python run_r4_bench.py --phase run  (can be resumed)
"""
...
def _wrapper_c(candidate):
    ...
    # For R=4 gen emits 'static inline void\nradix4_...(' - after 'static'
    # removal we still need to strip 'inline' so the function has external
    # linkage (plain 'inline' without 'static' in C99 = external-inline
    # definition, which creates confusing link semantics).
    for direction in ('fwd', 'bwd'):
        ...
        # Also strip 'inline ' if present immediately before the function.
        # Needed for R=4 generator output where 'static inline void' is emitted.
        inline_needle = f'inline void\n{fn_name}('
        if inline_needle in src:
            idx = src.find(inline_needle)
            src = src[:idx] + src[idx + len('inline '):]
...
if __name__ == '__main__':
    ...
    cc = os.environ.get('CC', 'gcc')
    print("-" * 64)
    print(" VectorFFT codelet bench (R=4)")
    print("-" * 64)
