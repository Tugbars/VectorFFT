/**
 * @file fft_radix32_avx512_ct_t1_ladder_dit.h
 * @brief DFT-32 AVX512 t1 DIT ladder (in-place, 5-base-twiddle log derivation, AVX-512) � NFUSE=4
 *
 * FFTW-style codelet for recursive CT executor.
 * Generated by gen_radix32.py --variant ct_t1_ladder_dit
 */

#ifndef FFT_RADIX32_AVX512_CT_CT_T1_LADDER_DIT_H
#define FFT_RADIX32_AVX512_CT_CT_T1_LADDER_DIT_H
...
radix32_t1_ladder_dit_fwd_avx512(
    ...
    size_t ios, size_t me)
{
    ...
    /* Ladder DIT � 5 base twiddles from flat W table at rows {0,1,3,7,15} */
    for (size_t m = 0; m < me; m += 8) {
        /* Load 5 base twiddle pairs */
        const __m512d b1_re = LD(&W_re[0*me+m]);
        const __m512d b1_im = LD(&W_im[0*me+m]);
        const __m512d b2_re = LD(&W_re[1*me+m]);
        const __m512d b2_im = LD(&W_im[1*me+m]);
        const __m512d b4_re = LD(&W_re[3*me+m]);
        const __m512d b4_im = LD(&W_im[3*me+m]);
        const __m512d b8_re = LD(&W_re[7*me+m]);
        const __m512d b8_im = LD(&W_im[7*me+m]);
        const __m512d b16_re = LD(&W_re[15*me+m]);
        const __m512d b16_im = LD(&W_im[15*me+m]);
...
__attribute__((target("avx512f,avx512dq,fma"))) void
radix32_t1_ladder_dit_bwd_avx512(
    double * __restrict__ rio_re, double * __restrict__ rio_im,
    const double * __restrict__ W_re, const double * __restrict__ W_im,
    size_t ios, size_t me)
{
...
#endif /* FFT_RADIX32_AVX512_CT_CT_T1_LADDER_DIT_H */
