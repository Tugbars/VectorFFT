...
//==============================================================================
// HELPER: Process a Range of Butterflies - FORWARD
//==============================================================================

static void radix3_process_range_native_soa_fv(
    ...
    int use_streaming)
{
    ...
    if (use_streaming)
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX3_PIPELINE_4_NATIVE_SOA_FV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX3_PIPELINE_4_NATIVE_SOA_FV_AVX512_STREAM(k + 4, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 7 < k_end; k += 8)
        {
            RADIX3_PIPELINE_4_NATIVE_SOA_FV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX3_PIPELINE_4_NATIVE_SOA_FV_AVX512_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX3_PIPELINE_4_NATIVE_SOA_FV_AVX512(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX3_PIPELINE_2_NATIVE_SOA_FV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX3_PIPELINE_2_NATIVE_SOA_FV_AVX2_STREAM(k + 2, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 3 < k_end; k += 4)
        {
            RADIX3_PIPELINE_2_NATIVE_SOA_FV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX3_PIPELINE_2_NATIVE_SOA_FV_AVX2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX3_PIPELINE_2_NATIVE_SOA_FV_AVX2(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
    ...
    if (use_streaming)
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX3_PIPELINE_1_NATIVE_SOA_FV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
            RADIX3_PIPELINE_1_NATIVE_SOA_FV_SSE2_STREAM(k + 1, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
        }
    }
    else
    {
        for (; k + 1 < k_end; k += 2)
        {
            RADIX3_PIPELINE_1_NATIVE_SOA_FV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, prefetch_dist, k_end);
    ...
    if (use_streaming)
    {
        for (; k < k_end; k++)
        {
            RADIX3_PIPELINE_1_NATIVE_SOA_FV_SSE2_STREAM(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
    else
    {
        for (; k < k_end; k++)
        {
            RADIX3_PIPELINE_1_NATIVE_SOA_FV_SSE2(k, K, in_re, in_im, out_re, out_im, stage_tw, 0, k_end);
        }
    }
#endif
...
//==============================================================================
// MAIN FUNCTION: Radix-3 Forward Transform - NATIVE SoA
//==============================================================================
    ...
    int K)
{
    ...
    // Radix-3: 3 output lanes × 2 arrays × K elements × sizeof(double)
    ...
    radix3_process_range_native_soa_fv(
        out_re, out_im,
        in_re, in_im,
        stage_tw,
        K,
