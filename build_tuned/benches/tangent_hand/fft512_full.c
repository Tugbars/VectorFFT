...
/* mid-32 tables @512: 8 groups x 248, d_b = w512^{c*b} */
...
static void route_1632(const double*x,double*y,const double*Tm32,int tg_leaf,int tg_mid,const double*Tdum){
    ...
    if(tg_mid)  radix32_z_w32tg_fwd_avx2(S,0,y,0,Tm32,0,16,0,16,0,16);
    else        radix32_z_t2b_fwd_avx2(S,0,y,0,Tm32,0,16,0,16,0,16);
}
/* (32,16): leaf32 16cols -> S[b*32+c]; mid16 over b, 32 cols */
static void route_3216(const double*x,double*y,const double*Tm16,int tg_mid,int tg_leaf,const double*Tdum){
    if(tg_leaf) radix32_z_w32tgL_fwd_avx2(x,0,S,0,Tdum,0,16,0,32,0,16);
    else        radix32_z_n1tb48_fwd_avx2(x,0,S,0,Tdum,0,16,0,32,0,16);
...
int main(void){
    ...
    double *arena=_aligned_malloc(192*1024,4096);
    double *x   = arena;                    /* 1024 dbl */
    S           = x    + 1024 + 8;
    double *y   = S    + 1024 + 8;
    double *Tm32= y    + 1024 + 8;          /* 8*248  = 1984 */
    double *Tf32= Tm32 + 1984 + 8;
    double *Td16= Tf32 + 1984 + 8;          /* 16*120 = 1920 */
    ...
    const char*nm[4]={"A leaf16+t2b48 (pure classic)","B leaf32+w16tg (mixed)",
                      "E tgL16+w32tg (PURE TANGENT 16,32)","F w32tgL+w16tg (PURE TANGENT 32,16)"};
    for(int r4=0;r4<4;r4++){
        if(r4==0) route_1632(x,y,Tm32,0,0,Tdum);
        else if(r4==1) route_3216(x,y,Tf16,1,0,Tdum);
        else if(r4==2) route_1632(x,y,Tf32,1,1,Tdum);
        else route_3216(x,y,Tf16,1,1,Tdum);
        double e=0;
    ...
    for(int r=0;r<ROUNDS;r++){
        for(int wch=0;wch<NLANE;wch++){
            ...
            void run_arm(void){
                if(arm==0||arm==4) route_1632(x,y,Tm32,0,0,Tdum);
                else if(arm==1) route_3216(x,y,Tf16,1,0,Tdum);
                else if(arm==2) route_1632(x,y,Tf32,1,1,Tdum);
                else route_3216(x,y,Tf16,1,1,Tdum);
            }
    ...
    const char*lbl[NLANE]={"A","B mixed","E PURE-TG (16,32)","F PURE-TG (32,16)","ctrl"};
