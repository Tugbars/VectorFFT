...
static void pace(int ms){ if(ms<=0)return; struct timespec ts={ms/1000,(long)(ms%1000)*1000000L}; nanosleep(&ts,NULL); }
...
static double best_proto(stride_plan_t *plan,double *re,double *im,size_t K,int reps){
    ...
    double best=1e18;
    for(int t=0;t<5;t++){ double t0=vfft_proto_now_ns(); for(int i=0;i<reps;i++) vfft_proto_execute_fwd(plan,re,im,K); double ns=(vfft_proto_now_ns()-t0)/reps; if(ns<best)best=ns; }
    return best;
}
static double best_mkl(DFTI_DESCRIPTOR_HANDLE d,double *re,double *im,int reps){
    ...
    double best=1e18;
    for(int t=0;t<5;t++){ double t0=vfft_proto_now_ns(); for(int i=0;i<reps;i++) DftiComputeForward(d,re,im); double ns=(vfft_proto_now_ns()-t0)/reps; if(ns<best)best=ns; }
    return best;
}
...
static double correctness(int N,int *f,int *v,int nf,size_t K,vfft_proto_registry_t *reg){
    ...
    srand(7+N+(int)K);
    for(size_t e=0;e<(size_t)N;e++) for(size_t l=0;l<K;l++){ double a=(double)rand()/RAND_MAX-0.5,b=(double)rand()/RAND_MAX-0.5; rk[e*K+l]=a;ik[e*K+l]=b;rp[e*Kp+l]=a;ip[e*Kp+l]=b; }
    for(size_t e=0;e<(size_t)N;e++) for(size_t l=K;l<Kp;l++){ rp[e*Kp+l]=0; ip[e*Kp+l]=0; }
    vfft_proto_execute_fwd(pk,rk,ik,K); vfft_proto_execute_fwd(pp,rp,ip,Kp);
    double md=0; for(size_t e=0;e<(size_t)N;e++) for(size_t l=0;l<K;l++){ double dr=fabs(rk[e*K+l]-rp[e*Kp+l]),di=fabs(ik[e*K+l]-ip[e*Kp+l]); double d=dr>di?dr:di; if(d>md)md=d; }
    afree(rk);afree(ik);afree(rp);afree(ip); return md;
}
...
int main(int argc, char **argv){
    ...
    if(!plan){ printf("K=%-4zu f%d  plan FAILED\n",K,flip); return 1; }
    size_t total=(size_t)N*K; int reps=(int)(50000000ull/total); if(reps<200)reps=200;
    double *re=ad(total),*im=ad(total),*rm=ad(total),*imk=ad(total);
    srand(42+(int)K);
    for(size_t i=0;i<total;i++){ double a=(double)rand()/RAND_MAX-0.5,b=(double)rand()/RAND_MAX-0.5; re[i]=a;im[i]=b;rm[i]=a;imk[i]=b; }
    DFTI_DESCRIPTOR_HANDLE d=mkl_make(N,K);

    double v,m;
    if(flip){ m=best_mkl(d,rm,imk,reps); cachebust(); pace(cool_ms); v=best_proto(plan,re,im,K,reps); }
    else    { v=best_proto(plan,re,im,K,reps); cachebust(); pace(cool_ms); m=best_mkl(d,rm,imk,reps); }
    printf("K=%-4zu rem=%zu flip=%d  corr=%.1e  vfft=%9.0f  mkl=%9.0f  ratio=%.3f\n",
           K, K%4, flip, md, v, m, m/v);
    DftiFreeDescriptor(&d); afree(re);afree(im);afree(rm);afree(imk);
    return 0;
}
