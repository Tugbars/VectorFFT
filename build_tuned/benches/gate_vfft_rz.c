...
static int cell(vfft_wisdom *w, int N, size_t K) {
    ...
    if (!pf || !pb || !pfz || !pbz) { printf("  [%d,%zu] create FAIL\n", N, K); return 0; }
    ...
    vfft_execute(pbz, VFFT_BACKWARD, z, NULL, yz, NULL);
    bad = 0;
    ...
    ok &= (mx < 1e-11);
    vfft_destroy(pf); vfft_destroy(pb); vfft_destroy(pfz); vfft_destroy(pbz);
...
static int cell2d(int N1, int N2)
{
    ...
    cf.layout = VFFT_LAYOUT_SPLIT;
    vfft_plan pb = vfft_create(&cf);              /* c2r SPLIT-in */
    if (!pf || !pb || !pfz || !pbz) { printf("  [2D %4d x%4d] create FAIL\n", N1, N2); return 0; }
