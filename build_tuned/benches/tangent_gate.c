/* ship_gate.c — tolerance gate for EVERY shipped tangent codelet, compiled
 * straight from the repo folder. Conventions:
 *   MID fwd : Y[o] = sum_l W_R^{ol} . w512^{lk} . x_l          (pre-twiddle)
 *   MID bwd : Y[o] = w512^{+ok} . sum_l W_R^{-ol} . x_l        (POST-twiddle)
...
K(radix16_z_t2tan_fwd_avx2);      K(radix16_z_n1ttan_fwd_avx2);
K(radix16_z_t2btan28_fwd_avx2);   K(radix16_z_n1tbtan28_fwd_avx2);
K(radix32_z_t2btan216_fwd_avx2);  K(radix32_z_n1tbtan216_fwd_avx2);
K(radix16_z_t2tan_bwd_avx2);
...
/* VTW2 records, our fold [-s,+s]; sg=-1 fwd, +1 bwd */
static void genT(double*T,int R,int kc,double sg){
    for(int l=1;l<R;l++){double*r=T+(l-1)*8;
        for(int c=0;c<2;c++){int kk=kc+c;
            cx d=cexp(sg*2.0*PI*I*(double)l*kk/512.0);
            r[c*2]=creal(d); r[c*2+1]=creal(d);
            r[4+c*2]=-cimag(d); r[4+c*2+1]=cimag(d);} }
}
static double gate_mid(krn fn,int R,int cols,double sg,int post){
    int rec=(R-1)*8, N2=R*cols*2;
    double *zin=_aligned_malloc((size_t)N2*8,64), *zo=_aligned_malloc((size_t)N2*8,64);
    ...
    for(int k=0;k<cols;k++) for(int o=0;o<R;o++){
        ...
        for(int l=0;l<R;l++){
            ...
            cx dg = post ? 1.0 : cexp(sg*2.0*PI*I*(double)l*k/512.0);
            ref += cexp(sg*2.0*PI*I*(double)o*l/(double)R)*dg*xv;
        }
        if(post) ref *= cexp(sg*2.0*PI*I*(double)o*k/512.0);
        cx got=zo[2*((size_t)o*cols+k)]+I*zo[2*((size_t)o*cols+k)+1];
        double e=cabs(got-ref); if(e>w)w=e;
    }
    _aligned_free(zin);_aligned_free(zo);_aligned_free(T); return w;
}
static double gate_leaf(krn fn,int R,int cols){
    ...
    _aligned_free(zin);_aligned_free(zo); return w;
}

int main(void){
    struct { const char*nm; double e; } r[7];
    r[0].nm="radix16_z_t2tan_fwd       (R16 mid, wing full-kernel)";
    r[0].e=gate_mid(radix16_z_t2tan_fwd_avx2,16,32,-1.0,0);
    r[1].nm="radix16_z_n1ttan_fwd      (R16 leaf, corner-turn)    ";
    r[1].e=gate_leaf(radix16_z_n1ttan_fwd_avx2,16,16);
    r[2].nm="radix16_z_t2btan28_fwd    (R16 mid, blocked 2.8)     ";
    r[2].e=gate_mid(radix16_z_t2btan28_fwd_avx2,16,32,-1.0,0);
    r[3].nm="radix16_z_n1tbtan28_fwd   (R16 leaf, blocked 2.8)    ";
    r[3].e=gate_leaf(radix16_z_n1tbtan28_fwd_avx2,16,16);
    r[4].nm="radix32_z_t2btan216_fwd   (R32 mid, blocked 2.16)    ";
    r[4].e=gate_mid(radix32_z_t2btan216_fwd_avx2,32,16,-1.0,0);
    r[5].nm="radix32_z_n1tbtan216_fwd  (R32 leaf, blocked 2.16)   ";
    r[5].e=gate_leaf(radix32_z_n1tbtan216_fwd_avx2,32,16);
    r[6].nm="radix16_z_t2tan_bwd       (R16 mid, BACKWARD)        ";
    r[6].e=gate_mid(radix16_z_t2tan_bwd_avx2,16,32,+1.0,1);
    int ok=1;
    for(int i=0;i<7;i++){
    ...
    printf("\n%s\n", ok ? "ALL 7 SHIPPED TANGENT CODELETS GATED CORRECT"
