...
int main(void)
{
    ...
    /* ── TURNED-axis edge variants (variant 4) ─────────────────────────
     * Leaf T256: PROMOTED at 128 (kv 64) and 512 (kv 67), 2026-08-16.
     * Mid M-128: loses every cell on this machine but stays in the pool
     * per owner policy (distinct constructions may win elsewhere). */
    ...
    ok &= check(256, 16, 16, 4, 0, "R16 mid M-128");
    ok &= check(512, 32, 16, 4, 0, "R32 mid M-128 (blocked)");
    ok &= check(240, 16, 15, 4, 0, "R16 mid M-128, ODD count");
    /* absent form: must DEGRADE bitwise to base */
