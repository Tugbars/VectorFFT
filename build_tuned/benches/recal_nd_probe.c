...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < 2; i++)   /* [0] warm the store, [1] the timed create */
    {
        ...
        cfg.order = is_r2c ? VFFT_ORDER_DEFAULT : VFFT_ORDER_SCRAMBLED;  /* r2c has no order axis */
        cfg.nthreads = 1;
        cfg.wisdom = W;
        cfg.wisdom_write = 1;
        ...
        if (i == 1)
            printf("%s %dx%dx%d split  recalibrate=%s  create %8.2f ms\n",
                   what, N, N, N, cfg.recalibrate ? "ON " : "off", t1 - t0);
        vfft_destroy(p);
    }
    vfft_wisdom_free(W);
    return 0;
