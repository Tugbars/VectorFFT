...
static double rt_err(const double *re, const double *im, const double *x, const double *xi, int N, size_t NK)
{
    ...
    {
        double a = re[i] / N - x[i], b = im[i] / N - xi[i];
        if (fabs(a) > e)
            e = fabs(a);
        if (fabs(b) > e)
            e = fabs(b);
...
static void ip_cell(int N, size_t K, vfft_wisdom *wis, const char *tag)
{
    ...
    vfft_config_t c = {.transform = VFFT_C2C, .placement = VFFT_INPLACE, .rigor = VFFT_MEASURE, .dims = 1, .n = {N, 0}, .howmany = K, .nthreads = 1, .wisdom = wis, .recalibrate = 0};
    vfft_plan p = vfft_create(&c);
    if (!p)
    {
        printf("  %-20s N=%-5d K=%-5zu  CREATE NULL\n", tag, N, K);
        return;
    }
    memcpy(re, x, NK * 8);
    memcpy(im, xi, NK * 8);
    ...
    if (p8)
    {
        memcpy(re, x, NK * 8);
        memcpy(im, xi, NK * 8);
        vfft_execute(p8, VFFT_FORWARD, re, im, re, im);
        mt = maxd(re, ref, NK);
        double mi = maxd(im, refi, NK);
        if (mi > mt)
            mt = mi;
        vfft_destroy(p8);
    }
...
static void oop_cell(int N, size_t K, vfft_wisdom *wis, const char *tag)
{
    ...
    vfft_config_t c = {.transform = VFFT_C2C, .placement = VFFT_OUTOFPLACE, .rigor = VFFT_MEASURE, .dims = 1, .n = {N, 0}, .howmany = K, .nthreads = 1, .wisdom = wis, .recalibrate = 0};
    vfft_plan p = vfft_create(&c);
    if (!p)
    {
        printf("  %-20s N=%-5d K=%-5zu  CREATE NULL\n", tag, N, K);
    ...
    double srcpres = maxd(sr, x, NK);
    {
        double e = maxd(si, xi, NK);
        if (e > srcpres)
            srcpres = e;
    }
    memcpy(X, dr, NK * 8);
    ...
    double dstpres = maxd(dr, X, NK);
    {
        double e = maxd(di, Xi, NK);
        if (e > dstpres)
            dstpres = e;
    }
...
static void r2c_cell(int N, size_t K, vfft_wisdom *wis)
{
    ...
    for (size_t i = 0; i < insz; i++)
        x[i] = (double)rand() / RAND_MAX - 0.5;
    vfft_config_t c = {.transform = VFFT_R2C, .placement = VFFT_OUTOFPLACE, .rigor = VFFT_MEASURE, .dims = 1, .n = {N, 0}, .howmany = K, .nthreads = 1, .wisdom = wis, .recalibrate = 0};
    vfft_plan p = vfft_create(&c);
    if (!p)
    {
        printf("  r2c                  N=%-5d K=%-5zu  CREATE NULL\n", N, K);
        return;
    }
    ...
    for (int k = 0; k <= halfN; k++)
    {
        ...
        for (int n = 0; n < N; n++)
        {
            ...
            rr += x[(size_t)n * K] * cos(a);
            ri += x[(size_t)n * K] * sin(a);
        }
...
{
    int halfN = N / 2;
    size_t insz = (size_t)N * K, csz = (size_t)(halfN + 1) * K;
    ...
    if (!rp || !cp)
    {
        printf("  c2r(r2c)             N=%-5d K=%-5zu  CREATE NULL (r2c=%p c2r=%p)\n", N, K, (void *)rp, (void *)cp);
        return;
    }
...
static void trig_cell(vfft_transform_t t, const char *nm, int N, size_t K, double scale, vfft_wisdom *wis)
{
    ...
    if (!p)
    {
        printf("  %-7s N=%-5d K=%-4zu  CREATE NULL\n", nm, N, K);
        return;
    }
...
/* 2D: c2c in-place roundtrip == N1*N2*x; r2c->c2r roundtrip == N1*N2*x. */
static void fft2d_cell(int N1, int N2, vfft_wisdom *wis)
{
    size_t plane = (size_t)N1 * N2, hp1 = (size_t)(N2 / 2 + 1), csz = (size_t)N1 * hp1;
    double *re = AAL(plane * 8), *im = AAL(plane * 8), *xr = AAL(plane * 8), *xi = AAL(plane * 8);
    srand(23 + N1 + N2);
    for (size_t i = 0; i < plane; i++) { xr[i] = (double)rand() / RAND_MAX - 0.5; xi[i] = (double)rand() / RAND_MAX - 0.5; }
    vfft_config_t cc = {.transform = VFFT_C2C, .placement = VFFT_INPLACE, .rigor = VFFT_MEASURE, .dims = 2, .n = {N1, N2}, .howmany = 1, .nthreads = 1, .wisdom = wis};
    vfft_plan pc = vfft_create(&cc);
    double c2c_rt = -1;
    if (pc) {
        memcpy(re, xr, plane * 8); memcpy(im, xi, plane * 8);
        vfft_execute(pc, VFFT_FORWARD, re, im, re, im);
        vfft_execute(pc, VFFT_BACKWARD, re, im, re, im);
        c2c_rt = 0;
        for (size_t i = 0; i < plane; i++) { double a = fabs(re[i] / plane - xr[i]), b = fabs(im[i] / plane - xi[i]); if (a > c2c_rt) c2c_rt = a; if (b > c2c_rt) c2c_rt = b; }
        vfft_destroy(pc);
    }
    double *x = AAL(plane * 8), *orr = AAL(csz * 8), *oii = AAL(csz * 8), *y = AAL(plane * 8);
    for (size_t i = 0; i < plane; i++) x[i] = (double)rand() / RAND_MAX - 0.5;
    vfft_config_t cr = {.transform = VFFT_R2C, .placement = VFFT_OUTOFPLACE, .rigor = VFFT_MEASURE, .dims = 2, .n = {N1, N2}, .howmany = 1, .nthreads = 1, .wisdom = wis};
    vfft_config_t cb = {.transform = VFFT_C2R, .placement = VFFT_OUTOFPLACE, .rigor = VFFT_MEASURE, .dims = 2, .n = {N1, N2}, .howmany = 1, .nthreads = 1, .wisdom = wis};
    vfft_plan pr = vfft_create(&cr), pb = vfft_create(&cb);
    double r2c_rt = -1;
    if (pr && pb) {
        vfft_execute(pr, VFFT_FORWARD, x, NULL, orr, oii);
        vfft_execute(pb, VFFT_BACKWARD, orr, oii, y, NULL);
        r2c_rt = 0;
        for (size_t i = 0; i < plane; i++) { double e = fabs(y[i] / plane - x[i]); if (e > r2c_rt) r2c_rt = e; }
    }
    if (pr) vfft_destroy(pr);
    if (pb) vfft_destroy(pb);
    printf("  2D %4dx%-4d  c2c_rt=%.0e  r2c->c2r_rt=%.0e  %s\n", N1, N2, c2c_rt, r2c_rt,
           (c2c_rt >= 0 && c2c_rt < 1e-9 && r2c_rt >= 0 && r2c_rt < 1e-9) ? "OK" : "CHECK");
    AFR(re); AFR(im); AFR(xr); AFR(xi); AFR(x); AFR(orr); AFR(oii); AFR(y);
}

int main(void)
{
    printf("== vfft PUBLIC API acceptance: c2c (ip/oop) + r2c + c2r + trig + 2D ==\n");
    printf("# ISA=%s version=%s\n", vfft_isa(), vfft_version());
    stride_env_init();
    stride_pin_thread(0); /* MT pool pins workers to cores 1..T-1; caller MUST hold core 0 */
    ...
    if (!w)
        w = vfft_wisdom_load("../../src/dag-fft-compiler/generator/generated");
    ...
    printf("-- 2D (dims=2; c2c in-place rt + r2c->c2r rt == N1*N2*x) --\n");
    fft2d_cell(64, 64, w);
    fft2d_cell(128, 64, w);
