...
enum { W_BAILEY = 0, W_ARMA, W_ARMA_IP, W_LEAFPASS, W_TRANSPOSE, W_T1, W_SCALAR, W_LEAFN, W_MONO64 };
...
int main(int argc, char **argv)
{
    ...
    for (int ni = 0; ni < nN; ni++) {
        ...
        c.tr_ = tr_; c.ti_ = ti_; c.dr = dr; c.di = di; c.wr = wr; c.wi = wi;
        ...
        for (int inp = 0; inp < 2; inp++) {
            ...
            for (int i = 0; i < nP; i++) {
                ...
                const char *bad = (eb > tol || ea > tol || ei > tol ||
                                   eb != eb || ea != ea || ei != ei) ? "  <FAIL>" : "";
                if (bad[0]) fails++;
                printf("  gate %-3s %3dx%-3d  B2=%.2e  A=%.2e  Aip=%.2e  cross=%.2e%s\n",
                       inp ? "rnd" : "det", pR1[i], pR2[i], eb, ea, ei, xd, bad);
            }
            if (c.leafN) {
                run(&c, W_LEAFN);
            ...
            if (N == 64) {
                ...
                const char *bad = (em > tol || em != em) ? "  <FAIL>" : "";
                if (bad[0]) fails++;
                printf("  gate %-3s MONO64    %.2e%s\n", inp ? "rnd" : "det", em, bad);
            }
        }
        if (fails) { printf("  GATE FAILURES (%d) — timings skipped\n", fails); continue; }

        /* ---- timings (rand input persists from gate loop) ---- */
        printf("  %-9s %10s %10s %10s %7s   %s\n", "pair", "B2(ns)", "A(ns)", "Aip(ns)", "A/B2", "A parts: leaf/tr/t1 (ns)");
        double bestB = 1e18, bestA = 1e18, bestI = 1e18; int bestBp = -1, bestAp = -1, bestIp = -1;
        for (int i = 0; i < nP; i++) {
            if (!plans[i]) continue;
            c.p = plans[i];
            ...
            printf("  %3dx%-5d %10.0f %10.0f %10.0f %7.2f   %0.f / %0.f / %0.f\n",
                   pR1[i], pR2[i], tb, ta, ti, ta / tb, tl, tt, t1);
        }
        if (bestIp >= 0)
        ...
        afree(tr_); afree(ti_); afree(dr); afree(di); afree(wr); afree(wi);
    }
    printf("\nDONE\n");
    return 0;
}
